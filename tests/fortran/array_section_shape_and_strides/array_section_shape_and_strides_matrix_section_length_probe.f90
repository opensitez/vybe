...
program array_section_shape_and_strides_matrix_section_length_probe
    ...
    if ((size(matrix(2:4,3:5))) /= 9) then
    print *, "FAIL: want [9] got [", size(matrix(2:4,3:5)), "]"
    stop 1
end if
    ...
    if ((size(matrix(2:4,3:5),2)) /= 3) then
    print *, "FAIL: want [3] got [", size(matrix(2:4,3:5),2), "]"
    stop 1
end if
    if ((matrix(2,3)) /= 12) then
    print *, "FAIL: want [12] got [", matrix(2,3), "]"
    stop 1
end if
    if ((matrix(4,5)) /= 24) then
    print *, "FAIL: want [24] got [", matrix(4,5), "]"
    stop 1
end if
