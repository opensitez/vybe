...
program array_section_shape_and_strides_two_dimensional_section_shape
    ...
    if ((lbound(matrix(2:3,1:3),1)) /= 2) then
    print *, "FAIL: want [2] got [", lbound(matrix(2:3,1:3),1), "]"
    stop 1
end if
    if ((ubound(matrix(2:3,1:3),1)) /= 3) then
    print *, "FAIL: want [3] got [", ubound(matrix(2:3,1:3),1), "]"
    stop 1
end if
    ...
    if ((sum(matrix(2:3,1:3))) /= 54) then
    print *, "FAIL: want [54] got [", sum(matrix(2:3,1:3)), "]"
    stop 1
end if
