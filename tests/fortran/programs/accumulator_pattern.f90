! vybe-test: fortran/programs/accumulator_pattern
! origin: languages/fortran/tests/fortran/test_programs.rs

program test
integer :: vybe_check_i = 0
integer :: vybe_check_w(2) = [ 15, 120 ]
    integer :: i, sum, product
    sum = 0
    product = 1
    do i = 1, 5
        sum = sum + i
        product = product * i
    end do
        vybe_check_i = vybe_check_i + 1
    if (vybe_check_i > 2) then
        print *, "FAIL: more than 2 line(s)"
        stop 1
    end if
    if ((sum) /= vybe_check_w(vybe_check_i)) then
        print *, "FAIL at ", vybe_check_i, " got [", sum, "]"
        stop 1
    end if
        vybe_check_i = vybe_check_i + 1
    if (vybe_check_i > 2) then
        print *, "FAIL: more than 2 line(s)"
        stop 1
    end if
    if ((product) /= vybe_check_w(vybe_check_i)) then
        print *, "FAIL at ", vybe_check_i, " got [", product, "]"
        stop 1
    end if
if (vybe_check_i /= 2) then
    print *, "FAIL: ", vybe_check_i, " line(s), wanted 2"
    stop 1
end if
end program test
