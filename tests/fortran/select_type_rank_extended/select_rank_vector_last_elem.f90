! vybe-test: fortran/select_type_rank_extended/select_rank_vector_last_elem
...
if ((x(size(x))) /= vybe_check_w(vybe_check_i)) then
    print *, "FAIL at ", vybe_check_i, " got [", x(size(x)), "]"
    stop 1
end if
rank default
vybe_check_i = vybe_check_i + 1
