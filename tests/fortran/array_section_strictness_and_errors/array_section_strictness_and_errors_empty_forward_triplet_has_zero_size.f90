! vybe-test: fortran/array_section_strictness_and_errors/array_section_strictness_and_errors_empty_forward_triplet_has_zero_size
...
program array_section_strictness_and_errors_empty_forward_triplet_has_zero_size
    integer :: values(1:5)
    values = (/1, 2, 3, 4, 5/)
    if ((size(values(5:4))) /= 0) then
    print *, "FAIL: want [0] got [", size(values(5:4)), "]"
    stop 1
end if
