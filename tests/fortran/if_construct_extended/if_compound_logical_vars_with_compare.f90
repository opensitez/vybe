! vybe-test: fortran/if_construct_extended/if_compound_logical_vars_with_compare
...
if (trim("wait") /= trim(vybe_check_w(vybe_check_i))) then
    print *, "FAIL at ", vybe_check_i, " got [", "wait", "]"
    stop 1
end if
end if
if (vybe_check_i /= 1) then
