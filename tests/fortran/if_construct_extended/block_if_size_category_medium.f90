! vybe-test: fortran/if_construct_extended/block_if_size_category_medium
...
if (trim("tiny") /= trim(vybe_check_w(vybe_check_i))) then
    print *, "FAIL at ", vybe_check_i, " got [", "tiny", "]"
    stop 1
end if
else if (n < 100) then
vybe_check_i = vybe_check_i + 1
...
if (trim("huge") /= trim(vybe_check_w(vybe_check_i))) then
    print *, "FAIL at ", vybe_check_i, " got [", "huge", "]"
    stop 1
end if
end if
if (vybe_check_i /= 1) then
