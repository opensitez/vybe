! vybe-test: fortran/sql/sql_insert_03
! origin: languages/fortran/tests/fortran/test_sql_cics.rs
program sql_insert_03
use iso_c_binding
implicit none
interface
...
db = c_null_ptr
errmsg = c_null_ptr
rc = sqlite3_open(c_char_"sql_insert_03.db"//c_null_char, db)
if (rc /= 0) then
...
rc = sqlite3_exec(db, c_char_"insert into t(id) values(42)"//c_null_char, c_null_funptr, c_null_ptr, errmsg)
if (rc /= 0) then
    print *, "FAIL: want [0] got [", rc, "]"
    stop 1
end if
