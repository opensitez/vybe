! vybe-test: fortran/sql/sql_cursor_fetch_10
! origin: languages/fortran/tests/fortran/test_sql_cics.rs
program sql_cursor_fetch_10
use iso_c_binding
implicit none
interface
...
db = c_null_ptr
errmsg = c_null_ptr
rc = sqlite3_open(c_char_"sql_cursor_fetch_10.db"//c_null_char, db)
if (rc /= 0) then
...
rc = sqlite3_exec(db, c_char_"insert into t values(7)"//c_null_char, c_null_funptr, c_null_ptr, errmsg)
if (rc /= 0) then
    print *, "FAIL: want [0] got [", rc, "]"
    stop 1
end if
rc = sqlite3_exec(db, c_char_"select id from t where id = 7"//c_null_char, c_null_funptr, c_null_ptr, errmsg)
if (rc /= 0) then
    print *, "FAIL: want [0] got [", rc, "]"
    stop 1
end if
...
end program sql_cursor_fetch_10
