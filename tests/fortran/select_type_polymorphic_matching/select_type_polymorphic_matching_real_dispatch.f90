! vybe-test: fortran/select_type_polymorphic_matching/select_type_polymorphic_matching_real_dispatch
...
integer :: vybe_check_w(1) = [ 3 ]
    ...
    type is (real)
        ...
        if ((int(value)) /= vybe_check_w(vybe_check_i)) then
            print *, "FAIL at ", vybe_check_i, " got [", int(value), "]"
            stop 1
        end if
    class default
                vybe_check_i = vybe_check_i + 1
