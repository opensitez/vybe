! vybe-test: fortran/select_type_polymorphic_matching/typeof_logical_scalar
...
program t
    ...
    tname = type_name(flag)
    if (trim(tname) /= "logical") then
        print *, "FAIL: want [logical] got [", trim(tname), "]"
        stop 1
    end if
contains
