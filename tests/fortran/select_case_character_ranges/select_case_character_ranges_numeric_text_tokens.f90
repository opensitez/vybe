! vybe-test: fortran/select_case_character_ranges/select_case_character_ranges_numeric_text_tokens
...
character(len=7) :: vybe_check_w(1) = [ "version" ]
    character(len=4) :: c
    c = 'v2x'
    select case (c)
    case ('a':'m')
    ...
    case ('v1':'v9')
        ...
        if (trim('version') /= trim(vybe_check_w(vybe_check_i))) then
            print *, "FAIL at ", vybe_check_i, " got [", 'version', "]"
            stop 1
        end if
    case default
                vybe_check_i = vybe_check_i + 1
