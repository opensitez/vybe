...
program array_bounds_and_lbound_ubound_referenced_from_nested_array
    ...
    nested(1) = -3
    nested(2) = 3
    if ((lbound(values(nested(1):nested(2)), 1)) /= -3) then
    print *, "FAIL: want [-3] got [", lbound(values(nested(1):nested(2)), 1), "]"
    stop 1
end if
    if ((ubound(values(nested(1):nested(2)), 1)) /= 3) then
    print *, "FAIL: want [3] got [", ubound(values(nested(1):nested(2)), 1), "]"
    stop 1
end if
