// vybe-test: pascal/array_algorithms/partial_sort_first_two_bubble
...
end; var a:array[0..3] of Integer; i,j,t:Integer; begin a[0]:=4; a[1]:=2; a[2]:=3; a[3]:=1; for i:=0 to 1 do for j:=0 to 3-i-1 do if a[j]>a[j+1] then begin t:=a[j]; a[j]:=a[j+1]; a[j+1]:=t; end; __p(__vs(a[0])); __p(__vs(a[1])); __vybeCheck('2' + #10 + '1');
end.
