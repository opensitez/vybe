// vybe-test: pascal/array_algorithms/gnome_sort_tiny
...
end; procedure GSort(var a:array of Integer); var i:Integer; begin i:=1; while i<=High(a) do begin if (i=0) or (a[i-1]<=a[i]) then Inc(i) else begin a[i-1]:=a[i-1] xor a[i]; a[i]:=a[i-1] xor a[i]; a[i-1]:=a[i-1] xor a[i]; Dec(i); end; end; end; var arr:array[0..3] of Integer; begin arr[0]:=3; arr[1]:=1; arr[2]:=4; arr[3]:=2; GSort(arr); __p(__vs(arr[0])); __p(__vs(arr[3])); __vybeCheck('1' + #10 + '4');
end.
