// vybe-test: pascal/array_algorithms/selection_sort_picks_min
...
end; procedure SelSort(var a:array of Integer); var i,j,m,t:Integer; begin for i:=0 to High(a) do begin m:=i; for j:=i+1 to High(a) do if a[j]<a[m] then m:=j; t:=a[i]; a[i]:=a[m]; a[m]:=t; end; end; var arr:array[0..2] of Integer; begin arr[0]:=5; arr[1]:=1; arr[2]:=3; SelSort(arr); __p(__vs(arr[0])); __vybeCheck('1');
end.
