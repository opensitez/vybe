// vybe-test: pascal/array_algorithms/insertion_sort_shift_right
...
end; procedure InsSort(var a:array of Integer); var i,j,k:Integer; begin for i:=1 to High(a) do begin k:=a[i]; j:=i-1; while (j>=0) and (a[j]>k) do begin a[j+1]:=a[j]; Dec(j); end; a[j+1]:=k; end; end; var arr:array[0..3] of Integer; begin arr[0]:=3; arr[1]:=1; arr[2]:=4; arr[3]:=2; InsSort(arr); __p(__vs(arr[1])); __p(__vs(arr[2])); __vybeCheck('2' + #10 + '3');
end.
