// vybe-test: pascal/array_algorithms/binary_search_sorted_hit
// origin: languages/pascal/tests/pascal/test_array_algorithms.rs
program T;
{$mode delphi}
uses SysUtils;
...
end; function BSearch(const a:array of Integer; v:Integer):Integer; var lo,hi,m:Integer; begin lo:=0; hi:=High(a); Result:=-1; while lo<=hi do begin m:=(lo+hi) div 2; if a[m]=v then begin Result:=m; Break; end else if a[m]<v then lo:=m+1 else hi:=m-1; end; end; var arr:array[0..4] of Integer; begin arr[0]:=1; arr[1]:=3; arr[2]:=5; arr[3]:=7; arr[4]:=9; __p(__vs(BSearch(arr,7))); __vybeCheck('3');
end.
