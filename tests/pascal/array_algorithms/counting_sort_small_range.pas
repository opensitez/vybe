// vybe-test: pascal/array_algorithms/counting_sort_small_range
...
end; var a,c:array[0..4] of Integer; i,v:Integer; begin a[0]:=3; a[1]:=1; a[2]:=2; a[3]:=1; a[4]:=3; for i:=0 to 4 do c[i]:=0; for i:=0 to 4 do Inc(c[a[i]]); v:=0; for i:=0 to 4 do while c[i]>0 do begin a[v]:=i; Dec(c[i]); Inc(v); end; __p(__vs(a[0])); __p(__vs(a[4])); __vybeCheck('1' + #10 + '3');
end.
