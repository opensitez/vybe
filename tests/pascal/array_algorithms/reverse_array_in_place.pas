// vybe-test: pascal/array_algorithms/reverse_array_in_place
...
end; procedure Rev(var a:array of Integer); var i,j,t:Integer; begin j:=High(a); for i:=0 to j div 2 do begin t:=a[i]; a[i]:=a[j-i]; a[j-i]:=t; end; end; var arr:array[0..3] of Integer; begin arr[0]:=1; arr[1]:=2; arr[2]:=3; arr[3]:=4; Rev(arr); __p(__vs(arr[0])); __p(__vs(arr[3])); __vybeCheck('4' + #10 + '1');
end.
