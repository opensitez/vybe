// vybe-test: pascal/array_algorithms/binary_search_sorted_miss
...
end; function BSearch(const a:array of Integer; v:Integer):Integer; var lo,hi,m:Integer; begin lo:=0; hi:=High(a); Result:=-1; while lo<=hi do begin m:=(lo+hi) div 2; if a[m]=v then begin Result:=m; Break; end else if a[m]<v then lo:=m+1 else hi:=m-1; end; end; var arr:array[0..3] of Integer; begin arr[0]:=2; arr[1]:=4; arr[2]:=6; arr[3]:=8; __p(__vs(BSearch(arr,5))); __vybeCheck('-1');
end.
