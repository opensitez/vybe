// vybe-test: pascal/array_algorithms/bubble_sort_ascending_small
...
end; procedure Sort(var a:array of Integer); var i,j,t,n:Integer; begin n:=High(a); for i:=0 to n do for j:=0 to n-1 do if a[j]>a[j+1] then begin t:=a[j]; a[j]:=a[j+1]; a[j+1]:=t; end; end; var arr:array[0..3] of Integer; begin arr[0]:=4; arr[1]:=1; arr[2]:=3; arr[3]:=2; Sort(arr); __p(__vs(arr[0])); __p(__vs(arr[3])); __vybeCheck('1' + #10 + '4');
end.
