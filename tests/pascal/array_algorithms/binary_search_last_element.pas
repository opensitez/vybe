// vybe-test: pascal/array_algorithms/binary_search_last_element
...
end; function BSearch(const a:array of Integer; v:Integer):Integer; var lo,hi,m:Integer; begin lo:=0; hi:=High(a); Result:=-1; while lo<=hi do begin m:=(lo+hi) div 2; if a[m]=v then begin Result:=m; Break; end else if a[m]<v then lo:=m+1 else hi:=m-1; end; end; var arr:array[0..2] of Integer; begin arr[0]:=2; arr[1]:=4; arr[2]:=6; __p(__vs(BSearch(arr,6))); __vybeCheck('2');
end.
