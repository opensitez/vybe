// vybe-test: pascal/array_algorithms/bubble_sort_already_sorted
...
end; procedure Sort(var a:array of Integer); var i,j,t,n:Integer; begin n:=High(a); for i:=0 to n do for j:=0 to n-1 do if a[j]>a[j+1] then begin t:=a[j]; a[j]:=a[j+1]; a[j+1]:=t; end; end; var arr:array[0..2] of Integer; begin arr[0]:=1; arr[1]:=2; arr[2]:=3; Sort(arr); __p(__vs(arr[2])); __vybeCheck('3');
end.
