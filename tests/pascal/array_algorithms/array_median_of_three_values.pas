// vybe-test: pascal/array_algorithms/array_median_of_three_values
...
end; function Med3(a,b,c:Integer):Integer; begin if ((a<=b) and (b<=c)) or ((c<=b) and (b<=a)) then Result:=b else if ((b<=a) and (a<=c)) or ((c<=a) and (a<=b)) then Result:=a else Result:=c; end; begin __p(__vs(Med3(3,1,2))); __vybeCheck('2');
end.
