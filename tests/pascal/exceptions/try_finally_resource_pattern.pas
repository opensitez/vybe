// vybe-test: pascal/exceptions/try_finally_resource_pattern
// origin: languages/pascal/tests/pascal/test_exceptions.rs
program T;
{$mode delphi}
uses SysUtils;
...
begin FName := name; __p(__vs('Opened ' + name)); end;

procedure TResource.DoWork;
begin __p(__vs('Working with ' + FName)); end;
...
begin
  ...
  try
    ...
    __p(__vs('Closing ' + r.FName));
    FreeAndNil(r);
  end;
__vybeCheck('Opened file.txt' + #10 + 'Working with file.txt' + #10 + 'Closing file.txt');
end.
