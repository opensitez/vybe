// vybe-test: pascal/interfaces_extended/interface_delegate_to_inner
...
end; type ICore=interface function Run:Integer; end; TCore=class(TInterfacedObject,ICore) function Run:Integer; end; function TCore.Run:Integer; begin Result:=7; end; type TProxy=class(TInterfacedObject,ICore) private Inner:ICore; public constructor Create(c:ICore); function Run:Integer; end; constructor TProxy.Create(c:ICore); begin Inner:=c; end; function TProxy.Run:Integer; begin Result:=Inner.Run+1; end; var p:ICore; begin p:=TProxy.Create(TCore.Create); __p(__vs(p.Run)); __vybeCheck('8');
end.
