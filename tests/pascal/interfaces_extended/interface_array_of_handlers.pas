// vybe-test: pascal/interfaces_extended/interface_array_of_handlers
// origin: languages/pascal/tests/pascal/test_interfaces_extended.rs
program T;
{$mode delphi}
uses SysUtils;
...
end; type IHandler=interface procedure Handle(v:Integer); end; TDouble=class(TInterfacedObject,IHandler) procedure Handle(v:Integer); end; procedure TDouble.Handle(v:Integer); begin __p(__vs(v*2)); end; procedure Dispatch(const hs:array of IHandler; v:Integer); var i:Integer; begin for i:=Low(hs) to High(hs) do hs[i].Handle(v); end; var h:IHandler; begin h:=TDouble.Create; Dispatch([h],3); __vybeCheck('6');
end.
