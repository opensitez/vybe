// vybe-test: pascal/collections_maps/map_default_when_missing
...
end; function GetDef(const items:array of Integer; idx:Integer; def:Integer):Integer; begin if (idx>=Low(items)) and (idx<=High(items)) then Result:=items[idx] else Result:=def; end; begin __p(__vs(GetDef([5],9,-1))); __vybeCheck('-1');
end.
