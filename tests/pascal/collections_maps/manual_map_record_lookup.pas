// vybe-test: pascal/collections_maps/manual_map_record_lookup
...
end; type TEntry=record Key:string; Value:Integer; end; var items:array[0..1] of TEntry; function Get(const k:string):Integer; var i:Integer; begin Result:=-1; for i:=0 to 1 do if items[i].Key=k then Result:=items[i].Value; end; begin items[0].Key:='a'; items[0].Value:=1; items[1].Key:='b'; items[1].Value:=2; __p(__vs(Get('b'))); __vybeCheck('2');
end.
