// vybe-test: pascal/generics_extended/generic_class_constraint_style_compare
// origin: languages/pascal/tests/pascal/test_generics_extended.rs
program T;
{$mode delphi}
uses SysUtils;
...
end; type TComparable=class public Value:Integer; constructor Create(v:Integer); function LessThan(o:TComparable):Boolean; end; constructor TComparable.Create(v:Integer); begin Value:=v; end; function TComparable.LessThan(o:TComparable):Boolean; begin Result:=Value<o.Value; end; function PickSmaller(a,b:TComparable):TComparable; begin if a.LessThan(b) then Result:=a else Result:=b; end; var x,y,z:TComparable; begin x:=TComparable.Create(4); y:=TComparable.Create(9); z:=PickSmaller(x,y); __p(__vs(z.Value)); x.Free; y.Free; __vybeCheck('4');
end.
