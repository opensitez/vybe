// vybe-test: pascal/properties_extended/property_read_in_for_loop
...
end; type TSeq=class private F:array[0..2] of Integer; function GetItem(i:Integer):Integer; public property Items[i:Integer]:Integer read GetItem; end; function TSeq.GetItem(i:Integer):Integer; begin Result:=F[i]; end; var s:TSeq; i,sum:Integer; begin s:=TSeq.Create; s.F[0]:=1; s.F[1]:=2; s.F[2]:=3; sum:=0; for i:=0 to 2 do sum:=sum+s.Items[i]; __p(__vs(sum)); __vybeCheck('6');
end.
