// vybe-test: pascal/properties_extended/property_dynamic_high_index_read
...
end; type TV=class private F:array of Integer; function GetLast:Integer; public property Last:Integer read GetLast; procedure Add(v:Integer); var n:Integer; begin n:=Length(F); SetLength(F,n+1); F[n]:=v; end; end; function TV.GetLast:Integer; begin Result:=F[High(F)]; end; var v:TV; begin v:=TV.Create; v.Add(1); v.Add(9); __p(__vs(v.Last)); __vybeCheck('9');
end.
