// vybe-test: pascal/oop_patterns/is_operator_inheritance_chain
...
begin
  c := TChild.Create;
  if c is TChild then __p(__vs('is child'));
  if c is TParent then __p(__vs('is parent'));
  if c is TGrandparent then __p(__vs('is grandparent'));
__vybeCheck('is child' + #10 + 'is parent' + #10 + 'is grandparent');
end.
