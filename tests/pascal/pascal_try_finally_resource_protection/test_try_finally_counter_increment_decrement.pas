// vybe-test: pascal/pascal_try_finally_resource_protection/test_try_finally_counter_increment_decrement
...
begin
  Inc(activeTasks);
  try
    __p(__vs('ActiveTasks:' + activeTasks.ToString));
  finally
    Dec(activeTasks);
  end;
end;
begin
  ...
  TaskRun;
  __p(__vs('ActiveTasks:' + activeTasks.ToString));
__vybeCheck('ActiveTasks:1' + #10 + 'ActiveTasks:0');
end.
