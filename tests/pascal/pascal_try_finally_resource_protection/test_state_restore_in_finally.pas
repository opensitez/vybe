// vybe-test: pascal/pascal_try_finally_resource_protection/test_state_restore_in_finally
...
begin
  ...
  globalState := True;
  try
    __p(__vs('StateModified:' + globalState.ToString));
  finally
    globalState := oldState;
    __p(__vs('StateRestored:' + globalState.ToString));
  end;
end;
begin
  globalState := False;
  ModifyState;
__vybeCheck('StateModified:True' + #10 + 'StateRestored:False');
end.
