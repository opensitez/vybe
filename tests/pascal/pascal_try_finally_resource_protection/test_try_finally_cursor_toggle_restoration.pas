// vybe-test: pascal/pascal_try_finally_resource_protection/test_try_finally_cursor_toggle_restoration
...
begin
  ...
  cursor := 'Wait';
  try
    __p(__vs('CurrentCursor:' + cursor));
  finally
    cursor := oldCursor;
    __p(__vs('RestoredCursor:' + cursor));
  end;
end;
begin
  cursor := 'Default';
  ShowWaitCursor;
__vybeCheck('CurrentCursor:Wait' + #10 + 'RestoredCursor:Default');
end.
