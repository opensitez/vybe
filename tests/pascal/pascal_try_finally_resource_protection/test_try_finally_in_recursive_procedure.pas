// vybe-test: pascal/pascal_try_finally_resource_protection/test_try_finally_in_recursive_procedure
...
begin
  try
    __p(__vs('Enter:' + depth.ToString));
    if depth > 1 then RecursiveProc(depth - 1);
  finally
    __p(__vs('Leave:' + depth.ToString));
  end;
end;
begin
  RecursiveProc(2);
__vybeCheck('Enter:2' + #10 + 'Enter:1' + #10 + 'Leave:1' + #10 + 'Leave:2');
end.
