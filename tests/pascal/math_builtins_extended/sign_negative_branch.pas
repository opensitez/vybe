// vybe-test: pascal/math_builtins_extended/sign_negative_branch
// origin: languages/pascal/tests/pascal/test_math_builtins_extended.rs
program T;
{$mode delphi}
uses SysUtils;
...
end; function Sign(n:Integer):Integer; begin if n>0 then Result:=1 else if n<0 then Result:=-1 else Result:=0; end; begin __p(__vs(Sign(-2))); __vybeCheck('-1');
end.
