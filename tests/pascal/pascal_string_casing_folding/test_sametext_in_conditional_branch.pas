// vybe-test: pascal/pascal_string_casing_folding/test_sametext_in_conditional_branch
...
begin
  if SameText(cmd, 'START') then __p(__vs('CommandStart'))
  else if SameText(cmd, 'STOP') then __p(__vs('CommandStop'));
end;
begin
  HandleCommand('Start');
  HandleCommand('stop');
__vybeCheck('CommandStart' + #10 + 'CommandStop');
end.
