// vybe-test: pascal/pascal_tstringlist_properties/test_tstringlist_key_value_values_property
...
begin
  ...
  sl.Add('port=8080');
  __p(__vs(sl.Values['host']));
  __p(__vs(sl.Values['port']));
  sl.Values['port'] := '9090';
  __p(__vs(sl.Values['port']));
  sl.Free;
__vybeCheck('localhost' + #10 + '8080' + #10 + '9090');
end.
