// vybe-test: pascal/pascal_generics_tobjectlist/test_nested_tobjectlist_hierarchy
// origin: languages/pascal/tests/pascal/test_pascal_generics_tobjectlist.rs
program Test;
{$mode delphi}
...
destructor TChild.Destroy; begin __p(__vs('ChildFreed')); inherited Destroy; end;
constructor TParent.Create; begin Children := TObjectList<TChild>.Create(True); end;
destructor TParent.Destroy; begin Children.Free; __p(__vs('ParentFreed')); inherited Destroy; end;
...
begin
  ...
  list.Add(p);
  list.Free;
__vybeCheck('ChildFreed' + #10 + 'ParentFreed');
end.
