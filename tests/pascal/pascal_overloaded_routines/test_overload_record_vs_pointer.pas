// vybe-test: pascal/pascal_overloaded_routines/test_overload_record_vs_pointer
...
begin
  __p(__vs('VALUE:' + r.Val.ToString));
end;
procedure Inspect(p: PRec); overload;
begin
  __p(__vs('POINTER:' + p^.Val.ToString));
end;
var rec: TRec;
begin
  rec.Val := 99;
  Inspect(rec);
  Inspect(@rec);
__vybeCheck('VALUE:99' + #10 + 'POINTER:99');
end.
