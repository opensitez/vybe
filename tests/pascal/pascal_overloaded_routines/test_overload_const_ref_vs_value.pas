// vybe-test: pascal/pascal_overloaded_routines/test_overload_const_ref_vs_value
...
begin
  __p(__vs('BY_ID:' + id.ToString));
end;
procedure LoadData(const d: TData); overload;
begin
  __p(__vs('BY_REC:' + d.ID.ToString));
end;
var rec: TData;
begin
  rec.ID := 55;
  LoadData(55);
  LoadData(rec);
__vybeCheck('BY_ID:55' + #10 + 'BY_REC:55');
end.
