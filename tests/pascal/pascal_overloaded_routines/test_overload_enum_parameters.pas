// vybe-test: pascal/pascal_overloaded_routines/test_overload_enum_parameters
...
begin
  __p(__vs('COLOR:' + Ord(c).ToString));
end;
procedure Render(s: TSize); overload;
begin
  __p(__vs('SIZE:' + Ord(s).ToString));
end;
begin
  Render(Green);
  Render(Large);
__vybeCheck('COLOR:1' + #10 + 'SIZE:2');
end.
