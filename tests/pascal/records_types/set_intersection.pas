// vybe-test: pascal/records_types/set_intersection
...
begin
  ...
  s2 := [B, C, D];
  both := s1 * s2;
  if A in both then __p(__vs('A')) else __p(__vs('no A'));
  if B in both then __p(__vs('B'));
  if C in both then __p(__vs('C'));
__vybeCheck('no A' + #10 + 'B' + #10 + 'C');
end.
