// vybe-test: pascal/records_types/set_include_exclude
...
begin
  ...
  Exclude(colors, Red);
  if Red in colors then __p(__vs('has red')) else __p(__vs('no red'));
  if Green in colors then __p(__vs('has green'));
__vybeCheck('no red' + #10 + 'has green');
end.
