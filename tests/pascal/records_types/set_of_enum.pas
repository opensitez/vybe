// vybe-test: pascal/records_types/set_of_enum
// origin: languages/pascal/tests/pascal/test_records_types.rs
program T;
{$mode delphi}
uses SysUtils;
...
begin
  ...
  if Mon in weekdays then __p(__vs('Monday is a weekday'));
  if Sat in weekdays then __p(__vs('Saturday is a weekday'))
  else __p(__vs('Saturday is weekend'));
__vybeCheck('Monday is a weekday' + #10 + 'Saturday is weekend');
end.
