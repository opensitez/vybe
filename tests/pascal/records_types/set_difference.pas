// vybe-test: pascal/records_types/set_difference
...
begin
  ...
  s2 := [B, D];
  diff := s1 - s2;
  if A in diff then __p(__vs('A'));
  if B in diff then __p(__vs('B')) else __p(__vs('no B'));
  if C in diff then __p(__vs('C'));
__vybeCheck('A' + #10 + 'no B' + #10 + 'C');
end.
