// vybe-test: pascal/pascal_unhandled_exception_hooks/test_exceptproc_global_counter_increment
...
begin
  Inc(unhandledCount);
  __p(__vs('UnhandledCount:' + unhandledCount.ToString));
end;
begin
  unhandledCount := 0;
  ExceptProc := CountHook;
  raise Exception.Create('CountTest');
__vybeCheck('UnhandledCount:1');
end.
