// vybe-test: pascal/pascal_unhandled_exception_hooks/test_exceptproc_econvert_error
...
begin
  if ExceptObj is EConvertError then
    __p(__vs('ConvertHookCaught:' + Exception(ExceptObj).Message));
end;
begin
  ExceptProc := ConvertHook;
  StrToInt('InvalidInt');
__vybeCheck('ConvertHookCaught:InvalidInt');
end.
