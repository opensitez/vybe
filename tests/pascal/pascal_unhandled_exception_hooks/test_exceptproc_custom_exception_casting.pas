// vybe-test: pascal/pascal_unhandled_exception_hooks/test_exceptproc_custom_exception_casting
...
begin
  if ExceptObj is ECustomHookErr then
    __p(__vs('Code:' + ECustomHookErr(ExceptObj).Code.ToString));
end;

begin
  ExceptProc := CustomHook;
  raise ECustomHookErr.CreateCode(500, 'ServerErr');
__vybeCheck('Code:500');
end.
