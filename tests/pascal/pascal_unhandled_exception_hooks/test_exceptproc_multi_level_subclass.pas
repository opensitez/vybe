// vybe-test: pascal/pascal_unhandled_exception_hooks/test_exceptproc_multi_level_subclass
...
begin
  if ExceptObj is EBase then
    __p(__vs('HierarchyMatched:' + ExceptObj.ClassName));
end;

begin
  ExceptProc := HierarchyHook;
  raise ESub.Create('SubClassError');
__vybeCheck('HierarchyMatched:ESub');
end.
