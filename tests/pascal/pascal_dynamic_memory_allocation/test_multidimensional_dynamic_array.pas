// vybe-test: pascal/pascal_dynamic_memory_allocation/test_multidimensional_dynamic_array
// origin: languages/pascal/tests/pascal/test_pascal_dynamic_memory_allocation.rs
program Test;
{$mode delphi}
uses SysUtils;
...
begin
  SetLength(grid, 2, 3);
  grid[1, 2] := 999;
  __p(__vs(Length(grid)));
  __p(__vs(Length(grid[1])));
  __p(__vs(grid[1, 2]));
__vybeCheck('2' + #10 + '3' + #10 + '999');
end.
