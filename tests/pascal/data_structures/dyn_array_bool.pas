// vybe-test: pascal/data_structures/dyn_array_bool
// origin: languages/pascal/tests/pascal/test_data_structures.rs
program T;
{$mode delphi}
uses SysUtils;
...
begin
  ...
  if flags[0] then __p(__vs('first is true'));
  if not flags[1] then __p(__vs('second is false'));
__vybeCheck('first is true' + #10 + 'second is false');
end.
