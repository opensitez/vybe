// vybe-test: pascal/type_system_extended/set_difference_operation
...
end; type TD = (A, B, C); var s1, s2, s3: set of TD; begin s1 := [A, B, C]; s2 := [B]; s3 := s1 - s2; if A in s3 then __p(__vs('a')); if B in s3 then __p(__vs('b')); __vybeCheck('a');
end.
