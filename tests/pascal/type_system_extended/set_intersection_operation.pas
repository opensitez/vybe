// vybe-test: pascal/type_system_extended/set_intersection_operation
// origin: languages/pascal/tests/pascal/test_type_system_extended.rs
program T;
{$mode delphi}
uses SysUtils;
...
end; type TD = (A, B, C); var s1, s2, s3: set of TD; begin s1 := [A, B]; s2 := [B, C]; s3 := s1 * s2; if B in s3 then __p(__vs('b')); if A in s3 then __p(__vs('a')); __vybeCheck('b' + #10 + 'a');
end.
