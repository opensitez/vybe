// vybe-test: pascal/pascal_procedure_overloading_default_args/test_overload_array_parameter_variants
...
begin __p(__vs('IntArrayLength:' + Length(arr).ToString)); end;

procedure ProcessArray(const arr: array of String); overload;
begin __p(__vs('StrArrayLength:' + Length(arr).ToString)); end;

begin
  ProcessArray([10, 20, 30]);
  ProcessArray(['A', 'B']);
__vybeCheck('IntArrayLength:3' + #10 + 'StrArrayLength:2');
end.
