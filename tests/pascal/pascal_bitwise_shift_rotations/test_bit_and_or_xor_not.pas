// vybe-test: pascal/pascal_bitwise_shift_rotations/test_bit_and_or_xor_not
// origin: languages/pascal/tests/pascal/test_pascal_bitwise_shift_rotations.rs
program Test;
{$mode delphi}
uses SysUtils;
...
begin
  a := $0F; b := $F0;
  __p(__vs(HexStr(a and b, 2)));
  __p(__vs(HexStr(a or b, 2)));
  __p(__vs(HexStr(a xor b, 2)));
  __p(__vs(HexStr(not a, 2)));
__vybeCheck('00' + #10 + 'FF' + #10 + 'FF' + #10 + 'F0');
end.
