// vybe-test: pascal/pascal_dynamic_array_multidimensional_ops/test_dynarray_2d_bounds_low_high
// origin: languages/pascal/tests/pascal/test_pascal_dynamic_array_multidimensional_ops.rs
program Test;
{$mode delphi}
uses SysUtils;
...
begin
  SetLength(m, 3, 4);
  __p(__vs(Low(m).ToString + '..' + High(m).ToString));
  __p(__vs(Low(m[0]).ToString + '..' + High(m[0]).ToString));
__vybeCheck('0..2' + #10 + '0..3');
end.
