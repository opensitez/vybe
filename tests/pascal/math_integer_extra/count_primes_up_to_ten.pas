// vybe-test: pascal/math_integer_extra/count_primes_up_to_ten
// origin: languages/pascal/tests/pascal/test_math_integer_extra.rs
program T;
{$mode delphi}
uses SysUtils;
...
end; function IsPrime(n:Integer):Boolean; var i:Integer; begin Result:=n>1; if Result then for i:=2 to n-1 do if (n mod i)=0 then Result:=false; end; var n,c:Integer; begin c:=0; for n:=2 to 10 do if IsPrime(n) then Inc(c); __p(__vs(c)); __vybeCheck('4');
end.
