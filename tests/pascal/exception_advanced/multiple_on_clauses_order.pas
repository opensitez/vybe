// vybe-test: pascal/exception_advanced/multiple_on_clauses_order
// origin: languages/pascal/tests/pascal/test_exception_advanced.rs
program T;
{$mode delphi}
uses SysUtils;
...
begin
  try
    ...
    on E: ESpecificError do __p(__vs('specific handler'));
    on E: EMyError do __p(__vs('my error handler'));
    on E: Exception do __p(__vs('base handler'));
  end;
__vybeCheck('specific handler');
end.
