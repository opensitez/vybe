// vybe-test: pascal/pointer_arithmetic/pointer_find_max_in_array_by_walking
// origin: languages/pascal/tests/pascal/test_pointer_arithmetic.rs
program T;
{$mode delphi}
uses SysUtils;
...
end; var a: array[0..3] of Integer; p: ^Integer; i, best: Integer; begin a[0]:=3; a[1]:=9; a[2]:=1; a[3]:=7; p:=@a[0]; best:=p^; for i:=1 to 3 do begin Inc(p); if p^>best then best:=p^; end; __p(__vs(best)); __vybeCheck('9');
end.
