// vybe-test: pascal/pointer_arithmetic/pointer_inc_walks_integer_array_forward
...
end; var a: array[0..3] of Integer; p: ^Integer; begin a[0]:=1; a[1]:=2; a[2]:=3; a[3]:=4; p:=@a[0]; __p(__vs(p^)); Inc(p); __p(__vs(p^)); Inc(p); __p(__vs(p^)); __vybeCheck('1' + #10 + '2' + #10 + '3');
end.
