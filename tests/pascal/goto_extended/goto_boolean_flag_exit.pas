// vybe-test: pascal/goto_extended/goto_boolean_flag_exit
// origin: languages/pascal/tests/pascal/test_goto_extended.rs
program T;
{$mode delphi}
uses SysUtils;
...
end; label exit_loop; var i:Integer; done:Boolean; begin i:=0; done:=false; while not done do begin Inc(i); if i=2 then done:=true; if done then goto exit_loop; __p(__vs('tick')); end; exit_loop: __p(__vs('out')); __vybeCheck('tick' + #10 + 'out');
end.
