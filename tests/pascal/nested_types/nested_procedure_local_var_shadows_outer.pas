// vybe-test: pascal/nested_types/nested_procedure_local_var_shadows_outer
// origin: languages/pascal/tests/pascal/test_nested_types.rs
program T;
{$mode delphi}
uses SysUtils;
...
var x: Integer;
  procedure Inner;
  begin x := 5; __p(__vs(x)); end;
begin
  x := 1;
  Inner;
  __p(__vs(x));
end;
begin x := 0; Outer; __p(__vs(x)); __vybeCheck('5' + #10 + '1' + #10 + '0');
end.
