// vybe-test: pascal/idioms_extended/idiom_boolean_and_short_circuit
// origin: languages/pascal/tests/pascal/test_idioms_extended.rs
program T;
{$mode delphi}
uses SysUtils;
...
begin
  called := False;
  if False and Side then __p(__vs('yes'));
  if called then __p(__vs('called')) else __p(__vs('skipped'));
__vybeCheck('skipped');
end.
