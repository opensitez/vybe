// vybe-test: pascal/pascal_binary_tree_traversal/test_tree_level_order_traversal_mock
// origin: languages/pascal/tests/pascal/test_pascal_binary_tree_traversal.rs
program Test;
{$mode delphi}
uses SysUtils;
...
begin
  if root <> nil then __p(__vs(root^.Key));
end;
procedure PrintLevel2(root: PNode);
begin
  if root^.Left <> nil then __p(__vs(root^.Left^.Key));
  if root^.Right <> nil then __p(__vs(root^.Right^.Key));
end;
