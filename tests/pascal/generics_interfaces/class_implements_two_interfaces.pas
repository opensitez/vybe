// vybe-test: pascal/generics_interfaces/class_implements_two_interfaces
// origin: languages/pascal/tests/pascal/test_generics_interfaces.rs
program T;
{$mode delphi}
uses SysUtils;
...
begin
  ...
  nameable := pet;
  describable := pet;
  __p(__vs(nameable.GetName()));
  __p(__vs(describable.Describe()));
__vybeCheck('Rex' + #10 + 'Rex the dog');
end.
