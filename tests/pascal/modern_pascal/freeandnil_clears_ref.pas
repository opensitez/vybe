// vybe-test: pascal/modern_pascal/freeandnil_clears_ref
...
begin
  f := TFoo.Create;
  if Assigned(f) then __p(__vs('assigned'));
  FreeAndNil(f);
  if not Assigned(f) then __p(__vs('freed'));
__vybeCheck('assigned' + #10 + 'freed');
end.
