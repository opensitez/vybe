// vybe-test: pascal/pascal_exceptobject_exceptaddr/test_exceptobject_nested_handlers
// origin: languages/pascal/tests/pascal/test_pascal_exceptobject_exceptaddr.rs
program Test;
{$mode delphi}
...
begin
  try
    try
      ...
      __p(__vs('InnerExceptObject:' + Exception(ExceptObject).Message));
      raise Exception.Create('OuterExc');
    end;
  except
    __p(__vs('OuterExceptObject:' + Exception(ExceptObject).Message));
  end;
__vybeCheck('InnerExceptObject:InnerExc' + #10 + 'OuterExceptObject:OuterExc');
end.
