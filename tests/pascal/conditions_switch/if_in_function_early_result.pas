// vybe-test: pascal/conditions_switch/if_in_function_early_result
// origin: languages/pascal/tests/pascal/test_conditions_switch.rs
program T;
{$mode delphi}
uses SysUtils;
...
end; function Classify(n:Integer):string; begin if n<0 then Result:='neg' else if n=0 then Result:='zero' else Result:='pos'; end; begin __p(__vs(Classify(-1))); __p(__vs(Classify(0))); __p(__vs(Classify(2))); __vybeCheck('neg' + #10 + 'zero' + #10 + 'pos');
end.
