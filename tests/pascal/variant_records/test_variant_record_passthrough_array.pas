// vybe-test: pascal/variant_records/test_variant_record_passthrough_array
...
begin
  ...
  for i := 0 to 1 do begin
    __pw(__vs(IntToStr(items[i].ID) + ':'));
    if items[i].IsPrimary then
      __p(__vs('p=' + IntToStr(items[i].Priority)))
    else
      __p(__vs('s=' + IntToStr(items[i].SecondaryID)));
  end;
__vybeCheck('1:p=10' + #10 + '2:s=99');
end.
