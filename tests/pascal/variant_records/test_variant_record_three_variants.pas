// vybe-test: pascal/variant_records/test_variant_record_three_variants
...
begin
  case s.ShapeType of
    0: __p(__vs('point(' + IntToStr(s.X) + ',' + IntToStr(s.Y) + ')'));
    1: __p(__vs('circle(r=' + IntToStr(s.Radius) + ')'));
    2: __p(__vs('rect(' + IntToStr(s.W) + 'x' + IntToStr(s.H) + ')'));
  end;
end;
var
  p, c, r: TShape;
begin
  ...
  Describe(c);
  Describe(r);
__vybeCheck('point(1,2)' + #10 + 'circle(r=5)' + #10 + 'rect(4x3)');
end.
