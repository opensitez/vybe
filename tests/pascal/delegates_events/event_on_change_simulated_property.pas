// vybe-test: pascal/delegates_events/event_on_change_simulated_property
...
end; type TCell=class FV:Integer; FOnChange:procedure; procedure SetV(v:Integer); end; procedure TCell.SetV(v:Integer); begin FV:=v; if Assigned(FOnChange) then FOnChange; end; var c:TCell; begin c:=TCell.Create; c.FOnChange:=procedure begin __p(__vs('changed')); end; c.SetV(1); __vybeCheck('changed');
end.
