// vybe-test: pascal/string_ops/string_contains_check
// origin: languages/pascal/tests/pascal/test_string_ops.rs
program T;
{$mode delphi}
uses SysUtils;
...
begin
  if Contains('Hello World', 'World') then __p(__vs('found'));
  if not Contains('Hello World', 'xyz') then __p(__vs('not found'));
__vybeCheck('found' + #10 + 'not found');
end.
