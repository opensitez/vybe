// vybe-test: pascal/interfaces_extra/iface_extra_param_2
...
end; type IName=interface function GetName:string; end; TNamed=class(TInterfacedObject,IName) private FN:string; public constructor Create(s:string); function GetName:string; end; constructor TNamed.Create(s:string); begin FN:=s; end; function TNamed.GetName:string; begin Result:=FN; end; procedure Show(const n:IName); begin __p(__vs(n.GetName)); end; var x:IName; begin x:=TNamed.Create('item2'); Show(x); __vybeCheck('item2');
end.
