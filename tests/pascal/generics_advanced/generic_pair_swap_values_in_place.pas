// vybe-test: pascal/generics_advanced/generic_pair_swap_values_in_place
...
end; type TPair<T>=record X,Y:T; end; procedure SwapPair<T>(var p:TPair<T>); var t:T; begin t:=p.X; p.X:=p.Y; p.Y:=t; end; var p:TPair<Integer>; begin p.X:=1; p.Y:=9; SwapPair<Integer>(p); __p(__vs(p.X)); __p(__vs(p.Y)); __vybeCheck('9' + #10 + '1');
end.
