// vybe-test: pascal/generics_advanced/generic_ring_buffer_wrap
...
end; type TRing<T>=class private FIdx:Integer; FVal:T; public constructor Create(v:T); procedure SetNext(v:T); function Get:T; end; constructor TRing<T>.Create(v:T); begin FVal:=v; FIdx:=0; end; procedure TRing<T>.SetNext(v:T); begin FVal:=v; Inc(FIdx); end; function TRing<T>.Get:T; begin Result:=FVal; end; var r:TRing<String>; begin r:=TRing<String>.Create('a'); r.SetNext('b'); __p(__vs(r.Get)); r.Free; __vybeCheck('b');
end.
