// vybe-test: pascal/record_methods_extra/record_method_shift_left
...
end; type TPt=record X,Y:Integer; procedure Shift(dx,dy:Integer); end; procedure TPt.Shift(dx,dy:Integer); begin X:=X+dx; Y:=Y+dy; end; var p:TPt; begin p.X:=0; p.Y:=0; p.Shift(2,3); __p(__vs(p.X)); __p(__vs(p.Y)); __vybeCheck('2' + #10 + '3');
end.
