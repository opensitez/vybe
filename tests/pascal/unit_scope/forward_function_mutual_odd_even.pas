// vybe-test: pascal/unit_scope/forward_function_mutual_odd_even
...
end; function IsEven(n: Integer): Boolean; forward; function IsOdd(n: Integer): Boolean; begin if n=0 then Result:=false else Result:=IsEven(n-1); end; function IsEven(n: Integer): Boolean; begin if n=0 then Result:=true else Result:=IsOdd(n-1); end; begin if IsEven(4) then __p(__vs('even')); __vybeCheck('even');
end.
