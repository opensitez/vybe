// vybe-test: pascal/functions_advanced/overload_procedure_by_param_count
// origin: languages/pascal/tests/pascal/test_functions_advanced.rs
program T;
{$mode delphi}
uses SysUtils;
...
procedure Log(msg: String); begin __p(__vs(msg)); end;
procedure Log(tag, msg: String); begin __p(__vs(tag + ': ' + msg)); end;
begin
  Log('plain');
  Log('warn', 'slow');
__vybeCheck('plain' + #10 + 'warn: slow');
end.
