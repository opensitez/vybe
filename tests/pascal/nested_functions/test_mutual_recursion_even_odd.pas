// vybe-test: pascal/nested_functions/test_mutual_recursion_even_odd
// origin: languages/pascal/tests/pascal/test_nested_functions.rs
program T;
{$mode delphi}
uses SysUtils;
...
begin
  if IsEven(4) then __p(__vs('even')) else __p(__vs('odd'));
  if IsOdd(3) then __p(__vs('odd')) else __p(__vs('even'));
__vybeCheck('even' + #10 + 'odd');
end.
