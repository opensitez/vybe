// vybe-test: pascal/parameter_modifiers/var_open_array_parameter_mutates_elements
// origin: languages/pascal/tests/pascal/test_parameter_modifiers.rs
program T;
{$mode delphi}
uses SysUtils;
...
end; procedure NegateAll(var a: array of Integer); var i: Integer; begin for i := Low(a) to High(a) do a[i] := -a[i]; end; var data: array of Integer; begin data := [1, -2, 3]; NegateAll(data); __p(__vs(data[0])); __p(__vs(data[1])); __vybeCheck('-1' + #10 + '2');
end.
