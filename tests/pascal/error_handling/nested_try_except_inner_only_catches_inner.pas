// vybe-test: pascal/error_handling/nested_try_except_inner_only_catches_inner
// origin: languages/pascal/tests/pascal/test_error_handling.rs
program T;
{$mode delphi}
uses SysUtils;
...
begin
  try
    try
      ...
      on E: Exception do __p(__vs('inner-handled'));
    end;
    __p(__vs('outer-continues'));
  except
    __p(__vs('outer-caught'));
  end;
__vybeCheck('inner-handled' + #10 + 'outer-continues');
end.
