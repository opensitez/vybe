// vybe-test: pascal/pascal_rtti_custom_attributes/test_rtti_custom_attribute_class_declaration
...
begin
  ...
  for attr in t.GetAttributes do
    if attr is TableAttribute then
      __p(__vs(TableAttribute(attr).TableName));
  ctx.Free;
__vybeCheck('users_table');
end.
