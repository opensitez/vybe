// vybe-test: pascal/pascal_rtti_custom_attributes/test_rtti_custom_attribute_multiple_params
// origin: languages/pascal/tests/pascal/test_pascal_rtti_custom_attributes.rs
program Test;
{$mode delphi}
...
begin
  ...
  for attr in p.GetAttributes do
    if attr is ColumnAttribute then
      __p(__vs(ColumnAttribute(attr).Name + ':' + ColumnAttribute(attr).IsNullable.ToString));
  ctx.Free;
__vybeCheck('user_age:True');
end.
