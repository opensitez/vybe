// vybe-test: c/switch_semantics/switch_can_use_enum_like_integer_constants
// origin: languages/c/tests/c/test_switch_semantics.rs
#include <stdio.h>
#include <string.h>
#include <assert.h>
int main() {
const char *__w[] = {"end\n"};
int __n = 1, __i = 0;
enum { START = 10, END = 20 }; int token = END; switch (token) { case START: { char __t[512]; snprintf(__t, sizeof(__t), "%s\n", "start");
  if (__i >= __n || strcmp(__t, __w[__i]) != 0) { printf("FAIL at line %d: got [%s]\n", __i, __t); assert(0); } __i++; } break; case END: { char __t[512]; snprintf(__t, sizeof(__t), "%s\n", "end");
  if (__i >= __n || strcmp(__t, __w[__i]) != 0) { printf("FAIL at line %d: got [%s]\n", __i, __t); assert(0); } __i++; } break; } if (__i != __n) { printf("FAIL: %d line(s), wanted %d\n", __i, __n); assert(0); }
return 0;
}

