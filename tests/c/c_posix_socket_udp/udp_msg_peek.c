// vybe-test: c/c_posix_socket_udp/udp_msg_peek
...
#include <unistd.h>
int main() {const char *__w[] = {"P P"};
int __n = 1, __i = 0;
 int s1 = socket(AF_INET, SOCK_DGRAM, 0); int s2 = socket(AF_INET, SOCK_DGRAM, 0); struct sockaddr_in a1={0}; a1.sin_family = AF_INET; a1.sin_addr.s_addr = htonl(INADDR_LOOPBACK); bind(s1, (struct sockaddr*)&a1, sizeof(a1)); socklen_t l1 = sizeof(a1); getsockname(s1, (struct sockaddr*)&a1, &l1); sendto(s2, "P", 1, 0, (struct sockaddr*)&a1, l1); char b1[2]={0}, b2[2]={0}; recvfrom(s1, b1, 1, MSG_PEEK, NULL, NULL); recvfrom(s1, b2, 1, 0, NULL, NULL); { char __t[512]; snprintf(__t, sizeof(__t), "%s %s", b1, b2);
