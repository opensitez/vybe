// vybe-test: c/c_stdio_vprintf_family/vfscanf_basic
// origin: languages/c/tests/c/test_c_stdio_vprintf_family.rs
#include <stdio.h>
#include <string.h>
#include <assert.h>
#include <stdarg.h>
void wrap_fscanf(FILE *f, const char *fmt, ...) { va_list args; va_start(args, fmt); vfscanf(f, fmt, args); va_end(args); }
int main() {const char *__w[] = {"42"};
int __n = 1, __i = 0;
 FILE *f = fopen("test_vfscanf.txt", "w+"); fputs("42", f); rewind(f); int val = 0; wrap_fscanf(f, "%d", &val); { char __t[512]; snprintf(__t, sizeof(__t), "%d", val);
  if (__i >= __n || strcmp(__t, __w[__i]) != 0) { printf("FAIL at line %d: got [%s]\n", __i, __t); assert(0); } __i++; } fclose(f); if (__i != __n) { printf("FAIL: %d line(s), wanted %d\n", __i, __n); assert(0); }
return 0; }

