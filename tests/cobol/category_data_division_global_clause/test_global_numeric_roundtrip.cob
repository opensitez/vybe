*> vybe-test: cobol/category_data_division_global_clause/test_global_numeric_roundtrip
*> origin: languages/cobol/tests/cobol/test_category_data_division_global_clause.rs
IDENTIFICATION DIVISION. PROGRAM-ID. T. DATA DIVISION. WORKING-STORAGE SECTION.
01 WS-VYBE-L PIC X(256). 01 G PIC S9(3) VALUE 0 IS GLOBAL. PROCEDURE DIVISION. MOVE 5 TO G CALL 'INCR' DISPLAY G.
    MOVE SPACES TO WS-VYBE-L
    STRING G DELIMITED SIZE INTO WS-VYBE-L
    IF WS-VYBE-L NOT = "015"
        DISPLAY "FAIL: want [015] got [" WS-VYBE-L "]"
        MOVE 1 TO RETURN-CODE
        RAISE EXCEPTION EC-PROGRAM
    END-IF. STOP RUN. IDENTIFICATION DIVISION. PROGRAM-ID. INCR. PROCEDURE DIVISION. ADD 10 TO G EXIT PROGRAM. END PROGRAM INCR. END PROGRAM T.

