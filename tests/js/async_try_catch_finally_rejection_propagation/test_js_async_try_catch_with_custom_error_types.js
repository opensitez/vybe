...
async function process(type) {
    try {
        ...
        if (e instanceof ValidationError) __p(__line("ValError: " + e.message));
        else if (e instanceof NetworkError) __p(__line("NetError: " + e.message));
    }
}
(async () => {
    await process("val");
...
__checkLater("ValError: Invalid Input\nNetError: Connection Timeout");
