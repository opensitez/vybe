// vybe-test: js/async_await_error_recovery/async_parallel_map_all_settled_errors
...
async function main(){const r=await Promise.allSettled([1,2].map(async x=>{if(x===2)throw "m"+x;return x;}));__p(__line(r[1].status+":"+r[1].reason));}main();
__checkLater("rejected:m2");
