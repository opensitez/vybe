...
async function main(){let sum=0;for(let i=1;i<=3;i++){try{sum+=await Promise.resolve(i);}catch{sum=-1;}if(i===2)try{await Promise.reject(0);}catch{}}__p(__line(sum));}main();
__checkLater("6");
