...
async function main(){const r=await Promise.allSettled([Promise.reject("a"),Promise.reject("b")]);__p(__line(r.map(x=>x.status).join(",")));}main();
__checkLater("rejected,rejected");
