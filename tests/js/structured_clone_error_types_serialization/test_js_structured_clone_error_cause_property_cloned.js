// vybe-test: js/structured_clone_error_types_serialization/test_js_structured_clone_error_cause_property_cloned
// origin: languages/js/tests/js/test_js_structured_clone_error_types_serialization.rs

function __line(...args) {
    // console.log joins its arguments with a single space. String() is the
    // coercion Vybe's logging host applies to each one.
    return args.map(String).join(" ");
}

function __check(got, want) {
    if (got !== want) {
        console.log("FAIL: want [" + want + "] got [" + got + "]");
        throw new Error("assertion failed");
    }
}

const causeErr = new Error("LowLevelCause");
const mainErr = new Error("HighLevelMain", { cause: causeErr });
const clone = structuredClone(mainErr);

__check(__line((clone.cause !== causeErr) + "|" + (clone.cause.message === "LowLevelCause")), "true|true");
