# vybe-test: powershell/engine_strict_mode/strict_mode_v1_pre_decrement_uninitialized_throws
Set-StrictMode -Version 1.0

$caughtError = $null
try {
    # In strict mode v1+, pre-decrementing an uninitialized variable cannot read it
    --$uninitializedPreDec
} catch {
    $caughtError = $_
}
...
if ($null -eq $caughtError) {
    Write-Host "FAIL: pre-decrementing uninitialized variable did not throw under v1.0"
    exit 1
}
