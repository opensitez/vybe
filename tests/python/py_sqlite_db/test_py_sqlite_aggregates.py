# vybe-test: python/py_sqlite_db/test_py_sqlite_aggregates
...
row = cursor.fetchone()
__check(__line(row[0]), "3")
__check(__line(row[1]), "60")
__check(__line(row[2]), "20.0")
__check(__line(row[3]), "10")
__check(__line(row[4]), "30")
conn.close()
