...
__p(__line(compute(1)))
__p(__line(compute(1)))
__p(__line(compute(2)))
__p(__line(compute(3)))  # evicts 1
__p(__line(compute(1)))  # recomputes 1

info = compute.cache_info()
__p(__line(info.hits))
__p(__line(info.misses))

compute.cache_clear()
info_cleared = compute.cache_info()
__p(__line(info_cleared.currsize))
__check(__buf, "2\n2\n4\n6\n2\n1\n4\n0")
