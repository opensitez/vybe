...
__p(__line(len(token_hex)))      # 32 hex chars for 16 bytes
__p(__line(all(c in "0123456789abcdef" for c in token_hex)))

token_bytes = secrets.token_bytes(16)
__p(__line(len(token_bytes)))

token_url = secrets.token_urlsafe(16)
__p(__line(len(token_url) >= 16))  # url-safe base64 is longer
__check(__buf, "32\nTrue\n16\nTrue")
