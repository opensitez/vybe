# vybe-test: python/py_decorators/test_py_decorator_inspecting_function_annotations
...
def print_annotations(func):
    __p(__line(func.__annotations__))
    return func

@print_annotations
def process(x: int, y: str) -> bool:
    return True
__check(__buf, "{'x': <class 'int'>, 'y': <class 'str'>, 'return': <class 'bool'>}")
