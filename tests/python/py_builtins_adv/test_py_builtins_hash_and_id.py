...
a = "hello"
b = "hello"
__p(__line(hash(a) == hash(b)))  # same content, same hash

x = [1, 2, 3]
y = x
__p(__line(id(x) == id(y)))
__p(__line(id(x) != id(list(x))))

__p(__line(hash(frozenset([1, 2])) == hash(frozenset([2, 1]))))  # order doesn't matter
__check(__buf, "True\nTrue\nTrue\nTrue")
