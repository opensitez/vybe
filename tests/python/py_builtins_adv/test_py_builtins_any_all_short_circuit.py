...
nums = [1, 3, 5, 7]
__p(__line(all(n % 2 != 0 for n in nums)))
__p(__line(any(n > 5 for n in nums)))
__p(__line(all(n > 5 for n in nums)))
__p(__line(any(n % 2 == 0 for n in nums)))
__p(__line(any([])))
__p(__line(all([])))  # vacuously true
__check(__buf, "True\nTrue\nFalse\nFalse\nFalse\nTrue")
