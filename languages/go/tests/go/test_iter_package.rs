...
go_compile_cases! {
    ...
    iter_pull_defer_stop_pattern => "package main; import \"iter\"; func main() { seq := func(yield func(int) bool) { yield(1) }; consume := func() { next, stop := iter.Pull(seq); defer stop(); _, _ = next() }; consume() }",
