...
#[test]
fn cast_integer_passthrough() {
    assert_eq!(
        run_pascal("program T; begin WriteLn(Integer(3.7)); end."),
        &["3.7"]
    );
}
...
fn cast_string() {
    assert_eq!(
        run_pascal("program T; begin WriteLn(String(42)); end."),
        &["42"]
    );
}
...
fn hex_literal_arithmetic() {
    assert_eq!(
        run_pascal("program T; begin WriteLn($10 + $20); end."),
        &["48"]
    );
}
...
fn char_literal_space() {
    assert_eq!(
        run_pascal("program T; begin WriteLn('hello' + #32 + 'world'); end."),
        &["hello world"]
    );
}
...
fn string_double_quote() {
    assert_eq!(
        run_pascal("program T; begin WriteLn('it''s'); end."),
        &["it's"]
    );
}
...
fn string_multiple_escapes() {
    assert_eq!(
        run_pascal("program T; begin WriteLn('he said ''hi'' to me'); end."),
        &["he said 'hi' to me"]
    );
}
...
fn program_heading() {
    assert_eq!(
        run_pascal("program MyApp; begin WriteLn('ok'); end."),
        &["ok"]
    );
}
...
#[test]
fn precedence_div_mod() {
    assert_eq!(
        run_pascal("program T; begin WriteLn(10 div 3); WriteLn(10 mod 3); end."),
        &["3", "1"]
    );
}
...
fn precedence_and_or() {
    assert_eq!(
        run_pascal("program T; begin if true or false then WriteLn('yes'); end."),
        &["yes"]
    );
}
...
fn precedence_not() {
    assert_eq!(
        run_pascal("program T; begin if not false then WriteLn('not false'); end."),
        &["not false"]
    );
}
...
fn shl_operator() {
    assert_eq!(
        run_pascal("program T; begin WriteLn(1 shl 3); end."),
        &["8"]
    );
}
...
fn shr_operator() {
    assert_eq!(
        run_pascal("program T; begin WriteLn(16 shr 2); end."),
        &["4"]
    );
}
...
fn xor_boolean() {
    assert_eq!(
        run_pascal("program T; begin if true xor false then WriteLn('xor works'); end."),
        &["xor works"]
    );
}
