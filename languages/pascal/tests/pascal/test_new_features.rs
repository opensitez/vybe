...
fn forin_array() {
    assert_eq!(
        run_pascal(
            r#"program T; var item: Integer; begin
      for item in [10, 20, 30] do WriteLn(item); end."#
        ),
        &["10", "20", "30"]
    );
}
...
fn forin_string() {
    assert_eq!(
        run_pascal(
            r#"program T; var ch: String; begin
      for ch in 'abc' do WriteLn(ch); end."#
        ),
        &["a", "b", "c"]
    );
}
...
begin a := [5, 10, 15]; for x in a do WriteLn(x); end."#
        ),
        &["5", "10", "15"]
    );
}
...
fn compound_plus() {
    assert_eq!(
        run_pascal("program T; var x: Integer; begin x := 10; x += 5; WriteLn(x); end."),
        &["15"]
    );
}
#[test]
fn compound_minus() {
    assert_eq!(
        run_pascal("program T; var x: Integer; begin x := 10; x -= 3; WriteLn(x); end."),
        &["7"]
    );
}
#[test]
fn compound_mul() {
    assert_eq!(
        run_pascal("program T; var x: Integer; begin x := 5; x *= 3; WriteLn(x); end."),
        &["15"]
    );
}
#[test]
fn compound_div() {
    assert_eq!(
        run_pascal("program T; var x: Real; begin x := 10.0; x /= 4.0; WriteLn(x); end."),
        &["2.5"]
    );
}
#[test]
fn compound_string_concat() {
    assert_eq!(
        run_pascal("program T; var s: String; begin s := 'hello'; s += ' world'; WriteLn(s); end."),
        &["hello world"]
    );
}
...
var d: TDay;
begin d := Wed;
  if d = Wed then WriteLn('midweek') else WriteLn('other'); end."#
        ),
        &["midweek"]
    );
}
...
begin a := TAnimal.Create('Rex');
  if a is TAnimal then WriteLn('yes') else WriteLn('no'); end."#
        ),
        &["yes"]
    );
}
...
var f: TFoo;
begin f := TFoo.Create(42); WriteLn((f as TFoo).FVal); end."#
        ),
        &["42"]
    );
}
...
var f: TFoo;
begin f := TFoo.Create(42); WriteLn(f.GetVal()); end."#
        ),
        &["42"]
    );
}
...
#[test]
fn str_stringreplace() {
    assert_eq!(
        run_pascal(
            "program T; begin WriteLn(StringReplace('hello world', 'world', 'pascal')); end."
        ),
        &["hello pascal"]
    );
}
...
fn str_stringofchar() {
    assert_eq!(
        run_pascal("program T; begin WriteLn(StringOfChar('*', 5)); end."),
        &["*****"]
    );
}
...
fn str_leftstr() {
    assert_eq!(
        run_pascal("program T; begin WriteLn(LeftStr('hello', 3)); end."),
        &["hel"]
    );
}
...
fn typed_const() {
    assert_eq!(
        run_pascal("program T; const MaxSize: Integer = 100; begin WriteLn(MaxSize); end."),
        &["100"]
    );
}

#[test]
fn typed_const_string() {
    assert_eq!(
        run_pascal("program T; const Greeting: String = 'Hello'; begin WriteLn(Greeting); end."),
        &["Hello"]
    );
}
