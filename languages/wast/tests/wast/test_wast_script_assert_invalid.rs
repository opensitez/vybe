...
fn assert_invalid_unknown_memory() {
    parse_ok(r#"(assert_invalid (module (func (result i32) i32.const 0 i32.load)) "unknown memory")"#);
}

#[test]
fn assert_invalid_unknown_table() {
    parse_ok(r#"(assert_invalid (module (func (result funcref) i32.const 0 table.get 0)) "unknown table")"#);
}
...
fn assert_invalid_alignment() {
    parse_ok(r#"(assert_invalid (module (memory 1) (func i32.const 0 i32.load align=8 drop)) "alignment must not be larger than natural")"#);
}

#[test]
fn assert_invalid_constant_expression() {
    parse_ok(r#"(assert_invalid (module (global i32 (i32.add (i32.const 1) (i32.const 2)))) "constant expression required")"#);
}
...
fn assert_invalid_start_function_params() {
    parse_ok(r#"(assert_invalid (module (func $start (param i32)) (start $start)) "start function")"#);
}

#[test]
fn assert_invalid_start_function_results() {
    parse_ok(r#"(assert_invalid (module (func $start (result i32) i32.const 0) (start $start)) "start function")"#);
}

#[test]
fn assert_invalid_duplicate_export() {
    parse_ok(r#"(assert_invalid (module (func (export "a")) (func (export "a"))) "duplicate export name")"#);
}
