...
fn assert_malformed_unclosed_string() {
    parse_ok(
        r#"(assert_malformed (module quote "(module (func (export \"f\"))") "unexpected token")"#,
    );
}
...
fn assert_malformed_unknown_opcode() {
    parse_ok(
        r#"(assert_malformed (module quote "(module (func invalid.opcode))") "unknown operator")"#,
    );
}
...
fn assert_malformed_invalid_integer() {
    parse_ok(
        r#"(assert_malformed (module quote "(module (func i32.const 9999999999999999999999999))") "constant out of range")"#,
    );
}
...
fn assert_malformed_invalid_float() {
    parse_ok(
        r#"(assert_malformed (module quote "(module (func f32.const invalid))") "unknown operator")"#,
    );
}
...
fn assert_malformed_invalid_utf8() {
    parse_ok(
        r#"(assert_malformed (module quote "(module (data \"\\ff\"))") "invalid utf-8 encoding")"#,
    );
}
...
fn assert_malformed_binary_invalid_section() {
    parse_ok(
        r#"(assert_malformed (module binary "\00asm\01\00\00\00\ff\00") "malformed section id")"#,
    );
}
