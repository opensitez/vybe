...
#[test]
fn array_is_list_sequential() {
    assert_eq!(
        run_prints(r#"<?php echo array_is_list([1,2,3]) ? 'yes' : 'no'; "#),
        vec!["yes"]
    );
}
#[test]
fn array_is_list_empty() {
    assert_eq!(
        run_prints(r#"<?php echo array_is_list([]) ? 'yes' : 'no'; "#),
        vec!["yes"]
    );
}
#[test]
fn array_is_list_string_keys() {
    assert_eq!(
        run_prints(r#"<?php echo array_is_list(['a'=>1,'b'=>2]) ? 'yes' : 'no'; "#),
        vec!["no"]
    );
}
#[test]
fn array_is_list_gap_in_keys() {
    assert_eq!(
        run_prints(r#"<?php $a = [0=>1,2=>3]; echo array_is_list($a) ? 'yes' : 'no'; "#),
        vec!["no"]
    );
}
#[test]
fn array_is_list_after_unset() {
    assert_eq!(
        run_prints(r#"<?php $a = [1,2,3]; unset($a[1]); echo array_is_list($a) ? 'yes' : 'no'; "#),
        vec!["no"]
    );
}
...
#[test]
fn readonly_property_throws_on_reassign() {
    assert_eq!(
        run_prints(
            r#"<?php
class User { public function __construct(public readonly string $name) {} }
...
#[test]
fn fiber_passes_value_on_suspend() {
    assert_eq!(
        run_prints(
            r#"<?php
