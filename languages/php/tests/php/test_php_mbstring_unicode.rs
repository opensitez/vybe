...
fn php_mbstring_ascii_framework_strings() {
    ...
    for text in words {
        ...
        assert_output(
            &format!("mb_strtolower({}, 'UTF-8')", quote_php(text)),
            &lower,
        );
        assert_output(
            &format!("mb_strtoupper({}, 'UTF-8')", quote_php(text)),
            &upper,
        );
        assert_int(
            &format!("mb_strlen({}, 'UTF-8')", quote_php(text)),
            len as i64,
        );
...
fn php_mbstring_unicode() {
    ...
    for text in unicode_words.iter() {
        ...
        assert_int(
            &format!(
                "mb_strlen(mb_substr({}, 0, 1, 'UTF-8'), 'UTF-8')",
                quote_php(text)
            ),
            1,
        );
        ...
        if len >= 2 {
            let needle = text.chars().next().expect("non-empty").to_string();
            ...
            assert_int(
                &format!("mb_strpos({}, '{}', 0, 'UTF-8')", quote_php(text), next),
                1,
            );
        }
        ...
        assert_int(
            &format!(
                "strlen(mb_convert_case({}, MB_CASE_UPPER, 'UTF-8'))",
                quote_php(text)
            ),
            len,
        );
