...
#[test]
fn union_type_int_string_param() {
    assert_eq!(
        run_prints(
            r#"<?php
...
fn str_contains_false() {
    assert_eq!(
        run_prints(r#"<?php echo str_contains('Hello World', 'xyz') ? 'yes' : 'no'; "#),
        vec!["no"]
    );
}
#[test]
fn str_starts_with_false() {
    assert_eq!(
        run_prints(r#"<?php echo str_starts_with('Hello World', 'World') ? 'yes' : 'no'; "#),
        vec!["no"]
    );
}
#[test]
fn str_ends_with_false() {
    assert_eq!(
        run_prints(r#"<?php echo str_ends_with('Hello World', 'Hello') ? 'yes' : 'no'; "#),
        vec!["no"]
    );
}
...
#[test]
fn get_debug_type_int() {
    assert_eq!(
        run_prints(r#"<?php echo get_debug_type(42); "#),
        vec!["int"]
    );
}
#[test]
fn get_debug_type_object() {
    assert_eq!(
        run_prints(r#"<?php class Foo {} echo get_debug_type(new Foo); "#),
        vec!["Foo"]
    );
}
#[test]
fn get_debug_type_array() {
    assert_eq!(
        run_prints(r#"<?php echo get_debug_type([]); "#),
        vec!["array"]
    );
}
