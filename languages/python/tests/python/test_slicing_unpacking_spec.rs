...
runtime_case!(
    slice_open_left_runtime,
    "x = [1, 2, 3, 4]\nprint(x[:2][1])\n",
    "2"
);
runtime_case!(
    slice_open_right_runtime,
    "x = [1, 2, 3, 4]\nprint(x[2:][0])\n",
    "3"
);
runtime_case!(
    slice_out_of_bounds_runtime,
    "x = [1, 2, 3]\nprint(len(x[:99]))\n",
    "3"
);
runtime_case!(
    slice_negative_step_runtime,
    "x = [1, 2, 3, 4]\nprint(x[::-1][0])\n",
    "4"
);
runtime_case!(
    string_reverse_runtime,
    "s = 'stressed'\nprint(s[::-1])\n",
    "desserts"
);
runtime_case!(
    string_stride_runtime,
    "s = 'abcdef'\nprint(s[::2])\n",
    "ace"
);
...
runtime_case!(
    slice_assign_expand_runtime,
    "x = [1, 4]\nx[1:1] = [2, 3]\nprint(x[2])\n",
    "3"
);
runtime_case!(
    slice_assign_shrink_runtime,
    "x = [1, 2, 3, 4]\nx[1:3] = [9]\nprint(x[1])\n",
    "9"
);
runtime_case!(
    slice_assign_replace_all_runtime,
    "x = [1, 2, 3]\nx[:] = [7, 8]\nprint(len(x))\n",
    "2"
);
compile_case!(
    slice_assign_step_compile,
    "x = [0, 1, 2, 3, 4, 5]\nx[::2] = [9, 9, 9]\n"
);
runtime_case!(
    del_slice_runtime,
...
compile_case!(
    del_step_slice_compile,
    "x = [1, 2, 3, 4, 5, 6]\ndel x[::2]\n"
);
compile_case!(
    starred_head_tail_compile,
...
compile_case!(
    dict_literal_star_compile,
    "a = {'x': 1}\nb = {'y': 2}\nc = {**a, **b}\n"
);
compile_case!(
    call_multiple_star_compile,
    "def f(a, b, c, d):\n    pass\nleft = [1, 2]\nright = [3, 4]\nf(*left, *right)\n"
);
compile_case!(
    call_star_and_doublestar_compile,
    "def f(a, b, c):\n    pass\nargs = [1]\nkw = {'b': 2, 'c': 3}\nf(*args, **kw)\n"
);
runtime_case!(
    swap_unpack_runtime,
...
compile_case!(
    subscript_with_slice_object_compile,
    "x = [1, 2, 3, 4, 5]\nsl = slice(1, 4)\ny = x[sl]\n"
);
compile_case!(
    unpack_generator_compile,
    "def gen():\n    yield 1\n    yield 2\na, b = gen()\n"
);
