...
//! mechanics come from `vybe_emitter::packing`.
...
fn emit_dynamic_byte_at_const(
    ...
    vybe_emitter::packing::emit_char_code_at_i32_const(chunks, current, data_slot, index, line);
...
fn emit_bytes_from_stack_array(chunks: &mut [Chunk], current: usize, count: u16, line: u32) {
    vybe_emitter::collections::emit_array_new(chunks, current, count, line);
...
fn emit_tuple_from_top(chunks: &mut [Chunk], current: usize, n: u16, line: u32) {
    vybe_emitter::tuples::emit_tuple(chunks, current, n, line);
}
...
fn emit_throw_exception(chunk: &mut Chunk, message: &str, line: u32) {
    ...
    vybe_emitter::errors::emit_exception_new_finalize(chunk, "Exception", line);
...
pub fn emit_struct_pack_into(chunks: &mut Vec<Chunk>, current: usize, argc: u8, line: u32) {
    ...
    for i in 0..4 {
        ...
        chunks[current].emit_op(Op::F64_ADD, line);
        vybe_emitter::packing::emit_char_code_at_i32_const(chunks, current, packed, i, line);
...
pub fn emit_struct_iter_unpack(chunks: &mut Vec<Chunk>, current: usize, argc: u8, line: u32) {
    ...
    vybe_emitter::collections::emit_array_new(chunks, current, 3, line);
    chunks[current].emit_else(line);
    vybe_emitter::collections::emit_array_new(chunks, current, 0, line);
    chunks[current].emit_end(line);
}
