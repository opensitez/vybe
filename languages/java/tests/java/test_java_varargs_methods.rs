...
jm!(overloaded_fixed_and_varargs, r#"System.out.println(Var.echo(1) + "," + Var.echo(1, 2));"#, "static class Var { static int echo(int a) { return 1; } static int echo(int a, int... rest) { return 1 + rest.length; } }", "1,2");
jm!(string_varargs_join, r#"System.out.println(Joiner.join("x", "y", "z"));"#, "static class Joiner { static String join(String first, String... rest) { String out = first; for(String p : rest){ out += p; } return out; } }", "xyz");
...
jm!(varargs_with_cast, r#"System.out.println(Var2.concat("a", 1, 2));"#, "static class Var2 { static String concat(String p, int... values) { String s = p; for (int v: values) { s += v; } return s; } }", "a12");
...
jm!(varargs_and_overload1, r#"System.out.println(Mixer.pick(1) + "," + Mixer.pick(1, 2));"#, r#"static class Mixer { static String pick(int a) { return "one"; } static String pick(int a, int... rest) { return "many"; } }"#, "one,many");
jm!(varargs_and_overload2, r#"System.out.println(Mixer2.pick(1) + "," + Mixer2.pick(1, 2));"#, r#"static class Mixer2 { static String pick(int a) { return "one"; } static String pick(int a, int b) { return "pair"; } static String pick(int a, int b, int c) { return "trip"; } }"#, "one,pair");
jm!(char_array_varargs, "System.out.println(CharVar.text('a', 'b', 'c'));", r#"static class CharVar { static String text(char... values) { String s = ""; for (char c : values) { s += c; } return s; } }"#, "abc");
...
jm!(generic_vararg_like_object, r#"System.out.println(ObjectVar.concat("A", 1, 2));"#, "static class ObjectVar { static String concat(Object first, Object... rest) { String s = String.valueOf(first); for (Object o : rest) { s += String.valueOf(o); } return s; } }", "A12");
jm!(varargs_multiple_calls, r#"System.out.println(Seq.total(1) + "," + Seq.total(1,2) + "," + Seq.total(1,2,3));"#, "static class Seq { static int total(int first, int... rest) { int s = first; for (int v : rest) { s += v; } return s; } }", "1,3,6");
...
jm!(varargs_and_non_varargs_conflict, r#"System.out.println(Pivot.a(1) + "," + Pivot.a(1,2));"#, "static class Pivot { static int a(int a) { return 1; } static int a(int a, int b) { return 2; } static int a(int a, int b, int c) { return 3; } }", "1,2");
