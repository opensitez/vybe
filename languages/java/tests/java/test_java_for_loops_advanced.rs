...
jt!(for_nested_outer, "int s = 0; for (int i = 0; i < 3; i++) { for (int j = 0; j < 2; j++) { s += (i * 2) + j; } } System.out.println(s);", "15");
...
jt!(for_complex_update, "int s = 1; for (int i = 1; i < 4; i *= 2) { s *= i; } System.out.println(s);", "2");
...
jt!(for_condition_changes, "int c = 0; for (int i = 0; i < 10; i++) { c += i; if (c > 10) break; } System.out.println(c);", "15");
