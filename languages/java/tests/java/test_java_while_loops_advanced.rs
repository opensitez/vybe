...
jt!(while_mutating_step, "int i = 1; int p = 1; while (i < 10) { p *= i; i += 2; } System.out.println(p);", "945");
jt!(while_with_boolean_flag, "int i = 0; int c = 0; boolean done = false; while (!done) { c++; if (c == 3) done = true; } System.out.println(c);", "3");
jt!(while_reassign_limit, "int limit = 2; int i = 0; int c = 0; while (i < limit) { c += 2; if (c > 3) limit = 10; i++; } System.out.println(c);", "20");
jt!(while_nested_sum, "int i = 0; int sum = 0; while (i < 4) { int j = 0; while (j <= i) { sum += j; j++; } i++; } System.out.println(sum);", "10");
...
jt!(while_or_condition, "int i = 0; int c = 0; while (i < 3 || c < 1) { c += i; i += 2; } System.out.println(c);", "2");
...
jt!(while_double_guard, "int i = 0; double d = 1.5; while (i < 3 && d > 0) { d -= 0.5; i++; } System.out.println((int)d);", "0");
