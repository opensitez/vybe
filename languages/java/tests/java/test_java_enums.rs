...
jm!(enum_switch, "System.out.println(sw(Color.YELLOW));", "enum Color { RED, GREEN, YELLOW, BLUE } static int sw(Color c) { switch(c){case RED:return 1;case GREEN:return 2;case YELLOW:return 3;default:return 0;} }", "3");
...
jm!(enum_sum_code, "System.out.println(Status2.sumCodes());", "enum Status2 { A(1), B(2), C(3); int code; Status2(int code) { this.code = code; } static int sumCodes() { int s=0; for(Status2 s2: values()) s += s2.code; return s; } }", "6");
...
jm!(enum_in_ctor, "System.out.println(Mode8.on(Mode8.HIGH));", "enum Mode8 { LOW(1), HIGH(2); int value; Mode8(int value) { this.value = value; } static int on(Mode8 m) { return m.value; } }", "2");
...
jm!(enum_ternary, r#"System.out.println(Priority.LOW.code + "," + Priority.HIGH.code);"#, "enum Priority { LOW(1), HIGH(10); int code; Priority(int code){ this.code = code; } }", "1,10");
...
jm!(enum_with_static_lookup, "System.out.println(Tag.fromCode(2));", "enum Tag { A(1), B(2); int code; Tag(int c){ code = c; } static Tag fromCode(int c) { for (Tag t: values()) if (t.code == c) return t; return null; } }", "B");
...
jm!(enum_plus_name, r#"System.out.println(Trait.A + "," + Trait.B);"#, "enum Trait { A, B }", "A,B");
