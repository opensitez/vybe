...
jm!(generic_array_list_emulation, "PairList p = new PairList(1,2); System.out.println(p.first() + p.second());", "static class PairBase<T> { T a; T b; PairBase(T a, T b){this.a=a; this.b=b;} T first(){ return a; } T second(){ return b; } } static class PairList extends PairBase<Integer> { PairList(Integer a, Integer b){ super(a,b); } }", "3");
