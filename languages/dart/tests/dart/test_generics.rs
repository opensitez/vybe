...
#[test]
fn generic_box() {
    compile_ok("class Box<T> { T value; Box(this.value); }");
}
#[test]
fn generic_pair() {
    compile_ok("class Pair<A, B> { A first; B second; Pair(this.first, this.second); }");
}
#[test]
fn generic_stack() {
    compile_ok(
...
#[test]
fn generic_fn() {
    compile_ok("T identity<T>(T val) => val;");
}
#[test]
fn generic_fn_two() {
    compile_ok("B transform<A, B>(A val, B Function(A) fn) => fn(val);");
}
#[test]
fn generic_fn_list() {
    compile_ok("List<T> repeat<T>(T val, int times) => List.generate(times, (_) => val);");
}

#[test]
fn generic_fn_result() {
    let out = run_prints("T id<T>(T v) => v; void main() { print(id(42)); }");
    assert_eq!(out, ["42"]);
}

#[test]
fn generic_fn_string_result() {
...
#[test]
fn list_int_typed() {
    compile_ok("List<int> nums = [1, 2, 3];");
}
#[test]
fn list_string_typed() {
    compile_ok("List<String> names = ['Alice', 'Bob'];");
}
#[test]
fn list_bool_typed() {
    compile_ok("List<bool> flags = [true, false, true];");
}
#[test]
fn map_string_int() {
    compile_ok("Map<String, int> scores = {'Alice': 90, 'Bob': 85};");
}
#[test]
fn map_int_list() {
    compile_ok("Map<int, List<String>> grouped = {1: ['a', 'b'], 2: ['c']};");
}
#[test]
fn set_int_typed() {
    compile_ok("Set<int> nums = {1, 2, 3};");
}
#[test]
fn set_string_typed() {
    compile_ok("Set<String> tags = {'dart', 'flutter'};");
}

#[test]
fn typed_list_operations() {
...
#[test]
fn generic_typedef() {
    compile_ok("typedef Mapper<T, R> = R Function(T);");
}
#[test]
fn generic_typedef_used() {
    compile_ok(
...
#[test]
fn future_typed() {
    compile_ok("Future<int> f = Future.value(42);");
}
#[test]
fn iterable_typed() {
    compile_ok("Iterable<int> it = [1, 2, 3];");
}
