...
fn equality_operator_compares_value_type_fields() {
    assert_eq!(
        run_csharp(
            r#"struct Color{public int R,G,B;
...
fn comparison_operators_less_and_greater() {
    assert_eq!(
        run_csharp(
            r#"class Weight:System.IComparable<Weight>{
public int Kg;
...
fn implicit_conversion_from_int_to_custom_type() {
    assert_eq!(
        run_csharp(
            r#"struct Meters{public double Value;
...
fn explicit_conversion_to_primitive_requires_cast() {
    assert_eq!(
        run_csharp(
            r#"struct Percent{public double Value;
