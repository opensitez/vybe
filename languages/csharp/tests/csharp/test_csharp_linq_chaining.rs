...
fn where_select_order_take_pipeline() {
    assert_eq!(
        run_csharp(
            r#"var result=new[]{5,3,8,1,9,2,7,4,6}
    .Where(n=>n>3)
    .Select(n=>n*n)
    .OrderBy(n=>n)
    .Take(3);
...
fn group_by_select_count_per_group() {
    assert_eq!(
        run_csharp(
            r#"var words=new[]{"cat","car","bar","bat","can"};
...
fn select_many_with_index_flattens_and_annotates() {
    assert_eq!(
        run_csharp(
            r#"var groups=new[]{new[]{1,2},new[]{3,4}};
...
fn chained_order_by_then_by_sorts_on_two_keys() {
    assert_eq!(
        run_csharp(
            r#"var data=new[]{(A:"b",B:2),(A:"a",B:3),(A:"a",B:1)};
...
fn zip_three_sequences_pairwise_sum() {
    assert_eq!(
        run_csharp(
            r#"var a=new[]{1,2,3}; var b=new[]{10,20,30};
