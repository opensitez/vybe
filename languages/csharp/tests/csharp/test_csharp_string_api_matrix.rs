...
fn string_matrix_basic_queries() {
    let words = ["", "a", "A", "ab", "abc", "foo", "bar", "baz", "xYz", "JSON", " space ", "Test"];
    ...
    for left in words {
        for right in words {
            ...
            assert_eq!(run_csharp(&len_src), &[ (left.len() + right.len()).to_string().as_str() ]);
            ...
            assert_eq!(run_csharp(&contains_src), &[ bool_text(left.contains(right)) ]);
            ...
            assert_eq!(run_csharp(&starts_src), &[ bool_text(left.starts_with(right)) ]);
...
fn string_matrix_case_and_trim_behaviors() {
    let words = ["", "a", "Abc", "foo", "Bar", " xYz ", "  ", "upper", "Lower", "JSON", "test", "mix-ed"]; 

    for word in words {
        let upper_src = format!("string a = {}; Console.WriteLine(a.ToUpperInvariant());", quote(word));
        assert_eq!(run_csharp(&upper_src), &[word.to_uppercase().as_str()]);

        let lower_src = format!("string a = {}; Console.WriteLine(a.ToLowerInvariant());", quote(word));
        assert_eq!(run_csharp(&lower_src), &[word.to_lowercase().as_str()]);

        let trim_src = format!("string a = {}; Console.WriteLine('[' + a.Trim() + ']');", quote(word));
        let expected_trim = format!("[{}]", word.trim());
        assert_eq!(run_csharp(&trim_src), &[expected_trim.as_str()]);

        let trim_start_src = format!("string a = {}; Console.WriteLine('[' + a.TrimStart() + ']');", quote(word));
        let expected_trim_start = format!("[{}]", word.trim_start());
        assert_eq!(run_csharp(&trim_start_src), &[expected_trim_start.as_str()]);

        let trim_end_src = format!("string a = {}; Console.WriteLine('[' + a.TrimEnd() + ']');", quote(word));
...
fn string_matrix_replace_index_and_substring() {
    let words = ["", "a", "aa", "aba", "alpha", "beta", "gamma", "foo", "bar", "xyz", "xYz", "end"]; 
    ...
    for source in words {
        for needle in needles {
            ...
            assert_eq!(run_csharp(&replace_src), &[replaced.len().to_string().as_str()]);
        }
        ...
        if source.len() > 0 {
            ...
            let last_src = format!("string s = {}; Console.WriteLine(s.Substring(s.Length - 1, 1));", quote(source));
