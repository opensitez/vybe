...
fn for_loop_even_odd_sum_matrix() {
    ...
    for limit in limits {
        ...
        assert_eq!(
            run_csharp(&src),
            vec![expected_sum_even.to_string(), expected_count_even.to_string()]
        );
    }
}
...
fn while_loop_halving_and_step_count_matrix() {
    ...
    for start in starts {
        ...
        assert_eq!(run_csharp(&src), vec![expected_steps.to_string(), value.to_string()]);
    }
}
...
fn nested_loop_matrix_dot_product() {
    ...
    for (rows, cols) in sizes {
        ...
        let expected_sum = (0..rows).flat_map(|r| (0..cols).map(move |c| r * c)).sum::<i32>();
