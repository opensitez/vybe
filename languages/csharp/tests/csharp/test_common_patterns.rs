...
#[test]
fn math_portable_stdlib_functions() {
    assert_eq!(
        run_csharp(
            r#"
...
#[test]
fn math_wasm_imports_avoid_vybe_math() {
    let wasm = compile_csharp_to_wasm(
        r#"
Console.WriteLine(Math.Sin(0));
Console.WriteLine(Math.Log(1));
Console.WriteLine(Math.Exp(0));
...
"#,
    ...
    assert!(
        !imports.iter().any(|(module, name)| module == "vybe:math"
            && ["sin", "log", "exp", "clamp"].contains(&name.as_str())),
        "expected portable math lowering, found vybe:math imports: {:?}",
        imports
    );
    assert!(
        imports
            .iter()
            .any(|(module, name)| module == "env" && name == "sin"),
        "expected stdlib/env import for sin, imports were {:?}",
        imports
    );
}
...
#[test]
fn property_with_backing_field() {
    assert_eq!(
        run_csharp(
            r#"
class Temperature {
