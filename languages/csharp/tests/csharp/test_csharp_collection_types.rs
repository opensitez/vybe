...
fn sorted_set_maintains_unique_sorted_elements() {
    assert_eq!(
        run_csharp(
            r#"var s=new System.Collections.Generic.SortedSet<int>{3,1,4,1,5};
...
fn sorted_set_remove_eliminates_element() {
    assert_eq!(
        run_csharp(
            r#"var s=new System.Collections.Generic.SortedSet<int>{1,2,3};
s.Remove(2);
Console.WriteLine(s.Count);"#
        ),
        &["2"]
...
fn observable_collection_collection_changed_fires_on_add() {
    assert_eq!(
        run_csharp(
            r#"var oc=new System.Collections.ObjectModel.ObservableCollection<int>();
int count=0;
...
fn queue_enqueue_dequeue_maintains_fifo() {
    assert_eq!(
        run_csharp(
            r#"var q=new System.Collections.Generic.Queue<string>();
...
fn stack_push_pop_maintains_lifo() {
    assert_eq!(
        run_csharp(
            r#"var s=new System.Collections.Generic.Stack<string>();
s.Push("a"); s.Push("b");
...
fn linked_list_add_after_inserts_between_nodes() {
    assert_eq!(
        run_csharp(
            r#"var ll=new System.Collections.Generic.LinkedList<int>();
...
fn sorted_dictionary_first_key_is_smallest() {
    assert_eq!(
        run_csharp(
            r#"var sd=new System.Collections.Generic.SortedDictionary<int,string>{{3,"c"},{1,"a"},{2,"b"}};
