...
fn params_accepts_variable_number_of_arguments() {
    assert_eq!(
        run_csharp(
            r#"int Sum(params int[] ns){int s=0;foreach(var n in ns)s+=n;return s;}
...
fn optional_parameter_uses_default_when_omitted() {
    assert_eq!(
        run_csharp(
            r#"string Greet(string name, string prefix="Hello") => prefix+" "+name;
...
fn named_argument_can_be_passed_out_of_order() {
    assert_eq!(
        run_csharp(
            r#"string Concat(string a, string b, string c) => a+b+c;
...
fn optional_with_null_default_allows_omission() {
    assert_eq!(
        run_csharp(
            r#"string Label(string text, string tag=null) => tag==null?text:$"[{tag}]{text}";
