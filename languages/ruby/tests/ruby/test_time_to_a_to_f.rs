...
ruby_test!(
    test_time_to_a_basic,
    "puts Time.utc(2000, 1, 1).to_a.length",
    "10"
); // [sec, min, hour, day, month, year, wday, yday, isdst, zone]
ruby_test!(
    test_time_to_a_values,
    "a = Time.utc(2000, 1, 2, 3, 4, 5).to_a; puts \"#{a[0]}-#{a[1]}-#{a[2]}-#{a[3]}-#{a[4]}-#{a[5]}\"",
    "5-4-3-2-1-2000"
); // sec-min-hour-day-mon-year
ruby_test!(
    test_time_to_a_dst_zone,
    "a = Time.utc(2000, 1, 1).to_a; puts \"#{a[8]}-#{a[9]}\"",
    "false-UTC"
);
