...
ruby_test!(
    test_complex_polar,
    "puts Complex(3, 4).polar.class.name",
    "Array"
);
ruby_test!(
    test_complex_rect,
    "puts Complex(3, 4).rect.join('-')",
    "3-4"
);
ruby_test!(test_complex_to_s, "puts Complex(1, -2).to_s", "1-2i");
ruby_test!(
    test_complex_compare,
    "puts Complex(1, 2) == Complex(1, 2)",
    "true"
);
