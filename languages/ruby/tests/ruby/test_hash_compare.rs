...
ruby_test!(
    test_hash_compare_eq,
    "puts ({a: 1, b: 2} == {b: 2, a: 1})",
    "true"
); // order independent
ruby_test!(
    test_hash_compare_not_eq,
    "puts ({a: 1, b: 2} == {a: 1, b: 3})",
    "false"
);
ruby_test!(
    test_hash_compare_size,
    "puts ({a: 1} == {a: 1, b: 2})",
    "false"
);
...
ruby_test!(
    test_hash_compare_disjoint,
    "puts ({a: 1} < {b: 2}).nil?",
    "false"
); // Wait, disjoint sets return false for <
ruby_test!(
    test_hash_compare_any_basic,
