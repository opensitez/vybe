...
ruby_test!(
    test_gc_stat_key,
    "puts GC.stat(:count).class.name",
    "Integer"
);
ruby_test!(test_gc_count, "puts GC.count.class.name", "Integer");
ruby_test!(
    test_gc_latest_gc_info,
    "puts GC.latest_gc_info.class.name",
    "Hash"
);
ruby_test!(
    test_gc_latest_gc_info_key,
    "puts GC.latest_gc_info(:major_by).class.name",
    "Symbol"
); // nil or symbol
