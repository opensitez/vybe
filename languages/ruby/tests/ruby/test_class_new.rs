...
ruby_test!(
    ...
    "c = Class.new; puts c.class.name",
    "Class"
);
ruby_test!(
    test_class_new_superclass,
    "c = Class.new(Array); puts c.new.is_a?(Array)",
    "true"
);
ruby_test!(
    test_class_new_block,
    "c = Class.new { def foo; 42; end }; puts c.new.foo",
    "42"
);
ruby_test!(
    test_class_allocate,
