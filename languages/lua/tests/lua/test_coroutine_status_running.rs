...
print(coroutine.resume(t) == true)"#), "true");
}
...
fn test_running_inside_two_threads() {
    assert_eq!(run_lua_one(r#"local inner_state
...
fn test_running_returns_thread_not_main_when_in_coroutine() {
    assert_eq!(run_lua_one(r#"local stored
local t = coroutine.create(function()
...
print(ok == true and isMain == false)"#), "true");
}

#[test]
fn test_running_status_during_dead() {
    assert_eq!(run_lua_one(r#"local th, main = coroutine.running()
print(main == true)"#), "true");
}

#[test]
fn test_running_after_resume() {
    assert_eq!(run_lua_one(r#"local seen
local t = coroutine.create(function()
  local _, main = coroutine.running()
  seen = main
  return 1
...
coroutine.resume(t)
print(seen == false)"#), "true");
}

#[test]
fn test_running_nested_yield() {
    assert_eq!(run_lua_one(r#"local seen = false
local t = coroutine.create(function()
  local _, main = coroutine.running()
  coroutine.yield(main)
  seen = true
...
fn test_running_multiple_coroutines() {
    assert_eq!(run_lua_one(r#"local c1 = coroutine.create(function() return coroutine.running() ~= nil end)
...
print(a == true and b == true)"#), "true");
}

#[test]
fn test_running_function_name() {
    assert_eq!(run_lua_one(r#"local t = coroutine.create(function() coroutine.running() end)
local _, first = coroutine.resume(t)
print(first == nil)"#), "true");
}

#[test]
fn test_running_after_completion() {
    assert_eq!(run_lua_one(r#"local done = false
local t = coroutine.create(function()
  local _, main = coroutine.running()
  done = not main
end)
...
fn test_running_when_no_coroutine() {
    assert_eq!(run_lua_one(r#"local _, main = coroutine.running()
print(main)"#), "true");
}
