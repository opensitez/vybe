lua_print! {
    ...
    test_pack_unpack_i4 => { "local s = string.pack('<i4', 1000); local v, p = string.unpack('<i4', s); print(v..' '..p)", "1000 5" },
    ...
    test_pack_unpack_fixed_string => { "local s = string.pack('z', 'hello'); local v, p = string.unpack('z', s); print(v..' '..p)", "hello 7" },
    test_pack_unpack_sized_string => { "local s = string.pack('z', 'hello'); local v, p = string.unpack('z', s); print(v..' '..p)", "hello 7" },
    test_pack_unpack_multiple => { "local s = string.pack('bBi', 42, 10, 3); local v1, v2, v3, p = string.unpack('bBi', s); print(v1..' '..v2..' '..v3..' '..p)", "42 10 3 4" },
    ...
    test_pack_invalid_format => { "local ok = pcall(function() string.pack('q', 1) end); print(tostring(ok))", "true" },
