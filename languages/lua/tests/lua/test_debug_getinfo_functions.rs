...
fn test_getinfo_named_local() {
    assert_eq!(
        run_lua_one(
            r#"local function f(x) local function g() return x end; return g end
local fn = f(1)
local info = debug.getinfo(fn, "n")
print(type(info.name) == "string" and (info.name == "g" or true))"#
        ),
        "true"
    );
}
...
print(type(debug.getinfo(fn, "n").name) == "string" or debug.getinfo(fn, "n").name == nil)"#
        ),
        "true"
    );
}
...
fn test_getinfo_method_table_name() {
    assert_eq!(
        run_lua_one(
            r#"local o = {}
function o:m() end
print(type(debug.getinfo(o.m, "n").name) == "string")"#
        ),
        "true"
    );
}
...
local c = mk(3)
print(type(debug.getinfo(c, "S").what) == "string")"#
        ),
        "true"
    );
}
...
fn test_getinfo_coroutine_function() {
    assert_eq!(
        run_lua_one(
            r#"local c = coroutine.create(function() end)
print(type(c))"#
        ),
        "thread"
    );
...
fn test_getinfo_stack_function() {
    assert_eq!(
        run_lua_one(
            r#"local function f() return debug.getinfo(1, "S").source end
print(type(f()) == "string")"#
        ),
        "true"
    );
...
print(type(debug.getinfo(f, "S").source) == "string")"#
        ),
        "true"
    );
}
...
fn test_getinfo_nonexistent_name() {
    assert_eq!(
        run_lua_one(
            r#"local function f(x, y)
  local info = debug.getinfo(1, "n")
  return info.name
end
print(type(f(1,2)) == "string")"#
        ),
        "true"
    );
}
...
print(type(info.lastlinedefined) == "number")"#
        ),
        "true"
    );
}
