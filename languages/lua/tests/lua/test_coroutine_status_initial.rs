...
print(coroutine.status(t) == "suspended")"#
        ),
        "true"
    );
}
...
fn test_status_after_yield_running() {
    assert_eq!(
        run_lua_one(
            r#"local t = coroutine.create(function() coroutine.yield(1) end)
coroutine.resume(t)
print(coroutine.status(t) == "suspended")"#
        ),
        "true"
...
fn test_status_create_multiple_threads() {
    assert_eq!(
        run_lua_one(
            r#"local a = coroutine.create(function() return 1 end)
local b = coroutine.create(function() return 2 end)
print(coroutine.status(a) .. "/" .. coroutine.status(b))"#
        ),
        "suspended/suspended"
    );
}
...
fn test_status_create_zero_arg() {
    assert_eq!(
        run_lua_one(
            r#"local t = coroutine.create(function(a) return a end)
print(coroutine.status(t) == "suspended")"#
        ),
        "true"
    );
...
fn test_status_resume_then_status() {
    assert_eq!(
        run_lua_one(
            r#"local t = coroutine.create(function(x) return x end)
print((function()
...
fn test_status_yield_and_send() {
    assert_eq!(
        run_lua_one(
            r#"local t = coroutine.create(function(v) coroutine.yield(v) return v end)
...
coroutine.resume(t2)
print(coroutine.status(t2) == "dead")"#
        ),
        "true"
    );
}
...
fn test_status_empty_resume_error() {
    assert_eq!(
        run_lua_one(
            r#"local t = coroutine.create(function() return 1/0 end)
...
fn test_status_yield_payload() {
    assert_eq!(
        run_lua_one(
            r#"local t = coroutine.create(function() coroutine.yield(0) end)
coroutine.resume(t)
print(coroutine.status(t) == "suspended")"#
        ),
        "true"
...
fn test_status_after_argument_resume() {
    assert_eq!(
        run_lua_one(
            r#"local t = coroutine.create(function(a)
