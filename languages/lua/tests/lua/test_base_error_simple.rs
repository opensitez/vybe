...
fn test_error_only_message() {
    assert_eq!(
        run_lua_one(
            r#"local ok, err = pcall(function() error("message") end)
print(err == "message")"#
        ),
        "true"
    );
...
fn test_error_empty_message() {
    assert_eq!(
        run_lua_one(
            r#"local ok, err = pcall(function() error("") end)
print(err == "")"#
        ),
        "true"
    );
...
fn test_error_number_message() {
    assert_eq!(
        run_lua_one(
            r#"local ok, err = pcall(function() error(15) end)
print(err == 15)"#
        ),
        "true"
    );
...
fn test_error_table_message() {
    assert_eq!(
        run_lua_one(
            r#"local ok, err = pcall(function() error({label = "x"}) end)
...
fn test_error_function_message() {
    assert_eq!(
        run_lua_one(
            r#"local marker = function() return 1 end
...
fn test_error_table_field_accessible() {
    assert_eq!(
        run_lua_one(
            r#"local payload = {code = 500}
local ok, err = pcall(function() error(payload) end)
print(type(err) == "table" and err.code == 500)"#
        ),
        "true"
    );
}
...
fn test_error_concat_with_message() {
    assert_eq!(
        run_lua_one(
            r#"local ok, err = pcall(function() error("a" .. "b") end)
print(err == "ab")"#
        ),
        "true"
    );
...
fn test_error_nested_return() {
    assert_eq!(
        run_lua_one(
            r#"local function f() error("ret") end
...
fn test_error_with_conditional_true() {
    assert_eq!(
        run_lua_one(
            r#"local ok, err = pcall(function() if true then error("cond") end end)
print(err == "cond")"#
        ),
        "true"
    );
...
fn test_error_with_conditional_false() {
    assert_eq!(
        run_lua_one(
            r#"local ok, err = pcall(function() if false then error("cond") else return true end end)
print(ok == true)"#
        ),
        "true"
    );
...
fn test_error_from_assert() {
    assert_eq!(
        run_lua_one(
            r#"local ok, err = pcall(function() assert(false, "asserted") end)
...
fn test_error_from_pcall() {
    assert_eq!(
        run_lua_one(
            r#"local ok, err = pcall(function() pcall(function() error("x") end) end)
print(ok == true)"#
        ),
        "true"
    );
...
fn test_error_long_text() {
    assert_eq!(
        run_lua_one(
            r#"local text = string.rep("e", 16)
local ok, err = pcall(function() error(text) end)
print(string.len(err) == 16)"#
        ),
        "true"
    );
}
...
fn test_error_with_newline() {
    assert_eq!(
        run_lua_one(
            r#"local ok, err = pcall(function() error("line1\\nline2") end)
print(string.find(err, "line1") ~= nil)"#
        ),
        "true"
    );
}
...
fn test_error_in_inner_function() {
    assert_eq!(
        run_lua_one(
            r#"local fn = function() error("innerfn") end
