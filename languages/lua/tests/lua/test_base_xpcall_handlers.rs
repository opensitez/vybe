...
fn test_xpcall_passes_function() {
    assert_eq!(
        run_lua_one(
            r#"local function ok() return 1 end
...
fn test_xpcall_handler_catches_error() {
    assert_eq!(
        run_lua_one(
            r#"local function bad() error("bad") end
local function handler(err) return "handled" end
local ok, v = xpcall(bad, handler)
print(ok == false and v == "handled")"#
        ),
        "true"
    );
}
...
fn test_xpcall_handler_receives_table() {
    assert_eq!(
        run_lua_one(
            r#"local function bad() error({m = 2}) end
local function handler(err) return err.m end
local ok, v = xpcall(bad, handler)
print(ok == false and v == 2)"#
        ),
        "true"
    );
}
...
fn test_xpcall_handler_with_args() {
    assert_eq!(
        run_lua_one(
            r#"local function good(x) return x * 2 end
...
fn test_xpcall_error_with_zero() {
    assert_eq!(
        run_lua_one(
            r#"local function bad() error(0) end
local function handler(err) return err + 1 end
local ok, v = xpcall(bad, handler)
print(ok == false and v == 1)"#
        ),
        "true"
    );
}
...
fn test_xpcall_error_with_false() {
    assert_eq!(
        run_lua_one(
            r#"local function bad() error(false) end
local function handler(err) return tostring(err) end
local ok, v = xpcall(bad, handler)
print(ok == false and v == "false")"#
        ),
        "true"
    );
}
...
fn test_xpcall_error_with_true() {
    assert_eq!(
        run_lua_one(
            r#"local function bad() error(true) end
...
print(ok and status == false and val == "handled")"#
        ),
        "true"
    );
}
...
fn test_xpcall_multi_result() {
    assert_eq!(
        run_lua_one(
            r#"local function good() return 1, 2 end
local function handler(err) return 0 end
...
fn test_xpcall_handler_boolean_result() {
    assert_eq!(
        run_lua_one(
            r#"local function bad() error("x") end
...
fn test_xpcall_error_message_length() {
    assert_eq!(
        run_lua_one(
            r#"local function bad() error("abcde") end
local function handler(err) return string.len(err) end
local ok, v = xpcall(bad, handler)
print(ok == false and v == 5)"#
        ),
        "true"
    );
}
...
fn test_xpcall_assert_payload() {
    assert_eq!(
        run_lua_one(
            r#"local function bad() assert(false, "bad") end
...
print(ok == false and string.find(v, "!") ~= nil)"#
        ),
        "true"
    );
}
...
fn test_xpcall_handler_reads_level() {
    assert_eq!(
        run_lua_one(
            r#"local function bad() error("lvl") end
...
fn test_xpcall_error_function_argument() {
    assert_eq!(
        run_lua_one(
            r#"local function bad() error(function() return 1 end) end
...
local ok, v = xpcall(bad, handler)
print(ok == false and string.find(v, "deep") ~= nil)"#
        ),
        "true"
    );
}
