lua_print! {
    ...
    test_debug_getinfo_invalid_level => { "local info = debug.getinfo(100); print(type(info))", "table" },
    test_debug_getinfo_fields => { "local info = debug.getinfo(1, 'nlS'); print(type(info.name)..' '..type(info.currentline)..' '..type(info.short_src))", "string number string" },
