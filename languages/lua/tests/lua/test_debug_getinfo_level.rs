...
fn test_getinfo_zero_level_fails() {
    assert_eq!(run_lua_one(r#"local info = debug.getinfo(0, "n")
print(type(info) == "table" or type(info) == "nil")"#), "true");
}
...
print(type(debug.getinfo(1, "u").isvararg) == "boolean")"#), "true");
}
...
fn test_getinfo_invalid_mode() {
    assert_eq!(run_lua_one(r#"local info = debug.getinfo(1, "xyz")
print(type(info) == "table" or type(info) == "nil")"#), "true");
}
