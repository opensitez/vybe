...
print(type(info.source))"#), "string");
}

#[test]
fn test_getinfo_current_linedefined() {
    assert_eq!(run_lua_one(r#"local info = debug.getinfo(1, \"S\")
print(type(info.linedefined) == \"number\")"#), "true");
}

#[test]
fn test_getinfo_current_namewhat() {
    assert_eq!(run_lua_one(r#"local info = debug.getinfo(1, \"n\")
print(info.namewhat == \"\" or type(info.namewhat) == \"string\")"#), "true");
}
...
fn test_getinfo_zero_level_fails() {
    assert_eq!(run_lua_one(r#"local ok, info = pcall(debug.getinfo, 0, \"n\")
print(ok)"#), "true");
}

#[test]
fn test_getinfo_negative_level() {
    assert_eq!(run_lua_one(r#"local ok, info = pcall(debug.getinfo, -1, \"n\")
print(ok == false or type(info) == \"nil\")"#), "true");
}

#[test]
fn test_getinfo_function_arg() {
    assert_eq!(run_lua_one(r#"local function g() end
local info = debug.getinfo(g, \"n\")
print(type(info.name) == \"string\")"#), "true");
}
...
print(f(1,2,3) > 0)"#), "true");
}

#[test]
fn test_getinfo_nparams_zero() {
    assert_eq!(run_lua_one(r#"local function f() return debug.getinfo(1, \"u\").nparams end
print(f())"#), "0");
}
...
print(type(info.currentline) == \"number\")"#), "true");
}
...
print(type(g()) == \"string\")"#), "true");
}

#[test]
fn test_getinfo_invalid_mode() {
    assert_eq!(run_lua_one(r#"local ok, info = pcall(debug.getinfo, 1, \"xyz\")
print(ok == true or type(info) == \"nil\")"#), "true");
}
...
fn test_getinfo_what() {
    assert_eq!(run_lua_one(r#"local info = debug.getinfo(1, \"S\")
print(info.what == \"Lua\" or info.what == \"C\")"#), "true");
}
