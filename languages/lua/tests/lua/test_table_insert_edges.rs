...
fn test_table_insert_edges_baseline() {
    assert_eq!(
        run_lua_one(r#"local t = {1,2,3}; table.insert(t, 2, 1); print(t[2] == 1)"#),
        "true"
    );
}
...
fn test_table_insert_edges_simple() {
    assert_eq!(
        run_lua_one(r#"local t = {1,2,3}; table.insert(t, 3, 2); print(t[3] == 2)"#),
        "true"
    );
}
...
fn test_table_insert_edges_trimmed() {
    assert_eq!(
        run_lua_one(r#"local t = {1,2,3}; table.insert(t, 4, 3); print(t[4] == 3)"#),
        "true"
    );
}
...
fn test_table_insert_edges_decimal() {
    assert_eq!(
        run_lua_one(
            r#"local t = {1,2,3}; local ok = pcall(function() table.insert(t, 5, 4) end); print(ok)"#
        ),
        "false"
    );
}
...
fn test_table_insert_edges_unicode_like() {
    assert_eq!(
        run_lua_one(
            r#"local t = {1,2,3}; local ok = pcall(function() table.insert(t, 21, 20) end); print(ok)"#
        ),
        "false"
    );
}
