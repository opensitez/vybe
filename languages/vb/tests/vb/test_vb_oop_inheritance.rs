...
#[test]
fn inherit_mybase_not_allowed_in_module() {
    assert_eq!(
        run_vb(
...
} // MyClass bypasses override
#[test]
fn inherit_me_method() {
    assert_eq!(
        run_vb(
...
} // Me respects override
#[test]
fn inherit_multiple_levels() {
    assert_eq!(
        run_vb(
            "Class A\nPublic V As Integer = 1\nEnd Class\nClass B\nInherits A\nEnd Class\nClass C\nInherits B\nEnd Class\nModule M\nSub Main()\nDim c1 As New C()\nConsole.WriteLine(c1.V)\nEnd Sub\nEnd Module"
        ),
...
#[test]
fn inherit_multiple_classes_fails() {
    assert_eq!(
        run_vb(
...
#[test]
fn inherit_from_object_implicit() {
    assert_eq!(
        run_vb(
...
#[test]
fn inherit_protected_member_access() {
    assert_eq!(
        run_vb(
...
#[test]
fn inherit_protected_member_no_access_outside() {
    assert_eq!(
        run_vb(
            "Class B\nProtected V As Integer = 42\nEnd Class\nModule M\nSub Main()\nDim b1 As New B()\n' b1.V ' Fails\nConsole.WriteLine(\"Parsed\")\nEnd Sub\nEnd Module"
        ),
...
#[test]
fn inherit_shadowing_field_base_access() {
    assert_eq!(
        run_vb(
            "Class B\nPublic V As Integer = 1\nEnd Class\nClass C\nInherits B\nPublic Shadows V As Integer = 2\nPublic Function GetBaseV() As Integer\nReturn MyBase.V\nEnd Function\nEnd Class\nModule M\nSub Main()\nDim c1 As New C()\nConsole.WriteLine(c1.GetBaseV())\nEnd Sub\nEnd Module"
        ),
...
#[test]
fn inherit_mustinherit_class_instantiation_fails() {
    assert_eq!(
        run_vb(
...
#[test]
fn inherit_notinheritable_class_derivation_fails() {
    assert_eq!(
        run_vb(
...
#[test]
fn inherit_constructor_chaining_implicit() {
    assert_eq!(
        run_vb(
...
#[test]
fn inherit_constructor_chaining_missing_parameterless_fails() {
    assert_eq!(
        run_vb(
...
#[test]
fn inherit_type_testing_typeof() {
    assert_eq!(
        run_vb(
...
#[test]
fn inherit_type_conversion_directcast() {
    assert_eq!(
        run_vb(
...
#[test]
fn inherit_type_conversion_trycast() {
    assert_eq!(
        run_vb(
...
#[test]
fn inherit_virtual_call_in_constructor() {
    assert_eq!(
        run_vb(
...
} // In VB, virtual calls in constructors reach derived implementations
#[test]
fn inherit_shadowed_method_access_via_base_ref() {
    assert_eq!(
        run_vb(
...
#[test]
fn inherit_overridable_method_access_via_base_ref() {
    assert_eq!(
        run_vb(
            "Class B\nPublic Overridable Sub M1()\nConsole.WriteLine(\"B\")\nEnd Sub\nEnd Class\nClass C\nInherits B\nPublic Overrides Sub M1()\nConsole.WriteLine(\"C\")\nEnd Sub\nEnd Class\nModule M\nSub Main()\nDim b1 As B = New C()\nb1.M1()\nEnd Sub\nEnd Module"
        ),
