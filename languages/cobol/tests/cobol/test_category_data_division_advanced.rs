...
cobol_test!(
    test_dd_global_in_group,
    "IDENTIFICATION DIVISION. PROGRAM-ID. T. DATA DIVISION. WORKING-STORAGE SECTION. 01 G PIC X VALUE 'G' IS GLOBAL. 05 V PIC X VALUE 'V'. PROCEDURE DIVISION. DISPLAY G. STOP RUN.",
    vec!["G"]
);
cobol_test!(
    test_dd_redefines_move_between_types,
    "IDENTIFICATION DIVISION. PROGRAM-ID. T. DATA DIVISION. WORKING-STORAGE SECTION. 01 A PIC X(4) VALUE '0012'. 01 B REDEFINES A PIC 9(4). PROCEDURE DIVISION. DISPLAY B. STOP RUN.",
    vec!["12"]
);
cobol_test!(
    test_dd_group_rename_and_value_move,
    "IDENTIFICATION DIVISION. PROGRAM-ID. T. DATA DIVISION. WORKING-STORAGE SECTION. 01 G. 05 A PIC X VALUE 'A'. 05 B PIC X VALUE 'B'. 05 C PIC X VALUE 'C'. 66 R RENAMES A THRU B. PROCEDURE DIVISION. MOVE 'ZZ' TO R DISPLAY R. STOP RUN.",
    vec!["ZZ"]
);
cobol_test!(
    test_dd_sign_clause_local_group,
    "IDENTIFICATION DIVISION. PROGRAM-ID. T. DATA DIVISION. WORKING-STORAGE SECTION. 01 V PIC S9(2) VALUE -12 SIGN IS LEADING. PROCEDURE DIVISION. DISPLAY V. STOP RUN.",
    vec!["-12"]
);
