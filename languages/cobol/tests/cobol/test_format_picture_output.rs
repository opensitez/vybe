...
fn pic_z_suppresses_leading_zero() {
    let out = run_prints(&p("01 N PIC Z(5) VALUE 42.", "    DISPLAY N."));
    assert_eq!(out, vec!["   42"]);
}

#[test]
fn pic_z_all_zeros_blank() {
    let out = run_prints(&p("01 N PIC Z(5) VALUE 0.", "    DISPLAY N."));
    assert_eq!(out, vec!["     "]);
}

#[test]
fn pic_z_full_value_displayed() {
    let out = run_prints(&p("01 N PIC Z(5) VALUE 99999.", "    DISPLAY N."));
    assert_eq!(out, vec!["99999"]);
}

#[test]
fn pic_asterisk_fill_leading() {
    let out = run_prints(&p("01 N PIC ***** VALUE 42.", "    DISPLAY N."));
    assert_eq!(out, vec!["***42"]);
}

#[test]
fn pic_asterisk_all_zeros_all_stars() {
    let out = run_prints(&p("01 N PIC ***** VALUE 0.", "    DISPLAY N."));
    assert_eq!(out, vec!["*****"]);
}

#[test]
fn pic_plus_positive_shows_sign() {
    let out = run_prints(&p("01 N PIC +9(4) VALUE 42.", "    DISPLAY N."));
...
fn pic_dot_inserts_decimal_point() {
    let out = run_prints(&p("01 N PIC 9(4).99 VALUE 1234.56.", "    DISPLAY N."));
    assert_eq!(out, vec!["1234.56"]);
}

#[test]
fn pic_comma_inserts_thousands() {
    let out = run_prints(&p("01 N PIC 9,9(3) VALUE 1999.", "    DISPLAY N."));
    assert_eq!(out, vec!["1,999"]);
}

#[test]
fn pic_b_inserts_blank() {
    let out = run_prints(&p("01 N PIC 9(3)B9(3) VALUE 123456.", "    DISPLAY N."));
    assert_eq!(out, vec!["123 456"]);
}

#[test]
fn pic_zero_inserts_literal_zero() {
    let out = run_prints(&p("01 N PIC 9(3)09(2) VALUE 12345.", "    DISPLAY N."));
    assert_eq!(out, vec!["1230045"]);
}

#[test]
fn pic_z_with_decimal() {
    let out = run_prints(&p("01 N PIC ZZ9.99 VALUE 0.75.", "    DISPLAY N."));
    assert_eq!(out, vec!["  0.75"]);
}

#[test]
fn pic_z_partial_suppression() {
    let out = run_prints(&p("01 N PIC ZZ99 VALUE 0099.", "    DISPLAY N."));
    assert_eq!(out, vec!["  99"]);
}

#[test]
fn pic_dollar_float_leading() {
    let out = run_prints(&p("01 N PIC $$$9 VALUE 42.", "    DISPLAY N."));
    assert_eq!(out, vec!["$$42"]);
}

#[test]
fn pic_cr_positive_blank() {
    let out = run_prints(&p("01 N PIC 9(4)CR VALUE 100.", "    DISPLAY N."));
...
fn pic_db_positive_blank() {
    let out = run_prints(&p("01 N PIC 9(4)DB VALUE 50.", "    DISPLAY N."));
...
fn pic_z_numeric_one() {
    let out = run_prints(&p("01 N PIC Z(4) VALUE 1.", "    DISPLAY N."));
    assert_eq!(out, vec!["   1"]);
}

#[test]
fn pic_comma_and_dot_combined() {
    let out = run_prints(&p("01 N PIC 9,9(3).99 VALUE 1234.56.", "    DISPLAY N."));
    assert_eq!(out, vec!["1,234.56"]);
}

#[test]
fn pic_plus_z_and_decimal() {
    compile_ok(&p("01 N PIC +Z(4).99 VALUE -12.5.", "    DISPLAY N."));
}

#[test]
fn pic_star_with_decimal() {
    let out = run_prints(&p("01 N PIC ***9.99 VALUE 1.25.", "    DISPLAY N."));
...
fn pic_plus_with_decimal_compiles() {
    compile_ok(&p("01 N PIC +9(5).99 VALUE -123.45.", "    DISPLAY N."));
}
