...
fn cics_receive() {
    compile_ok(&p(
        d(),
        "    EXEC CICS RECEIVE MAP(MENUMAP) INTO(WS-INPUT) END-EXEC.",
    ));
}
#[test]
fn cics_read() {
    compile_ok(&p(
        d(),
        "    EXEC CICS READ FILE(CUSTFILE) INTO(WS-RECORD) RIDFLD(WS-KEY) END-EXEC.",
    ));
}
#[test]
fn cics_write() {
    compile_ok(&p(
        d(),
        "    EXEC CICS WRITE FILE(CUSTFILE) FROM(WS-RECORD) RIDFLD(WS-KEY) END-EXEC.",
    ));
}
#[test]
fn cics_return() {
    compile_ok(&p(d(), "    EXEC CICS RETURN TRANSID(MENU) END-EXEC."));
}
#[test]
fn cics_link() {
    compile_ok(&p(d(), "    EXEC CICS LINK PROGRAM(SUBPROG) END-EXEC."));
}
#[test]
fn cics_xctl() {
    compile_ok(&p(d(), "    EXEC CICS XCTL PROGRAM(NEXTPROG) END-EXEC."));
}
#[test]
fn cics_asktime() {
    compile_ok(&p(d(), "    EXEC CICS ASKTIME ABSTIME(WS-A) END-EXEC."));
}
#[test]
fn cics_formattime() {
    compile_ok(&p(
        d(),
        "    EXEC CICS FORMATTIME ABSTIME(WS-A) DDMMYYYY(WS-NAME) END-EXEC.",
    ));
}
#[test]
fn cics_getmain() {
...
#[test]
fn cics_endbr() {
    compile_ok(&p(d(), "    EXEC CICS ENDBR FILE(CUSTFILE) END-EXEC."));
}
...
#[test]
fn dli_get_unique() {
    compile_ok(&p(d(), "    EXEC DLI GU INTO(WS-SEGMENT) END-EXEC."));
}
#[test]
fn dli_get_next() {
    compile_ok(&p(d(), "    EXEC DLI GN INTO(WS-SEGMENT) END-EXEC."));
}
#[test]
fn dli_insert() {
    compile_ok(&p(d(), "    EXEC DLI ISRT FROM(WS-SEGMENT) END-EXEC."));
}
#[test]
fn dli_replace() {
    compile_ok(&p(d(), "    EXEC DLI REPL FROM(WS-SEGMENT) END-EXEC."));
}
#[test]
fn dli_delete() {
    compile_ok(&p(d(), "    EXEC DLI DLET END-EXEC."));
}
...
#[test]
fn usage_comp() {
    compile_ok(&p("01 WS-X PIC 9(9) USAGE COMP.", "    DISPLAY WS-X."));
}
#[test]
fn usage_comp3() {
    compile_ok(&p("01 WS-X PIC 9(9) COMP-3.", "    DISPLAY WS-X."));
}
#[test]
fn usage_binary() {
    compile_ok(&p("01 WS-X PIC 9(9) USAGE BINARY.", "    DISPLAY WS-X."));
}
...
#[test]
fn compute_basic_overflow() {
    compile_ok(&p(d(), "    COMPUTE WS-A = WS-B * WS-C."));
}
...
#[test]
fn external_item() {
    compile_ok(&p("01 WS-EXT PIC X(50) EXTERNAL.", "    DISPLAY WS-EXT."));
}
