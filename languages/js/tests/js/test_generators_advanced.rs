...
#[test]
fn generator_optional_param_uses_default_when_omitted() {
    assert_eq!(
        run_js(
            r#"
...
"#
        ...
        vec!["1,2,3,4,5", "0,2,4,6,8"]
    );
}

#[test]
fn class_with_symbol_iterator_followed_by_log() {
    // Diagnostic: does a class with [Symbol.iterator] prevent subsequent console.log from running?
    assert_eq!(
        run_js(
            r#"
class Range {
...
#[test]
fn generator_after_class_simple_call() {
    // Diagnostic: does spread of range work after a class?
    assert_eq!(
        run_js(
            r#"
class Range {
...
#[test]
fn generator_optional_param_with_adjacent_class() {
    // Verify the default-param fix still works when a class with [Symbol.iterator]
    // is declared in the same module (regression: class compilation could poison
    // rest_fixed_arities or other module-wide state used by the call-site padder).
    assert_eq!(
        run_js(
            r#"
class Range {
...
#[test]
fn generator_three_arg_with_adjacent_class_iterator() {
    // Check if even the fully-specified call hangs when a class with [Symbol.iterator]
    // is present (isolating whether the iterator method body causes a hang vs default param).
    assert_eq!(
        run_js(
            r#"
class Range {
