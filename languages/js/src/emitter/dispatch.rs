...
pub fn dispatch(name: &str, chunks: &mut Vec<Chunk>, current: usize, argc: u8, line: u32) -> bool {
    match name {
        "js.net_create_connection" => {
            vybe_platform_dotnet::emitter::core::node_socket_adapter::emit_net_create_connection(
                chunks, current, argc, line,
            )
        }
        "js.net_create_server" => {
            vybe_platform_dotnet::emitter::core::node_socket_adapter::emit_net_create_server(
                chunks, current, argc, line,
            )
        }
        "js.dgram_create_socket" => {
            vybe_platform_dotnet::emitter::core::node_socket_adapter::emit_dgram_create_socket(
                chunks, current, argc, line,
            )
        }
