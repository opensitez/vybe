...
fn synthesize_exception_class(name: &str, parent: &str) -> Statement {
    ...
    let canon = vybe_compiler::primitives::errors::canonical_exception_name(name).to_string();
...
fn dotnet_exception_types_expr(name: &str, parent: &str) -> Expression {
    ...
    for canon in vybe_compiler::primitives::errors::exception_ancestors(name) {
        push_unique(&mut names, canon);
    }
    let canonical = vybe_compiler::primitives::errors::canonical_exception_name(name);
