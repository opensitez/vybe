...
fn host_fn_ref_by_idx(module: &str, name: &str, idx: usize) -> Value {
    ...
    obj.properties
        .insert("__host_module".into(), str_value(module));
...
fn receiver_fn(name: &str, idx: usize) -> Value {
    vybe_platform_ecma::receiver_host_fn_ref(MODULE, name, idx)
}
...
fn sync_pairs_object(obj: &Arc<Mutex<Object>>, pairs: &[(String, String)]) {
    ...
    lock.properties
        .insert("size".into(), Value::F64(pairs.len() as f64));
}
...
fn make_search_params_object(
    ...
    obj.properties.insert(
        "getAll".into(),
        receiver_fn("searchParamsGetAll", fns.params_get_all),
    );
    obj.properties
        .insert("has".into(), receiver_fn("searchParamsHas", fns.params_has));
    obj.properties.insert(
        "toString".into(),
        bound_host_fn_ref_by_idx(
            MODULE,
            "searchParamsToString",
            fns.params_to_string,
            Vec::new(),
        ),
    );
    obj.properties.insert(
        "append".into(),
        receiver_fn("searchParamsAppend", fns.params_append),
    );
    obj.properties
        .insert("set".into(), receiver_fn("searchParamsSet", fns.params_set));
    obj.properties.insert(
        "delete".into(),
        receiver_fn("searchParamsDelete", fns.params_delete),
    );
    obj.properties.insert(
        "sort".into(),
        receiver_fn("searchParamsSort", fns.params_sort),
    );
    obj.properties.insert(
        "keys".into(),
        receiver_fn("searchParamsKeys", fns.params_keys),
    );
    obj.properties.insert(
        "values".into(),
        receiver_fn("searchParamsValues", fns.params_values),
    );
    obj.properties.insert(
        "entries".into(),
        receiver_fn("searchParamsEntries", fns.params_entries),
    );
    obj.properties.insert(
        "iterator".into(),
        bound_host_fn_ref_by_idx(
    ...
    obj.properties.insert(
        "forEach".into(),
        receiver_fn("searchParamsForEach", fns.params_for_each),
    );
    if let Some(owner) = owner {
        obj.properties
            .insert("__url_owner".into(), Value::Object(owner));
    }
    let value = Value::Object(vybe_runtime::heap::alloc(obj));
        ...
        if matches!(lock.properties.get("toString"), Some(Value::Object(method)) if matches!(method.lock().unwrap().kind, ObjectKind::HostFunction(_)))
        {
            lock.properties.insert(
                ...
                bound_host_fn_ref_by_idx(
                    ...
                    "searchParamsToString",
                    fns.params_to_string,
                    vec![Value::Object(params_obj.clone())],
                ),
            );
            lock.properties.insert(
                "iterator".into(),
...
fn sync_url_object(obj: &Arc<Mutex<Object>>, url: &Url) {
    ...
    let hash = url
        ...
        .unwrap_or_default();
    let pathname = if url.path().is_empty() {
        "/".into()
    } else {
        url.path().to_string()
    };
...

    {
        ...
        lock.properties
            .insert("origin".into(), str_value(origin_string(url)));
        lock.properties
            .insert("protocol".into(), str_value(format!("{}:", url.scheme())));
        lock.properties
            .insert("username".into(), str_value(url.username()));
        lock.properties.insert(
            "password".into(),
            str_value(url.password().unwrap_or_default()),
        );
        lock.properties.insert("host".into(), str_value(host));
        lock.properties
            .insert("hostname".into(), str_value(hostname));
        lock.properties.insert("port".into(), str_value(port));
        lock.properties
            .insert("pathname".into(), str_value(pathname));
        ...
        lock.properties.insert(
            "__set_href".into(),
            host_fn_ref_by_idx(MODULE, "urlSetHref", fns.set_href),
        );
        lock.properties.insert(
            "__set_protocol".into(),
            host_fn_ref_by_idx(MODULE, "urlSetProtocol", fns.set_protocol),
        );
        lock.properties.insert(
            "__set_username".into(),
            host_fn_ref_by_idx(MODULE, "urlSetUsername", fns.set_username),
        );
        lock.properties.insert(
            "__set_password".into(),
            host_fn_ref_by_idx(MODULE, "urlSetPassword", fns.set_password),
        );
        lock.properties.insert(
            "__set_host".into(),
            host_fn_ref_by_idx(MODULE, "urlSetHost", fns.set_host),
        );
        lock.properties.insert(
            "__set_hostname".into(),
            host_fn_ref_by_idx(MODULE, "urlSetHostname", fns.set_hostname),
        );
        lock.properties.insert(
            "__set_port".into(),
            host_fn_ref_by_idx(MODULE, "urlSetPort", fns.set_port),
        );
        lock.properties.insert(
            "__set_pathname".into(),
            host_fn_ref_by_idx(MODULE, "urlSetPathname", fns.set_pathname),
        );
        lock.properties.insert(
            "__set_search".into(),
            host_fn_ref_by_idx(MODULE, "urlSetSearch", fns.set_search),
        );
        lock.properties.insert(
            "__set_hash".into(),
            host_fn_ref_by_idx(MODULE, "urlSetHash", fns.set_hash),
        );
    }
    ...
    obj.lock()
        .unwrap()
        .properties
        .insert("searchParams".into(), search_params);
}

fn make_url_object(url: &Url) -> Value {
    let value = Value::Object(vybe_runtime::heap::alloc(Object::new()));
...
fn make_bound_array_iterator(values: Vec<Value>) -> Value {
    let iterator = vybe_platform_ecma::array::make_array_iterator(values);
        ...
        if let Some(next_idx) = next_idx {
            iterator_obj.lock().unwrap().properties.insert(
                ...
                bound_host_fn_ref_by_idx(
                    ...
                    "iterNext",
                    next_idx,
                    vec![Value::Object(iterator_obj.clone())],
                ),
            );
        }
    }
