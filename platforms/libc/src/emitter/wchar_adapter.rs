...
pub fn wide_to_string(arr: Expression) -> Expression {
    ...
    call(ident("__libc_wide_to_string"), vec![arr])
}
...
pub fn runtime_helpers() -> Vec<Statement> {
    ...
    let wide_to_string_body = vec![
        ...
        var_decl("i", lit_int(0)),
        var_decl("n", member(ident("arr"), "length")),
        while_stmt(
            and(
                lt(ident("i"), ident("n")),
                ne(index(ident("arr"), ident("i")), lit_int(0)),
            ),
            vec![
                expr_stmt(assign(
                    ident("res"),
    ...
    vec![
        decl,
        function("__libc_wide_to_string", vec!["arr"], wide_to_string_body),
