...
fn build_url_obj(url: &Url) -> Value {
    ...
    o.properties
        .insert("protocol".into(), s(format!("{}:", url.scheme())));
    o.properties.insert("host".into(), s(host_with_port(url)));
    o.properties
        .insert("hostname".into(), s(url.host_str().unwrap_or("")));
    ...
    o.properties
        .insert("password".into(), s(url.password().unwrap_or("")));
...
fn set_kv(params: &mut Object, keys: Vec<String>, vals: Vec<String>) {
    params.properties.insert(
        "__keys".into(),
        arr_val(keys.into_iter().map(|k| s(&k)).collect()),
    );
    params.properties.insert(
        "__vals".into(),
        arr_val(vals.into_iter().map(|v| s(&v)).collect()),
    );
}
