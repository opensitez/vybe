...
pub fn register(vm: &mut VM) {
    vm.register_host_fn(
        ...
        "mode",
        Box::new(
            |_ctx: &mut HostContext, _args: &[Value]| match with_context(|_c| ()) {
    ...
    vm.register_host_fn(
        "node:http",
        "server_software",
        Box::new(|_ctx, _| Value::String(Arc::from(concat!("vybex/", env!("CARGO_PKG_VERSION"))))),
    );
    ...
    vm.register_host_fn(
        "node:http",
        "php_sapi_name",
        Box::new(|_ctx, _| match with_context(|_c| ()) {
