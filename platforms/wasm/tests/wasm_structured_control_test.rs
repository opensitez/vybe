...
fn decoded_function(instructions: &[u8]) -> Chunk {
    ...
    let mut chunks = vybe_platform_wasm::read_wasm(&wasm).expect("standard wasm should read");
...
fn reader_preserves_br_if_depth() {
    let chunk = decoded_function(&[
        ...
        0x1a, //   drop (empty block must exit at entry height)
        0x0b, // end
...
fn reader_uses_function_section_type_indices() {
    ...
    let chunks = vybe_platform_wasm::read_wasm(&wasm).expect("standard wasm should read");
