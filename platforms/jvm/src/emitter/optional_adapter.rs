//! JVM `java.util.Optional`.
//!
//! The whole class, moved from `languages/java`, which had all 17
//! emitters while this file had three. The three overlapped and were
//! byte-identical in EMITTED BYTECODE — they differed only in local
//! variable names and one blank line — so nothing was reconciled and
//! nothing was lost; the stub is simply the superset now.
//!
//! An `Optional` is `[present, value]`: a two-element array, so `isPresent`
//! is an index read rather than a type test.
