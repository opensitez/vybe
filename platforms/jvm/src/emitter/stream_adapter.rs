...
fn emit_iterate_with_predicate_timing(
    ...
    if predicate_before_push && argc == 3 {
        vybe_compiler::primitives::callable::emit_callback_on(
            chunks,
            current,
            predicate_slot,
            value_slot,
    ...
    vybe_compiler::primitives::callable::emit_callback_on(
        chunks, current, next_slot, value_slot, line,
    );
    chunks[current].emit_op_u16(Op::LOCAL_SET, value_slot, line);
    increment_index(chunks, current, index_slot, line);
    chunks[current].emit_br(0, line);
...
pub fn emit_sum(chunks: &mut [Chunk], current: usize, line: u32) {
    ...
    let state =
        vybe_compiler::primitives::loops::emit_for_in_start(chunks, current, arr, idx, line);
...
pub fn emit_collect(chunks: &mut [Chunk], current: usize, line: u32) {
    ...
    emit_string_concat(chunks, current, line);
    chunks[current].emit_op_u16(Op::LOCAL_GET, suffix_slot, line);
    emit_string_concat(chunks, current, line);
    chunks[current].emit_op_u16(Op::LOCAL_SET, result_slot, line);
...
fn emit_collect_mark_handled(chunks: &mut [Chunk], current: usize, handled_slot: u16, line: u32) {
...
fn emit_collect_to_collection(
    ...
    chunks[current].emit_op_u16(Op::LOCAL_GET, result_slot, line);
    chunks[current].emit_op_u16(Op::LOCAL_GET, value_slot, line);
    super::collection_adapter::emit_add(chunks, current, 2, line);
    chunks[current].emit_op(Op::DROP, line);
...
fn emit_string_concat(chunks: &mut [Chunk], current: usize, line: u32) {
