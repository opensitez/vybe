...
fn deep_clone(
    ...
    match v {
        ...
        Value::Symbol(_) => Err(crate::error::new_error_flat(
            "DataCloneError",
...
fn clone_object(
    ...
    if is_uncloneable_weak_collection(obj) {
        return Err(crate::error::new_error_flat(
            "DataCloneError",
    ...
    let result = match kind_tag {
        ...
        KindTag::Function | KindTag::HostFunction | KindTag::Continuation => {
            Err(crate::error::new_error_flat(
                "DataCloneError",
        ...
        s.properties
            ...
            .filter_map(|(k, getter)| {
                ...
                if prop.starts_with("__")
                    ...
                    || crate::object::is_nonenum(&s, prop)
                {
                    return None;
                }
...
fn clone_array(
    ...
    let (elems, holes): (Vec<Value>, Vec<usize>) = {
        ...
        if let ObjectKind::Array(ref v) = s.kind {
            let holes = (0..v.len())
                .filter(|index| crate::array::is_array_hole(&s, *index))
                .collect();
            (v.clone(), holes)
        } else {
            (Vec::new(), Vec::new())
    ...
    };
    {
        ...
        crate::array::store_hole_indices(&mut t, &hole_set);
    }
    Ok(target_val)
}
...
fn clone_typedarray(
    ...
    let out = if spans_entire_buffer {
        ...
        let out = crate::typedarray::new_typed_array(elem, length);
...
fn is_uncloneable_weak_collection(src: &Arc<Mutex<Object>>) -> bool {
    ...
    ) || s.properties.contains_key(crate::weakmap::WEAKMAP_TAG)
        || s.properties.contains_key(crate::weakmap::WEAKSET_TAG)
}
