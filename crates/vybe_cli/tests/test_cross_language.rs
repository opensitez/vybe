...
"#;
    let php_prog = vybe_parser_php::parse(php_src).expect("PHP parse failed");
    let php_comp = vybe_compiler_php::compile_component(&php_prog, "math_php").expect("PHP comp failed");
...
fn all_languages_as_components() {
    ...
    // PHP
    let php_prog = vybe_parser_php::parse("<?php $x = 1;").expect("PHP");
    let php_comp = vybe_compiler_common::components::build_component("php_mod",
        vybe_bytecode::component::Language::Php,
        vybe_compiler_php::Compiler::new().compile(&php_prog).expect("PHP compile"));
    ...
    // Dart
    let dart_prog = vybe_parser_dart::parse("var x = 1;").expect("Dart");
    let dart_comp = vybe_compiler_common::components::build_component("dart_mod",
        vybe_bytecode::component::Language::Dart,
        vybe_compiler_dart::Compiler::new().compile(&dart_prog).expect("Dart compile"));
