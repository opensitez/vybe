...
#[test]
fn test_builtins_functionality() {
    let source = std::fs::read_to_string("../../tests/test_builtins.vb").expect("Failed to read test file");
    ...
    assert!(has_success, "Built-ins test failed - did not see SUCCESS");
    assert!(!has_failure, "Built-ins test failed - saw FAILURE");
}
