...
fn test_file_io_execution() {
    ...
    interpreter.env.define_const("TestFilePath", vybe_runtime::Value::String(temp_file.to_string_lossy().to_string()));
    interpreter.env.define_const("TestLegacyPath", vybe_runtime::Value::String(temp_legacy.to_string_lossy().to_string()));
    ...
    while let Some(effect) = interpreter.side_effects.pop_front() {
        match effect {
            vybe_runtime::RuntimeSideEffect::MsgBox(msg) => outputs.push(msg),
            vybe_runtime::RuntimeSideEffect::ConsoleOutput(msg) => outputs.push(msg),
            _ => {}
        }
    }
