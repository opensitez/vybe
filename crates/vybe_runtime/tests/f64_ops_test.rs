...
#[test]
fn f64_const_zero() {
    assert_eq!(run(|c| push(c, 0.0)).as_f64(), 0.0);
}
#[test]
fn f64_const_pi() {
    let r = run(|c| push(c, std::f64::consts::PI));
    assert!((r.as_f64() - std::f64::consts::PI).abs() < 1e-15);
}
#[test]
fn f64_const_negative() {
    assert_eq!(run(|c| push(c, -1.5)).as_f64(), -1.5);
}
#[test]
fn f64_const_inf() {
    assert!(run(|c| push(c, f64::INFINITY)).as_f64().is_infinite());
}
#[test]
fn f64_const_nan() {
    assert!(run(|c| push(c, f64::NAN)).as_f64().is_nan());
}
