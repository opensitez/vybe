...
/// Run VB, return VM so we can inspect globals and invoke functions
fn run_vb_vm(source: &str) -> (VM, Rc<RefCell<Vec<String>>>) {
    let program = vybe_parser_basic::parse_program(source)
        .unwrap_or_else(|e| panic!("Parse error: {e}"));
    let mut vm = VM::new();
    let output: Rc<RefCell<Vec<String>>> = Rc::new(RefCell::new(Vec::new()));
...
/// Run VB with GUI host functions registered (for form tests)
...
fn d29_form_class_with_initialize_component() {
    let (_vm, queue, _output) = run_vb_gui(r#"
Imports System.Windows.Forms
Imports System.Drawing
...
"#);
    ...
    let add_controls: Vec<_> = effects.iter()
        .filter(|e| matches!(e, vybe_host::SideEffect::AddControl { .. }))
        .collect();
    assert_eq!(add_controls.len(), 2, "Expected 2 AddControl side effects, got {}", add_controls.len());

    let run_app = effects.iter().any(|e| matches!(e, vybe_host::SideEffect::RunApplication { .. }));
    assert!(run_app, "Expected RunApplication side effect");
}
...
"#);
    let effects = queue.borrow_mut().drain();
    let adds: Vec<_> = effects.iter().filter_map(|e| {
        if let vybe_host::SideEffect::AddControl { control_name, left, top, width, height, .. } = e {
            Some((control_name.clone(), *left, *top, *width, *height))
        } else { None }
    }).collect();
    assert_eq!(adds.len(), 1);
    assert_eq!(adds[0].0, "btn1");
    assert_eq!(adds[0].1, 10);
    assert_eq!(adds[0].2, 20);
    assert_eq!(adds[0].3, 80);
    assert_eq!(adds[0].4, 30);
}
...
"#);
    ...
    assert!(handler.is_some(), "Expected Click handler registered for btn1");
}
...
"#);
    let q = queue.borrow();
    assert!(q.get_event_handler("btn1", "Click").is_some(), "btn1.Click handler");
    assert!(q.get_event_handler("btn2", "Click").is_some(), "btn2.Click handler");
}
...
"#);
    let effects = queue.borrow_mut().drain();
    let control_names: Vec<String> = effects.iter().filter_map(|e| {
        if let vybe_host::SideEffect::AddControl { control_name, .. } = e {
            Some(control_name.clone())
        } else { None }
    }).collect();
    assert!(control_names.contains(&"btnOK".to_string()), "Expected btnOK");
    assert!(control_names.contains(&"txtInput".to_string()), "Expected txtInput");
}

/// D36. Control properties set in InitializeComponent visible in side effects
#[test]
fn d36_control_properties_in_side_effects() {
    let (_vm, queue, _output) = run_vb_gui(r#"
Imports System.Windows.Forms
Imports System.Drawing
Public Class Form1
...
"#);
    let effects = queue.borrow_mut().drain();
    let add: Option<_> = effects.iter().find_map(|e| {
        if let vybe_host::SideEffect::AddControl { control_name, control_type, left, top, width, height, .. } = e {
            Some((control_name.clone(), control_type.clone(), *left, *top, *width, *height))
        } else { None }
    });
    let add = add.expect("Expected AddControl side effect for label");
    assert_eq!(add.0, "lblTitle");
    assert_eq!(add.1, "Label");
    assert_eq!(add.2, 50);   // left
    assert_eq!(add.3, 100);  // top
    assert_eq!(add.4, 150);  // width
    assert_eq!(add.5, 25);   // height
}
...
"#);
    ...
    let method = if let Value::Object(obj) = &instance {
        obj.borrow().properties.get("increment").cloned()
    } else { None }
...
fn e38_invoke_without_me_documents_bug() {
    let (mut vm, _output) = run_vb_vm(r#"
...
"#);
    ...
    let method = if let Value::Object(obj) = &instance {
        obj.borrow().properties.get("greet").cloned()
    } else { None }.unwrap();
...
"#);
    ...
    let method = if let Value::Object(obj) = &instance {
        obj.borrow().properties.get("addtwonumbers").cloned()
    } else { None }.expect("addtwonumbers method");

    let result = vm.invoke(&method, &[instance.clone(), Value::F64(3.0), Value::F64(7.0)]);
    assert!(result.is_ok(), "invoke should succeed: {:?}", result.err());
...
"#);
    ...
    let bump = if let Value::Object(obj) = &instance {
        obj.borrow().properties.get("bump").cloned()
    } else { None }.expect("bump method");
    vm.invoke(&bump, &[instance.clone()]).unwrap();

    // Invoke Show
    let show = if let Value::Object(obj) = &instance {
        obj.borrow().properties.get("show").cloned()
    } else { None }.expect("show method");
...
"#);
    ...
    assert!(matches!(f.unwrap(), Value::Object(_)), "f should be an Object");
}
...
"#);
    ...
    let method = if let Value::Object(obj) = &instance {
        obj.borrow().properties.get("sayhello").cloned()
    } else { None }.expect("sayhello method");

    let result = vm.invoke(&method, &[instance.clone(), Value::String(Rc::from("World"))]);
    assert!(result.is_ok(), "invoke should succeed: {:?}", result.err());
...
/// G57. Control created then properties set then Controls.Add — all correct in side effect
#[test]
...
"#);
    let effects = queue.borrow_mut().drain();
    let add = effects.iter().find_map(|e| {
        if let vybe_host::SideEffect::AddControl { control_name, left, top, width, height, .. } = e {
            Some((control_name.clone(), *left, *top, *width, *height))
        } else { None }
    }).expect("Expected AddControl");
    assert_eq!(add.0, "btnTest");
    assert_eq!(add.1, 25);
    assert_eq!(add.2, 35);
    assert_eq!(add.3, 100);
    assert_eq!(add.4, 40);
...
/// G58. Multiple controls added — all appear in side effects
#[test]
...
"#);
    let effects = queue.borrow_mut().drain();
    let names: Vec<String> = effects.iter().filter_map(|e| {
        if let vybe_host::SideEffect::AddControl { control_name, .. } = e {
            Some(control_name.clone())
        } else { None }
    }).collect();
    assert_eq!(names.len(), 3);
    assert!(names.contains(&"btn1".to_string()));
    assert!(names.contains(&"btn2".to_string()));
    assert!(names.contains(&"txt1".to_string()));
}
...
"#);
    assert_eq!(out, vec!["Count: 42", "Pi: 3.14", "Active: true", "Hello World"]);
}
