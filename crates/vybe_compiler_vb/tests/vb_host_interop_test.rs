...
"#);
    ...
    vm.invoke(&method, &[instance.clone(), Value::String(Rc::from("World"))]).unwrap();
...
"#);
    ...
    assert_eq!(output.borrow().last().map(|s| s.as_str()), Some("3"));
}
...
"#);
    ...
    let method = if let Value::Object(obj) = &instance {
        obj.borrow().properties.get("showname").cloned()
    } else { None }.unwrap();

    vm.invoke(&method, &[instance.clone()]).unwrap();
    assert_eq!(output.borrow().last().map(|s| s.as_str()), Some("Alice"));
}
...
"#);
    ...
    let update = if let Value::Object(obj) = &instance {
        obj.borrow().properties.get("update").cloned()
    } else { None }.unwrap();

    vm.invoke(&update, &[instance.clone(), Value::String(Rc::from("changed"))]).unwrap();

    // Check the field was modified
    if let Value::Object(obj) = &instance {
        let v = obj.borrow().properties.get("value").cloned();
