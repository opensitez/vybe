...
use dioxus::prelude::*;
use vybe_forms::{ControlType, Form};
...
pub struct FormSnapshot {
    pub controls: Vec<vybe_forms::Control>,
    pub width: i32,
    pub height: i32,
    pub text: String,
...
pub struct AppState {
    ...
    pub clipboard_controls: Signal<Vec<vybe_forms::Control>>,
...
impl AppState {
    ...
    pub fn new_project(&self) {
        ...
        let designer_code = vybe_forms::serialization::designer_codegen::generate_designer_code(&form);
        let user_code = vybe_forms::serialization::designer_codegen::generate_user_code_stub("Form1");
        let form_module = vybe_project::FormModule::new_vbnet(form, designer_code, user_code);
        project.forms.push(form_module);
        project.startup_object = vybe_project::StartupObject::Form("Form1".to_string());
    ...
    pub fn open_project_dialog(&self) {
        if let Some(path) = FileDialog::new()
            .add_filter("vybe Project", &["vbp", "vbproj"])
            .pick_file()
        {
    ...
    pub fn save_project(&self) {
        ...
        if let Some(path) = current_path {
            if let Err(e) = vybe_project::save_project_auto(project, &path) {
    ...
    pub fn add_control_at(&self, control_type: ControlType, x: i32, y: i32) {
        ...
        if let (Some(proj), Some(name)) = (project_write.as_mut(), form_name.as_ref()) {
            if let Some(form_module) = proj.get_form_mut(name) {
                ...
                let new_control = vybe_forms::Control::new(
                    control_type,
                    control_name,
                    x,
                    y
    ...
    pub fn add_new_form(&self) {
        ...
        if let Some(proj) = project_write.as_mut() {
            ...
            let designer_code = vybe_forms::serialization::designer_codegen::generate_designer_code(&form);
            let user_code = vybe_forms::serialization::designer_codegen::generate_user_code_stub(&name);
    ...
    pub fn add_new_classic_form(&self) {
        ...
        if let Some(proj) = project_write.as_mut() {
            ...
            proj.forms.push(vybe_project::FormModule::new_classic(form));
    ...
    pub fn add_code_file(&self) {
        ...
        if let Some(proj) = project_write.as_mut() {
            ...
            let mut code_file = vybe_project::CodeFile::new(&name);
        ...
        if let Some(paths) = picked {
            for path in paths {
                ...
                let result = match ext.as_str() {
                    "vb" => vybe_project::load_form_vb(&path),
                    _ => continue,
                };
        ...
        if let Some(paths) = picked {
            for path in paths {
                ...
                if let Some(proj) = project_write.as_mut() {
                    ...
                    let mut code_file = vybe_project::CodeFile::new(&file_name);
                    code_file.code = code;
                    proj.add_code_file(code_file);
    ...
    pub fn reparent_control(&self, control_id: uuid::Uuid, new_parent_id: Option<uuid::Uuid>) {
        ...
        if let (Some(proj), Some(name)) = (project_write.as_mut(), form_name.as_ref()) {
            if let Some(form_module) = proj.get_form_mut(name) {
                ...
                let get_global_pos = |target_id: Option<Uuid>, map: &std::collections::HashMap<Uuid, (vybe_forms::Bounds, Option<Uuid>)>| -> (i32, i32) {
                    let mut x = 0;
                    let mut y = 0;
