...
#[test]
fn alias_str_tostring() {
    let aliases = classes::cross_language_aliases("__str__");
    assert!(aliases.contains(&"toString"), "Python __str__ should alias to JS toString");
    assert!(aliases.contains(&"tostring"), "Python __str__ should alias to VB/C# tostring");
    assert!(aliases.contains(&"__get_tostring"), "Python __str__ should alias to __get_tostring");
}
