...
use vybe_compiler_common::strings;
...
fn emit_concat_zero_parts() {
    ...
    let has_empty = chunk.constants.iter().any(|c| matches!(c, Value::String(s) if s.as_ref() == ""));
...
fn emit_literal_part_adds_constant() {
    ...
    let has_hello = chunk.constants.iter().any(|c| matches!(c, Value::String(s) if s.as_ref() == "hello"));
