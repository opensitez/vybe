...
pub fn emit_set_const_key(chunks: &mut [Chunk], current: usize, key: &str, line: u32) {
    ...
    crate::collections::emit_push(chunks, current, line); chunks[current].emit_op(Op::DROP, line);
}
...
pub fn emit_set_dynamic(chunks: &mut [Chunk], current: usize, line: u32) {
    crate::collections::emit_set(chunks, current, line); chunks[current].emit_op(Op::DROP, line);
}
...
pub fn emit_method_size(chunks: &mut [Chunk], current: usize, line: u32) {
    let keys_key = chunks[current].add_constant(Value::String(Arc::from("__keys")));
    chunks[current].emit_op_u16(Op::STRUCT_GET, keys_key, line);
    crate::collections::emit_len(chunks, current, line);
}
...
pub fn emit_values_from_local(chunks: &mut [Chunk], current: usize, dict_slot: u16, keys_slot: u16, result_slot: u16, idx_slot: u16, line: u32) {
    ...
    crate::collections::emit_get(chunks, current, line); // keys[i] → key string
    crate::collections::emit_get(chunks, current, line); // dict[key] → value
    crate::collections::emit_push(chunks, current, line); chunks[current].emit_op(Op::DROP, line); // result.push(value)
    chunks[current].emit_op(Op::DROP, line);

    crate::loops::emit_for_in_end(chunks, current, idx_slot, state, line);
...
pub fn emit_items_from_local(chunks: &mut [Chunk], current: usize, dict_slot: u16, keys_slot: u16, result_slot: u16, idx_slot: u16, line: u32) {
    ...
    chunks[current].emit_op_u16(Op::LOCAL_GET, keys_slot, line);
    chunks[current].emit_op_u16(Op::LOCAL_GET, idx_slot, line);
    crate::collections::emit_get(chunks, current, line); // key = keys[i]
    ...
    crate::collections::emit_get(chunks, current, line);
    // pair = [key, value]
    crate::collections::emit_array_pair(chunks, current, line);
    // result.push(pair)
    chunks[current].emit_op_u16(Op::LOCAL_GET, result_slot, line);
    ...
    chunks[current].emit_op_u16(Op::LOCAL_GET, keys_slot, line);
    chunks[current].emit_op_u16(Op::LOCAL_GET, idx_slot, line);
    crate::collections::emit_get(chunks, current, line); // key
    chunks[current].emit_op_u16(Op::LOCAL_GET, dict_slot, line);
    chunks[current].emit_op_u16(Op::LOCAL_GET, keys_slot, line);
    chunks[current].emit_op_u16(Op::LOCAL_GET, idx_slot, line);
    crate::collections::emit_get(chunks, current, line); // key again
    crate::collections::emit_get(chunks, current, line); // dict[key] = value
    crate::collections::emit_array_pair(chunks, current, line); // [key, value]
    crate::collections::emit_push(chunks, current, line); chunks[current].emit_op(Op::DROP, line); // result.push(pair)
    chunks[current].emit_op(Op::DROP, line);
