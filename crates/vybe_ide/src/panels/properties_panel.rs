...
enum PropItem {
    Section(String),
    Row(String, String), // (key, value) — text editable
    CheckboxRow(String, bool), // (key, checked)
    DropdownRow(String, String, Vec<String>), // (key, current_value, options)
}
...
pub struct PropertiesPanel {
    ...
    /// App.rs checks this after handle_click and generates the handler.
    ...
    /// Pending action: "build_conn" or "test_conn" for app.rs to handle.
    pub pending_action: Option<String>,
    /// Set to true when an inline property edit (e.g. checkbox) should be committed immediately.
...
impl PropertiesPanel {
    ...
    pub fn render(
        ...
        rect: Rect, scale: f32, form: Option<&Form>, selected_control: Option<&str>,
    ) {
        let s = scale;
        let mut paint = Paint::default();

        // Background — match left panel (#fafafa)
    ...
    fn get_prop<'a>(ctrl: &'a vybe_forms::Control, key: &str, default: &str) -> String {
    ...
    fn collect_props(&self, form: Option<&Form>, selected_control: Option<&str>) -> Vec<PropItem> {
        ...
        if let (Some(form), Some(ctrl_name)) = (form, selected_control) {
            if let Some(ctrl) = form.controls.iter().find(|c| c.name == ctrl_name) {
                ...
                // DataAdapter — full connection builder
    ...
    fn row_count(items: &[PropItem]) -> usize {
        ...
        if self.color_picker.open {
            ...
            match self.color_picker.handle_click(mx, my, popup_x, popup_y) {
                ColorPickerEvent::Changed(c) => {
                    // Commit the color to the property
                ...
                PropItem::Row(key, value) => {
                    if my >= y && my < y + ROW_H {
                        if self.tab == PropTab::Properties && mx >= val_x {
                            ...
                            // Clicking an event → set pending_event for app.rs to pick up
                ...
                PropItem::CheckboxRow(key, checked) => {
                    if my >= y && my < y + ROW_H {
                        if self.tab == PropTab::Properties && mx >= val_x {
                            ...
                            // Events tab usually doesn't have checkboxes, but handled uniformly if any
    ...
    pub fn scroll(&mut self, delta: f32, rect: Rect, form: Option<&Form>, selected_control: Option<&str>) {
        let items = match self.tab {
            PropTab::Properties => self.collect_props(form, selected_control),
            PropTab::Events => self.collect_events(form, selected_control),
        };
