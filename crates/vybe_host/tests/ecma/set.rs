...
fn new_set_from_iterable_deduplicates_members() {
    let set = invoke("new", vec![array(vec![Value::I32(1), Value::I32(2), Value::I32(1)])]);
...
fn add_delete_and_clear_update_membership_and_size() {
    ...
    assert_eq!(invoke("has", vec![set.clone(), Value::String(Arc::from("x"))]), Value::Bool(true));
    assert_eq!(invoke("size", vec![set.clone()]), Value::I32(1));
    assert_eq!(invoke("delete", vec![set.clone(), Value::String(Arc::from("x"))]), Value::Bool(true));
...
fn from_iterable_on_string_materializes_characters() {
    ...
    assert_eq!(invoke("size", vec![set.clone()]), Value::I32(2));
    assert_eq!(iterator_values(invoke("values", vec![set])), vec![Value::String(Arc::from("a")), Value::String(Arc::from("b"))]);
}

#[test]
fn values_and_entries_preserve_insertion_order() {
    let set = invoke("new", vec![array(vec![Value::I32(3), Value::I32(1), Value::I32(4)])]);
    assert_eq!(iterator_values(invoke("values", vec![set.clone()])), vec![Value::I32(3), Value::I32(1), Value::I32(4)]);

    let entries = iterator_values(invoke("entries", vec![set]));
    assert_eq!(entries.len(), 3);
    let Value::Object(pair) = &entries[0] else { panic!("entry should be pair array") };
    let pair = pair.lock().unwrap();
    let ObjectKind::Array(values) = &pair.kind else { panic!("entry should be pair array") };
    assert_eq!(values, &vec![Value::I32(3), Value::I32(3)]);
}

#[test]
fn union_intersection_difference_and_symmetric_difference_follow_set_algebra() {
    let left = invoke("new", vec![array(vec![Value::I32(1), Value::I32(2), Value::I32(3)])]);
    ...
    assert_eq!(iterator_values(invoke("values", vec![union])), vec![Value::I32(1), Value::I32(2), Value::I32(3), Value::I32(4)]);
    assert_eq!(iterator_values(invoke("values", vec![intersection])), vec![Value::I32(3)]);
    assert_eq!(iterator_values(invoke("values", vec![difference])), vec![Value::I32(1), Value::I32(2)]);
    assert_eq!(iterator_values(invoke("values", vec![symmetric])), vec![Value::I32(1), Value::I32(2), Value::I32(4)]);
}
...
fn relational_predicates_report_subset_superset_and_disjointness() {
    ...
    let right = invoke("new", vec![array(vec![Value::I32(1), Value::I32(2), Value::I32(3)])]);
    let other = invoke("new", vec![array(vec![Value::I32(9)])]);

    assert_eq!(invoke("isSubsetOf", vec![left.clone(), right.clone()]), Value::I32(1));
    assert_eq!(invoke("isSupersetOf", vec![right.clone(), left]), Value::I32(1));
    assert_eq!(invoke("isDisjointFrom", vec![right.clone(), other.clone()]), Value::I32(1));
...
fn mutating_set_algebra_updates_receiver_in_place() {
    ...
    assert_eq!(iterator_values(invoke("values", vec![base.clone()])), vec![Value::I32(1), Value::I32(2), Value::I32(3)]);

    let _ = invoke("exceptWith", vec![base.clone(), invoke("new", vec![array(vec![Value::I32(2)])])]);
    assert_eq!(iterator_values(invoke("values", vec![base.clone()])), vec![Value::I32(1), Value::I32(3)]);

    let _ = invoke("symmetricExceptWith", vec![base.clone(), invoke("new", vec![array(vec![Value::I32(3), Value::I32(4)])])]);
    assert_eq!(iterator_values(invoke("values", vec![base.clone()])), vec![Value::I32(1), Value::I32(4)]);
