...
fn is_ip_returns_6_for_ipv4_mapped_ipv6() {
    assert_eq!(call_net("isIP", vec![s("::ffff:192.168.1.1")]), Value::I32(6));
}
...
fn is_ipv6_false_for_ipv4() {
    assert_eq!(call_net("isIPv6", vec![s("192.168.1.1")]), Value::Bool(false));
}

#[test]
fn is_ipv6_false_for_hostname() {
    assert_eq!(call_net("isIPv6", vec![s("example.com")]), Value::Bool(false));
}
...
fn create_server_has_get_connections_method() {
    let server = call_net("createServer", vec![]);
    assert!(has_method(&server, "getConnections"), "Server.getConnections must exist");
}

#[test]
fn create_server_has_on_method() {
    let server = call_net("createServer", vec![]);
    assert!(has_method(&server, "on"), "Server.on (EventEmitter) must exist");
}
...
fn socket_has_set_encoding_method() {
    let socket = call_net("Socket", vec![]);
    assert!(has_method(&socket, "setEncoding"), "Socket.setEncoding must exist");
}

#[test]
fn socket_has_set_timeout_method() {
    let socket = call_net("Socket", vec![]);
    assert!(has_method(&socket, "setTimeout"), "Socket.setTimeout must exist");
}
...
fn socket_has_bytes_written_property() {
    ...
    assert!(matches!(bw, Value::I32(_) | Value::I64(_) | Value::F64(_) | Value::Undefined));
}
...
fn socket_has_bytes_read_property() {
    ...
    assert!(matches!(br, Value::I32(_) | Value::I64(_) | Value::F64(_) | Value::Undefined));
}
...
fn block_list_has_add_address_method() {
    let bl = call_net("BlockList", vec![]);
    assert!(has_method(&bl, "addAddress"), "BlockList.addAddress must exist");
}
...
fn block_list_has_add_subnet_method() {
    let bl = call_net("BlockList", vec![]);
    assert!(has_method(&bl, "addSubnet"), "BlockList.addSubnet must exist");
}
...
fn block_list_has_rules_property() {
    ...
    assert!(!matches!(rules, Value::Undefined), "BlockList.rules must exist");
}
...
fn socket_address_has_port_property() {
    ...
    assert!(matches!(port, Value::I32(_) | Value::F64(_) | Value::Undefined));
}
...
fn server_constructor_returns_object() {
    ...
    assert!(matches!(server, Value::Object(_)), "Server constructor must return object");
}
...
fn create_server_has_set_timeout_method() {
    let server = call_net("createServer", vec![]);
    assert!(has_method(&server, "setTimeout"), "Server.setTimeout must exist");
}
...
fn block_list_check_returns_bool_for_unknown_address() {
    ...
    assert!(
        ...
        "BlockList.check must return bool, got {:?}", result
    );
}
