...
fn stat_at_symlink_to_file_reports_regular_file() {
    ...
    let stat = types("[method]descriptor.stat-at", vec![root, Value::I32(0), s("alias.txt")]);
...
fn stat_at_symlink_to_directory_reports_directory() {
    ...
    let stat = types("[method]descriptor.stat-at", vec![root, Value::I32(0), s("nested-link")]);
...
fn stat_at_symlink_follow_flag_matches_default_behavior() {
    ...
    let default_stat = types("[method]descriptor.stat-at", vec![root.clone(), Value::I32(0), s("alias.txt")]);
    let follow_flag_stat = types("[method]descriptor.stat-at", vec![root, Value::I32(1), s("alias.txt")]);
...
fn open_at_symlink_to_file_get_type_reports_regular_file() {
    ...
    let descriptor = types(
        ...
        vec![root, Value::I32(0), s("alias.txt"), Value::I32(0), Value::I32(0)],
    );
...
fn open_at_symlink_to_directory_with_directory_flag_succeeds() {
    ...
    let descriptor = types(
        "[method]descriptor.open-at",
        vec![root, Value::I32(0), s("nested-link"), Value::I32(2), Value::I32(0)],
    );
    assert!(is_error(&descriptor).is_none());
...
fn read_via_stream_on_symlinked_file_reads_target_bytes() {
    ...
    let stream = types("[method]descriptor.read-via-stream", vec![descriptor, Value::F64(0.0)]);
    let bytes = io_streams("[method]input-stream.blocking-read", vec![stream, Value::F64(5.0)]);
...
fn read_directory_reports_file_symlink_entry_as_symbolic_link() {
    ...
    assert!(directory_entries(stream).contains(&(String::from("alias.txt"), String::from("symbolic-link"))));
}
...
fn read_directory_reports_directory_symlink_entry_as_symbolic_link() {
    ...
    assert!(directory_entries(stream).contains(&(String::from("nested-link"), String::from("symbolic-link"))));
}
...
fn is_same_object_is_false_for_symlink_and_target_descriptors() {
    ...
    let alias = types(
        "[method]descriptor.open-at",
        vec![root.clone(), Value::I32(0), s("alias.txt"), Value::I32(0), Value::I32(0)],
    );
    let target = types(
        "[method]descriptor.open-at",
        vec![root, Value::I32(0), s("target.txt"), Value::I32(0), Value::I32(0)],
    );

    assert_eq!(types("[method]descriptor.is-same-object", vec![alias, target]), Value::Bool(false));
}
...
fn readlink_at_on_directory_symlink_returns_target_name() {
    ...
    let result = types("[method]descriptor.readlink-at", vec![root, s("nested-link")]);
