...
fn register_types(vm: &mut VM, type_ids: HttpTypeIds) {
    ...
    vm.register_host_fn("wasi:http/types", "[constructor]request-options", Box::new(move |_ctx: &mut HostContext, _args: &[Value]| {
        let mut registry = registry().lock().unwrap();
        let id = registry.alloc_id();
        registry.request_options.insert(id, RequestOptionsResource { connect_timeout_ns: None, first_byte_timeout_ns: None, between_bytes_timeout_ns: None });
    ...
    vm.register_host_fn("wasi:http/types", "[method]future-incoming-response.subscribe", Box::new(|_ctx: &mut HostContext, _args: &[Value]| {
    ...
    vm.register_host_fn("wasi:http/types", "[method]fields.set", Box::new(|_ctx: &mut HostContext, args: &[Value]| {
        let Some(headers_id) = resource_id(&args[0], KIND_HEADERS) else { return err("invalid-argument"); };
        let Some(name) = string_arg(args, 1) else { return err("invalid-argument"); };
        let mut registry = registry().lock().unwrap();
        let Some(headers) = registry.headers.get_mut(&headers_id) else { return err("invalid-argument"); };
        let key = name.to_ascii_lowercase();
        headers.entries.retain(|(k, _)| !k.eq_ignore_ascii_case(&key));
    ...
    vm.register_host_fn("wasi:http/types", "[method]fields.delete", Box::new(|_ctx: &mut HostContext, args: &[Value]| {
        let Some(headers_id) = resource_id(&args[0], KIND_HEADERS) else { return err("invalid-argument"); };
        let Some(name) = string_arg(args, 1) else { return err("invalid-argument"); };
        let mut registry = registry().lock().unwrap();
        let Some(headers) = registry.headers.get_mut(&headers_id) else { return err("invalid-argument"); };
        headers.entries.retain(|(k, _)| !k.eq_ignore_ascii_case(&name));
        Value::Null
    }));

    // fields.append(name, value) → result
    vm.register_host_fn("wasi:http/types", "[method]fields.append", Box::new(|_ctx: &mut HostContext, args: &[Value]| {
        let Some(headers_id) = resource_id(&args[0], KIND_HEADERS) else { return err("invalid-argument"); };
        let Some(name) = string_arg(args, 1) else { return err("invalid-argument"); };
    ...
    vm.register_host_fn("wasi:http/types", "[method]fields.clone", Box::new(move |_ctx: &mut HostContext, args: &[Value]| {
        let Some(src_id) = resource_id(&args[0], KIND_HEADERS) else { return err("invalid-argument"); };
        let mut registry = registry().lock().unwrap();
        let entries = registry.headers.get(&src_id).map(|h| h.entries.clone()).unwrap_or_default();
    ...
    vm.register_host_fn("wasi:http/types", "[static]fields.from-list", Box::new(move |_ctx: &mut HostContext, args: &[Value]| {
                ...
                for pair in pairs {
                    if let Value::Object(p) = pair {
                        ...
                        if let vybe_bytecode::value::ObjectKind::Array(ref kv) = p.kind {
                            let name = kv.first().map(|v| format!("{}", v)).unwrap_or_default().to_ascii_lowercase();
                            let val = kv.get(1).map(|v| format!("{}", v).into_bytes()).unwrap_or_default();
    ...
    vm.register_host_fn("wasi:http/types", "[method]outgoing-request.method", Box::new(|_ctx: &mut HostContext, args: &[Value]| {
        let Some(id) = resource_id(&args[0], KIND_OUTGOING_REQUEST) else { return err("invalid-argument"); };
        let registry = registry().lock().unwrap();
        let Some(req) = registry.outgoing_requests.get(&id) else { return err("invalid-argument"); };
        Value::String(Arc::from(req.method.as_str()))
    }));

    vm.register_host_fn("wasi:http/types", "[method]outgoing-request.path-with-query", Box::new(|_ctx: &mut HostContext, args: &[Value]| {
        let Some(id) = resource_id(&args[0], KIND_OUTGOING_REQUEST) else { return err("invalid-argument"); };
        let registry = registry().lock().unwrap();
        let Some(req) = registry.outgoing_requests.get(&id) else { return err("invalid-argument"); };
        req.path_with_query.as_deref().map(|s| Value::String(Arc::from(s))).unwrap_or(Value::Null)
    }));

    vm.register_host_fn("wasi:http/types", "[method]outgoing-request.scheme", Box::new(|_ctx: &mut HostContext, args: &[Value]| {
        let Some(id) = resource_id(&args[0], KIND_OUTGOING_REQUEST) else { return err("invalid-argument"); };
        let registry = registry().lock().unwrap();
        let Some(req) = registry.outgoing_requests.get(&id) else { return err("invalid-argument"); };
        req.scheme.as_deref().map(|s| Value::String(Arc::from(s))).unwrap_or(Value::Null)
    }));

    vm.register_host_fn("wasi:http/types", "[method]outgoing-request.authority", Box::new(|_ctx: &mut HostContext, args: &[Value]| {
        let Some(id) = resource_id(&args[0], KIND_OUTGOING_REQUEST) else { return err("invalid-argument"); };
        let registry = registry().lock().unwrap();
        let Some(req) = registry.outgoing_requests.get(&id) else { return err("invalid-argument"); };
        req.authority.as_deref().map(|s| Value::String(Arc::from(s))).unwrap_or(Value::Null)
    }));

    // [method]outgoing-request.body → result<outgoing-body, error-code>
    vm.register_host_fn("wasi:http/types", "[method]outgoing-request.body", Box::new(move |_ctx: &mut HostContext, args: &[Value]| {
        let Some(_request_id) = resource_id(&args[0], KIND_OUTGOING_REQUEST) else { return err("invalid-argument"); };
        let mut registry = registry().lock().unwrap();
        let body_id = registry.alloc_id();
        registry.outgoing_bodies.insert(body_id, OutgoingBodyResource { bytes: Vec::new() });
    ...
    vm.register_host_fn("wasi:http/types", "[method]request-options.connect-timeout", Box::new(|_ctx: &mut HostContext, args: &[Value]| {
        let Some(id) = resource_id(&args[0], KIND_REQUEST_OPTIONS) else { return err("invalid-argument"); };
        let registry = registry().lock().unwrap();
        let Some(opt) = registry.request_options.get(&id) else { return err("invalid-argument"); };
        opt.connect_timeout_ns.map(|n| Value::F64(n as f64)).unwrap_or(Value::Null)
    }));

    vm.register_host_fn("wasi:http/types", "[method]request-options.set-connect-timeout", Box::new(|_ctx: &mut HostContext, args: &[Value]| {
        let Some(id) = resource_id(&args[0], KIND_REQUEST_OPTIONS) else { return err("invalid-argument"); };
        let ns = args.get(1).map(|v| v.as_f64() as u64);
        let mut registry = registry().lock().unwrap();
        let Some(opt) = registry.request_options.get_mut(&id) else { return err("invalid-argument"); };
        opt.connect_timeout_ns = ns;
        Value::Null
    }));

    vm.register_host_fn("wasi:http/types", "[method]request-options.first-byte-timeout", Box::new(|_ctx: &mut HostContext, args: &[Value]| {
        let Some(id) = resource_id(&args[0], KIND_REQUEST_OPTIONS) else { return err("invalid-argument"); };
        let registry = registry().lock().unwrap();
        let Some(opt) = registry.request_options.get(&id) else { return err("invalid-argument"); };
        opt.first_byte_timeout_ns.map(|n| Value::F64(n as f64)).unwrap_or(Value::Null)
    }));

    vm.register_host_fn("wasi:http/types", "[method]request-options.set-first-byte-timeout", Box::new(|_ctx: &mut HostContext, args: &[Value]| {
        let Some(id) = resource_id(&args[0], KIND_REQUEST_OPTIONS) else { return err("invalid-argument"); };
        let ns = args.get(1).map(|v| v.as_f64() as u64);
        let mut registry = registry().lock().unwrap();
        let Some(opt) = registry.request_options.get_mut(&id) else { return err("invalid-argument"); };
    ...
    vm.register_host_fn("wasi:http/types", "[method]request-options.between-bytes-timeout", Box::new(|_ctx: &mut HostContext, args: &[Value]| {
        let Some(id) = resource_id(&args[0], KIND_REQUEST_OPTIONS) else { return err("invalid-argument"); };
        let registry = registry().lock().unwrap();
        let Some(opt) = registry.request_options.get(&id) else { return err("invalid-argument"); };
        opt.between_bytes_timeout_ns.map(|n| Value::F64(n as f64)).unwrap_or(Value::Null)
    }));

    vm.register_host_fn("wasi:http/types", "[method]request-options.set-between-bytes-timeout", Box::new(|_ctx: &mut HostContext, args: &[Value]| {
        let Some(id) = resource_id(&args[0], KIND_REQUEST_OPTIONS) else { return err("invalid-argument"); };
        let ns = args.get(1).map(|v| v.as_f64() as u64);
        let mut registry = registry().lock().unwrap();
        let Some(opt) = registry.request_options.get_mut(&id) else { return err("invalid-argument"); };
    ...
    vm.register_host_fn("wasi:http/types", "[method]incoming-response.consume", Box::new(move |_ctx: &mut HostContext, args: &[Value]| {
        let Some(response_id) = resource_id(&args[0], KIND_INCOMING_RESPONSE) else { return err("invalid-argument"); };
        let mut registry = registry().lock().unwrap();
        let Some(response) = registry.incoming_responses.get(&response_id) else { return err("invalid-argument"); };
        let body_bytes = response.body.as_bytes().to_vec();
        let body_id = registry.alloc_id();
        registry.incoming_bodies.insert(body_id, IncomingBodyResource { body: body_bytes, position: 0 });
    ...
    vm.register_host_fn("wasi:http/types", "[method]incoming-body.%stream", Box::new(move |_ctx: &mut HostContext, args: &[Value]| {
        let Some(body_id) = resource_id(&args[0], KIND_INCOMING_BODY) else { return err("invalid-argument"); };
        let body_bytes = {
            let registry = registry().lock().unwrap();
            let Some(body) = registry.incoming_bodies.get(&body_id) else { return err("invalid-argument"); };
            body.body.clone()
        };
        let stream_id = super::filesystem::register_buffer_stream(body_bytes);
        let mut obj = Object::new();
        obj.properties.insert("__wasi_kind".into(), Value::String(Arc::from("input-stream")));
        obj.properties.insert("__wasi_id".into(), Value::F64(stream_id as f64));
        Value::Object(Arc::new(Mutex::new(obj)))
    }));

    // [static]incoming-body.finish → future-trailers
    vm.register_host_fn("wasi:http/types", "[static]incoming-body.finish", Box::new(move |_ctx: &mut HostContext, _args: &[Value]| {
        let mut registry = registry().lock().unwrap();
        let id = registry.alloc_id();
    ...
    vm.register_host_fn("wasi:http/types", "[method]future-trailers.get", Box::new(|_ctx: &mut HostContext, _args: &[Value]| {
        // No trailers in our HTTP/1.1 client — return Some(Ok(None))
        Value::Null
    }));

    // future-trailers.subscribe → always-ready pollable
    vm.register_host_fn("wasi:http/types", "[method]future-trailers.subscribe", Box::new(|_ctx: &mut HostContext, _args: &[Value]| {
        let mut obj = Object::new();
        obj.properties.insert("__type".into(), Value::String(Arc::from("Pollable")));
        obj.properties.insert("__ready".into(), Value::Bool(true));
        Value::Object(Arc::new(Mutex::new(obj)))
    ...
    vm.register_host_fn("wasi:http/types", "[constructor]outgoing-response", Box::new(move |_ctx: &mut HostContext, args: &[Value]| {
        let headers_id = resource_id(&args[0], KIND_HEADERS).unwrap_or(0);
        let mut registry = registry().lock().unwrap();
        let id = registry.alloc_id();
        registry.outgoing_responses.insert(id, OutgoingResponseResource { status: 200, headers_id, body_id: None });
        make_resource(KIND_OUTGOING_RESPONSE, id, type_ids.outgoing_response)
    }));

    vm.register_host_fn("wasi:http/types", "[method]outgoing-response.status-code", Box::new(|_ctx: &mut HostContext, args: &[Value]| {
        let Some(id) = resource_id(&args[0], KIND_OUTGOING_RESPONSE) else { return err("invalid-argument"); };
        let registry = registry().lock().unwrap();
        let Some(resp) = registry.outgoing_responses.get(&id) else { return err("invalid-argument"); };
        Value::F64(resp.status as f64)
    }));

    vm.register_host_fn("wasi:http/types", "[method]outgoing-response.set-status-code", Box::new(|_ctx: &mut HostContext, args: &[Value]| {
        let Some(id) = resource_id(&args[0], KIND_OUTGOING_RESPONSE) else { return err("invalid-argument"); };
        let code = args.get(1).map(|v| v.as_f64() as u16).unwrap_or(200);
        let mut registry = registry().lock().unwrap();
        let Some(resp) = registry.outgoing_responses.get_mut(&id) else { return err("invalid-argument"); };
        resp.status = code;
        Value::Null
    }));

    vm.register_host_fn("wasi:http/types", "[method]outgoing-response.headers", Box::new(move |_ctx: &mut HostContext, args: &[Value]| {
        let Some(id) = resource_id(&args[0], KIND_OUTGOING_RESPONSE) else { return err("invalid-argument"); };
        let registry = registry().lock().unwrap();
        let Some(resp) = registry.outgoing_responses.get(&id) else { return err("invalid-argument"); };
    ...
    vm.register_host_fn("wasi:http/types", "[method]outgoing-response.body", Box::new(move |_ctx: &mut HostContext, args: &[Value]| {
        let Some(id) = resource_id(&args[0], KIND_OUTGOING_RESPONSE) else { return err("invalid-argument"); };
        let mut registry = registry().lock().unwrap();
        let body_id = registry.alloc_id();
        registry.outgoing_bodies.insert(body_id, OutgoingBodyResource { bytes: Vec::new() });
    ...
    vm.register_host_fn("wasi:http/types", "[method]outgoing-body.write", Box::new(move |_ctx: &mut HostContext, args: &[Value]| {
        let Some(body_id) = resource_id(&args[0], KIND_OUTGOING_BODY) else { return err("invalid-argument"); };
    ...
    vm.register_host_fn("wasi:http/types", "[static]outgoing-body.finish", Box::new(|_ctx: &mut HostContext, _args: &[Value]| {
        Value::Null
    }));

    // http-error-code(err) → option<error-code>
    vm.register_host_fn("wasi:http/types", "http-error-code", Box::new(|_ctx: &mut HostContext, args: &[Value]| {
    ...
    for fn_name in &["[method]incoming-request.method", "[method]incoming-request.path-with-query",
                      "[method]incoming-request.scheme", "[method]incoming-request.authority",
                      "[method]incoming-request.headers", "[method]incoming-request.consume"] {
        vm.register_host_fn("wasi:http/types", fn_name, Box::new(|_ctx: &mut HostContext, _args: &[Value]| {
            Value::Null
        }));
    }

    // [static]response-outparam.set — server-side; stubs for link compatibility.
    vm.register_host_fn("wasi:http/types", "[static]response-outparam.set", Box::new(|_ctx: &mut HostContext, _args: &[Value]| {
        Value::Null
    }));
}
...
fn register_wasi3(vm: &mut VM, type_ids: HttpTypeIds) {
    ...
    vm.register_host_fn("wasi:http/types", "[static]request.new", Box::new(move |_ctx: &mut HostContext, args: &[Value]| {
        let headers_id = resource_id(&args[0], KIND_HEADERS).unwrap_or(0);
        let mut reg = registry().lock().unwrap();
        let id = reg.alloc_id();
        reg.outgoing_requests.insert(id, OutgoingRequestResource {
            headers_id,
            method: "GET".into(),
            path_with_query: None,
            scheme: None,
    ...
    vm.register_host_fn("wasi:http/types", "[static]request.consume-body", Box::new(|ctx: &mut HostContext, args: &[Value]| {
        let bytes = if let Some(body_id) = resource_id(&args[0], KIND_OUTGOING_BODY) {
            registry().lock().unwrap().outgoing_bodies.get(&body_id).map(|b| b.bytes.clone()).unwrap_or_default()
        } else {
            Vec::new()
        };
        ...
        Value::Object(Arc::new(Mutex::new(Object::new_array(vec![stream_val, future_val]))))
    }));

    // [static]response.new — WASI 0.3 constructor alongside [constructor]outgoing-response.
    vm.register_host_fn("wasi:http/types", "[static]response.new", Box::new(move |_ctx: &mut HostContext, args: &[Value]| {
        let headers_id = resource_id(&args[0], KIND_HEADERS).unwrap_or(0);
        let mut reg = registry().lock().unwrap();
        let id = reg.alloc_id();
        reg.outgoing_responses.insert(id, OutgoingResponseResource { status: 200, headers_id, body_id: None });
        make_resource(KIND_OUTGOING_RESPONSE, id, type_ids.outgoing_response)
    }));

    // [static]response.consume-body — returns body bytes as stream<u8> + resolved trailers future.
    vm.register_host_fn("wasi:http/types", "[static]response.consume-body", Box::new(|ctx: &mut HostContext, args: &[Value]| {
        let bytes = if let Some(body_id) = resource_id(&args[0], KIND_OUTGOING_BODY) {
            registry().lock().unwrap().outgoing_bodies.get(&body_id).map(|b| b.bytes.clone()).unwrap_or_default()
        } else if let Some(body_id) = resource_id(&args[0], KIND_INCOMING_BODY) {
            registry().lock().unwrap().incoming_bodies.get(&body_id).map(|b| b.body.clone()).unwrap_or_default()
        } else {
            Vec::new()
        };
        let (stream_val, stream_id) = ctx.create_stream();
