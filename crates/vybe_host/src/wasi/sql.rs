...
use state::{next_id, state, ConnEntry, StmtEntry};
use driver::{open, SqlDriver};

// ── Shared scalar parser (used by postgres.rs and mysql.rs) ──────────────────

pub(super) fn parse_scalar(s: &str) -> Value {
    if s.is_empty() || s.eq_ignore_ascii_case("null") { return Value::Null; }
    if s == "true"  || s == "t" || s == "TRUE"  { return Value::Bool(true); }
    if s == "false" || s == "f" || s == "FALSE" { return Value::Bool(false); }
...
fn make_conn_obj(id: u64, url: &str) -> Value {
    ...
    obj.properties.insert("__wasi_kind".into(),      Value::String(Arc::from("sql-connection")));
    obj.properties.insert("__wasi_id".into(),        Value::F64(id as f64));
    ...
    obj.properties.insert("connectionstring".into(), Value::String(Arc::from(url)));
    obj.properties.insert("provider".into(),         string_value(driver_provider_name(url)));
    obj.properties.insert("serverversion".into(),    string_value(driver_server_version(url)));
    obj.properties.insert("connectiontimeout".into(), Value::F64(30.0));
...
fn make_stmt_obj(id: u64) -> Value {
    let mut obj = Object::new();
    obj.properties.insert("__wasi_kind".into(), Value::String(Arc::from("sql-statement")));
    obj.properties.insert("__wasi_id".into(),   Value::F64(id as f64));
    Value::Object(Arc::new(Mutex::new(obj)))
}
...
fn make_error_obj(msg: &str) -> Value {
    let mut obj = Object::new();
    obj.properties.insert("__wasi_kind".into(), Value::String(Arc::from("sql-error")));
    obj.properties.insert("message".into(),     Value::String(Arc::from(msg)));
    Value::Object(Arc::new(Mutex::new(obj)))
}
...
fn f64_prop(obj: &Arc<Mutex<Object>>, key: &str) -> f64 {
    obj.lock().unwrap().properties.get(key).map(Value::as_f64).unwrap_or(0.0)
}

fn string_prop(obj: &Arc<Mutex<Object>>, key: &str) -> String {
    obj.lock().unwrap().properties.get(key).map(|v| format!("{}", v)).unwrap_or_default()
}
...
fn row_value_by_index(row: &Value, index: usize) -> Value {
    ...
    let col_names = {
        ...
        if let Some(Value::Object(names)) = guard.properties.get("__col_names") {
            let names_guard = names.lock().unwrap();
            if let ObjectKind::Array(ref elems) = names_guard.kind {
                elems.iter().map(|value| format!("{}", value)).collect::<Vec<_>>()
            } else {
                vec![]
            }
        } else {
...
fn data_table_from_rows(name: &str, rows: &[Value], col_names: &[String]) -> Value {
    ...
    table.properties.insert("__type".into(), string_value("DataTable"));
    table.properties.insert("tablename".into(), string_value(name));
    table.properties.insert(
        "columns".into(),
        rows_array(col_names.iter().map(|col| string_value(col)).collect()),
    );
    table.properties.insert("rows".into(), rows_array(rows.to_vec()));
    Value::Object(Arc::new(Mutex::new(table)))
}

fn make_params_obj() -> Value {
    let mut obj = Object::new();
    obj.properties.insert("__type".into(), string_value("SqlParameterCollection"));
...
fn make_command_obj(type_name: &str, conn_id: u64, conn_string: &str) -> Value {
    let mut obj = Object::new();
    obj.properties.insert("__type".into(), string_value(type_name));
    obj.properties.insert("__conn_id".into(), Value::F64(conn_id as f64));
    obj.properties.insert("commandtext".into(), string_value(""));
    obj.properties.insert("commandtimeout".into(), Value::F64(30.0));
    obj.properties.insert("commandtype".into(), Value::F64(1.0));
    obj.properties.insert("connectionstring".into(), string_value(conn_string));
    obj.properties.insert("parameters".into(), make_params_obj());
    Value::Object(Arc::new(Mutex::new(obj)))
}

fn make_transaction_obj(conn_id: u64) -> Value {
    let mut obj = Object::new();
    obj.properties.insert("__type".into(), string_value("SqlTransaction"));
    obj.properties.insert("__conn_id".into(), Value::F64(conn_id as f64));
...
fn make_reader_obj_with_cols(rows: Vec<Value>, col_names: Vec<String>) -> Value {
    let mut obj = Object::new();
    obj.properties.insert("__type".into(), string_value("SqlDataReader"));
    obj.properties.insert("__rows".into(), rows_array(rows.clone()));
    obj.properties.insert(
        "__col_names".into(),
        rows_array(col_names.iter().map(|col| string_value(col)).collect()),
    );
    obj.properties.insert("__pos".into(), Value::F64(-1.0));
    obj.properties.insert("hasrows".into(), Value::Bool(!rows.is_empty()));
    obj.properties.insert("fieldcount".into(), Value::F64(col_names.len() as f64));
    obj.properties.insert("isclosed".into(), Value::Bool(false));
    Value::Object(Arc::new(Mutex::new(obj)))
}
...
fn make_data_adapter_obj(sql: &str, conn_id: u64, conn_string: &str) -> Value {
    let mut obj = Object::new();
    obj.properties.insert("__type".into(), string_value("SqlDataAdapter"));
    obj.properties.insert("selectcommand".into(), string_value(sql));
    obj.properties.insert("__conn_id".into(), Value::F64(conn_id as f64));
    obj.properties.insert("connectionstring".into(), string_value(conn_string));
    Value::Object(Arc::new(Mutex::new(obj)))
}
...
fn command_sql_and_params(command: &Arc<Mutex<Object>>) -> (String, Vec<String>) {
    ...
    let Some(Value::Object(params)) = command.lock().unwrap().properties.get("parameters").cloned() else {
        return (sql, vec![]);
    };
...
fn get_sql(args: &[Value]) -> String {
    if let Some(Value::Object(obj)) = args.first() {
        let o = obj.lock().unwrap();
        let ct = o.properties.get("commandtext")
            .map(|v| format!("{}", v))
            .unwrap_or_default();
...
pub fn register(vm: &mut VM) {
    ...
    vm.register_host_fn("wasi:sql/types", "connection.new",
        Box::new(|_ctx: &mut HostContext, args: &[Value]| {
            let raw = arg_str(args, 0);
            let normalized = if raw.is_empty() { String::new() } else { normalize_conn_str_full(&raw) };
            let mut obj = Object::new();
            obj.properties.insert("__type".into(), string_value("SqlConnection"));
            obj.properties.insert("__conn_id".into(), Value::F64(0.0));
            obj.properties.insert("connectionstring".into(), string_value(&raw));
            obj.properties.insert("provider".into(), string_value(driver_provider_name(&normalized)));
            obj.properties.insert("serverversion".into(), string_value(""));
            obj.properties.insert("connectiontimeout".into(), Value::F64(30.0));
            obj.properties.insert("state".into(), string_value("Closed"));
            Value::Object(Arc::new(Mutex::new(obj)))
        }));

    vm.register_host_fn("wasi:sql/types", "[static]connection.open",
        Box::new(|_ctx: &mut HostContext, args: &[Value]| {
            let raw = arg_str(args, 0);
            ...
            match open(&url) {
                Ok(driver) => {
                    let id = next_id();
                    state().lock().unwrap().conns.insert(id, ConnEntry { driver, url: url.clone() });
    ...
    vm.register_host_fn("wasi:sql/types", "[method]connection.open",
        Box::new(|_ctx: &mut HostContext, args: &[Value]| {
            ...
            match open(&url) {
                Ok(driver) => {
                    let id = next_id();
                    state().lock().unwrap().conns.insert(id, ConnEntry { driver, url });
                    ...
                    set_prop(obj, "serverversion", string_value(driver_server_version(&raw)));
    ...
    vm.register_host_fn("wasi:sql/types", "[static]statement.prepare",
        Box::new(|_ctx: &mut HostContext, args: &[Value]| {
            let query  = arg_str(args, 0);
            let params = extract_params(args, 1);
            if query.is_empty() { return make_error_obj("empty query"); }
            let id = next_id();
            state().lock().unwrap().stmts.insert(id, StmtEntry { query, params });
            make_stmt_obj(id)
        }));

    vm.register_host_fn("wasi:sql/types", "[method]error.trace",
        Box::new(|_ctx: &mut HostContext, args: &[Value]| {
            if let Some(Value::Object(obj)) = args.first() {
    ...
    vm.register_host_fn("wasi:sql/types", "[method]connection.close",
        Box::new(|_ctx: &mut HostContext, args: &[Value]| {
            let conn_id = get_conn_id(args);
            if conn_id != 0 {
    ...
    vm.register_host_fn("wasi:sql/types", "[method]connection.create-command",
        Box::new(|_ctx: &mut HostContext, args: &[Value]| create_command_from_connection(args, "SqlCommand")));

    vm.register_host_fn("wasi:sql/types", "[method]connection.begin-transaction",
        Box::new(|_ctx: &mut HostContext, args: &[Value]| {
            let conn_id = get_conn_id(args);
            if conn_id == 0 {
                return Value::Null;
    ...
    vm.register_host_fn("wasi:sql/types", "[method]connection.get-schema",
        Box::new(|_ctx: &mut HostContext, args: &[Value]| {
            let conn_id = get_conn_id(args);
    ...
    vm.register_host_fn("wasi:sql/types", "command.new",
        Box::new(|_ctx: &mut HostContext, args: &[Value]| {
    ...
    vm.register_host_fn("wasi:sql/types", "data-adapter.new",
        Box::new(|_ctx: &mut HostContext, args: &[Value]| {
    ...
    vm.register_host_fn("wasi:sql/types", "[method]command.execute-non-query",
        Box::new(|_ctx: &mut HostContext, args: &[Value]| {
    ...
    vm.register_host_fn("wasi:sql/types", "[method]command.execute-scalar",
        Box::new(|_ctx: &mut HostContext, args: &[Value]| {
            let Some(Value::Object(command)) = args.first() else {
                return Value::Null;
            };
    ...
    vm.register_host_fn("wasi:sql/types", "[method]command.execute-reader",
        Box::new(|_ctx: &mut HostContext, args: &[Value]| {
            let Some(Value::Object(command)) = args.first() else {
    ...
    vm.register_host_fn("wasi:sql/types", "[method]command.create-parameter",
        Box::new(|_ctx: &mut HostContext, args: &[Value]| {
            let name = arg_str(args, 1);
            let value = args.get(4).cloned().unwrap_or(Value::Null);
            let mut obj = Object::new();
            obj.properties.insert("__type".into(), string_value("SqlParameter"));
            obj.properties.insert("name".into(), string_value(&name));
            obj.properties.insert("value".into(), value);
            Value::Object(Arc::new(Mutex::new(obj)))
        }));

    vm.register_host_fn("wasi:sql/types", "[method]params.add-with-value",
        Box::new(|_ctx: &mut HostContext, args: &[Value]| {
            let Some(Value::Object(params)) = args.first() else {
                return Value::Null;
            };
            let mut param = Object::new();
            param.properties.insert("__type".into(), string_value("SqlParameter"));
            param.properties.insert("name".into(), string_value(&arg_str(args, 1)));
            param.properties.insert("value".into(), args.get(2).cloned().unwrap_or(Value::Null));
            let Some(Value::Object(items)) = params.lock().unwrap().properties.get("__items").cloned() else {
                return Value::Null;
            };
    ...
    vm.register_host_fn("wasi:sql/types", "[method]params.clear",
        Box::new(|_ctx: &mut HostContext, args: &[Value]| {
            let Some(Value::Object(params)) = args.first() else {
                return Value::Null;
            };
    ...
    vm.register_host_fn("wasi:sql/types", "[method]params.count",
        Box::new(|_ctx: &mut HostContext, args: &[Value]| {
            let Some(Value::Object(params)) = args.first() else {
    ...
    vm.register_host_fn("wasi:sql/types", "[method]transaction.commit",
        Box::new(|_ctx: &mut HostContext, args: &[Value]| {
    ...
    vm.register_host_fn("wasi:sql/types", "[method]transaction.rollback",
        Box::new(|_ctx: &mut HostContext, args: &[Value]| {
    ...
    vm.register_host_fn("wasi:sql/types", "[method]reader.read",
        Box::new(|_ctx: &mut HostContext, args: &[Value]| {
    ...
    vm.register_host_fn("wasi:sql/types", "[method]reader.get-value",
        Box::new(|_ctx: &mut HostContext, args: &[Value]| {
            let Some(Value::Object(reader)) = args.first() else {
                return Value::Null;
            };
    ...
    vm.register_host_fn("wasi:sql/types", "[method]reader.get-string",
        Box::new(|_ctx: &mut HostContext, args: &[Value]| {
            let Some(Value::Object(reader)) = args.first() else {
                return string_value("");
            };
            ...
            string_value(&format!("{}", row_value_by_index(&row, args.get(1).map(Value::as_f64).unwrap_or(0.0) as usize)))
        }));

    vm.register_host_fn("wasi:sql/types", "[method]reader.get-name",
        Box::new(|_ctx: &mut HostContext, args: &[Value]| {
            let Some(Value::Object(reader)) = args.first() else {
                return string_value("");
            };
    ...
    vm.register_host_fn("wasi:sql/types", "[method]reader.is-dbnull",
        Box::new(|_ctx: &mut HostContext, args: &[Value]| {
            let Some(Value::Object(reader)) = args.first() else {
            ...
            Value::Bool(matches!(row_value_by_index(&row, args.get(1).map(Value::as_f64).unwrap_or(0.0) as usize), Value::Null))
        }));

    vm.register_host_fn("wasi:sql/types", "[method]reader.close",
        Box::new(|_ctx: &mut HostContext, args: &[Value]| {
    ...
    vm.register_host_fn("wasi:sql/types", "[method]reader.get-schema-table",
        Box::new(|_ctx: &mut HostContext, args: &[Value]| {
            let Some(Value::Object(reader)) = args.first() else {
            ...
            for (index, value) in names.iter().enumerate() {
                ...
                row.properties.insert("ColumnOrdinal".into(), Value::F64(index as f64));
    ...
    vm.register_host_fn("wasi:sql/types", "[method]adapter.fill",
        Box::new(|_ctx: &mut HostContext, args: &[Value]| {
            ...
            if target_type.eq_ignore_ascii_case("dataset") {
                ...
                set_prop(target, "columns", rows_array(col_names.iter().map(|name| string_value(name)).collect()));
    ...
    vm.register_host_fn("wasi:sql/readwrite", "query",
        Box::new(|_ctx: &mut HostContext, args: &[Value]| {
            ...
            match do_query(conn_id, &sql, &params) {
                ...
                Err(e)   => { eprintln!("wasi:sql/readwrite query: {}", e); rows_array(vec![]) }
            }
        }));

    vm.register_host_fn("wasi:sql/readwrite", "exec",
        Box::new(|_ctx: &mut HostContext, args: &[Value]| {
            let conn_id = args.first().map(|v| wasi_id(v)).unwrap_or(0);
            let stmt_id = args.get(1).map(|v| wasi_id(v)).unwrap_or(0);
            let (sql, params) = {
            ...
            match do_exec(conn_id, &sql, &params) {
                ...
                Err(e) => { eprintln!("wasi:sql/readwrite exec: {}", e); Value::F64(-1.0) }
            }
        }));
...
fn register_flat_api(vm: &mut VM) {
    vm.register_host_fn("wasi:sql", "connect",
        Box::new(|_ctx: &mut HostContext, args: &[Value]| {
            let raw = arg_str(args, 0);
            if raw.is_empty() || raw == "null" {
                let mut obj = Object::new();
            ...
            match open(&url) {
                ...
                Err(e) => { eprintln!("db.connect: {}", e); Value::Null }
            }
        }));

    vm.register_host_fn("wasi:sql", "open",
        Box::new(|_ctx: &mut HostContext, args: &[Value]| {
            if let Some(Value::Object(obj)) = args.first() {
                let raw = obj.lock().unwrap()
    ...
    vm.register_host_fn("wasi:sql", "close",
        Box::new(|_ctx: &mut HostContext, args: &[Value]| {
            state().lock().unwrap().conns.remove(&get_conn_id(args));
            Value::Null
        }));

    vm.register_host_fn("wasi:sql", "createCommand",
        Box::new(|_ctx: &mut HostContext, args: &[Value]| {
            let conn_id = get_conn_id(args);
            let mut obj = Object::new();
    ...
    vm.register_host_fn("wasi:sql", "query",
        Box::new(|_ctx: &mut HostContext, args: &[Value]| {
            ...
            match do_query(id, &sql, &params) {
                Ok(rows) => rows_array(rows),
                Err(e)   => { eprintln!("db.query: {}", e); rows_array(vec![]) }
            }
        }));

    vm.register_host_fn("wasi:sql", "execute",
        Box::new(|_ctx: &mut HostContext, args: &[Value]| {
            let id     = get_conn_id(args);
            let sql    = get_sql(args);
            let params = extract_params(args, 2);
            match do_exec(id, &sql, &params) {
                Ok(n)  => Value::F64(n as f64),
                Err(e) => { eprintln!("db.execute: {}", e); Value::F64(-1.0) }
            }
        }));

    vm.register_host_fn("wasi:sql", "scalar",
        Box::new(|_ctx: &mut HostContext, args: &[Value]| {
            let id     = get_conn_id(args);
            let sql    = get_sql(args);
            let params = extract_params(args, 2);
    ...
    vm.register_host_fn("wasi:sql", "beginTransaction",
        Box::new(|_ctx: &mut HostContext, args: &[Value]| {
            Value::Bool(do_exec(get_conn_id(args), "BEGIN", &[]).is_ok())
        }));

    vm.register_host_fn("wasi:sql", "commit",
        Box::new(|_ctx: &mut HostContext, args: &[Value]| {
            Value::Bool(do_exec(get_conn_id(args), "COMMIT", &[]).is_ok())
        }));

    vm.register_host_fn("wasi:sql", "rollback",
        Box::new(|_ctx: &mut HostContext, args: &[Value]| {
            Value::Bool(do_exec(get_conn_id(args), "ROLLBACK", &[]).is_ok())
        }));

    vm.register_host_fn("wasi:sql", "tables",
        Box::new(|_ctx: &mut HostContext, args: &[Value]| {
            let id = get_conn_id(args);
            let driver = {
                let s = state();
            ...
            let Some(driver) = driver else { return rows_array(vec![]); };
            match driver.query(driver.tables_sql(), &[]) {
                Ok(rows) => rows_array(rows.into_iter().filter_map(|v| {
    ...
    vm.register_host_fn("wasi:sql", "columns",
        Box::new(|_ctx: &mut HostContext, args: &[Value]| {
            let id    = get_conn_id(args);
            ...
            match driver.query(&sql, &[]) {
                Ok(rows) => rows_array(rows.into_iter().filter_map(|v| {
                    if let Value::Object(o) = v {
                        let row = Value::Object(Arc::clone(&o));
                        let col_names = col_names_from_rows(&[row.clone()]);
                        col_names.get(col_idx).map(|name| row_value_by_name(&row, name))
                    } else { None }
                }).collect()),
                Err(_) => rows_array(vec![]),
            }
...
pub fn normalize_conn_str_full(raw: &str) -> String {
    ...
    if raw.starts_with("sqlite:") || raw.starts_with("postgres:")
        || raw.starts_with("postgresql:") || raw.starts_with("mysql:")
        || raw.starts_with("mysql2:") || raw.contains("://")
    {
    ...
    if lower.starts_with("data source=") || lower.starts_with("datasource=") {
        let path = raw.splitn(2, '=').nth(1).unwrap_or("")
            .split(';').next().unwrap_or("").trim().trim_matches('"');
        return format!("sqlite:{}?mode=rwc", path);
    }

    if lower.contains("server=") || lower.contains("host=") {
        let pairs: HashMap<String, String> = raw.split(';').filter_map(|p| {
            let mut kv = p.splitn(2, '=');
            Some((kv.next()?.trim().to_lowercase(), kv.next()?.trim().to_string()))
        }).collect();
...
pub fn test_connection_and_list_tables(conn_str: &str) -> Result<Vec<String>, String> {
    ...
    let rows = driver.query(sql, &[]).map_err(|e| format!("Query failed: {}", e))?;
    Ok(rows.into_iter().filter_map(|v| {
        let names = col_names_from_rows(std::slice::from_ref(&v));
        if let Some(name) = names.first().map(|column| row_value_by_name(&v, column)).map(|value| format!("{}", value)) {
...
pub fn fetch_columns_for_query(conn_str: &str, select_cmd: &str) -> Result<Vec<String>, String> {
    if conn_str.is_empty() || select_cmd.is_empty() { return Ok(vec![]); }
    let url = normalize_conn_str_full(conn_str);
    let driver = open(&url)?;
    driver.query_columns(select_cmd, &[])
}

pub fn query_rows(conn_str: &str, sql: &str)
    -> Result<(Vec<String>, Vec<HashMap<String, String>>), String>
{
    ...
    let result = rows.into_iter().filter_map(|v| {
        if matches!(v, Value::Object(_)) {
            Some(columns.iter().map(|column| (column.clone(), format!("{}", row_value_by_name(&v, column)))).collect())
        } else {
            None
        }
    }).collect();
