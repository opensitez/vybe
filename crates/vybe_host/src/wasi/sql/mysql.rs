...
impl MySqlDriver {
    pub(super) fn open(url: &str) -> Result<Self, String> {
        ...
        Ok(Self { conn: Mutex::new(conn), url })
    }
}

fn to_param(s: &str) -> mysql::Value {
    if let Ok(n) = s.parse::<i64>()  { return mysql::Value::Int(n); }
    if let Ok(f) = s.parse::<f64>()  { return mysql::Value::Double(f); }
    mysql::Value::Bytes(s.as_bytes().to_vec())
}

fn row_to_obj(row: &mysql::Row) -> Value {
    let col_names: Vec<String> = row.columns_ref()
        .iter()
    ...
    for (i, name) in col_names.iter().enumerate() {
        let val = match row.get_opt::<mysql::Value, _>(i) {
            ...
            Some(Ok(mysql::Value::Bytes(b))) => {
                super::parse_scalar(&String::from_utf8_lossy(&b))
            }
...
impl SqlDriver for MySqlDriver {
    ...
    fn query_columns(&self, sql: &str, _params: &[String]) -> Result<Vec<String>, String> {
        ...
        Ok(stmt.columns().iter().map(|col| col.name_str().to_string()).collect())
    }
