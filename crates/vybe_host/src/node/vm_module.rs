...
impl<'a> Eval<'a> {
    ...
    fn mul(&mut self) -> Value {
        ...
        loop {
            match self.lex.peek() {
                Tok::Star => {
                    self.lex.next();
                    left = val_mul(left, self.pow());
                }
                Tok::Slash => {
                    self.lex.next();
                    left = val_div(left, self.pow());
                }
                Tok::Percent => {
                    self.lex.next();
    ...
    fn primary(&mut self) -> Value {
        match self.lex.next() {
            ...
            Tok::Ident(name) => self.vars.get(&name).cloned().unwrap_or(Value::Undefined),
            Tok::LParen => {
                let v = self.expr();
...
fn val_mul(a: Value, b: Value) -> Value {
    num_result(to_f64(&a) * to_f64(&b))
}
fn val_div(a: Value, b: Value) -> Value {
    Value::F64(to_f64(&a) / to_f64(&b))
}
fn val_rem(a: Value, b: Value) -> Value {
    num_result(to_f64(&a) % to_f64(&b))
}
fn val_pow(a: Value, b: Value) -> Value {
    num_result(to_f64(&a).powf(to_f64(&b)))
}
fn val_neg(a: Value) -> Value {
    num_result(-to_f64(&a))
}
        ...
        Box::new(|_ctx, args| match args.first() {
            Some(Value::Object(obj)) => {
                obj.lock()
                    .unwrap()
                    .properties
                    .insert("__isContext".into(), Value::Bool(true));
