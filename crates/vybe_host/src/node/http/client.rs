...
fn make_client_request(method: &str, host: &str, path: &str) -> Value {
    ...
    for m in ["end","write","setHeader","getHeader","removeHeader","destroy",
              "setTimeout","abort","on","once","emit","pipe"] {
...
fn make_agent(max_sockets: i32) -> Value {
    let mut o = Object::new();
    o.properties.insert("maxSockets".into(), Value::I32(max_sockets));
    o.properties.insert("maxFreeSockets".into(), Value::I32(256));
    o.properties.insert("maxTotalSockets".into(), Value::F64(f64::INFINITY));
    ...
    for m in ["destroy","getName","createConnection","addRequest","reuseSocket","removeSocket"] {
        o.properties.insert(m.into(), stub());
    }
    Value::Object(Arc::new(Mutex::new(o)))
}
...
pub fn register(vm: &mut VM) {
    // STATUS_CODES
    vm.register_host_fn("node:http", "STATUS_CODES", Box::new(|_ctx, _args| {
        let mut o = Object::new();
        let codes: &[(&str, &str)] = &[
        ...
        for (code, text) in codes {
            o.properties.insert((*code).into(), Value::String(Arc::from(*text)));
        }
        Value::Object(Arc::new(Mutex::new(o)))
    }));

    // METHODS
    vm.register_host_fn("node:http", "METHODS", Box::new(|_ctx, _args| {
        let methods = ["ACL","BIND","CHECKOUT","CONNECT","COPY","DELETE","GET","HEAD",
                       "LINK","LOCK","M-SEARCH","MERGE","MKACTIVITY","MKCALENDAR","MKCOL",
                       "MOVE","NOTIFY","OPTIONS","PATCH","POST","PROPFIND","PROPPATCH",
                       "PURGE","PUT","REBIND","REPORT","SEARCH","SOURCE","SUBSCRIBE",
                       "TRACE","UNBIND","UNLINK","UNLOCK","UNSUBSCRIBE"];
        let elems: Vec<Value> = methods.iter().map(|m| Value::String(Arc::from(*m))).collect();
        Value::Object(Arc::new(Mutex::new(Object::new_array(elems))))
    }));

    // maxHeaderSize
    vm.register_host_fn("node:http", "maxHeaderSize", Box::new(|_ctx, _args| Value::I32(16384)));

    // globalAgent
    vm.register_host_fn("node:http", "globalAgent", Box::new(|_ctx, _args| make_agent(256)));

    // validateHeaderName
    vm.register_host_fn("node:http", "validateHeaderName", Box::new(|_ctx, args| {
        let name = match args.first() { Some(Value::String(s)) => s.to_string(), _ => return Value::Bool(false) };
        ...
        if valid { Value::Undefined } else { Value::Bool(false) }
    }));

    // validateHeaderValue
    vm.register_host_fn("node:http", "validateHeaderValue", Box::new(|_ctx, _args| Value::Undefined));

    // setMaxIdleHTTPParsers
    vm.register_host_fn("node:http", "setMaxIdleHTTPParsers", Box::new(|_ctx, _args| Value::Undefined));

    // createServer([opts, cb]) → Server
    vm.register_host_fn("node:http", "createServer", Box::new(|_ctx, _args| {
        let mut o = Object::new();
        o.properties.insert("listening".into(), Value::Bool(false));
        o.properties.insert("timeout".into(), Value::I32(0));
        o.properties.insert("keepAliveTimeout".into(), Value::I32(5000));
        o.properties.insert("headersTimeout".into(), Value::I32(60000));
        o.properties.insert("requestTimeout".into(), Value::I32(300000));
        o.properties.insert("maxConnections".into(), Value::Undefined);
        for m in ["listen","close","address","setTimeout","getConnections","on","once","emit","ref","unref"] {
            o.properties.insert(m.into(), stub());
        }
        Value::Object(Arc::new(Mutex::new(o)))
    }));
    ...
    vm.register_host_fn("node:http", "request", Box::new(|_ctx, args| {
    ...
    vm.register_host_fn("node:http", "get", Box::new(|_ctx, args| {
    ...
    // Agent([opts]) → Agent
    vm.register_host_fn("node:http", "Agent", Box::new(|_ctx, args| {
    ...
    vm.register_host_fn("node:http", "IncomingMessage", Box::new(|_ctx, _args| {
        let mut o = Object::new();
        ...
        o.properties.insert("httpVersionMajor".into(), Value::I32(1));
        o.properties.insert("httpVersionMinor".into(), Value::I32(1));
        ...
        o.properties.insert("destroyed".into(), Value::Bool(false));
        for m in ["on","once","emit","pipe","pause","resume","destroy","read","setTimeout"] {
            o.properties.insert(m.into(), stub());
        }
        Value::Object(Arc::new(Mutex::new(o)))
    }));
    ...
    vm.register_host_fn("node:http", "ServerResponse", Box::new(|_ctx, _args| {
        let mut o = Object::new();
        o.properties.insert("statusCode".into(), Value::I32(200));
        o.properties.insert("statusMessage".into(), s("OK"));
        o.properties.insert("headersSent".into(), Value::Bool(false));
        o.properties.insert("writableEnded".into(), Value::Bool(false));
        o.properties.insert("writableFinished".into(), Value::Bool(false));
        o.properties.insert("finished".into(), Value::Bool(false));
        o.properties.insert("writable".into(), Value::Bool(true));
        o.properties.insert("chunkedEncoding".into(), Value::Bool(false));
        for m in ["writeHead","setHeader","getHeader","getHeaders","getHeaderNames",
                  "hasHeader","removeHeader","write","end","flushHeaders",
                  "on","once","emit","setTimeout","addTrailers"] {
            o.properties.insert(m.into(), stub());
        }
        Value::Object(Arc::new(Mutex::new(o)))
    }));
