...
pub fn register(vm: &mut VM) {
    // Raw accessors ────────────────────────────────────────────────────────
    vm.register_host_fn("node:http", "method", Box::new(|_ctx: &mut HostContext, _args: &[Value]| {
        with_context(|c| Value::String(Arc::from(c.method.as_str())))
            .unwrap_or_else(|| Value::String(Arc::from("")))
    }));

    vm.register_host_fn("node:http", "uri", Box::new(|_ctx, _| {
        with_context(|c| Value::String(Arc::from(c.uri.as_str())))
            .unwrap_or_else(|| Value::String(Arc::from("")))
    }));

    vm.register_host_fn("node:http", "path", Box::new(|_ctx, _| {
        with_context(|c| Value::String(Arc::from(c.path.as_str())))
            .unwrap_or_else(|| Value::String(Arc::from("")))
    }));

    vm.register_host_fn("node:http", "query", Box::new(|_ctx, _| {
        with_context(|c| Value::String(Arc::from(c.query.as_str())))
            .unwrap_or_else(|| Value::String(Arc::from("")))
    }));

    vm.register_host_fn("node:http", "scheme", Box::new(|_ctx, _| {
        with_context(|c| Value::String(Arc::from(c.scheme.as_str())))
            .unwrap_or_else(|| Value::String(Arc::from("")))
    }));

    vm.register_host_fn("node:http", "host", Box::new(|_ctx, _| {
        with_context(|c| Value::String(Arc::from(c.host.as_str())))
            .unwrap_or_else(|| Value::String(Arc::from("")))
    }));

    vm.register_host_fn("node:http", "port", Box::new(|_ctx, _| {
        with_context(|c| Value::F64(c.port as f64))
            .unwrap_or(Value::F64(0.0))
    }));

    vm.register_host_fn("node:http", "protocol", Box::new(|_ctx, _| {
        with_context(|c| Value::String(Arc::from(c.protocol.as_str())))
            .unwrap_or_else(|| Value::String(Arc::from("")))
    }));

    vm.register_host_fn("node:http", "remote_addr", Box::new(|_ctx, _| {
        with_context(|c| Value::String(Arc::from(c.remote_addr.as_str())))
            .unwrap_or_else(|| Value::String(Arc::from("")))
    }));

    vm.register_host_fn("node:http", "remote_port", Box::new(|_ctx, _| {
        with_context(|c| Value::F64(c.remote_port as f64))
            .unwrap_or(Value::F64(0.0))
    }));

    // Headers ──────────────────────────────────────────────────────────────
    vm.register_host_fn("node:http", "header", Box::new(|_ctx, args| {
    ...
    vm.register_host_fn("node:http", "header_all", Box::new(|_ctx, args| {
        let name = string_arg(args, 0).to_ascii_lowercase();
        with_context(|c| {
            let mut out = Vec::new();
            for (n, v) in &c.headers {
    ...
    vm.register_host_fn("node:http", "headers", Box::new(|_ctx, _| {
        with_context(|c| {
    ...
    // CGI env ──────────────────────────────────────────────────────────────
    vm.register_host_fn("node:http", "env", Box::new(|_ctx, args| {
    ...
    vm.register_host_fn("node:http", "envs", Box::new(|_ctx, _| {
        with_context(|c| {
    ...
    // Body streaming ───────────────────────────────────────────────────────
    vm.register_host_fn("node:http", "body_length", Box::new(|_ctx, _| {
    ...
    vm.register_host_fn("node:http", "body_eof", Box::new(|_ctx, _| {
        with_context(|c| Value::Bool(c.body.lock().unwrap().eof()))
            .unwrap_or(Value::Bool(true))
    }));

    vm.register_host_fn("node:http", "body_read", Box::new(|_ctx, args| {
    ...
    vm.register_host_fn("node:http", "body_read_all", Box::new(|_ctx, _| {
        with_context(|c| {
    ...
    vm.register_host_fn("node:http", "query_pairs", Box::new(|_ctx, _| {
        with_context(|c| {
