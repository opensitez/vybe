...
fn qs_parse(input: &str, sep: char, eq: char) -> Value {
    ...
    for (k, mut vals) in map {
        if vals.len() == 1 {
            ...
            obj.properties.insert(k, Value::Object(Arc::new(Mutex::new(Object::new_array(arr)))));
        }
    }
...
fn qs_stringify(obj: &Value, sep: char, eq: char) -> String {
    let Value::Object(o) = obj else { return String::new(); };
    let o = o.lock().unwrap();
...
pub fn register(vm: &mut VM) {
    vm.register_host_fn("node:querystring", "parse", Box::new(|_ctx, args| {
    ...
    vm.register_host_fn("node:querystring", "stringify", Box::new(|_ctx, args| {
    ...
    vm.register_host_fn("node:querystring", "escape", Box::new(|_ctx, args| {
        let input = match args.first() {
            Some(Value::String(s)) => s.to_string(),
            _ => String::new(),
        };
    ...
    vm.register_host_fn("node:querystring", "unescape", Box::new(|_ctx, args| {
        let input = match args.first() {
            Some(Value::String(s)) => s.to_string(),
            _ => String::new(),
        };
