...
fn make_server() -> Value {
    ...
    for m in ["listen","close","address","setTimeout","setSecureContext",
              "getConnections","on","once","emit","ref","unref"] {
        o.properties.insert(m.into(), stub());
    }
    Value::Object(Arc::new(Mutex::new(o)))
}
...
fn make_agent(opts: Option<&Value>) -> Value {
    ...
    if let Some(Value::Object(opts_obj)) = opts {
        ...
        for key in ["ca","cert","key","rejectUnauthorized","checkServerIdentity"] {
...
pub fn register(vm: &mut VM) {
    ...
    vm.register_host_fn("node:https", "createServer", Box::new(|_ctx, _args| make_server()));

    // Server class alias
    vm.register_host_fn("node:https", "Server", Box::new(|_ctx, _args| make_server()));

    // request(options[, cb]) → ClientRequest
    vm.register_host_fn("node:https", "request", Box::new(|_ctx, args| {
    ...
    vm.register_host_fn("node:https", "get", Box::new(|_ctx, args| {
    ...
    // Agent([opts]) → Agent
    vm.register_host_fn("node:https", "Agent", Box::new(|_ctx, args| {
        make_agent(args.first())
    }));

    // globalAgent
    vm.register_host_fn("node:https", "globalAgent", Box::new(|_ctx, _args| {
        make_agent(None)
    }));
}
