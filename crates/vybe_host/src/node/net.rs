...
fn ee_methods() -> &'static [&'static str] {
    &["on","once","off","emit","addListener","removeListener",
      "removeAllListeners","listeners","listenerCount","eventNames"]
}

fn make_socket() -> Value {
    let mut o = Object::new();
    o.properties.insert("remoteAddress".into(), Value::Undefined);
    ...
    for m in ["connect","write","end","destroy","pipe","setEncoding",
              "setTimeout","ref","unref","pause","resume","address"] {
...
fn make_server() -> Value {
    ...
    for m in ["listen","close","address","getConnections","setTimeout","ref","unref"] {
        o.properties.insert(m.into(), Value::Undefined);
    }
    for m in ee_methods() {
        o.properties.insert((*m).into(), Value::Undefined);
...
fn classify_ip(s: &str) -> i32 {
    if is_ipv4(s) { 4 } else if is_ipv6(s) { 6 } else { 0 }
}

pub fn register(vm: &mut VM) {
    vm.register_host_fn("node:net", "isIP", Box::new(|_ctx, args| {
        let s = match args.first() { Some(Value::String(s)) => s.to_string(), _ => String::new() };
        Value::I32(classify_ip(&s))
    }));

    vm.register_host_fn("node:net", "isIPv4", Box::new(|_ctx, args| {
        let s = match args.first() { Some(Value::String(s)) => s.to_string(), _ => String::new() };
        Value::Bool(is_ipv4(&s))
    }));

    vm.register_host_fn("node:net", "isIPv6", Box::new(|_ctx, args| {
        let s = match args.first() { Some(Value::String(s)) => s.to_string(), _ => String::new() };
        Value::Bool(is_ipv6(&s))
    }));

    vm.register_host_fn("node:net", "createServer", Box::new(|_ctx, _args| make_server()));
    vm.register_host_fn("node:net", "Server", Box::new(|_ctx, _args| make_server()));

    vm.register_host_fn("node:net", "createConnection", Box::new(|_ctx, _args| make_socket()));
    ...
    vm.register_host_fn("node:net", "BlockList", Box::new(|_ctx, _args| {
        let mut o = Object::new();
        let rules = Value::Object(Arc::new(Mutex::new(
    ...
    vm.register_host_fn("node:net", "SocketAddress", Box::new(|_ctx, args| {
        ...
        let mut o = Object::new();
        o.properties.insert("address".into(), Value::String(Arc::from(addr.as_str())));
        o.properties.insert("port".into(), Value::I32(port));
        o.properties.insert("family".into(), Value::String(Arc::from(family.as_str())));
        Value::Object(Arc::new(Mutex::new(o)))
    }));

    vm.register_host_fn("node:net", "NetConnectOpts", Box::new(|_ctx, _args| {
        Value::Object(Arc::new(Mutex::new(Object::new())))
    }));

    // Top-level addAddress / check forwarded to BlockList instance (first arg)
    vm.register_host_fn("node:net", "addAddress", Box::new(|_ctx, _args| Value::Undefined));
    vm.register_host_fn("node:net", "check", Box::new(|_ctx, _args| Value::Bool(false)));
}
