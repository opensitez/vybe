...
fn ev_key(event: &str) -> String { format!("__ev_{event}") }
fn evo_key(event: &str) -> String { format!("__evo_{event}") }
...
fn make_emitter() -> Value {
    ...
    for m in ["on","once","off","emit","addListener","removeListener",
...
pub fn register(vm: &mut VM) {
    vm.register_host_fn("node:events", "EventEmitter", Box::new(|_ctx, _args| {
        make_emitter()
    }));

    // on / addListener
    for name in ["on", "addListener"] {
        vm.register_host_fn("node:events", name, Box::new(|_ctx, args| {
            let event = match args.get(1) { Some(Value::String(s)) => s.to_string(), _ => return Value::Undefined };
    ...
    // prependListener
    vm.register_host_fn("node:events", "prependListener", Box::new(|_ctx, args| {
        let event = match args.get(1) { Some(Value::String(s)) => s.to_string(), _ => return Value::Undefined };
        let listener = args.get(2).cloned().unwrap_or(Value::Null);
        if let Some(mut em) = get_emitter_mut(args.first().unwrap_or(&Value::Undefined)) {
            let k = ev_key(&event);
    ...
    // once
    vm.register_host_fn("node:events", "once", Box::new(|_ctx, args| {
        let event = match args.get(1) { Some(Value::String(s)) => s.to_string(), _ => return Value::Undefined };
        let listener = args.get(2).cloned().unwrap_or(Value::Null);
        if let Some(mut em) = get_emitter_mut(args.first().unwrap_or(&Value::Undefined)) {
            let k = evo_key(&event);
    ...
    // prependOnceListener
    vm.register_host_fn("node:events", "prependOnceListener", Box::new(|_ctx, args| {
        let event = match args.get(1) { Some(Value::String(s)) => s.to_string(), _ => return Value::Undefined };
        let listener = args.get(2).cloned().unwrap_or(Value::Null);
        if let Some(mut em) = get_emitter_mut(args.first().unwrap_or(&Value::Undefined)) {
            let k = evo_key(&event);
    ...
    vm.register_host_fn("node:events", "removeAllListeners", Box::new(|_ctx, args| {
        if let Some(mut em) = get_emitter_mut(args.first().unwrap_or(&Value::Undefined)) {
            if let Some(Value::String(event)) = args.get(1) {
                ...
                let keys: Vec<String> = em.properties.keys()
                    .filter(|k| k.starts_with("__ev_") || k.starts_with("__evo_"))
                    .cloned()
                    .collect();
                for k in keys { em.properties.remove(&k); }
            }
        }
        Value::Undefined
    }));
    ...
    // emit(ee, event) → Bool
    vm.register_host_fn("node:events", "emit", Box::new(|_ctx, args| {
        let event = match args.get(1) { Some(Value::String(s)) => s.to_string(), _ => return Value::Bool(false) };
        if let Some(mut em) = get_emitter_mut(args.first().unwrap_or(&Value::Undefined)) {
    ...
    vm.register_host_fn("node:events", "listenerCount", Box::new(|_ctx, args| {
        let event = match args.get(1) { Some(Value::String(s)) => s.to_string(), _ => return Value::I32(0) };
    ...
    vm.register_host_fn("node:events", "listeners", Box::new(|_ctx, args| {
        let event = match args.get(1) { Some(Value::String(s)) => s.to_string(), _ => return empty_array() };
        if let Some(em) = get_emitter_mut(args.first().unwrap_or(&Value::Undefined)) {
    ...
    vm.register_host_fn("node:events", "rawListeners", Box::new(|_ctx, args| {
        let event = match args.get(1) { Some(Value::String(s)) => s.to_string(), _ => return empty_array() };
        if let Some(em) = get_emitter_mut(args.first().unwrap_or(&Value::Undefined)) {
            let mut all = get_array(&em, &ev_key(&event));
            all.extend(get_array(&em, &evo_key(&event)));
    ...
    vm.register_host_fn("node:events", "eventNames", Box::new(|_ctx, args| {
        if let Some(em) = get_emitter_mut(args.first().unwrap_or(&Value::Undefined)) {
            ...
            let names: Vec<Value> = seen.into_iter()
    ...
    vm.register_host_fn("node:events", "getMaxListeners", Box::new(|_ctx, args| {
        if let Some(em) = get_emitter_mut(args.first().unwrap_or(&Value::Undefined)) {
            return em.properties.get("__maxlisteners").cloned().unwrap_or(Value::I32(10));
        }
        Value::I32(10)
    }));

    // setMaxListeners(ee, n)
    vm.register_host_fn("node:events", "setMaxListeners", Box::new(|_ctx, args| {
    ...
    vm.register_host_fn("node:events", "defaultMaxListeners", Box::new(|_ctx, _args| {
        Value::I32(10)
    }));

    // getEventListeners(ee, event) → Array
    vm.register_host_fn("node:events", "getEventListeners", Box::new(|_ctx, args| {
        let event = match args.get(1) { Some(Value::String(s)) => s.to_string(), _ => return empty_array() };
        if let Some(em) = get_emitter_mut(args.first().unwrap_or(&Value::Undefined)) {
            let mut all = get_array(&em, &ev_key(&event));
            all.extend(get_array(&em, &evo_key(&event)));
    ...
    vm.register_host_fn("node:events", "errorMonitor", Box::new(|_ctx, _args| {
        Value::String(Arc::from("Symbol(events.errorMonitor)"))
    }));
}
