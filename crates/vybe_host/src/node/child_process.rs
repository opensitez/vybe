...
fn null_stream() -> Value {
    ...
    for m in ["write","read","on","once","off","emit","pipe","destroy","resume","pause"] {
...
fn make_child_process(pid: u32) -> Value {
    ...
    o.properties.insert("stdio".into(), Value::Object(Arc::new(Mutex::new(Object::new_array(stdio)))));
    for m in ["send","disconnect","ref","unref","on","once","off","emit",
...
pub fn register(vm: &mut VM) {
    ...
    vm.register_host_fn("node:child_process", "spawnSync", Box::new(|_ctx, args| {
        ...
        if let Some(ref inp) = input {
            ...
            let mut o = Object::new();
            o.properties.insert("pid".into(), Value::F64(pid as f64));
            o.properties.insert("stdout".into(), s_val(&String::from_utf8_lossy(&out.stdout)));
            o.properties.insert("stderr".into(), s_val(&String::from_utf8_lossy(&out.stderr)));
            o.properties.insert("status".into(), Value::F64(out.status.code().unwrap_or(-1) as f64));
            o.properties.insert("signal".into(), Value::Null);
    ...
    vm.register_host_fn("node:child_process", "spawn", Box::new(|_ctx, args| {
        let command = s_arg(args, 0, "");
        let cmd_args: Vec<String> = match args.get(1) {
            Some(arr) => extract_string_array(arr),
            None => Vec::new(),
    ...
    vm.register_host_fn("node:child_process", "exec", Box::new(|_ctx, args| {
        let command = s_arg(args, 0, "");
        let mut cmd = if cfg!(windows) {
            let mut c = Command::new("cmd");
            c.args(["/d", "/s", "/c", &command]);
        ...
        let pid = match cmd.spawn() {
            Ok(child) => { let p = child.id(); std::mem::forget(child); p }
            Err(_) => std::process::id() + 1,
        };
        make_child_process(pid)
    }));
    ...
    vm.register_host_fn("node:child_process", "execFile", Box::new(|_ctx, args| {
        let file = s_arg(args, 0, "");
        let cmd_args: Vec<String> = match args.get(1) {
            Some(arr) => extract_string_array(arr),
            None => Vec::new(),
    ...
    vm.register_host_fn("node:child_process", "fork", Box::new(|_ctx, _args| {
    ...
    vm.register_host_fn("node:child_process", "kill", Box::new(|_ctx, args| {
