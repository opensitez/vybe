...
pub fn register(vm: &mut VM) {
    vm.register_host_fn(
        "node:os",
        "platform",
        Box::new(|_ctx, _args| s_val(node_platform())),
    );
    vm.register_host_fn(
        "node:os",
        "arch",
        Box::new(|_ctx, _args| s_val(node_arch())),
    );
    vm.register_host_fn(
        "node:os",
        "type",
        Box::new(|_ctx, _args| s_val(node_type())),
    );
    vm.register_host_fn(
        "node:os",
        "release",
    ...
    vm.register_host_fn(
        "node:os",
        "hostname",
        Box::new(|_ctx, _args| s_val(&hostname())),
    );
    vm.register_host_fn(
        "node:os",
        "tmpdir",
        Box::new(|_ctx, _args| s_val(std::env::temp_dir().to_string_lossy().as_ref())),
    );
    vm.register_host_fn(
        "node:os",
        "homedir",
        Box::new(|_ctx, _args| s_val(&home_dir())),
    );
    ...
    vm.register_host_fn(
        ...
        "endianness",
        Box::new(|_ctx, _args| {
            s_val(if cfg!(target_endian = "big") {
                "BE"
            } else {
                "LE"
            })
    ...
    vm.register_host_fn(
        "node:os",
        "EOL",
        Box::new(|_ctx, _args| s_val(if cfg!(windows) { "\r\n" } else { "\n" })),
    );
    ...
    vm.register_host_fn(
        "node:os",
        "totalmem",
        Box::new(|_ctx, _args| Value::F64(read_meminfo("MemTotal").unwrap_or(1_073_741_824.0))),
    );
    vm.register_host_fn(
        "node:os",
        "freemem",
        Box::new(|_ctx, _args| Value::F64(read_meminfo("MemAvailable").unwrap_or(0.0))),
    );
    vm.register_host_fn(
        "node:os",
        "uptime",
        Box::new(|_ctx, _args| Value::F64(read_uptime().unwrap_or(0.0))),
    );
    ...
    vm.register_host_fn(
        ...
        Box::new(|_ctx, _args| {
            ...
            for _ in 0..count {
                ...
                o.properties
                    .insert("times".into(), Value::Object(Arc::new(Mutex::new(times))));
    ...
    vm.register_host_fn(
        ...
        Box::new(|_ctx, _args| {
            ...
            entry
                .properties
                .insert("address".into(), s_val("127.0.0.1"));
            entry
                .properties
                .insert("netmask".into(), s_val("255.0.0.0"));
            entry.properties.insert("family".into(), s_val("IPv4"));
            entry.properties.insert("cidr".into(), s_val("127.0.0.1/8"));
            entry
                .properties
                .insert("mac".into(), s_val("00:00:00:00:00:00"));
            entry
                .properties
    ...
    vm.register_host_fn(
        "node:os",
        "machine",
        Box::new(|_ctx, _args| s_val(std::env::consts::ARCH)),
    );
    ...
    vm.register_host_fn(
        ...
        Box::new(|_ctx, _args| {
            ...
            priority_obj
                .properties
                .insert("PRIORITY_LOW".into(), Value::I32(19));
            priority_obj
                .properties
                .insert("PRIORITY_BELOW_NORMAL".into(), Value::I32(10));
            priority_obj
                .properties
                .insert("PRIORITY_NORMAL".into(), Value::I32(0));
            priority_obj
                .properties
                .insert("PRIORITY_ABOVE_NORMAL".into(), Value::I32(-7));
            priority_obj
                .properties
                .insert("PRIORITY_HIGH".into(), Value::I32(-14));
            priority_obj
                .properties
                .insert("PRIORITY_HIGHEST".into(), Value::I32(-20));

            let mut root = Object::new();
            root.properties.insert(
                "errno".into(),
                Value::Object(Arc::new(Mutex::new(errno_obj))),
            );
            root.properties.insert(
                "signals".into(),
                Value::Object(Arc::new(Mutex::new(signals_obj))),
            );
            root.properties.insert(
                "priority".into(),
                Value::Object(Arc::new(Mutex::new(priority_obj))),
            );
            Value::Object(Arc::new(Mutex::new(root)))
        }),
    );
