...
fn bytes_from_value(v: &Value) -> Vec<u8> {
    match v {
        Value::Object(obj) => {
            ...
            match &obj.kind {
                ObjectKind::Array(elems) => elems.iter().map(|e| match e {
                    Value::I32(n) => *n as u8,
                    Value::F64(f) => *f as u8,
                    _ => 0,
                }).collect(),
...
fn base64_decode(s: &str) -> Vec<u8> {
    ...
    let chars: Vec<u8> = s.chars().filter(|c| *c != '=' && *c != '\n' && *c != '\r')
        .map(decode_char).filter(|&b| b != 255).collect();
    let mut out = Vec::new();
    let mut i = 0;
    while i + 1 < chars.len() {
...
fn encode_with_encoding(bytes: &[u8], enc: &str) -> String {
    match enc.to_lowercase().as_str() {
        ...
        "base64url" => base64_encode(bytes).replace('+', "-").replace('/', "_").replace('=', ""),
...
pub fn register(vm: &mut VM) {
    ...
    vm.register_host_fn("node:buffer", "alloc", Box::new(|_ctx, args| {
    ...
    vm.register_host_fn("node:buffer", "from", Box::new(|_ctx, args| {
        match args.first() {
            Some(Value::String(s)) => {
            ...
            Some(Value::Object(obj)) => {
                ...
                match &obj.kind {
                    ObjectKind::Array(elems) => {
                        let bytes: Vec<u8> = elems.iter().map(|e| match e {
    ...
    vm.register_host_fn("node:buffer", "byteLength", Box::new(|_ctx, args| {
    ...
    vm.register_host_fn("node:buffer", "concat", Box::new(|_ctx, args| {
        ...
        if let Some(limit) = args.get(1) {
            let len = match limit { Value::I32(n) => *n as usize, Value::F64(f) => *f as usize, _ => all.len() };
            all.truncate(len);
        }
        bytes_to_buf(all)
    }));
    ...
    // isBuffer(value)
    vm.register_host_fn("node:buffer", "isBuffer", Box::new(|_ctx, args| {
    ...
    // isEncoding(encoding)
    vm.register_host_fn("node:buffer", "isEncoding", Box::new(|_ctx, args| {
        let enc = match args.first() { Some(Value::String(s)) => s.to_lowercase(), _ => return Value::Bool(false) };
        let valid = ["utf8","utf-8","hex","base64","base64url","ascii","latin1",
                     "binary","ucs2","ucs-2","utf16le","utf-16le"];
        Value::Bool(valid.contains(&enc.as_str()))
    }));

    // compare(buf1, buf2) → -1 | 0 | 1
    vm.register_host_fn("node:buffer", "compare", Box::new(|_ctx, args| {
    ...
    vm.register_host_fn("node:buffer", "toString", Box::new(|_ctx, args| {
    ...
    // slice(buf, start, end)
    vm.register_host_fn("node:buffer", "slice", Box::new(|_ctx, args| {
        let bytes = bytes_from_value(args.first().unwrap_or(&Value::Undefined));
    ...
    vm.register_host_fn("node:buffer", "subarray", Box::new(|_ctx, args| {
        let bytes = bytes_from_value(args.first().unwrap_or(&Value::Undefined));
        let len = bytes.len() as i32;
        let start = {
            let s = get_i32(args, 1, 0);
    ...
    vm.register_host_fn("node:buffer", "copy", Box::new(|_ctx, args| {
        ...
        let src_slice = if src_start < src_len { &src[src_start..src_end.min(src_len)] } else { &[] };
        if let Some(Value::Object(dst_obj)) = args.get(1) {
            ...
            if let ObjectKind::Array(ref mut elems) = dst_obj.kind {
                ...
                for (i, &b) in src_slice.iter().enumerate() {
                    ...
                    if ti < elems.len() { elems[ti] = Value::I32(b as i32); copied += 1; }
                }
    ...
    vm.register_host_fn("node:buffer", "indexOf", Box::new(|_ctx, args| {
        ...
        if needle.is_empty() { return Value::I32(offset as i32); }
        for i in offset..haystack.len() {
            if haystack[i..].starts_with(&needle) { return Value::I32(i as i32); }
        }
        Value::I32(-1)
    }));

    // includes(buf, val) → bool
    vm.register_host_fn("node:buffer", "includes", Box::new(|_ctx, args| {
        let haystack = bytes_from_value(args.first().unwrap_or(&Value::Undefined));
        let needle: Vec<u8> = match args.get(1) {
            Some(Value::I32(n)) => vec![(*n & 0xff) as u8],
            Some(Value::F64(f)) => vec![(*f as i32 & 0xff) as u8],
        ...
        if needle.is_empty() { return Value::Bool(true); }
        for i in 0..haystack.len() {
            if haystack[i..].starts_with(&needle) { return Value::Bool(true); }
        }
        Value::Bool(false)
    }));

    // equals(buf1, buf2) → bool
    vm.register_host_fn("node:buffer", "equals", Box::new(|_ctx, args| {
        let a = bytes_from_value(args.first().unwrap_or(&Value::Undefined));
        let b = bytes_from_value(args.get(1).unwrap_or(&Value::Undefined));
        Value::Bool(a == b)
    }));
    ...
    vm.register_host_fn("node:buffer", "fill", Box::new(|_ctx, args| {
    ...
    vm.register_host_fn("node:buffer", "swap16", Box::new(|_ctx, args| {
        let mut bytes = bytes_from_value(args.first().unwrap_or(&Value::Undefined));
        let mut i = 0;
        while i + 1 < bytes.len() { bytes.swap(i, i + 1); i += 2; }
        bytes_to_buf(bytes)
    }));

    // swap32(buf) — swap byte order in quads
    vm.register_host_fn("node:buffer", "swap32", Box::new(|_ctx, args| {
        let mut bytes = bytes_from_value(args.first().unwrap_or(&Value::Undefined));
        let mut i = 0;
        while i + 3 < bytes.len() {
            bytes.swap(i, i + 3); bytes.swap(i + 1, i + 2); i += 4;
        }
        bytes_to_buf(bytes)
    }));

    // readUInt8(buf, offset)
    vm.register_host_fn("node:buffer", "readUInt8", Box::new(|_ctx, args| {
        let bytes = bytes_from_value(args.first().unwrap_or(&Value::Undefined));
    ...
    // readInt8(buf, offset)
    vm.register_host_fn("node:buffer", "readInt8", Box::new(|_ctx, args| {
        let bytes = bytes_from_value(args.first().unwrap_or(&Value::Undefined));
        let off = get_i32(args, 1, 0) as usize;
    ...
    // readUInt16BE
    vm.register_host_fn("node:buffer", "readUInt16BE", Box::new(|_ctx, args| {
        let bytes = bytes_from_value(args.first().unwrap_or(&Value::Undefined));
        let off = get_i32(args, 1, 0) as usize;
        if off + 1 < bytes.len() {
    ...
    // readUInt16LE
    vm.register_host_fn("node:buffer", "readUInt16LE", Box::new(|_ctx, args| {
        let bytes = bytes_from_value(args.first().unwrap_or(&Value::Undefined));
        let off = get_i32(args, 1, 0) as usize;
        if off + 1 < bytes.len() {
    ...
    // readUInt32BE
    vm.register_host_fn("node:buffer", "readUInt32BE", Box::new(|_ctx, args| {
        let bytes = bytes_from_value(args.first().unwrap_or(&Value::Undefined));
        let off = get_i32(args, 1, 0) as usize;
        if off + 3 < bytes.len() {
            Value::F64(u32::from_be_bytes([bytes[off],bytes[off+1],bytes[off+2],bytes[off+3]]) as f64)
        } else { Value::I32(0) }
    }));

    // readUInt32LE
    vm.register_host_fn("node:buffer", "readUInt32LE", Box::new(|_ctx, args| {
        let bytes = bytes_from_value(args.first().unwrap_or(&Value::Undefined));
        let off = get_i32(args, 1, 0) as usize;
        if off + 3 < bytes.len() {
            Value::F64(u32::from_le_bytes([bytes[off],bytes[off+1],bytes[off+2],bytes[off+3]]) as f64)
        } else { Value::I32(0) }
    }));

    // writeUInt8(buf, value, offset) → offset + 1
    vm.register_host_fn("node:buffer", "writeUInt8", Box::new(|_ctx, args| {
        let val = (get_i32(args, 1, 0) & 0xff) as u8;
        let off = get_i32(args, 2, 0) as usize;
        if let Some(Value::Object(obj)) = args.first() {
            let mut obj = obj.lock().unwrap();
            if let ObjectKind::Array(ref mut elems) = obj.kind {
                if off < elems.len() { elems[off] = Value::I32(val as i32); }
            }
        }
    ...
    vm.register_host_fn("node:buffer", "writeInt8", Box::new(|_ctx, args| {
    ...
    vm.register_host_fn("node:buffer", "writeUInt16BE", Box::new(|_ctx, args| {
        ...
        if let Some(Value::Object(obj)) = args.first() {
            ...
            if let ObjectKind::Array(ref mut elems) = obj.kind {
                for (i, &b) in val.iter().enumerate() {
                    if off + i < elems.len() { elems[off + i] = Value::I32(b as i32); }
                }
            }
        }
    ...
    vm.register_host_fn("node:buffer", "writeUInt16LE", Box::new(|_ctx, args| {
    ...
    vm.register_host_fn("node:buffer", "writeUInt32BE", Box::new(|_ctx, args| {
    ...
    vm.register_host_fn("node:buffer", "writeUInt32LE", Box::new(|_ctx, args| {
        let val = match args.get(1) {
    ...
    vm.register_host_fn("node:buffer", "readInt16BE", Box::new(|_ctx, args| {
        let bytes = bytes_from_value(args.first().unwrap_or(&Value::Undefined));
        let off = get_i32(args, 1, 0) as usize;
        if off + 1 < bytes.len() {
    ...
    vm.register_host_fn("node:buffer", "readInt16LE", Box::new(|_ctx, args| {
        let bytes = bytes_from_value(args.first().unwrap_or(&Value::Undefined));
        let off = get_i32(args, 1, 0) as usize;
        if off + 1 < bytes.len() {
    ...
    vm.register_host_fn("node:buffer", "readInt32BE", Box::new(|_ctx, args| {
        let bytes = bytes_from_value(args.first().unwrap_or(&Value::Undefined));
        let off = get_i32(args, 1, 0) as usize;
        if off + 3 < bytes.len() {
            Value::I32(i32::from_be_bytes([bytes[off],bytes[off+1],bytes[off+2],bytes[off+3]]))
        } else { Value::I32(0) }
    }));
    vm.register_host_fn("node:buffer", "readInt32LE", Box::new(|_ctx, args| {
        let bytes = bytes_from_value(args.first().unwrap_or(&Value::Undefined));
        let off = get_i32(args, 1, 0) as usize;
        if off + 3 < bytes.len() {
            Value::I32(i32::from_le_bytes([bytes[off],bytes[off+1],bytes[off+2],bytes[off+3]]))
        } else { Value::I32(0) }
    }));

    // readFloatBE / readFloatLE
    vm.register_host_fn("node:buffer", "readFloatBE", Box::new(|_ctx, args| {
        let bytes = bytes_from_value(args.first().unwrap_or(&Value::Undefined));
        let off = get_i32(args, 1, 0) as usize;
        if off + 3 < bytes.len() {
            Value::F64(f32::from_be_bytes([bytes[off],bytes[off+1],bytes[off+2],bytes[off+3]]) as f64)
        } else { Value::F64(0.0) }
    }));
    vm.register_host_fn("node:buffer", "readFloatLE", Box::new(|_ctx, args| {
        let bytes = bytes_from_value(args.first().unwrap_or(&Value::Undefined));
        let off = get_i32(args, 1, 0) as usize;
        if off + 3 < bytes.len() {
            Value::F64(f32::from_le_bytes([bytes[off],bytes[off+1],bytes[off+2],bytes[off+3]]) as f64)
        } else { Value::F64(0.0) }
    }));

    // readDoubleBE / readDoubleLE
    vm.register_host_fn("node:buffer", "readDoubleBE", Box::new(|_ctx, args| {
        let bytes = bytes_from_value(args.first().unwrap_or(&Value::Undefined));
        let off = get_i32(args, 1, 0) as usize;
        if off + 7 < bytes.len() {
    ...
    vm.register_host_fn("node:buffer", "readDoubleLE", Box::new(|_ctx, args| {
        let bytes = bytes_from_value(args.first().unwrap_or(&Value::Undefined));
        let off = get_i32(args, 1, 0) as usize;
        if off + 7 < bytes.len() {
            let arr: [u8; 8] = bytes[off..off+8].try_into().unwrap_or([0;8]);
    ...
    vm.register_host_fn("node:buffer", "readBigUInt64BE", Box::new(|_ctx, args| {
        let bytes = bytes_from_value(args.first().unwrap_or(&Value::Undefined));
        let off = get_i32(args, 1, 0) as usize;
        if off + 7 < bytes.len() {
            let arr: [u8; 8] = bytes[off..off+8].try_into().unwrap_or([0;8]);
    ...
    vm.register_host_fn("node:buffer", "readBigUInt64LE", Box::new(|_ctx, args| {
        let bytes = bytes_from_value(args.first().unwrap_or(&Value::Undefined));
        let off = get_i32(args, 1, 0) as usize;
        if off + 7 < bytes.len() {
            let arr: [u8; 8] = bytes[off..off+8].try_into().unwrap_or([0;8]);
    ...
    vm.register_host_fn("node:buffer", "readBigInt64BE", Box::new(|_ctx, args| {
        let bytes = bytes_from_value(args.first().unwrap_or(&Value::Undefined));
        let off = get_i32(args, 1, 0) as usize;
        if off + 7 < bytes.len() {
            let arr: [u8; 8] = bytes[off..off+8].try_into().unwrap_or([0;8]);
    ...
    vm.register_host_fn("node:buffer", "readBigInt64LE", Box::new(|_ctx, args| {
        let bytes = bytes_from_value(args.first().unwrap_or(&Value::Undefined));
        let off = get_i32(args, 1, 0) as usize;
        if off + 7 < bytes.len() {
            let arr: [u8; 8] = bytes[off..off+8].try_into().unwrap_or([0;8]);
    ...
    vm.register_host_fn("node:buffer", "writeInt16BE", Box::new(|_ctx, args| {
    ...
    vm.register_host_fn("node:buffer", "writeInt16LE", Box::new(|_ctx, args| {
    ...
    vm.register_host_fn("node:buffer", "writeInt32BE", Box::new(|_ctx, args| {
    ...
    vm.register_host_fn("node:buffer", "writeInt32LE", Box::new(|_ctx, args| {
    ...
    vm.register_host_fn("node:buffer", "writeFloatBE", Box::new(|_ctx, args| {
        let f = match args.get(1) { Some(Value::F64(f)) => *f as f32, Some(Value::I32(n)) => *n as f32, _ => 0.0f32 };
    ...
    vm.register_host_fn("node:buffer", "writeFloatLE", Box::new(|_ctx, args| {
        let f = match args.get(1) { Some(Value::F64(f)) => *f as f32, Some(Value::I32(n)) => *n as f32, _ => 0.0f32 };
    ...
    vm.register_host_fn("node:buffer", "writeDoubleBE", Box::new(|_ctx, args| {
        let f = match args.get(1) { Some(Value::F64(f)) => *f, Some(Value::I32(n)) => *n as f64, _ => 0.0f64 };
        let val = f.to_be_bytes();
        let off = get_i32(args, 2, 0) as usize;
        if let Some(Value::Object(obj)) = args.first() {
            let mut obj = obj.lock().unwrap();
    ...
    vm.register_host_fn("node:buffer", "writeDoubleLE", Box::new(|_ctx, args| {
        let f = match args.get(1) { Some(Value::F64(f)) => *f, Some(Value::I32(n)) => *n as f64, _ => 0.0f64 };
        let val = f.to_le_bytes();
        let off = get_i32(args, 2, 0) as usize;
        if let Some(Value::Object(obj)) = args.first() {
    ...
    vm.register_host_fn("node:buffer", "swap64", Box::new(|_ctx, args| {
        let mut bytes = bytes_from_value(args.first().unwrap_or(&Value::Undefined));
        let mut i = 0;
        while i + 7 < bytes.len() {
            bytes.swap(i, i+7); bytes.swap(i+1, i+6); bytes.swap(i+2, i+5); bytes.swap(i+3, i+4);
            i += 8;
        }
        bytes_to_buf(bytes)
    }));
    ...
    vm.register_host_fn("node:buffer", "reverse", Box::new(|_ctx, args| {
        let mut bytes = bytes_from_value(args.first().unwrap_or(&Value::Undefined));
        bytes.reverse();
        bytes_to_buf(bytes)
    }));
