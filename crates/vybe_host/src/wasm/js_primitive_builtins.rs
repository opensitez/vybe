...
fn register_number(vm: &mut VM) {
    ...
    vm.register_host_fn("wasm:js-number", "testI32", Box::new(|_ctx: &mut HostContext, args: &[Value]| {
        Value::I32(match args.first() {
            Some(Value::I32(_)) => 1,
            Some(Value::F64(n)) => {
                if is_neg_zero(*n) { return Value::I32(0); }
    ...
    vm.register_host_fn("wasm:js-number", "testU32", Box::new(|_ctx: &mut HostContext, args: &[Value]| {
        Value::I32(match args.first() {
    ...
    vm.register_host_fn("wasm:js-number", "fromU32", Box::new(|_ctx: &mut HostContext, args: &[Value]| {
        Value::F64((args.first().map(|v| v.as_i32()).unwrap_or(0) as u32) as f64)
    }));

    // toF64(externref) -> f64 — trap if not a number
    vm.register_host_fn("wasm:js-number", "toF64", Box::new(|ctx: &mut HostContext, args: &[Value]| {
        match args.first() {
            Some(Value::F64(n)) => Value::F64(*n),
            Some(Value::I32(n)) => Value::F64(*n as f64),
            Some(Value::I64(n)) => Value::F64(*n as f64),
            _ => {
                ctx.throw_value(Value::String(Arc::from("TypeError: wasm:js-number.toF64 — not a number")));
                Value::Null
            }
        }
    }));
    ...
    vm.register_host_fn("wasm:js-number", "toI32", Box::new(|ctx: &mut HostContext, args: &[Value]| {
        let n = match args.first() {
            Some(Value::I32(n)) => return Value::I32(*n),
            Some(Value::F64(n)) => *n,
            Some(Value::I64(n)) => *n as f64,
            _ => {
                ctx.throw_value(Value::String(Arc::from("TypeError: wasm:js-number.toI32 — not a number")));
                return Value::Null;
            }
        };
        if is_neg_zero(n) || (n as i32) as f64 != n {
            ctx.throw_value(Value::String(Arc::from("TypeError: wasm:js-number.toI32 — not an i32 integer")));
            return Value::Null;
        }
        Value::I32(n as i32)
    }));
    ...
    vm.register_host_fn("wasm:js-number", "toU32", Box::new(|ctx: &mut HostContext, args: &[Value]| {
        let n = match args.first() {
            Some(Value::I32(n)) if *n >= 0 => return Value::I32(*n),
            Some(Value::F64(n)) => *n,
            Some(Value::I64(n)) if *n >= 0 && *n <= u32::MAX as i64 => return Value::I32(*n as i32),
            _ => {
                ctx.throw_value(Value::String(Arc::from("TypeError: wasm:js-number.toU32 — not a number")));
                return Value::Null;
            }
        };
        if is_neg_zero(n) || (n as u32) as f64 != n {
            ctx.throw_value(Value::String(Arc::from("TypeError: wasm:js-number.toU32 — not a u32 integer")));
            return Value::Null;
        }
...
fn register_symbol(vm: &mut VM) {
    ...
    vm.register_host_fn("wasm:js-symbol", "test", Box::new(|_ctx: &mut HostContext, args: &[Value]| {
        Value::I32(if matches!(args.first(), Some(Value::Symbol(_))) { 1 } else { 0 })
    }));

    // equals(externref, externref) -> i32
    // Identity equality (Arc::ptr_eq). Traps if either arg is not a symbol or null.
    vm.register_host_fn("wasm:js-symbol", "equals", Box::new(|ctx: &mut HostContext, args: &[Value]| {
        let x = args.first().unwrap_or(&Value::Null);
        let y = args.get(1).unwrap_or(&Value::Null);
        if !matches!(x, Value::Symbol(_) | Value::Null) || !matches!(y, Value::Symbol(_) | Value::Null) {
            ctx.throw_value(Value::String(Arc::from("TypeError: wasm:js-symbol.equals — not a symbol or null")));
            return Value::Null;
        }
        Value::I32(match (x, y) {
            (Value::Symbol(a), Value::Symbol(b)) => if Arc::ptr_eq(a, b) { 1 } else { 0 },
...
fn register_bigint(vm: &mut VM) {
    ...
    vm.register_host_fn("wasm:js-bigint", "test", Box::new(|_ctx: &mut HostContext, args: &[Value]| {
        Value::I32(if matches!(args.first(), Some(Value::BigInt(_))) { 1 } else { 0 })
    }));
}
