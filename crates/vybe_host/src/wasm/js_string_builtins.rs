...
pub fn register(vm: &mut VM) {
    ...
    vm.register_host_fn("wasm:js-string", "cast", Box::new(|ctx: &mut HostContext, args: &[Value]| {
        match args.first() {
            Some(v @ Value::String(_)) => v.clone(),
            _ => { trap(ctx, "TypeError: wasm:js-string.cast — not a string"); Value::Null }
        }
    }));

    // length(externref) -> i32
    // UTF-16 code unit count. Traps on null or non-string.
    vm.register_host_fn("wasm:js-string", "length", Box::new(|ctx: &mut HostContext, args: &[Value]| {
        match args.first() {
            Some(Value::String(s)) => Value::I32(s.encode_utf16().count() as i32),
            _ => { trap(ctx, "TypeError: wasm:js-string.length — not a string"); Value::Null }
        }
    }));

    // concat(externref, externref) -> (ref extern)
    // Traps if either argument is null or non-string.
    vm.register_host_fn("wasm:js-string", "concat", Box::new(|ctx: &mut HostContext, args: &[Value]| {
        let a = match args.first() {
            Some(Value::String(s)) => s.clone(),
            _ => { trap(ctx, "TypeError: wasm:js-string.concat — first arg not a string"); return Value::Null; }
        };
        let b = match args.get(1) {
            Some(Value::String(s)) => s.clone(),
            _ => { trap(ctx, "TypeError: wasm:js-string.concat — second arg not a string"); return Value::Null; }
        };
    ...
    vm.register_host_fn("wasm:js-string", "substring", Box::new(|ctx: &mut HostContext, args: &[Value]| {
        let s = match args.first() {
            Some(Value::String(s)) => s.clone(),
            _ => { trap(ctx, "TypeError: wasm:js-string.substring — not a string"); return Value::Null; }
        };
        ...
        Value::String(Arc::from(String::from_utf16_lossy(&units[start..end]).as_ref()))
    }));

    // equals(externref, externref) -> i32
    // Allows null (null == null is 1, null != string is 0).
    // Traps if non-null and non-string.
    vm.register_host_fn("wasm:js-string", "equals", Box::new(|ctx: &mut HostContext, args: &[Value]| {
        let a = args.first().unwrap_or(&Value::Null);
        let b = args.get(1).unwrap_or(&Value::Null);
        if !matches!(a, Value::Null) && !is_string(a) {
            trap(ctx, "TypeError: wasm:js-string.equals — first arg not a string or null");
            return Value::Null;
        }
        if !matches!(b, Value::Null) && !is_string(b) {
            trap(ctx, "TypeError: wasm:js-string.equals — second arg not a string or null");
            return Value::Null;
        }
        Value::I32(match (a, b) {
            (Value::String(x), Value::String(y)) => if x == y { 1 } else { 0 },
    ...
    vm.register_host_fn("wasm:js-string", "compare", Box::new(|ctx: &mut HostContext, args: &[Value]| {
        let a = match args.first() {
            Some(Value::String(s)) => s.clone(),
            _ => { trap(ctx, "TypeError: wasm:js-string.compare — first arg not a string"); return Value::Null; }
        };
        let b = match args.get(1) {
            Some(Value::String(s)) => s.clone(),
            _ => { trap(ctx, "TypeError: wasm:js-string.compare — second arg not a string"); return Value::Null; }
        };
    ...
    vm.register_host_fn("wasm:js-string", "charCodeAt", Box::new(|ctx: &mut HostContext, args: &[Value]| {
        let s = match args.first() {
            Some(Value::String(s)) => s.clone(),
            _ => { trap(ctx, "TypeError: wasm:js-string.charCodeAt — not a string"); return Value::Null; }
        };
        let units: Vec<u16> = s.encode_utf16().collect();
        let idx = (args.get(1).map(|v| v.as_i32()).unwrap_or(0) as u32) as usize;
        if idx >= units.len() {
            trap(ctx, "RangeError: wasm:js-string.charCodeAt — index out of bounds");
            return Value::Null;
        }
    ...
    vm.register_host_fn("wasm:js-string", "codePointAt", Box::new(|ctx: &mut HostContext, args: &[Value]| {
        let s = match args.first() {
            Some(Value::String(s)) => s.clone(),
            _ => { trap(ctx, "TypeError: wasm:js-string.codePointAt — not a string"); return Value::Null; }
        };
        let units: Vec<u16> = s.encode_utf16().collect();
        let idx = (args.get(1).map(|v| v.as_i32()).unwrap_or(0) as u32) as usize;
        if idx >= units.len() {
            trap(ctx, "RangeError: wasm:js-string.codePointAt — index out of bounds");
            return Value::Null;
        }
        let hi = units[idx];
    ...
    vm.register_host_fn("wasm:js-string", "fromCodePoint", Box::new(|ctx: &mut HostContext, args: &[Value]| {
        let code = args.first().map(|v| v.as_i32()).unwrap_or(0) as u32;
        if code > 0x10FFFF {
            trap(ctx, "RangeError: wasm:js-string.fromCodePoint — code point out of range");
            return Value::Null;
        }
        match char::from_u32(code) {
            Some(c) => Value::String(Arc::from(c.to_string().as_str())),
            None => { trap(ctx, "RangeError: wasm:js-string.fromCodePoint — invalid code point"); Value::Null }
        }
    }));
    ...
    vm.register_host_fn("wasm:js-string", "fromCharCodeArray", Box::new(|ctx: &mut HostContext, args: &[Value]| {
        let arr = match args.first() {
            Some(Value::Object(o)) => o.clone(),
            _ => { trap(ctx, "TypeError: wasm:js-string.fromCharCodeArray — not an array"); return Value::Null; }
        };
        let obj = arr.lock().unwrap();
        let elems = match &obj.kind {
            ObjectKind::Array(e) => e.clone(),
            _ => { drop(obj); trap(ctx, "TypeError: wasm:js-string.fromCharCodeArray — not an array"); return Value::Null; }
        };
        drop(obj);
        ...
        if start > end || end > arr_len {
            trap(ctx, "RangeError: wasm:js-string.fromCharCodeArray — range out of bounds");
            return Value::Null;
        }
        let units: Vec<u16> = elems[start..end].iter()
    ...
    vm.register_host_fn("wasm:js-string", "intoCharCodeArray", Box::new(|ctx: &mut HostContext, args: &[Value]| {
        let s = match args.first() {
            Some(Value::String(s)) => s.clone(),
            _ => { trap(ctx, "TypeError: wasm:js-string.intoCharCodeArray — not a string"); return Value::Null; }
        };
        let arr = match args.get(1) {
            Some(Value::Object(o)) => o.clone(),
            _ => { trap(ctx, "TypeError: wasm:js-string.intoCharCodeArray — not an array"); return Value::Null; }
        };
        ...
        let elems = match &mut obj.kind {
            ...
            _ => { drop(obj); trap(ctx, "TypeError: wasm:js-string.intoCharCodeArray — not an array"); return Value::Null; }
        };
        if start + count > elems.len() {
            drop(obj);
            trap(ctx, "RangeError: wasm:js-string.intoCharCodeArray — string doesn't fit in array");
            return Value::Null;
        }
    ...
    vm.register_host_fn("wasm:js-string", "fromI32", Box::new(|_ctx: &mut HostContext, args: &[Value]| {
        Value::String(Arc::from(format!("{}", args.first().map(|v| v.as_i32()).unwrap_or(0)).as_str()))
    }));

    // fromU32(i32) -> (ref extern) — unsigned decimal string (reinterpret i32 as u32)
    vm.register_host_fn("wasm:js-string", "fromU32", Box::new(|_ctx: &mut HostContext, args: &[Value]| {
    ...
    vm.register_host_fn("wasm:js-string", "fromI64", Box::new(|_ctx: &mut HostContext, args: &[Value]| {
        Value::String(Arc::from(format!("{}", args.first().map(|v| v.as_i64()).unwrap_or(0)).as_str()))
    }));

    // fromU64(i64) -> (ref extern) — unsigned decimal string (reinterpret i64 as u64)
    vm.register_host_fn("wasm:js-string", "fromU64", Box::new(|_ctx: &mut HostContext, args: &[Value]| {
