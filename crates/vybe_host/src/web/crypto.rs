...
fn random_u64() -> u64 {
    ...
    let m = SEED.get_or_init(|| {
        Mutex::new(SystemTime::now().duration_since(UNIX_EPOCH).unwrap_or_default().as_nanos() as u64)
    });
...
pub fn register(vm: &mut VM) {
    ...
    vm.register_host_fn("web:crypto", "randomUUID", Box::new(|_ctx: &mut HostContext, _args: &[Value]| {
        let a = random_u64();
        let b = random_u64();
        let s = format!(
    ...
    vm.register_host_fn("web:crypto", "getRandomValues", Box::new(|_ctx: &mut HostContext, args: &[Value]| {
    ...
    vm.register_host_fn("web:crypto", "digest", Box::new(|_ctx: &mut HostContext, args: &[Value]| {
        let algo = args.first().map(|v| format!("{}", v).to_uppercase()).unwrap_or_default();
                ...
                _ => {
                    if let Some(Value::Object(buf)) = o.properties.get("buffer") {
                        ...
                        if let ObjectKind::ArrayBuffer(ref ab) = bo.kind {
                            ...
                            let len = o.properties.get("byteLength").map(|v| v.as_f64() as usize).unwrap_or(d.len());
