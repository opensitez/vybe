...
//!
//! Vybe's Promise model is sync-by-default (see `crate::ecma::promise`).
...
fn make_promise_fulfilled(value: Value) -> Value {
    ...
    obj.properties.insert("__state".into(), Value::String(Arc::from("fulfilled")));
...
fn make_promise_rejected(reason: Value) -> Value {
    let mut obj = Object::new();
    obj.properties.insert("__type".into(), Value::String(Arc::from("Promise")));
    obj.properties.insert("__state".into(), Value::String(Arc::from("rejected")));
...
fn headers_obj(map: HashMap<String, String>) -> Value {
    let mut obj = Object::new();
    obj.properties.insert("__type".into(), Value::String(Arc::from("Headers")));
    ...
    obj.properties.insert("__entries".into(), Value::Object(Arc::new(Mutex::new(Object::new_array(entries)))));
    Value::Object(Arc::new(Mutex::new(obj)))
}

fn make_response(status: u16, status_text: &str, body: String, headers: HashMap<String, String>) -> Value {
    let mut obj = Object::new();
    obj.properties.insert("__type".into(), Value::String(Arc::from("Response")));
    obj.properties.insert("status".into(), Value::F64(status as f64));
    obj.properties.insert("statusText".into(), Value::String(Arc::from(status_text)));
    obj.properties.insert("ok".into(), Value::Bool((200..300).contains(&status)));
    obj.properties.insert("headers".into(), headers_obj(headers));
    obj.properties.insert("__body".into(), Value::String(Arc::from(body.as_str())));
    Value::Object(Arc::new(Mutex::new(obj)))
}
...
pub fn register(vm: &mut VM) {
    ...
    vm.register_host_fn("web:fetch", "fetch", Box::new(|_ctx: &mut HostContext, args: &[Value]| {
        let url = match args.first() {
            Some(Value::String(s)) => s.to_string(),
            Some(Value::Object(obj)) => {
                let o = obj.lock().unwrap();
                o.properties.get("url").map(|v| format!("{}", v))
        ...
        match http_get_blocking(&url) {
            Ok((status, headers, body)) => make_promise_fulfilled(make_response(status, "OK", body, headers)),
            Err(e) => make_promise_rejected(Value::String(Arc::from(e.to_string().as_str()))),
        }
    }));

    // new Request(input, init?) — args[0]=input, args[1]=init.
    vm.register_host_fn("web:fetch", "requestNew", Box::new(|_ctx: &mut HostContext, args: &[Value]| {
        let url = args.first().map(|v| format!("{}", v)).unwrap_or_default();
        let method = args.get(1).and_then(|init| {
            if let Value::Object(io) = init {
                io.lock().unwrap().properties.get("method").map(|v| format!("{}", v))
            } else { None }
        }).unwrap_or_else(|| "GET".into());
        let mut obj = Object::new();
        obj.properties.insert("__type".into(), Value::String(Arc::from("Request")));
        obj.properties.insert("url".into(), Value::String(Arc::from(url.as_str())));
        obj.properties.insert("method".into(), Value::String(Arc::from(method.as_str())));
        Value::Object(Arc::new(Mutex::new(obj)))
    }));

    // new Response(body?, init?) — args[0]=body, args[1]=init.
    vm.register_host_fn("web:fetch", "responseNew", Box::new(|_ctx: &mut HostContext, args: &[Value]| {
        ...
        let (status, status_text) = if let Some(Value::Object(init)) = args.get(1) {
            ...
            let st = io.properties.get("status").map(|v| v.as_f64() as u16).unwrap_or(200);
            let stt = io.properties.get("statusText").map(|v| format!("{}", v)).unwrap_or_else(|| "OK".into());
            (st, stt)
        } else {
            (200, "OK".into())
        };
        Value::Object(Arc::new(Mutex::new({
            ...
            obj.properties.insert("__type".into(), Value::String(Arc::from("Response")));
            obj.properties.insert("status".into(), Value::F64(status as f64));
            obj.properties.insert("statusText".into(), Value::String(Arc::from(status_text.as_str())));
            obj.properties.insert("ok".into(), Value::Bool((200..300).contains(&status)));
            obj.properties.insert("__body".into(), Value::String(Arc::from(body.as_str())));
            obj.properties.insert("headers".into(), headers_obj(HashMap::new()));
            obj
        })))
    }));

    // response.text() → Promise<string>
    vm.register_host_fn("web:fetch", "responseText", Box::new(|_ctx: &mut HostContext, args: &[Value]| {
        if let Some(Value::Object(obj)) = args.first() {
            let o = obj.lock().unwrap();
            let body = o.properties.get("__body").map(|v| format!("{}", v)).unwrap_or_default();
    ...
    vm.register_host_fn("web:fetch", "responseJson", Box::new(|_ctx: &mut HostContext, args: &[Value]| {
        if let Some(Value::Object(obj)) = args.first() {
            let o = obj.lock().unwrap();
            let body = o.properties.get("__body").map(|v| format!("{}", v)).unwrap_or_default();
    ...
    // ── Headers ────────────────────────────────────────────────────
    vm.register_host_fn("web:fetch", "headersNew", Box::new(|_ctx: &mut HostContext, args: &[Value]| {
...
fn http_get_blocking(url: &str) -> Result<(u16, HashMap<String, String>, String), Box<dyn std::error::Error>> {
