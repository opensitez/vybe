...
pub fn register(vm: &mut VM) {
    ...
    set_prop(
        &crypto,
        "randomUUID",
        host_fn_ref(vm, "web:crypto", "randomUUID"),
    );
    set_prop(
        &crypto,
        "getRandomValues",
        host_fn_ref(vm, "web:crypto", "getRandomValues"),
    );
    ...
    vm.globals
        .insert("fetch".to_string(), host_fn_ref(vm, "web:fetch", "fetch"));
    ...
    vm.globals.insert(
        ...
        host_fn_ref(vm, "web:timers", "setTimeout"),
    );
    vm.globals.insert(
        "clearTimeout".to_string(),
        host_fn_ref(vm, "web:timers", "clearTimeout"),
    );
    vm.globals.insert(
        "setInterval".to_string(),
        host_fn_ref(vm, "web:timers", "setInterval"),
    );
    vm.globals.insert(
        "clearInterval".to_string(),
        host_fn_ref(vm, "web:timers", "clearInterval"),
    );
    vm.globals.insert(
        "queueMicrotask".to_string(),
        host_fn_ref(vm, "web:timers", "queueMicrotask"),
    );
    ...
    set_prop(
        &dom_parser,
        "parseFromString",
        host_fn_ref(vm, "web:dom-parser", "parseFromString"),
    );
    let xml_serializer = ensure_namespace(vm, &["XMLSerializer"]);
    set_prop(
        &xml_serializer,
        "serializeToString",
        host_fn_ref(vm, "web:dom-parser", "serializeToString"),
    );
    ...
    set_prop(
        &xml,
        "parseString",
        host_fn_ref(vm, "web:dom-parser", "parse"),
    );
    set_prop(&xml, "load", host_fn_ref(vm, "web:dom-parser", "load"));
    set_prop(
        &xml,
        "toString",
        host_fn_ref(vm, "web:dom-parser", "toString"),
    );
    ...
    set_prop(
        &xdocument,
        "Load",
        host_fn_ref(vm, "web:dom-parser", "load"),
    );
