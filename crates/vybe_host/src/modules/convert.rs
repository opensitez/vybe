...
pub fn register(vm: &mut VM) {
    ...
    vm.register_host_fn("vybe:convert", "isFinite", Box::new(|_ctx: &mut HostContext, args: &[Value]| {
        Value::Bool(args.first().map(|v| v.as_f64().is_finite()).unwrap_or(false))
    }));

    // Number.isInteger(n)
    vm.register_host_fn("vybe:convert", "isInteger", Box::new(|_ctx: &mut HostContext, args: &[Value]| {
