//! # `wasm:js-fixedarray` host handlers
//!
...
pub fn register(vm: &mut VM) {
    ...
    vm.register_host_fn("wasm:js-fixedarray", "newWithLength",
        Box::new(|_ctx, args| {
    ...
    vm.register_host_fn("wasm:js-fixedarray", "fromArray",
        Box::new(|_ctx, args| {
    ...
    vm.register_host_fn("wasm:js-fixedarray", "toArray",
        Box::new(|_ctx, args| {
            let elements: Vec<Value> = match args.first() {
                Some(Value::Object(src)) => {
                    let s = src.lock().unwrap();
    ...
    vm.register_host_fn("wasm:js-fixedarray", "length",
        Box::new(|_ctx, args| {
    ...
    vm.register_host_fn("wasm:js-fixedarray", "get",
        Box::new(|_ctx, args| {
    ...
    vm.register_host_fn("wasm:js-fixedarray", "isFixedArray",
        Box::new(|_ctx, args| {
            if let Some(arr) = array_of(args, 0) {
                let o = arr.lock().unwrap();
                return Value::I32(
    ...
    vm.register_host_fn("wasm:js-fixedarray", "freeze",
        Box::new(|_ctx, args| {
            if let Some(arr) = array_of(args, 0) {
    ...
    vm.register_host_fn("wasm:js-fixedarray", "isFrozen",
        Box::new(|_ctx, args| {
            if let Some(arr) = array_of(args, 0) {
                let o = arr.lock().unwrap();
                return Value::I32(
