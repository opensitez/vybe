...
pub fn register(vm: &mut VM) {
    ...
    vm.register_host_fn("wasi:clocks", "vbNow", Box::new(|_args: &[Value]| {
    ...
    vm.register_host_fn("wasi:clocks", "vbDate", Box::new(|_args: &[Value]| {
        let now = std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .unwrap_or_default()
            .as_secs();
    ...
    vm.register_host_fn("wasi:clocks", "vbTime", Box::new(|_args: &[Value]| {
        let now = std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .unwrap_or_default()
            .as_secs();
    ...
    vm.register_host_fn("wasi:clocks", "vbTimer", Box::new(|_args: &[Value]| {
        let now = std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .unwrap_or_default()
            .as_secs();
    ...
    vm.register_host_fn("wasi:clocks", "vbYear", Box::new(|args: &[Value]| {
    ...
    vm.register_host_fn("wasi:clocks", "vbMonth", Box::new(|args: &[Value]| {
        let ts = parse_vb_date(args);
    ...
    // day(date) → day number
    vm.register_host_fn("wasi:clocks", "vbDay", Box::new(|args: &[Value]| {
        let ts = parse_vb_date(args);
    ...
    // hour(date) → hour
    vm.register_host_fn("wasi:clocks", "vbHour", Box::new(|args: &[Value]| {
        let ts = parse_vb_date(args);
    ...
    // minute(date) → minute
    vm.register_host_fn("wasi:clocks", "vbMinute", Box::new(|args: &[Value]| {
        let ts = parse_vb_date(args);
    ...
    // second(date) → second
    vm.register_host_fn("wasi:clocks", "vbSecond", Box::new(|args: &[Value]| {
        let ts = parse_vb_date(args);
