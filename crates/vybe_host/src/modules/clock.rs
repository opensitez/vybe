...
pub fn register(vm: &mut VM) {
    ...
    vm.register_host_fn("wasi:clocks/monotonic-clock", "subscribe-instant", Box::new(|_ctx: &mut HostContext, args: &[Value]| {
        let ready_at_ns = args.first().map(|v| v.as_f64()).unwrap_or(0.0);
        let mut obj = Object::new();
        obj.properties.insert("__type".into(), Value::String(Arc::from("TimerPollable")));
        obj.properties.insert("__ready_at_ns".into(), Value::F64(ready_at_ns));
        Value::Object(Arc::new(Mutex::new(obj)))
    }));

    // subscribe-duration(how-long: duration) → pollable
    // Returns a TimerPollable ready after `how-long` ns from now.
    vm.register_host_fn("wasi:clocks/monotonic-clock", "subscribe-duration", Box::new(|_ctx: &mut HostContext, args: &[Value]| {
    ...
    vm.register_host_fn("wasi:clocks/monotonic-clock", "get-resolution", Box::new(|_ctx: &mut HostContext, _args: &[Value]| {
        Value::F64(1.0)
    }));
    ...
    vm.register_host_fn("wasi:clocks/monotonic-clock", "wait-until", Box::new(|ctx: &mut HostContext, args: &[Value]| {
        static START: OnceLock<Instant> = OnceLock::new();
        let start = START.get_or_init(Instant::now);
        let mark_ns = args.first().map(|v| v.as_f64()).unwrap_or(0.0);
        let elapsed_ns = start.elapsed().as_nanos() as f64;
        if mark_ns > elapsed_ns {
            std::thread::sleep(std::time::Duration::from_nanos((mark_ns - elapsed_ns) as u64));
        }
    ...
    vm.register_host_fn("wasi:clocks/monotonic-clock", "wait-for", Box::new(|ctx: &mut HostContext, args: &[Value]| {
    ...
    vm.register_host_fn("wasi:clocks/timezone", "display", Box::new(|_ctx: &mut HostContext, _args: &[Value]| {
        let mut rec = Object::new();
        rec.properties.insert("utc-offset".into(), Value::I32(0));
        rec.properties.insert("name".into(), Value::String(Arc::from("UTC")));
        rec.properties.insert("in-daylight-saving-time".into(), Value::Bool(false));
        Value::Object(Arc::new(Mutex::new(rec)))
    }));

    // utc-offset(when: datetime) → s32  — seconds east of UTC
    vm.register_host_fn("wasi:clocks/timezone", "utc-offset", Box::new(|_ctx: &mut HostContext, _args: &[Value]| {
        Value::I32(0)
    }));
