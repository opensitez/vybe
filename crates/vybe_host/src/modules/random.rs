...
pub fn register(vm: &mut VM) {
    // Random float in [0, 1)
    vm.register_host_fn("wasi:random", "random", Box::new(|_ctx: &mut HostContext, _args: &[Value]| {
        Value::F64(next_f64())
    }));

    // Random integer in [min, max] inclusive
    vm.register_host_fn("wasi:random", "randomInt", Box::new(|_ctx: &mut HostContext, args: &[Value]| {
    ...
    // Seed the RNG
    vm.register_host_fn("wasi:random", "seed", Box::new(|_ctx: &mut HostContext, args: &[Value]| {
    ...
    vm.register_host_fn("wasi:random", "randomBytes", Box::new(|_ctx: &mut HostContext, args: &[Value]| {
    ...
    // Random UUID v4
    vm.register_host_fn("wasi:random", "uuid", Box::new(|_ctx: &mut HostContext, _args: &[Value]| {
        let a = next_u64();
        let b = next_u64();
        let s = format!(
