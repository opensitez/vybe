...
pub fn register(vm: &mut VM) {
    // Random float in [0, 1)
    vm.register_host_fn("vybe:random", "random", Box::new(|_args: &[Value]| {
        Value::F64(next_f64())
    }));

    // Random integer in [min, max] inclusive
    vm.register_host_fn("vybe:random", "randomInt", Box::new(|args: &[Value]| {
    ...
    // Seed the RNG
    vm.register_host_fn("vybe:random", "seed", Box::new(|args: &[Value]| {
    ...
    vm.register_host_fn("vybe:random", "randomBytes", Box::new(|args: &[Value]| {
    ...
    // Random UUID v4
    vm.register_host_fn("vybe:random", "uuid", Box::new(|_args: &[Value]| {
        let a = next_u64();
        let b = next_u64();
        let s = format!(
