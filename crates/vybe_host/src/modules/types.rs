...
fn register_list(vm: &mut VM) {
    ...
    // to `ecma:array/*` (ECMA-262 §23.1 Array). `__get_count` /
...
fn register_dictionary(vm: &mut VM) {
    ...
    // Methods route through `ecma:object/*` (see builtin_types.rs)
    ...
    // `d.Count` property reads auto-invoke at struct_get time.
    let getter_idx = count_getter_idx(vm);
...
fn register_queue_stack(vm: &mut VM) {
    ...
    // Queue — backed by JS Array, FIFO semantics via `ecma:array/push`
    // + `ecma:array/shift` (see builtin_types.rs Queue type).
    ...
    // Stack — backed by JS Array, LIFO semantics via `ecma:array/push`
    // + `ecma:array/pop`.
    ...
    // through `ecma:set/*` (see builtin_types.rs HashSet type).
