...
pub fn register(vm: &mut VM) {
    ...
    vm.register_host_fn("vybe:net", "tcpListenerNew", Box::new(|args: &[Value]| {
    ...
    vm.register_host_fn("vybe:net", "tcpListenerStart", Box::new(|_args: &[Value]| {
        Value::Null
    }));

    // TcpListener.Stop()
    vm.register_host_fn("vybe:net", "tcpListenerStop", Box::new(|args: &[Value]| {
        let id = get_id(args);
    ...
    vm.register_host_fn("vybe:net", "tcpListenerAccept", Box::new(|args: &[Value]| {
        let id = get_id(args);
        let state = get_state();
    ...
    vm.register_host_fn("vybe:net", "tcpGetStream", Box::new(|args: &[Value]| {
    ...
    // StreamWriter.Flush()
    vm.register_host_fn("vybe:net", "streamWriterFlush", Box::new(|args: &[Value]| {
        let id = get_id(args);
        if id > 0 {
            let state = get_state();
            let mut guard = state.lock().unwrap();
    ...
    // New UdpClient(port)
    vm.register_host_fn("vybe:net", "udpNew", Box::new(|args: &[Value]| {
    ...
    vm.register_host_fn("vybe:net", "udpSend", Box::new(|args: &[Value]| {
        let id = get_id(args);
    ...
    vm.register_host_fn("vybe:net", "udpReceive", Box::new(|args: &[Value]| {
        let id = get_id(args);
        let state = get_state();
        let guard = state.lock().unwrap();
        if let Some(socket) = guard.udp_sockets.get(&id) {
    ...
    // UdpClient.Close()
    vm.register_host_fn("vybe:net", "udpClose", Box::new(|args: &[Value]| {
        let id = get_id(args);
