...
pub fn register(vm: &mut VM) {
    ...
    vm.register_host_fn("wasi:filesystem", "readFileBytes", Box::new(|_ctx: &mut HostContext, args: &[Value]| {
        let path = s(args, 0);
    ...
    vm.register_host_fn("wasi:filesystem", "writeFile", Box::new(|_ctx: &mut HostContext, args: &[Value]| {
        let path = s(args, 0);
        let data = s(args, 1);
    ...
    vm.register_host_fn("wasi:filesystem", "appendFile", Box::new(|_ctx: &mut HostContext, args: &[Value]| {
        use std::io::Write;
        let path = s(args, 0);
        let data = s(args, 1);
    ...
    vm.register_host_fn("wasi:filesystem", "exists", Box::new(|_ctx: &mut HostContext, args: &[Value]| {
        Value::Bool(std::path::Path::new(&s(args, 0)).exists())
    }));

    vm.register_host_fn("wasi:filesystem", "isFile", Box::new(|_ctx: &mut HostContext, args: &[Value]| {
        Value::Bool(std::path::Path::new(&s(args, 0)).is_file())
    }));

    vm.register_host_fn("wasi:filesystem", "isDir", Box::new(|_ctx: &mut HostContext, args: &[Value]| {
        Value::Bool(std::path::Path::new(&s(args, 0)).is_dir())
    }));

    vm.register_host_fn("wasi:filesystem", "remove", Box::new(|_ctx: &mut HostContext, args: &[Value]| {
        let path = s(args, 0);
    ...
    vm.register_host_fn("wasi:filesystem", "listDir", Box::new(|_ctx: &mut HostContext, args: &[Value]| {
        let path = s(args, 0);
    ...
    vm.register_host_fn("wasi:filesystem", "mkdir", Box::new(|_ctx: &mut HostContext, args: &[Value]| {
        Value::Bool(std::fs::create_dir_all(&s(args, 0)).is_ok())
    }));

    vm.register_host_fn("wasi:filesystem", "fileSize", Box::new(|_ctx: &mut HostContext, args: &[Value]| {
    ...
    vm.register_host_fn("wasi:filesystem", "rename", Box::new(|_ctx: &mut HostContext, args: &[Value]| {
        let old = s(args, 0);
        let new = s(args, 1);
    ...
    // copy(src, dest)
    vm.register_host_fn("wasi:filesystem", "copy", Box::new(|_ctx: &mut HostContext, args: &[Value]| {
        let src = s(args, 0);
        let dest = s(args, 1);
    ...
    vm.register_host_fn("wasi:filesystem", "stat", Box::new(|_ctx: &mut HostContext, args: &[Value]| {
        let path = s(args, 0);
    ...
    vm.register_host_fn("wasi:filesystem", "readDirEntries", Box::new(|_ctx: &mut HostContext, args: &[Value]| {
        let path = s(args, 0);
        match std::fs::read_dir(&path) {
            Ok(entries) => {
    ...
    vm.register_host_fn("wasi:filesystem", "openFile", Box::new(|_ctx: &mut HostContext, args: &[Value]| {
        let path = s(args, 0);
        let mode = s(args, 1);
    ...
    vm.register_host_fn("wasi:filesystem", "closeFile", Box::new(|_ctx: &mut HostContext, args: &[Value]| {
    ...
    vm.register_host_fn("wasi:filesystem", "printFile", Box::new(|_ctx: &mut HostContext, args: &[Value]| {
        let fnum = args.first().map(|v| v.as_i32()).unwrap_or(1);
    ...
    vm.register_host_fn("wasi:filesystem", "writeFile_handle", Box::new(|_ctx: &mut HostContext, args: &[Value]| {
        let fnum = args.first().map(|v| v.as_i32()).unwrap_or(1);
    ...
    vm.register_host_fn("wasi:filesystem", "lineInput", Box::new(|_ctx: &mut HostContext, args: &[Value]| {
        let fnum = args.first().map(|v| v.as_i32()).unwrap_or(1);
        if let Ok(mut handles) = FILE_HANDLES.lock() {
            if let Some(h) = handles.get_mut(&fnum) {
    ...
    vm.register_host_fn("wasi:filesystem", "inputFile", Box::new(|_ctx: &mut HostContext, args: &[Value]| {
        let fnum = args.first().map(|v| v.as_i32()).unwrap_or(1);
        if let Ok(mut handles) = FILE_HANDLES.lock() {
            if let Some(h) = handles.get_mut(&fnum) {
                if let Some(ref mut r) = h.reader {
