...
pub fn register(vm: &mut VM) {
    // Dynamic arithmetic
    vm.register_host_fn("vybe:rt", "dyn_add", Box::new(|_vm: &mut VM, args: &[Value]| {
    ...
    vm.register_host_fn("vybe:rt", "dyn_sub", Box::new(|_vm: &mut VM, args: &[Value]| {
        Value::F64(args.first().map(|v| v.as_f64()).unwrap_or(0.0) - args.get(1).map(|v| v.as_f64()).unwrap_or(0.0))
    }));

    vm.register_host_fn("vybe:rt", "dyn_mul", Box::new(|_vm: &mut VM, args: &[Value]| {
        Value::F64(args.first().map(|v| v.as_f64()).unwrap_or(0.0) * args.get(1).map(|v| v.as_f64()).unwrap_or(0.0))
    }));

    vm.register_host_fn("vybe:rt", "dyn_div", Box::new(|_vm: &mut VM, args: &[Value]| {
        Value::F64(args.first().map(|v| v.as_f64()).unwrap_or(0.0) / args.get(1).map(|v| v.as_f64()).unwrap_or(1.0))
    }));

    vm.register_host_fn("vybe:rt", "dyn_mod", Box::new(|_vm: &mut VM, args: &[Value]| {
        Value::F64(args.first().map(|v| v.as_f64()).unwrap_or(0.0) % args.get(1).map(|v| v.as_f64()).unwrap_or(1.0))
    }));

    vm.register_host_fn("vybe:rt", "dyn_neg", Box::new(|_vm: &mut VM, args: &[Value]| {
        Value::F64(-args.first().map(|v| v.as_f64()).unwrap_or(0.0))
    }));

    // Dynamic comparison
    vm.register_host_fn("vybe:rt", "dyn_eq", Box::new(|_vm: &mut VM, args: &[Value]| {
        let a = args.first().unwrap_or(&Value::Null);
        let b = args.get(1).unwrap_or(&Value::Null);
        Value::Bool(a.eq(b))
    }));

    vm.register_host_fn("vybe:rt", "dyn_ne", Box::new(|_vm: &mut VM, args: &[Value]| {
        let a = args.first().unwrap_or(&Value::Null);
        let b = args.get(1).unwrap_or(&Value::Null);
        Value::Bool(!a.eq(b))
    }));

    vm.register_host_fn("vybe:rt", "dyn_lt", Box::new(|_vm: &mut VM, args: &[Value]| {
        Value::Bool(args.first().map(|v| v.as_f64()).unwrap_or(0.0) < args.get(1).map(|v| v.as_f64()).unwrap_or(0.0))
    }));

    vm.register_host_fn("vybe:rt", "dyn_gt", Box::new(|_vm: &mut VM, args: &[Value]| {
        Value::Bool(args.first().map(|v| v.as_f64()).unwrap_or(0.0) > args.get(1).map(|v| v.as_f64()).unwrap_or(0.0))
    }));

    vm.register_host_fn("vybe:rt", "dyn_le", Box::new(|_vm: &mut VM, args: &[Value]| {
        Value::Bool(args.first().map(|v| v.as_f64()).unwrap_or(0.0) <= args.get(1).map(|v| v.as_f64()).unwrap_or(0.0))
    }));

    vm.register_host_fn("vybe:rt", "dyn_ge", Box::new(|_vm: &mut VM, args: &[Value]| {
        Value::Bool(args.first().map(|v| v.as_f64()).unwrap_or(0.0) >= args.get(1).map(|v| v.as_f64()).unwrap_or(0.0))
    }));

    // Dynamic type operations
    vm.register_host_fn("vybe:rt", "dyn_not", Box::new(|_vm: &mut VM, args: &[Value]| {
        Value::Bool(!truthy(args.first().unwrap_or(&Value::Null)))
    }));

    vm.register_host_fn("vybe:rt", "dyn_to_bool", Box::new(|_vm: &mut VM, args: &[Value]| {
        Value::Bool(truthy(args.first().unwrap_or(&Value::Null)))
    }));

    // String operations
    vm.register_host_fn("vybe:rt", "str_concat", Box::new(|_vm: &mut VM, args: &[Value]| {
    ...
    vm.register_host_fn("vybe:rt", "get_prop", Box::new(|_vm: &mut VM, args: &[Value]| {
    ...
    vm.register_host_fn("vybe:rt", "set_prop", Box::new(|_vm: &mut VM, args: &[Value]| {
        if let Some(Value::Object(obj)) = args.first() {
            let name = args.get(1).map(|v| format!("{}", v)).unwrap_or_default();
    ...
    vm.register_host_fn("vybe:rt", "new_object", Box::new(|_vm: &mut VM, _args: &[Value]| {
        Value::Object(Rc::new(RefCell::new(Object::new())))
    }));

    vm.register_host_fn("vybe:rt", "new_array", Box::new(|_vm: &mut VM, args: &[Value]| {
        let elems: Vec<Value> = args.to_vec();
        Value::Object(Rc::new(RefCell::new(Object::new_array(elems))))
    }));

    vm.register_host_fn("vybe:rt", "array_get", Box::new(|_vm: &mut VM, args: &[Value]| {
        if let Some(Value::Object(obj)) = args.first() {
    ...
    vm.register_host_fn("vybe:rt", "array_set", Box::new(|_vm: &mut VM, args: &[Value]| {
        if let Some(Value::Object(obj)) = args.first() {
            let key = args.get(1).map(|v| format!("{}", v)).unwrap_or_default();
            let val = args.get(2).cloned().unwrap_or(Value::Null);
    ...
    // Type checking
    vm.register_host_fn("vybe:rt", "typeof", Box::new(|_vm: &mut VM, args: &[Value]| {
    ...
    vm.register_host_fn("vybe:rt", "global_get", Box::new(|_vm: &mut VM, _args: &[Value]| {
        // This is resolved by the VM at runtime through vm.globals
        // The host function is a placeholder — the VM intercepts it
        Value::Null
    }));
    vm.register_host_fn("vybe:rt", "global_set", Box::new(|_vm: &mut VM, _args: &[Value]| {
        Value::Null
    }));

    // Indexed struct field access (WASM GC style — by field index, not string)
    vm.register_host_fn("vybe:rt", "struct_get_idx", Box::new(|_vm: &mut VM, args: &[Value]| {
        if let Some(Value::Object(obj)) = args.first() {
    ...
    vm.register_host_fn("vybe:rt", "struct_set_idx", Box::new(|_vm: &mut VM, args: &[Value]| {
        if let Some(Value::Object(obj)) = args.first() {
            let idx = args.get(1).map(|v| v.as_f64() as usize).unwrap_or(0);
            let val = args.get(2).cloned().unwrap_or(Value::Null);
