...
pub fn register(vm: &mut VM) {
    ...
    vm.register_host_fn("ecma:bigint", "toString", Box::new(|_ctx: &mut HostContext, args: &[Value]| {
        let n = to_bigint(args.first().unwrap_or(&Value::Null));
        let radix = args.get(1).map(|v| v.as_f64() as u32).unwrap_or(10);
        if !(2..=36).contains(&radix) { return Value::String(std::sync::Arc::from(n.to_string().as_str())); }
        let s = if radix == 10 {
            n.to_string()
        } else {
    ...
    binop!("div", |a: i64, b: i64| if b == 0 { 0 } else { a.wrapping_div(b) });
    binop!("rem", |a: i64, b: i64| if b == 0 { 0 } else { a.wrapping_rem(b) });
