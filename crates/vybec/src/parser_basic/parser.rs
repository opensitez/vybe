...
#[derive(Parser)]
#[grammar = "src/parser_basic/grammar.pest"]
pub struct VBParser;
...
fn parse_class_decl(pair: Pair<Rule>) -> ParseResult<ClassDecl> {
    ...
    for p in inner {
        ...
        Rule::dim_statement => {
            if let Ok(super::ast::stmt::Statement::Dim(decls)) = parse_statement(p) {
                fields.extend(decls);
            }
        }
            Rule::field_decl => {
    ...
    match pair.as_rule() {
        ...
        Rule::resume_statement => {
            ...
            if text.contains("next") {
                Ok(Statement::Resume(super::ast::ResumeTarget::Next))
            } else {
                let mut inner = pair.into_inner();
                if let Some(label) = inner.next() {
                    Ok(Statement::Resume(super::ast::ResumeTarget::Label(label.as_str().to_string())))
                } else {
                    Ok(Statement::Resume(super::ast::ResumeTarget::Implicit))
                }
            }
        }
    ...
    match pair.as_rule() {
        ...
        Rule::cast_expression => {
            ...
            let kind = if text[..5].eq_ignore_ascii_case("CType") {
                super::ast::expr::CastKind::CType
            } else if text[..10].eq_ignore_ascii_case("DirectCast") {
                super::ast::expr::CastKind::DirectCast
            } else {
                super::ast::expr::CastKind::TryCast
            };
            let mut inner = pair.into_inner();
            let expr = parse_expression(inner.next().unwrap())?;
        ...
        Rule::interpolated_string => {
            ...
            while let Some(ch) = chars.next() {
                if ch == '{' {
                    ...
                    match super::parse_program(&expr_code) {
                        Ok(program) => {
...
fn parse_query_expression(pair: Pair<Rule>) -> ParseResult<Expression> {
    ...
    while let Some(id_pair) = from_inner.next() {
        if id_pair.as_rule() == Rule::identifier {
            ...
            if let Some(collection) = collection_expr {
                ranges.push(super::ast::query::RangeVariable {
                    name,
                    collection,
                    type_name,
                });
            ...
            Rule::query_operator => {
                ...
                match op.as_rule() {
                    Rule::where_clause => {
                        ...
                        clauses.push(super::ast::query::QueryClause::Where(expr));
                    }
                    Rule::order_by_clause => {
                        ...
                        for ord in op.into_inner() {
                            ...
                            let direction = match ord_inner.next().map(|x| x.as_str().to_lowercase()).as_deref() {
                                Some("descending") => super::ast::query::OrderDirection::Descending,
                                _ => super::ast::query::OrderDirection::Ascending,
                            };
                            orderings.push(super::ast::query::Ordering { expression: expr, direction });
                        }
                        clauses.push(super::ast::query::QueryClause::OrderBy(orderings));
                    }
                    Rule::let_clause => {
                        ...
                        clauses.push(super::ast::query::QueryClause::Let { name, value });
                    }
                    _ => {}
                }
            }
            Rule::select_or_group_clause => {
                ...
                match inner_sg.as_rule() {
                    Rule::select_clause => {
                        ...
                        select_or_group = Some(super::ast::query::SelectOrGroupClause::Select(exprs));
                    }
                    Rule::group_clause => {
                        ...
                        if exprs.len() >= 2 {
                             ...
                             select_or_group = Some(super::ast::query::SelectOrGroupClause::Group(Box::new(super::ast::query::GroupClause { item, key })));
    ...
    Ok(Expression::Query(Box::new(super::ast::query::QueryExpression {
        from_clause: super::ast::query::FromClause { ranges },
        body: super::ast::query::QueryBody {
            clauses,
            select_or_group: select_or_group.unwrap_or(super::ast::query::SelectOrGroupClause::Select(vec![Expression::Nothing])), 
        },
    })))
}
...
fn parse_xml_node(pair: Pair<Rule>) -> ParseResult<super::ast::xml::XmlNode> {
    match pair.as_rule() {
        ...
        Rule::xml_element => {
            ...
            for p in inner {
                match p.as_rule() {
                    Rule::xml_attribute => {
                        ...
                        let val_node = if val_pair.as_rule() == Rule::xml_embedded_expression {
                            super::ast::xml::XmlNode::EmbeddedExpression(parse_expression(val_pair.into_inner().next().unwrap())?)
                        } else {
                             let s = val_pair.as_str().trim_matches('"').to_string();
                             let s = s.replace("&quot;", "\"").replace("&lt;", "<").replace("&gt;", ">").replace("&amp;", "&");
                             super::ast::xml::XmlNode::Text(s)
                        };
                        attributes.push(super::ast::xml::XmlAttribute { name: attr_name, value: vec![val_node] });
                    }
                    Rule::xml_content => {
                         ...
                         match content.as_rule() {
                             ...
                             Rule::xml_text => {
                                 ...
                                 if !t.trim().is_empty() {
                                     children.push(super::ast::xml::XmlNode::Text(t));
                                 }
                             }
                             Rule::xml_embedded_expression => {
                                 children.push(super::ast::xml::XmlNode::EmbeddedExpression(parse_expression(content.into_inner().next().unwrap())?));
                             }
                             _ => {}
                         }
                    }
             ...
             Ok(super::ast::xml::XmlNode::Element(super::ast::xml::XmlElement {
                 name,
                 attributes,
                 children,
...
fn parse_xml_name(pair: Pair<Rule>) -> super::ast::xml::XmlName {
    let s = pair.as_str();
    if let Some(idx) = s.find(':') {
        super::ast::xml::XmlName {
            prefix: Some(s[..idx].to_string()),
            local: s[idx+1..].to_string(),
        }
    } else {
        super::ast::xml::XmlName { prefix: None, local: s.to_string() }
    }
}
