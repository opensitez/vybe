...
impl Compiler {
        ...
        match decl {
            ...
            Declaration::Interface(iface) => {
                ...
                let method_names: Vec<String> = iface.methods.iter().map(|m| {
                    match m {
                        crate::parser_basic::ast::InterfaceMember::Sub { name, .. } => name.as_str().to_lowercase(),
                        crate::parser_basic::ast::InterfaceMember::Function { name, .. } => name.as_str().to_lowercase(),
                        crate::parser_basic::ast::InterfaceMember::Property { name, .. } => name.as_str().to_lowercase(),
                        crate::parser_basic::ast::InterfaceMember::Event { name, .. } => name.as_str().to_lowercase(),
                    }
                }).collect();
    ...
    pub(crate) fn emit_ref_func(&mut self, func_idx: usize, upvalues: &[super::scope::UpvalueDesc]) {
        let line = self.line;
