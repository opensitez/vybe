...
fn php_function_js_calls() {
    ...
    let php_prog = vybec::parser_php::parse(php_src).expect("PHP parse");
    vm.run(vybec::compiler_php::Compiler::new().compile(&php_prog).expect("PHP compile")).expect("PHP run");
...
fn ruby_function_dart_calls() {
    ...
    let dart_prog = vybec::parser_dart::parse(dart_src).expect("Dart parse");
    vm.run(vybec::compiler_dart::Compiler::new().compile(&dart_prog).expect("Dart compile")).expect("Dart run");
...
fn five_languages_share_global() {
    ...
    let php = vybec::parser_php::parse("<?php $counter = $counter + 1000;").expect("PHP");
    vm.run(vybec::compiler_php::Compiler::new().compile(&php).expect("PHP")).expect("PHP");
...
fn all_8_languages_output() {
    ...
    let tests: Vec<(&str, Box<dyn Fn(&mut VM)>)> = vec![
        ...
        ("PHP", Box::new(|vm: &mut VM| {
            let p = vybec::parser_php::parse("<?php echo 'PHP';").unwrap();
            vm.run(vybec::compiler_php::Compiler::new().compile(&p).unwrap()).unwrap();
        })),
        ...
        ("Dart", Box::new(|vm: &mut VM| {
            let p = vybec::parser_dart::parse("print('Dart');").unwrap();
            vm.run(vybec::compiler_dart::Compiler::new().compile(&p).unwrap()).unwrap();
        })),
