...
#[test] fn stringbuilder() { compile_ok(r#"<?php
...
#[test] fn stringbuilder_replace() { compile_ok(r#"<?php
...
#[test] fn hashset() { compile_ok(r#"<?php
$set = new HashSet();
$set->add('apple');
$set->add('banana');
...
#[test] fn dictionary() { compile_ok(r#"<?php
$dict = new Dictionary();
"#); }

// ── Random (same as VB/C# System.Random) ────────────────────
#[test] fn random_obj() { compile_ok(r#"<?php
$rng = new Random();
$n = $rng->nextInt(1, 100);
$f = $rng->nextFloat();
"#); }
...
#[test] fn stopwatch() { compile_ok(r#"<?php
$sw = new Stopwatch();
$elapsed = $sw->elapsed();
"#); }

// ── SplDoublyLinkedList (same as VB/C# LinkedList) ──────────
#[test] fn linked_list() { compile_ok(r#"<?php
$list = new SplDoublyLinkedList();
"#); }

// ── File handles (same as VB Open/Print/Input/Close) ────────
#[test] fn fopen_fwrite_fclose() { compile_ok(r#"<?php
$fp = fopen('test.txt', 'w');
fwrite($fp, 'Hello World');
fclose($fp);
"#); }

#[test] fn fopen_fgets() { compile_ok(r#"<?php
$fp = fopen('test.txt', 'r');
$line = fgets($fp);
fclose($fp);
"#); }

#[test] fn feof_loop() { compile_ok(r#"<?php
$fp = fopen('data.csv', 'r');
while (!feof($fp)) {
    $line = fgets($fp);
    echo $line;
...
#[test] fn exception_has_cross_lang_shape() { compile_ok(r#"<?php
...
#[test] fn runtime_exception_canonical() { compile_ok(r#"<?php
try {
...
#[test] fn type_error_canonical() { compile_ok(r#"<?php
try {
...
#[test] fn method_aliases() { compile_ok(r#"<?php
...
#[test] fn cross_lang_class_inheritance() { compile_ok(r#"<?php
...
#[test] fn component_export() {
    // PHP functions get exported as Component Model exports
    // Other languages can import them via the Linker
    let program = helpers::parse(r#"<?php
...
"#);
    let component = vybec::compiler_php::compile_component(&program, "php_utils").unwrap();
    assert_eq!(component.language, vybe_bytecode::component::Language::Php);
    assert!(!component.exports.is_empty(), "should export functions");
}
