...
echo $a->speak();
"#;
    let prog = vybec::parser_php::parse(src).expect("parse");
    vm.run(vybec::compiler_php::Compiler::new().compile(&prog).expect("compile")).expect("run");
    assert_eq!(output.borrow().as_slice(), &["Rex speaks"]);
}
...
print(a.speak());
"#;
    let prog = vybec::parser_dart::parse(src).expect("parse");
    vm.run(vybec::compiler_dart::Compiler::new().compile(&prog).expect("compile")).expect("run");
    assert_eq!(output.borrow().as_slice(), &["Rex speaks"]);
}
