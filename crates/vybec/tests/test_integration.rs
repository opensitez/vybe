...
fn all_languages_produce_output() {
    ...
    // PHP
    let php = vybec::parser_php::parse("<?php echo 'PHP';").expect("PHP");
    ...
    // Dart
    let dart = vybec::parser_dart::parse("print('Dart');").expect("Dart");
    vm.run(vybec::compiler_dart::Compiler::new().compile(&dart).expect("Dart")).expect("Dart");
    // COBOL
