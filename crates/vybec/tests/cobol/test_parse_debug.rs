...
fn parse_data_items_standalone() {
    ...
    let mut lexer = vybec::parser_cobol::lexer::Lexer::new(src);
    let tokens = lexer.tokenize().expect("tokenize failed");
    let token_kinds: Vec<String> = tokens.iter().map(|t| format!("{:?}", t.kind)).collect();

    // Find where DATA token is
    let data_pos = tokens.iter().position(|t| t.kind == vybec::parser_cobol::token::TokenKind::Data);
    let ws_pos = tokens.iter().position(|t| t.kind == vybec::parser_cobol::token::TokenKind::WorkingStorage);

    let prog = vybec::parser_cobol::parse(src).expect("parse failed");
