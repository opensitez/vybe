...
fn compile_python(src: &str) -> Vec<vybe_bytecode::Chunk> {
    let module = vybex::languages::python::parse(src).expect("parse");
    let profile = vybex::profile::parse_profile(vybex::languages::python::profile_source())
        .expect("profile");
    vybex::compiler::Compiler::with_profile(profile).compile(&module).expect("compile")
}
...
fn compile_vb(src: &str) -> Vec<vybe_bytecode::Chunk> {
    let module = vybex::languages::vb::parse(src).expect("parse");
