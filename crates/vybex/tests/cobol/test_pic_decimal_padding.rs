...
// ── Z (zero suppression) ───────────────────────────────────
#[test] fn pic_z_basic() { compile_ok(&p("01 WS-X PIC Z(5)9 VALUE 42.", "    DISPLAY WS-X.")); }
#[test] fn pic_zz99() { compile_ok(&p("01 WS-X PIC ZZ99 VALUE 42.", "    DISPLAY WS-X.")); }
#[test] fn pic_zzz_all_zero() { compile_ok(&p("01 WS-X PIC Z(5) VALUE 0.", "    DISPLAY WS-X.")); }

// ── $ (currency) ───────────────────────────────────────────
#[test] fn pic_dollar() { compile_ok(&p("01 WS-AMT PIC $9(5).99 VALUE 1234.56.", "    DISPLAY WS-AMT.")); }
#[test] fn pic_dollar_suppress() { compile_ok(&p("01 WS-AMT PIC $ZZ,ZZ9.99 VALUE 1234.56.", "    DISPLAY WS-AMT.")); }
#[test] fn pic_dollar_large() { compile_ok(&p("01 WS-AMT PIC $$$,$$9.99 VALUE 75000.00.", "    DISPLAY WS-AMT.")); }

// ── , (comma insertion) ────────────────────────────────────
#[test] fn pic_comma() { compile_ok(&p("01 WS-X PIC 9(3),9(3) VALUE 123456.", "    DISPLAY WS-X.")); }
#[test] fn pic_comma_z() { compile_ok(&p("01 WS-X PIC ZZZ,ZZ9 VALUE 12345.", "    DISPLAY WS-X.")); }

// ── . (decimal point) ──────────────────────────────────────
#[test] fn pic_decimal() { compile_ok(&p("01 WS-X PIC 9(5).99 VALUE 123.45.", "    DISPLAY WS-X.")); }
#[test] fn pic_v_decimal() { compile_ok(&p("01 WS-X PIC 9(5)V99 VALUE 123.45.", "    DISPLAY WS-X.")); }

// ── - / + (sign display) ──────────────────────────────────
#[test] fn pic_minus_trail() { compile_ok(&p("01 WS-X PIC 9(5)- VALUE -100.", "    DISPLAY WS-X.")); }
#[test] fn pic_plus_lead() { compile_ok(&p("01 WS-X PIC +9(5) VALUE 100.", "    DISPLAY WS-X.")); }
#[test] fn pic_s_signed() { compile_ok(&p("01 WS-X PIC S9(5) VALUE -500.", "    DISPLAY WS-X.")); }

// ── * (asterisk fill / check protection) ──────────────────
#[test] fn pic_asterisk() { compile_ok(&p("01 WS-X PIC **(5)9.99 VALUE 42.50.", "    DISPLAY WS-X.")); }

// ── B (blank insertion) ────────────────────────────────────
#[test] fn pic_blank() { compile_ok(&p("01 WS-X PIC 9(3)B9(3) VALUE 123456.", "    DISPLAY WS-X.")); }

// ── / (slash insertion) ────────────────────────────────────
#[test] fn pic_slash_date() { compile_ok(&p("01 WS-DATE PIC 99/99/9999 VALUE 12252024.", "    DISPLAY WS-DATE.")); }

// ── 0 (zero insertion) ─────────────────────────────────────
#[test] fn pic_zero_ins() { compile_ok(&p("01 WS-X PIC 9(3)09(3) VALUE 123456.", "    DISPLAY WS-X.")); }

// ═══════════════════════════════════════════════════════════
// COMP-3 PACKED DECIMAL — Exact Arithmetic
// ═══════════════════════════════════════════════════════════

#[test] fn comp3_add() { compile_ok(&p(
    "01 WS-A PIC 9(5)V99 COMP-3 VALUE 100.50.\n01 WS-B PIC 9(5)V99 COMP-3 VALUE 200.75.\n01 WS-C PIC 9(5)V99 COMP-3 VALUE 0.",
    "    ADD WS-A TO WS-B.\n    DISPLAY WS-B."
)); }

#[test] fn comp3_multiply() { compile_ok(&p(
...
#[test] fn comp3_divide() { compile_ok(&p(
...
#[test] fn comp3_compute() { compile_ok(&p(
...
#[test] fn comp3_rounding() { compile_ok(&p(
...
#[test] fn comp_binary() { compile_ok(&p("01 WS-X PIC 9(9) COMP VALUE 12345.", "    DISPLAY WS-X.")); }
#[test] fn comp5() { compile_ok(&p("01 WS-X PIC 9(9) USAGE BINARY VALUE 255.", "    DISPLAY WS-X.")); }
...
#[test] fn move_alpha_pad() { compile_ok(&p(
    "01 WS-A PIC X(20) VALUE \"Hello\".",
    "    DISPLAY WS-A."
)); }

#[test] fn move_alpha_short() { compile_ok(&p(
    "01 WS-SRC PIC X(5) VALUE \"Hi\".\n01 WS-DST PIC X(10).",
    "    MOVE WS-SRC TO WS-DST.\n    DISPLAY WS-DST."
)); }

#[test] fn move_alpha_truncate() { compile_ok(&p(
    "01 WS-SRC PIC X(20) VALUE \"Hello World\".\n01 WS-DST PIC X(5).",
    "    MOVE WS-SRC TO WS-DST.\n    DISPLAY WS-DST."
)); }

// ── Numeric: left-fill with zeros ──────────────────────────
#[test] fn move_num_zero_fill() { compile_ok(&p(
    "01 WS-SRC PIC 9(3) VALUE 42.\n01 WS-DST PIC 9(5).",
    "    MOVE WS-SRC TO WS-DST.\n    DISPLAY WS-DST."
)); }

#[test] fn move_num_to_larger() { compile_ok(&p(
    "01 WS-SRC PIC 9(3) VALUE 5.\n01 WS-DST PIC 9(8).",
    "    MOVE WS-SRC TO WS-DST.\n    DISPLAY WS-DST."
)); }

// ── Numeric to alpha ───────────────────────────────────────
#[test] fn move_num_to_alpha() { compile_ok(&p(
    "01 WS-NUM PIC 9(5) VALUE 12345.\n01 WS-STR PIC X(10).",
    "    MOVE WS-NUM TO WS-STR.\n    DISPLAY WS-STR."
)); }

// ── Spaces/Zeros ───────────────────────────────────────────
#[test] fn move_spaces_to_alpha() { compile_ok(&p(
    "01 WS-X PIC X(20) VALUE \"Old data\".",
    "    MOVE SPACES TO WS-X.\n    DISPLAY WS-X."
)); }

#[test] fn move_zeros_to_num() { compile_ok(&p(
