...
pub fn run_python(src: &str) -> Vec<String> {
    let module = vybex::languages::python::parse(src).expect("Python parse failed");
    ...
    let chunks = vybex::compiler::Compiler::with_profile(profile)
        .compile(&module).expect("Python compile failed");
...
pub fn parse_ok(src: &str) {
    vybex::languages::python::parse(src).expect("Python parse failed");
}
