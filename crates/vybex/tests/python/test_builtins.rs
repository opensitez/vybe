...
#[test] fn builtin_abs() { compile_ok("x = abs(-5)\n"); }
#[test] fn builtin_len() { compile_ok("x = len([1,2,3])\n"); }
#[test] fn builtin_int() { compile_ok("x = int('5')\n"); }
#[test] fn builtin_float() { compile_ok("x = float('3.14')\n"); }
#[test] fn builtin_str() { compile_ok("x = str(42)\n"); }
#[test] fn builtin_hex() { compile_ok("s = hex(255)\n"); }
#[test] fn builtin_oct() { compile_ok("s = oct(8)\n"); }
#[test] fn builtin_bin() { compile_ok("s = bin(42)\n"); }
#[test] fn builtin_id() { compile_ok("x = [1, 2]\nn = id(x)\n"); }
#[test] fn builtin_hash() { compile_ok("n = hash('hello')\n"); }
#[test] fn builtin_callable() { compile_ok("def f(): pass\nb = callable(f)\n"); }
#[test] fn builtin_frozenset() { compile_ok("s = frozenset([1, 2, 3])\n"); }
#[test] fn builtin_vars() { compile_ok("class C:\n    def __init__(self):\n        self.x = 1\nc = C()\nd = vars(c)\n"); }
#[test] fn builtin_dir() { compile_ok("x = {'a': 1, 'b': 2}\nkeys = dir(x)\n"); }
#[test] fn builtin_pow_two() { compile_ok("x = pow(2, 10)\n"); }
#[test] fn builtin_pow_three() { compile_ok("x = pow(2, 10, 100)\n"); }
#[test] fn builtin_divmod() { compile_ok("q, r = divmod(17, 5)\n"); }
#[test] fn builtin_format() { compile_ok("s = format(3.14, '.2f')\n"); }
#[test] fn builtin_iter() { compile_ok("it = iter([1, 2, 3])\n"); }
#[test] fn builtin_next() { compile_ok("items = [1, 2, 3]\nx = next(items)\n"); }

// String methods

#[test] fn str_upper() { compile_ok("x = 'hello'.upper()\n"); }
#[test] fn str_lower() { compile_ok("x = 'HELLO'.lower()\n"); }
#[test] fn str_strip() { compile_ok("x = '  hi  '.strip()\n"); }
#[test] fn str_split() { compile_ok("x = 'a b c'.split()\n"); }
#[test] fn str_join() { compile_ok("x = ' '.join(['a', 'b', 'c'])\n"); }
#[test] fn str_replace() { compile_ok("x = 'hello'.replace('l', 'r')\n"); }
...
#[test] fn str_startswith() { compile_ok("x = 'hello'.startswith('he')\n"); }
#[test] fn str_endswith() { compile_ok("x = 'hello'.endswith('lo')\n"); }
...
#[test] fn str_title() { compile_ok("x = 'hello world'.title()\n"); }
#[test] fn str_swapcase() { compile_ok("x = 'Hello'.swapcase()\n"); }
#[test] fn str_removeprefix() { compile_ok("s = 'HelloWorld'\nr = s.removeprefix('Hello')\n"); }
#[test] fn str_removesuffix() { compile_ok("s = 'HelloWorld'\nr = s.removesuffix('World')\n"); }
...
#[test] fn list_insert() { compile_ok("x = [1,3]\nx.insert(1, 2)\n"); }
#[test] fn list_remove() { compile_ok("x = [1,2,3]\nx.remove(2)\n"); }
...
#[test] fn dict_update() { compile_ok("d = {'a': 1}\nd.update({'b': 2})\n"); }
#[test] fn dict_setdefault() { compile_ok("d = {}\nv = d.setdefault('key', 42)\n"); }

// Set methods

#[test] fn set_add() { compile_ok("s = {1, 2, 3}\ns.add(4)\n"); }
#[test] fn set_discard() { compile_ok("s = {1, 2, 3}\ns.discard(2)\n"); }
#[test] fn set_union() { compile_ok("a = {1, 2}\nb = {3, 4}\nc = a.union(b)\n"); }
#[test] fn set_intersection() { compile_ok("a = {1, 2, 3}\nb = {2, 3, 4}\nc = a.intersection(b)\n"); }
#[test] fn set_difference() { compile_ok("a = {1, 2, 3}\nb = {2, 3}\nc = a.difference(b)\n"); }
...
#[test] fn sorted_with_key() { compile_ok("words = ['banana', 'apple', 'cherry']\nresult = sorted(words, key=len)\n"); }
#[test] fn sorted_with_lambda_key() { compile_ok("pairs = [(1, 'b'), (3, 'a'), (2, 'c')]\nresult = sorted(pairs, key=lambda x: x[1])\n"); }

// f-string format specs

#[test] fn fstring_format_float() { compile_ok("x = 3.14159\ns = f\"{x:.2f}\"\n"); }
#[test] fn fstring_format_width() { compile_ok("name = \"hello\"\ns = f\"{name:>20}\"\n"); }
#[test] fn fstring_format_int() { compile_ok("n = 42\ns = f\"{n:04d}\"\n"); }
