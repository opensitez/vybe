...
"#);
    ...
    assert!(g.should_run, "Expected should_run to be true after Application.Run");
}
...
"#);
    ...
    assert!(g.control_names.len() >= 2, "Expected at least 2 forms, got {}", g.control_names.len());
}
...
"#);
    let g = gui.lock().unwrap();
    assert!(g.control_names.contains(&"btnok".to_string()), "Expected control btnok, got {:?}", g.control_names);
}
...
"#);
    let g = gui.lock().unwrap();
    assert!(g.control_names.contains(&"txtinput".to_string()), "Expected control txtinput");
}
...
"#);
    let mut g = gui.lock().unwrap();
    assert!(g.control_names.contains(&"lbltitle".to_string()), "Expected control lbltitle");
...
"#);
    let g = gui.lock().unwrap();
    assert!(g.control_names.contains(&"chkagree".to_string()), "Expected control chkagree");
}
...
"#);
    let g = gui.lock().unwrap();
    assert!(g.control_names.contains(&"cboitems".to_string()), "Expected control cboitems");
}
...
"#);
    let g = gui.lock().unwrap();
    assert!(g.control_names.contains(&"rboption1".to_string()), "Expected control rboption1");
}
...
"#);
    let g = gui.lock().unwrap();
    assert!(g.control_names.contains(&"lstitems".to_string()), "Expected control lstitems");
}
...
"#);
    let g = gui.lock().unwrap();
    assert!(g.control_names.contains(&"pnlmain".to_string()), "Expected control pnlmain");
}
...
"#);
    let g = gui.lock().unwrap();
    assert!(g.control_names.contains(&"grpsettings".to_string()), "Expected control grpsettings");
}
...
"#);
    let g = gui.lock().unwrap();
    assert!(g.control_names.contains(&"piclogo".to_string()), "Expected control piclogo");
}
...
"#);
    let g = gui.lock().unwrap();
    assert!(g.control_names.contains(&"btn1".to_string()), "Expected control btn1");
}
...
"#);
    let g = gui.lock().unwrap();
    assert!(g.control_names.contains(&"txtname".to_string()), "Expected control txtname");
}
...
"#);
    let g = gui.lock().unwrap();
    assert!(g.control_names.contains(&"btn1".to_string()), "Expected control btn1");
    assert!(g.control_names.contains(&"btn2".to_string()), "Expected control btn2");
}
...
"#);
    let g = gui.lock().unwrap();
    assert!(g.event_handlers.contains_key("btn1.click"), "Expected Click handler for btn1, got keys: {:?}", g.event_keys());
}
...
"#);
    let g = gui.lock().unwrap();
    assert!(g.event_handlers.contains_key("btn1.click"), "Missing handler for btn1.click");
    assert!(g.event_handlers.contains_key("btn2.click"), "Missing handler for btn2.click");
}
...
"#);
    let g = gui.lock().unwrap();
    assert!(g.event_handlers.contains_key("form1.load"), "Expected Load handler for form1, got keys: {:?}", g.event_keys());
}
...
"#);
    ...
    assert!(!matches!(h, vybe_bytecode::Value::Null), "Handler should not be Null, got {:?}", h);
}
...
"#);
    let g = gui.lock().unwrap();
    assert!(g.event_handlers.contains_key("btn1.click"), "Expected handler after InitializeComponent");
}
...
"#);
    let g = gui.lock().unwrap();
    assert!(g.event_handlers.contains_key("txt1.textchanged"), "Expected TextChanged handler for txt1");
}
...
"#);
    let g = gui.lock().unwrap();
    assert!(g.event_handlers.contains_key("btn1.click"), "Missing Click handler");
    assert!(g.event_handlers.contains_key("btn1.mousedown"), "Missing MouseDown handler");
}
...
"#);
    let g = gui.lock().unwrap();
    assert!(g.event_handlers.contains_key("btn1.click"), "Expected handler that accesses fields");
}
...
"#);
    let g = gui.lock().unwrap();
    assert!(g.event_handlers.contains_key("cbo1.selectedindexchanged"), "Expected SelectedIndexChanged handler for cbo1");
}
...
"#);
    let g = gui.lock().unwrap();
    assert!(g.event_handlers.contains_key("chk1.checkedchanged"), "Expected CheckedChanged handler for chk1");
}
...
"#);
    let g = gui.lock().unwrap();
    assert_eq!(g.control_names.len(), 2, "Expected 2 controls, got {:?}", g.control_names);
...
"#);
    let g = gui.lock().unwrap();
    assert_eq!(g.control_names.len(), 3, "Expected 3 controls, got {:?}", g.control_names);
...
"#);
    ...
    assert!(g.event_handlers.contains_key("btn1.click"), "Handler should be registered after InitializeComponent");
}
...
"#);
    let g = gui.lock().unwrap();
    assert_eq!(g.control_names.len(), 5, "Expected 5 controls, got {:?}", g.control_names);
}
...
"#);
    let g = gui.lock().unwrap();
    assert!(g.control_names.contains(&"pb1".to_string()), "Expected control pb1");
    assert!(g.control_names.contains(&"tb1".to_string()), "Expected control tb1");
}
...
"#);
    let g = gui.lock().unwrap();
    assert!(g.control_names.contains(&"mybutton".to_string()), "Expected control named mybutton");
}
...
"#);
    let mut g = gui.lock().unwrap();
    let text = g.get_property("btn1", "text");
    assert_eq!(text, "Before Add", "Expected Text='Before Add' for btn1, got '{}'", text);
}
...
"#);
    let g = gui.lock().unwrap();
    assert!(g.control_names.contains(&"txt1".to_string()), "Expected txt1 control");
}
...
"#);
    let g = gui.lock().unwrap();
    assert!(g.close_requested, "Expected close_requested after Me.Close()");
}
...
fn h06_console_writeline_with_forms() {
    let (_vm, _gui, output) = run_vb_gui(r#"
Imports System.Windows.Forms
