...
use super::helpers::{run_vb, run_vb_vm, run_vb_gui};
...
"#);
    ...
    assert_eq!(g.form.control_count(), 2, "Expected 2 controls, got {}", g.form.control_count());
...
"#);
    let g = gui.lock().unwrap();
    assert!(g.get_event_handler("btn1", "Click").is_some(), "btn1.Click handler");
    assert!(g.get_event_handler("btn2", "Click").is_some(), "btn2.Click handler");
}
...
"#);
    let g = gui.lock().unwrap();
    assert!(g.control_names.contains(&"btnok".to_string()), "Expected btnOK");
    assert!(g.control_names.contains(&"txtinput".to_string()), "Expected txtInput");
}
...
"#);
    ...
    assert!(g.control_names.contains(&"lbltitle".to_string()), "Expected lblTitle control");
}
...
"#);
    ...
    let result = vm.invoke(&method, &[instance.clone(), Value::String(Arc::from("World"))]);
    assert!(result.is_ok(), "invoke should succeed: {:?}", result.err());
...
"#);
    let g = gui.lock().unwrap();
    assert_eq!(g.form.control_count(), 1);
    assert!(g.control_names.contains(&"btntest".to_string()), "Expected control 'btnTest'");
}
