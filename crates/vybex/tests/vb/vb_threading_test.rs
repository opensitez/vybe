...
End Module
"#);
    assert!(out.contains(&"task running".to_string()), "task callback should execute: {:?}", out);
    assert!(out.contains(&"main continues".to_string()), "main should continue: {:?}", out);
}
...
End Module
"#);
    assert!(out.contains(&"thread ran".to_string()), "thread should run: {:?}", out);
    assert!(out.contains(&"joined".to_string()), "join should complete: {:?}", out);
}
...
End Module
"#);
    assert!(out.contains(&"t1 start".to_string()), "t1 should start: {:?}", out);
