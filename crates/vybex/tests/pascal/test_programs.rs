...
#[test] fn prog_fizzbuzz() {
    let out = run_pascal(r#"program T; var i: Integer;
begin for i := 1 to 15 do begin
  if (i mod 15) = 0 then WriteLn('FizzBuzz')
    ...
    assert_eq!(out, &["1","2","Fizz","4","Buzz","Fizz","7","8","Fizz","Buzz","11","Fizz","13","14","FizzBuzz"]);
}

#[test] fn prog_sum_1_to_100() {
    assert_eq!(run_pascal("program T; var i, s: Integer; begin s := 0; for i := 1 to 100 do s := s + i; WriteLn(s); end."), &["5050"]);
}
...
end."#), &["true","false","true","false","true"]);
}

#[test] fn prog_power_iterative() {
    assert_eq!(run_pascal(r#"program T;
function Pow(base, exp: Integer): Integer;
var i, r: Integer;
begin r := 1; for i := 1 to exp do r := r * base; Result := r; end;
begin WriteLn(Pow(2, 10)); end."#), &["1024"]);
}

#[test] fn prog_reverse_string() {
    assert_eq!(run_pascal(r#"program T;
function ReverseStr(s: String): String;
var i: Integer;
begin Result := ''; for i := Length(s) - 1 downto 0 do Result := Result + s[i]; end;
begin WriteLn(ReverseStr('hello')); end."#), &["olleh"]);
}

#[test] fn prog_count_digits() {
    assert_eq!(run_pascal(r#"program T;
...
begin WriteLn(CountDigits(12345)); end."#), &["5"]);
}

#[test] fn prog_sum_of_digits() {
    assert_eq!(run_pascal(r#"program T;
...
begin WriteLn(SumDigits(12345)); end."#), &["15"]);
}

#[test] fn prog_max_of_three() {
    assert_eq!(run_pascal(r#"program T;
...
begin WriteLn(Max3(3, 7, 5)); WriteLn(Max3(10, 2, 8)); end."#), &["7", "10"]);
}

#[test] fn prog_triangle_area() {
    assert_eq!(run_pascal(r#"program T;
function TriangleArea(base, height: Integer): Real;
begin Result := base * height / 2; end;
begin WriteLn(TriangleArea(10, 5)); end."#), &["25"]);
}

#[test] fn prog_accumulate_in_loop() {
    assert_eq!(run_pascal(r#"program T;
...
begin WriteLn(CollatzSteps(6)); end."#), &["8"]);
}
