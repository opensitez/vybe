...
fn showmessage_emits_call() {
    let (_vm, _gui, msgs) = run_pascal_gui_capture_msgbox(r#"
program Test;
begin ShowMessage('Hello!'); end.
"#);
    let msgs = msgs.lock().unwrap();
    assert!(msgs.iter().any(|(text, _)| text == "Hello!"), "Expected ShowMessage, got {:?}", *msgs);
}
...
"#);
    let msgs = msgs.lock().unwrap();
    assert!(msgs.iter().any(|(text, _)| text == "Are you sure?"), "Expected MessageDlg, got {:?}", *msgs);
}
