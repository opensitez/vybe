...
fn dyn_array_empty() {
    assert_eq!(
        run_pascal("program T; var a: array of Integer; begin a := []; WriteLn(Length(a)); end."),
        &["0"]
    );
}
...
fn dyn_array_single() {
    assert_eq!(
        run_pascal("program T; var a: array of Integer; begin a := [42]; WriteLn(a[0]); end."),
        &["42"]
    );
}

#[test]
fn dyn_array_modify_loop() {
    assert_eq!(
        run_pascal(
            r#"program T;
