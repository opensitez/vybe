...
#[test] fn proc_no_params() {
    assert_eq!(run_pascal("program T; procedure Hello; begin WriteLn('hi'); end; begin Hello; end."), &["hi"]);
}
#[test] fn proc_one_param() {
    assert_eq!(run_pascal("program T; procedure Greet(name: String); begin WriteLn('Hello ' + name); end; begin Greet('World'); end."), &["Hello World"]);
}
#[test] fn proc_two_params() {
    assert_eq!(run_pascal("program T; procedure Show(a, b: Integer); begin WriteLn(a + b); end; begin Show(3, 4); end."), &["7"]);
}
#[test] fn proc_multiple_calls() {
    assert_eq!(run_pascal("program T; procedure P(x: Integer); begin WriteLn(x); end; begin P(1); P(2); P(3); end."), &["1","2","3"]);
}
...
// Functions
#[test] fn func_add() {
    assert_eq!(run_pascal("program T; function Add(a, b: Integer): Integer; begin Result := a + b; end; begin WriteLn(Add(3, 4)); end."), &["7"]);
}
#[test] fn func_no_params() {
    assert_eq!(run_pascal("program T; function GetFortyTwo: Integer; begin Result := 42; end; begin WriteLn(GetFortyTwo()); end."), &["42"]);
}
...
#[test] fn func_multiple_params() {
    assert_eq!(run_pascal("program T; function Sum(a, b, c: Integer): Integer; begin Result := a + b + c; end; begin WriteLn(Sum(1, 2, 3)); end."), &["6"]);
}
#[test] fn func_string_result() {
    assert_eq!(run_pascal("program T; function Greet(name: String): String; begin Result := 'Hello ' + name; end; begin WriteLn(Greet('World')); end."), &["Hello World"]);
}
