...
#[test] fn edge_empty_program()    { assert_eq!(run_pascal("program T; begin end."), &[] as &[&str]); }
#[test] fn edge_empty_proc()       { assert_eq!(run_pascal("program T; procedure P; begin end; begin P; end."), &[] as &[&str]); }
#[test] fn edge_many_locals() {
    assert_eq!(run_pascal("program T; var a,b,c,d,e: Integer; begin a:=1; b:=2; c:=3; d:=4; e:=5; WriteLn(a+b+c+d+e); end."), &["15"]);
}
...
#[test] fn edge_single_char_string() {
    assert_eq!(run_pascal("program T; begin WriteLn(Length('x')); end."), &["1"]);
}
#[test] fn edge_assigned_nil() {
    assert_eq!(run_pascal("program T; begin if not Assigned(nil) then WriteLn('y'); end."), &["y"]);
}

// Scoping
#[test] fn scope_local_shadows_global() {
    assert_eq!(run_pascal(r#"program T; var x: Integer;
procedure Test; var x: Integer; begin x := 99; WriteLn(x); end;
begin x := 1; Test; WriteLn(x); end."#), &["99", "1"]);
}
#[test] fn scope_nested_blocks() {
    assert_eq!(run_pascal("program T; var x: Integer; begin x := 1; begin x := 2; WriteLn(x); end; WriteLn(x); end."), &["2", "2"]);
}
...
#[test] fn nested_loop_sum() {
    assert_eq!(run_pascal(r#"program T; var i, j, s: Integer;
begin s := 0;
