...
#[test] fn bitwise_compound_and() {
    let out = run_prints("package main; import \"fmt\"; func main() { x := 12; x &= 10; fmt.Println(x); }");
    assert_eq!(out, vec!["8"]);
}
#[test] fn bitwise_compound_or() {
    let out = run_prints("package main; import \"fmt\"; func main() { x := 12; x |= 10; fmt.Println(x); }");
    assert_eq!(out, vec!["14"]);
}
#[test] fn bitwise_compound_xor() {
    let out = run_prints("package main; import \"fmt\"; func main() { x := 12; x ^= 10; fmt.Println(x); }");
    assert_eq!(out, vec!["6"]);
}
#[test] fn bitwise_compound_and_not() {
    let out = run_prints("package main; import \"fmt\"; func main() { x := 12; x &^= 10; fmt.Println(x); }");
    assert_eq!(out, vec!["4"]);
}
#[test] fn bitwise_compound_left_shift() {
    let out = run_prints("package main; import \"fmt\"; func main() { x := 3; x <<= 2; fmt.Println(x); }");
    assert_eq!(out, vec!["12"]);
}
#[test] fn bitwise_compound_right_shift() {
    let out = run_prints("package main; import \"fmt\"; func main() { x := 12; x >>= 2; fmt.Println(x); }");
    assert_eq!(out, vec!["3"]);
}
