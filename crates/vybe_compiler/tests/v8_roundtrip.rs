...
fn extract_imports(wasm: &[u8]) -> Vec<(String, String)> {
    ...
    while i < wasm.len() {
        ...
        if sid == 2 {
            // import section
            let (count, r) = read_leb(wasm, i);
            let mut p = i + r;
            for _ in 0..count {
                let (ml, r) = read_leb(wasm, p);
                p += r;
                let m = std::str::from_utf8(&wasm[p..p + ml])
                    .unwrap_or("")
                    .to_string();
                p += ml;
                let (nl, r) = read_leb(wasm, p);
                p += r;
                let n = std::str::from_utf8(&wasm[p..p + nl])
                    .unwrap_or("")
                    .to_string();
                p += nl;
                let kind = wasm[p];
...
fn v8_array_imports_present_in_wasm() {
    ...
    assert!(
        !js_array.is_empty(),
        "no vybe:js-array.* imports in emitted .wasm! imports={:?}",
        imports
    );
    // Must have at least the three ops our source exercises.
    for needed in ["newWithLength", "push", "length"] {
        assert!(
            js_array.iter().any(|&n| n == needed),
            "missing `vybe:js-array.{}` in emitted imports: {:?}",
            needed,
            js_array
        );
    }
    ...
    match run_on_v8(&wasm, "main") {
        ...
        Err(e) => {
            if e.contains("Unknown import") || e.contains("import ") && e.contains("not defined") {
                panic!(
                    "v8 rejected a `vybe:js-array.*` import — migration incomplete: {}",
                    e
                );
            }
...
fn v8_math_imports_are_portable() {
    ...
    assert!(
        !imports
            .iter()
            .any(|(m, n)| m == "vybe:math" && ["sin", "log", "exp"].contains(&n.as_str())),
        "math migration incomplete, found vybe:math imports: {:?}",
        imports
    );
    for needed in ["sin", "log", "exp"] {
        assert!(
            imports.iter().any(|(m, n)| m == "env" && n == needed),
            "missing env.{} import in emitted wasm: {:?}",
            needed,
            imports
        );
    }
