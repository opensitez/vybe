...
fn char_is_digit_true_for_ascii_digit() {
    assert_eq!(run_csharp(r#"Console.WriteLine(char.IsDigit('7'));"#), &["True"]);
}

#[test]
fn char_is_letter_true_for_alphabetic_char() {
    assert_eq!(run_csharp(r#"Console.WriteLine(char.IsLetter('a'));"#), &["True"]);
}

#[test]
fn char_is_upper_distinguishes_case() {
    assert_eq!(
        run_csharp(r#"Console.WriteLine(char.IsUpper('A')); Console.WriteLine(char.IsUpper('a'));"#),
        &["True", "False"]
    );
}
...
fn char_to_upper_converts_lowercase() {
    assert_eq!(run_csharp(r#"Console.WriteLine(char.ToUpper('b'));"#), &["B"]);
}

#[test]
fn char_to_lower_converts_uppercase() {
    assert_eq!(run_csharp(r#"Console.WriteLine(char.ToLower('Z'));"#), &["z"]);
}

#[test]
fn char_is_whitespace_true_for_space_and_tab() {
    assert_eq!(
        run_csharp(r#"Console.WriteLine(char.IsWhiteSpace(' ')); Console.WriteLine(char.IsWhiteSpace('\t'));"#),
        &["True", "True"]
    );
}
...
fn char_is_punctuation_true_for_dot() {
    assert_eq!(run_csharp(r#"Console.WriteLine(char.IsPunctuation('.'));"#), &["True"]);
}
...
fn char_arithmetic_adds_offset_to_produce_next_letter() {
    assert_eq!(run_csharp(r#"char c = (char)('A' + 2); Console.WriteLine(c);"#), &["C"]);
}
