...
fn static_field_shared_across_all_instances() {
    assert_eq!(
        run_csharp(r#"class Ctr{public static int Count=0; public Ctr(){Count++;}}
...
fn memberwise_clone_creates_shallow_copy() {
    assert_eq!(
        run_csharp(r#"class Point:System.ICloneable{public int X,Y;public object Clone()=>MemberwiseClone();}
...
fn override_to_string_produces_custom_output() {
    assert_eq!(
        run_csharp(r#"class Color{int R,G,B;public Color(int r,int g,int b){R=r;G=g;B=b;}
...
fn get_hash_code_override_consistent_for_same_data() {
    assert_eq!(
        run_csharp(r#"class Key{int V;public Key(int v){V=v;}public override int GetHashCode()=>V.GetHashCode();}
