...
fn ref_parameter_mutates_caller_variable() {
    assert_eq!(
        run_csharp(r#"void Double(ref int x){x*=2;}
...
fn out_parameter_initialised_inside_callee() {
    assert_eq!(
        run_csharp(r#"void Minmax(int[] a, out int min, out int max){
    min=a[0]; max=a[0];
...
fn out_inline_declaration_in_method_call() {
    assert_eq!(
        run_csharp(r#"bool ok = int.TryParse("42", out int result);
...
fn in_parameter_prevents_copy_and_is_readonly() {
    assert_eq!(
        run_csharp(r#"int Sum3(in int a, in int b, in int c) => a+b+c;
...
fn multiple_out_parameters_assign_multiple_return_values() {
    assert_eq!(
        run_csharp(r#"void Split(string s, out string head, out string tail){
    int mid=s.Length/2;
