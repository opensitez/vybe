...
fn strategy_pattern_swaps_algorithm_at_runtime() {
    assert_eq!(
        run_csharp(r#"interface ISort{int[] Sort(int[] a);}
...
fn observer_pattern_notifies_multiple_subscribers() {
    assert_eq!(
        run_csharp(r#"class Button{public event System.Action Clicked;}
var b=new Button();
int a=0,c=0;
b.Clicked+=()=>a++;
b.Clicked+=()=>c++;
...
fn builder_pattern_assembles_complex_object_step_by_step() {
    assert_eq!(
        run_csharp(r#"class Query{public string Table="";public string Filter="";}
class QueryBuilder{
    Query q=new Query();
