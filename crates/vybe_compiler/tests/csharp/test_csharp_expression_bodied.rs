...
fn expression_bodied_property_getter() {
    assert_eq!(
        run_csharp(
            r#"class Circle{public double R;public double Area=>System.Math.PI*R*R;}
...
fn expression_bodied_indexer() {
    assert_eq!(
        run_csharp(
            r#"class Bag{int[]data={1,2,3};public int this[int i]=>data[i];}
...
fn expression_bodied_static_method() {
    assert_eq!(
        run_csharp(
            r#"static class Utils{public static int Clamp(int v,int lo,int hi)=>v<lo?lo:v>hi?hi:v;}
...
fn expression_bodied_void_method_using_statement_form() {
    assert_eq!(
        run_csharp(
            r#"class Logger{public void Log(string msg)=>Console.WriteLine(msg);}
...
fn expression_bodied_operator() {
    assert_eq!(
        run_csharp(
            r#"struct Num{public int V;public static Num operator+(Num a,Num b)=>new Num{V=a.V+b.V};}
