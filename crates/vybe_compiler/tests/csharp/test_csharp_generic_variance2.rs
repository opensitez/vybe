...
fn covariant_interface_allows_derived_where_base_expected() {
    assert_eq!(
        run_csharp(r#"interface IReader<out T>{T Read();}
...
fn contravariant_interface_allows_base_where_derived_expected() {
    assert_eq!(
        run_csharp(r#"interface IWriter<in T>{void Write(T v);}
...
fn ienumerable_is_covariant_over_its_element_type() {
    assert_eq!(
        run_csharp(r#"System.Collections.Generic.IEnumerable<string> strings=new[]{"a","b"};
...
fn func_return_type_is_covariant() {
    assert_eq!(
        run_csharp(r#"System.Func<string> getStr=()=>"hello";
