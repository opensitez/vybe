...
fn recursive_factorial_returns_correct_result() {
    assert_eq!(
        run_csharp(r#"long Fact(int n)=>n<=1?1:n*Fact(n-1);
...
fn recursive_fibonacci_returns_correct_nth_number() {
    assert_eq!(
        run_csharp(r#"int Fib(int n)=>n<=1?n:Fib(n-1)+Fib(n-2);
...
fn recursive_gcd_computes_greatest_common_divisor() {
    assert_eq!(
        run_csharp(r#"int Gcd(int a,int b)=>b==0?a:Gcd(b,a%b);
...
fn recursive_binary_search_finds_target_index() {
    assert_eq!(
        run_csharp(r#"int BinSearch(int[] a,int lo,int hi,int t){
    if(lo>hi) return -1;
    int mid=(lo+hi)/2;
...
fn recursive_quicksort_sorts_array_in_place() {
    assert_eq!(
        run_csharp(r#"void QSort(int[] a,int lo,int hi){
    if(lo>=hi) return;
    int p=a[hi],i=lo;
...
fn mutual_recursion_even_odd_check() {
    assert_eq!(
        run_csharp(r#"bool IsEven(int n){if(n==0)return true; return IsOdd(n-1);}
