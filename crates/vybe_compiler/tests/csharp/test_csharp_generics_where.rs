...
fn where_class_constraint_accepts_reference_types() {
    assert_eq!(
        run_csharp(
            r#"T Wrap<T>(T v) where T:class=>v;
...
fn where_struct_constraint_accepts_value_types() {
    assert_eq!(
        run_csharp(
            r#"T Default<T>() where T:struct=>default;
