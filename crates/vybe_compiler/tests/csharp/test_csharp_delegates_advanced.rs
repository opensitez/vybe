...
fn delegate_stored_in_dictionary_and_dispatched_by_key() {
    assert_eq!(
        run_csharp(r#"var ops=new System.Collections.Generic.Dictionary<string,System.Func<int,int,int>>{
    {"add",(a,b)=>a+b},
    {"mul",(a,b)=>a*b}
};
...
fn chained_func_composition() {
    assert_eq!(
        run_csharp(r#"System.Func<int,int> double_it=x=>x*2;
...
fn predicate_combined_with_and() {
    assert_eq!(
        run_csharp(r#"System.Predicate<int> positive=x=>x>0;
...
fn lambda_closed_over_mutable_list_builds_result() {
    assert_eq!(
        run_csharp(r#"var log=new System.Collections.Generic.List<string>();
...
fn func_returns_func_partial_application() {
    assert_eq!(
        run_csharp(r#"System.Func<int,System.Func<int,int>> multiply=factor=>n=>n*factor;
