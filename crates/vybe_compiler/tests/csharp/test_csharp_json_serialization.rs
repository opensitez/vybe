...
fn serialize_simple_object_to_json_string() {
    assert_eq!(
        run_csharp(r#"var obj=new{Name="Alice",Age=30};
...
fn json_serialize_list_produces_array_syntax() {
    assert_eq!(
        run_csharp(r#"var list=new System.Collections.Generic.List<int>{1,2,3};
...
fn json_options_case_insensitive_deserialization() {
    assert_eq!(
        run_csharp(r#"class Item{public string Label{get;set;}}
...
fn json_deserialize_dictionary_from_object_json() {
    assert_eq!(
        run_csharp(r#"var d=System.Text.Json.JsonSerializer.Deserialize<System.Collections.Generic.Dictionary<string,int>>("{\"a\":1,\"b\":2}");
...
fn json_roundtrip_preserves_nested_object() {
    assert_eq!(
        run_csharp(r#"class Inner{public int X{get;set;}}
