...
fn tuple_pattern_in_switch_expression_dispatches_on_pair() {
    assert_eq!(
        run_csharp(r#"string Classify(int x,int y)=>(x,y) switch{
    (0,0)=>"origin",
    (>0,0)=>"pos-x",
    (0,>0)=>"pos-y",
    _=>"other"};
...
fn tuple_deconstruction_assigns_to_multiple_locals() {
    assert_eq!(
        run_csharp(r#"(string name,int age)=("Alice",30);
...
fn tuple_deconstruction_with_discard() {
    assert_eq!(
        run_csharp(r#"(string name,_,int score)=("Bob",99,"skip",55) switch{
