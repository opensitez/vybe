...
fn two_d_array_set_and_get_by_index() {
    assert_eq!(
        run_csharp(r#"int[,] m=new int[3,3];
m[1,2]=99;
...
fn two_d_array_row_and_column_lengths() {
    assert_eq!(
        run_csharp(r#"int[,] m=new int[4,5];
...
fn three_d_array_dimension_count() {
    assert_eq!(
        run_csharp(r#"int[,,] t=new int[2,3,4];
...
fn two_d_array_length_is_total_element_count() {
    assert_eq!(
        run_csharp(r#"int[,] m=new int[3,4];
...
fn two_d_array_initializer_syntax() {
    assert_eq!(
        run_csharp(r#"int[,] m={{1,2,3},{4,5,6}};
