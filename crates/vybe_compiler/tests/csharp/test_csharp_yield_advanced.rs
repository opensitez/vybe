...
fn yield_break_stops_iteration_early() {
    assert_eq!(
        run_csharp(
            r#"System.Collections.Generic.IEnumerable<int> Take(int[] a,int max){
    int count=0;
    foreach(var n in a){
...
fn nested_iterators_produce_flat_result_when_chained() {
    assert_eq!(
        run_csharp(
            r#"System.Collections.Generic.IEnumerable<int> Doubles(int n){
...
fn yield_return_with_complex_state_machine() {
    assert_eq!(
        run_csharp(
            r#"System.Collections.Generic.IEnumerable<string> Words(string s){
    var parts=s.Split(' ');
