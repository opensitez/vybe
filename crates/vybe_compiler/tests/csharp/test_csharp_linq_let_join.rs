...
fn join_correlates_two_sequences_on_matching_key() {
    assert_eq!(
        run_csharp(r#"var ids=new[]{1,2,3};
...
fn group_by_query_syntax_groups_by_first_char() {
    assert_eq!(
        run_csharp(r#"var words=new[]{"apple","ant","banana"};
...
fn into_continues_query_after_group() {
    assert_eq!(
        run_csharp(r#"var nums=new[]{1,2,3,4,5,6};
...
fn multiple_from_clauses_produce_cartesian_product() {
    assert_eq!(
        run_csharp(r#"var result=from a in new[]{1,2} from b in new[]{10,20} select a*b;
...
fn order_by_in_query_syntax_sorts_ascending() {
    assert_eq!(
        run_csharp(r#"var q=from n in new[]{3,1,2} orderby n select n;
foreach(var x in q) Console.WriteLine(x);"#),
        &["1", "2", "3"]
    );
}
