...
fn json_encode_array() {
    assert_eq!(
        run_prints(r#"<?php echo json_encode([1,2,3]); "#),
        vec!["[1,2,3]"]
    );
}
#[test]
...
fn json_encode_nested() {
    assert_eq!(
        run_prints(r#"<?php echo json_encode(['user'=>['name'=>'Bob','scores'=>[10,20,30]]]); "#),
        vec![r#"{"user":{"name":"Bob","scores":[10,20,30]}}"#]
    );
}
...
#[test]
fn json_decode_returns_null_on_error() {
    assert_eq!(
        run_prints(
            r#"<?php $r = json_decode('invalid json'); echo $r === null ? 'null' : 'not'; "#
        ),
        vec!["null"]
    );
}
...
#[test]
fn json_last_error_no_error() {
    assert_eq!(
        run_prints(r#"<?php json_encode([1,2,3]); echo json_last_error(); "#),
        vec!["0"]
    );
}
#[test]
fn json_last_error_on_invalid() {
    assert_eq!(
        run_prints(
            r#"<?php json_decode('{invalid}'); echo json_last_error() !== JSON_ERROR_NONE ? 'error' : 'ok'; "#
        ),
        vec!["error"]
    );
}
...
fn json_last_error_msg() {
    assert_eq!(
        run_prints(
            r#"<?php json_decode('{bad}'); echo strlen(json_last_error_msg()) > 0 ? 'has_msg' : 'empty'; "#
        ),
        vec!["has_msg"]
    );
}
...
fn json_encode_numeric_check() {
    assert_eq!(
        run_prints(r#"<?php echo json_encode(['n'=>'1.5'], JSON_NUMERIC_CHECK); "#),
        vec![r#"{"n":1.5}"#]
    );
}
#[test]
fn json_encode_force_object() {
    assert_eq!(
        run_prints(r#"<?php echo json_encode([1,2,3], JSON_FORCE_OBJECT); "#),
        vec![r#"{"0":1,"1":2,"2":3}"#]
    );
}
