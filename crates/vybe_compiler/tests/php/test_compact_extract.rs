...
#[test] fn compact_basic_variables() {
...
#[test] fn compact_with_array_of_names() {
    assert_eq!(run_prints(r#"<?php
$x = 1; $y = 2; $z = 3;
$vars = compact('x', 'y', 'z');
echo array_sum($vars);
...
#[test] fn compact_nested_array_argument() {
    assert_eq!(run_prints(r#"<?php
$a = 10; $b = 20;
$result = compact(['a', 'b']);
...
#[test] fn compact_skips_undefined_var() {
    assert_eq!(run_prints(r#"<?php
$defined = 'yes';
...
#[test] fn compact_used_in_function_context() {
    assert_eq!(run_prints(r#"<?php
...
#[test] fn extract_imports_to_local_scope() {
    assert_eq!(run_prints(r#"<?php
$data = ['foo' => 'bar', 'num' => 42];
extract($data);
echo $foo . ':' . $num;
...
#[test] fn extract_returns_count_of_vars() {
    assert_eq!(run_prints(r#"<?php
$data = ['a' => 1, 'b' => 2, 'c' => 3];
$count = extract($data);
echo $count;
...
#[test] fn extract_overwrite_default() {
    assert_eq!(run_prints(r#"<?php
$x = 'old';
extract(['x' => 'new']);
echo $x;
...
#[test] fn extract_prefix_all() {
    assert_eq!(run_prints(r#"<?php
extract(['id' => 1, 'name' => 'Joe'], EXTR_PREFIX_ALL, 'usr');
echo $usr_id . ':' . $usr_name;
"#), vec!["1:Joe"]);
}
#[test] fn extract_prefix_conflicts_only() {
    assert_eq!(run_prints(r#"<?php
$name = 'existing';
...
#[test] fn compact_extract_roundtrip() {
    assert_eq!(run_prints(r#"<?php
...
#[test] fn variable_variable_basic() {
    assert_eq!(run_prints(r#"<?php $var = 'hello'; $name = 'var'; echo $$name; "#), vec!["hello"]);
}
#[test] fn variable_variable_set() {
    assert_eq!(run_prints(r#"<?php $key = 'x'; $$key = 42; echo $x; "#), vec!["42"]);
}
#[test] fn variable_variable_in_loop() {
    assert_eq!(run_prints(r#"<?php
foreach (['a','b','c'] as $i => $name) $$name = $i * 10;
echo $a . ',' . $b . ',' . $c;
"#), vec!["0,10,20"]);
}
#[test] fn variable_variable_with_array() {
    assert_eq!(run_prints(r#"<?php
$arr = [1,2,3];
$varname = 'arr';
echo count($$varname);
...
#[test] fn dynamic_function_call_via_variable() {
    assert_eq!(run_prints(r#"<?php
$fn = 'strtoupper';
echo $fn('hello');
"#), vec!["HELLO"]);
}
#[test] fn dynamic_method_call_via_variable() {
    assert_eq!(run_prints(r#"<?php
...
#[test] fn dynamic_static_call_via_variable() {
    assert_eq!(run_prints(r#"<?php
