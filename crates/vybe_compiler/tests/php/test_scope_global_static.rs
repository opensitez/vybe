...
#[test] fn global_var_accessible_in_function() {
...
#[test] fn function_scope_isolates_from_global() {
    assert_eq!(run_prints(r#"<?php
$x = 99;
...
#[test] fn static_var_persists_between_calls() {
    assert_eq!(run_prints(r#"<?php
function counter(): int { static $n = 0; return ++$n; }
echo counter() . ',' . counter() . ',' . counter();
"#), vec!["1,2,3"]);
}
#[test] fn static_var_initialized_once() {
    assert_eq!(run_prints(r#"<?php
function once(): int { static $x = 100; $x += 5; return $x; }
echo once() . ',' . once();
"#), vec!["105,110"]);
}
#[test] fn static_var_in_recursive_function() {
    assert_eq!(run_prints(r#"<?php
function depth(): int {
    static $d = 0;
    $d++;
...
#[test] fn static_var_per_function_scope() {
    assert_eq!(run_prints(r#"<?php
...
#[test] fn static_class_property_shared() {
    assert_eq!(run_prints(r#"<?php
class Registry {
...
#[test] fn static_class_method_without_instance() {
    assert_eq!(run_prints(r#"<?php
class MathUtil { public static function cube(int $n): int { return $n ** 3; } }
echo MathUtil::cube(4);
"#), vec!["64"]);
}
#[test] fn static_property_inheritance_not_shared() {
    assert_eq!(run_prints(r#"<?php
...
#[test] fn define_constant() {
    assert_eq!(run_prints(r#"<?php define('APP_NAME', 'Vybe'); echo APP_NAME; "#), vec!["Vybe"]);
}
#[test] fn class_constant_access() {
    assert_eq!(run_prints(r#"<?php
class Color { const RED = 'red'; const GREEN = 'green'; }
echo Color::RED . ',' . Color::GREEN;
"#), vec!["red,green"]);
}
#[test] fn interface_constant_access() {
    assert_eq!(run_prints(r#"<?php
...
#[test] fn constant_in_expression() {
    assert_eq!(run_prints(r#"<?php
define('TAX_RATE', 0.2);
$price = 100;
...
#[test] fn nested_function_defined_on_call() {
    assert_eq!(run_prints(r#"<?php
function outer(): void {
...
#[test] fn closure_vs_function_scope() {
    assert_eq!(run_prints(r#"<?php
$x = 5;
