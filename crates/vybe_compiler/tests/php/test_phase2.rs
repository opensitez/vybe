...
// ── Intersection types ──────────────────────────────────────
#[test]
fn intersection_type_parse() {
    assert!(
        parse_ok("<?php function foo(A&B $x): void {}"),
        "intersection type parse"
    );
}

#[test]
fn intersection_return_type() {
    assert!(
        parse_ok("<?php function bar(): Countable&Iterator { return null; }"),
...
fn attribute_on_function() {
    assert!(parse_ok(
        "<?php #[Pure] function add(int $a, int $b): int { return $a + $b; }"
    ));
}
...
fn attribute_with_args() {
    assert!(parse_ok(
        "<?php #[Route('/api', methods: ['GET'])] function handler() {}"
    ));
}

// ── Readonly properties ─────────────────────────────────────
#[test]
fn readonly_property_class() {
    compile_ok(
        r#"<?php
class User {
