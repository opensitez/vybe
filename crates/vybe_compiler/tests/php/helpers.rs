...
//!
//! `compile_ok(src)` runs `vybe_compiler::languages::php::parse` → walker →
//! `vybe_compiler::compiler::Compiler::with_profile` and asserts the result is
...
pub fn parse(src: &str) {
    match vybe_compiler::languages::php::parse(src) {
        Ok(_) => {}
...
pub fn parse_ok(src: &str) -> bool {
    vybe_compiler::languages::php::parse(src).is_ok()
}
