...
#[test] fn reference_basic_alias() {
    assert_eq!(run_prints(r#"<?php $a = 1; $b = &$a; $b = 99; echo $a; "#), vec!["99"]);
}
#[test] fn reference_unset_does_not_destroy_original() {
    assert_eq!(run_prints(r#"<?php $a = 42; $b = &$a; unset($b); echo $a; "#), vec!["42"]);
}
...
#[test] fn reference_global_in_function() {
    assert_eq!(run_prints(r#"<?php
$x = 10;
...
#[test] fn foreach_reference_modification() {
    assert_eq!(run_prints(r#"<?php
$arr = [1,2,3];
...
#[test] fn foreach_reference_pitfall_unset() {
    assert_eq!(run_prints(r#"<?php
$a = [1,2,3];
foreach ($a as &$v) {}
unset($v);
...
#[test] fn closure_use_by_reference_modifies_outer() {
    assert_eq!(run_prints(r#"<?php
$sum = 0;
...
#[test] fn multiple_closures_share_reference() {
    assert_eq!(run_prints(r#"<?php
$counter = 0;
...
#[test] fn function_returns_reference() {
    assert_eq!(run_prints(r#"<?php
class Config {
...
#[test] fn preg_match_sets_matches_by_ref() {
    assert_eq!(run_prints(r#"<?php
preg_match('/(\d+)-(\d+)/', 'year-2024', $m);
echo $m[1] . '-' . $m[2];
"#), vec!["2024"]);
}
#[test] fn sort_modifies_array_by_ref() {
    assert_eq!(run_prints(r#"<?php
$a = [3,1,2];
sort($a);
echo implode(',', $a);
...
#[test] fn sscanf_returns_matches_or_populates_vars() {
    assert_eq!(run_prints(r#"<?php
...
#[test] fn weak_reference_null_after_unset() {
    assert_eq!(run_prints(r#"<?php
class Resource {}
$obj = new Resource;
$weak = WeakReference::create($obj);
...
#[test] fn reference_numeric_increment() {
    assert_eq!(run_prints(r#"<?php
$counters = ['a' => 0, 'b' => 0];
$ref = &$counters['a'];
for ($i = 0; $i < 5; $i++) $ref++;
