...
#[test] fn splqueue_enqueue_dequeue() {
    assert_eq!(run_prints(r#"<?php
$q = new SplQueue;
...
#[test] fn splminheap_extracts_minimum_first() {
    assert_eq!(run_prints(r#"<?php
$h = new SplMinHeap;
...
#[test] fn splmaxheap_extracts_maximum_first() {
    assert_eq!(run_prints(r#"<?php
$h = new SplMaxHeap;
$h->insert(5); $h->insert(2); $h->insert(8); $h->insert(1);
$out = [];
...
#[test] fn splfixedarray_size_and_access() {
    assert_eq!(run_prints(r#"<?php
...
#[test] fn splfixedarray_from_array() {
    assert_eq!(run_prints(r#"<?php
$a = SplFixedArray::fromArray([1,2,3,4,5]);
echo $a->getSize() . ':' . $a[2];
"#), vec!["5:3"]);
}
#[test] fn splfixedarray_to_array() {
    assert_eq!(run_prints(r#"<?php
$a = SplFixedArray::fromArray([10,20,30]);
echo implode(',', $a->toArray());
"#), vec!["10,20,30"]);
}
#[test] fn splfixedarray_iteration() {
    assert_eq!(run_prints(r#"<?php
...
#[test] fn spldoublylinkedlist_push_pop() {
    assert_eq!(run_prints(r#"<?php
...
#[test] fn spldoublylinkedlist_unshift() {
    assert_eq!(run_prints(r#"<?php
$l = new SplDoublyLinkedList;
