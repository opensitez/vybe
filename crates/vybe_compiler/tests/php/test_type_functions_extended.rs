...
#[test]
fn settype_change_in_place() { compile_ok(r#"<?php
$v = '42';
settype($v, 'integer');
echo $v;
"#); }
...
#[test]
fn get_debug_type_builtin() { compile_ok(r#"<?php
...
#[test]
fn intval_with_hex_base() { compile_ok(r#"<?php
$n = intval('1F', 16);
echo $n;
"#); }

#[test]
fn intval_with_octal_base() { compile_ok(r#"<?php
$n = intval('777', 8);
echo $n;
"#); }

#[test]
fn intval_with_binary_base() { compile_ok(r#"<?php
$n = intval('1010', 2);
echo $n;
"#); }
...
#[test]
fn is_finite_regular_float() { compile_ok(r#"<?php
echo is_finite(3.14) ? 'yes' : 'no';
echo is_finite(INF) ? 'yes' : 'no';
"#); }

#[test]
fn is_infinite_check() { compile_ok(r#"<?php
echo is_infinite(INF) ? 'yes' : 'no';
echo is_infinite(3.14) ? 'yes' : 'no';
"#); }

#[test]
fn is_nan_check() { compile_ok(r#"<?php
...
// ── ctype functions ──────────────────────────────────────────────
#[test]
fn ctype_alpha_all_letters() { compile_ok(r#"<?php
echo ctype_alpha('Hello') ? 'yes' : 'no';
echo ctype_alpha('Hello1') ? 'yes' : 'no';
"#); }

#[test]
fn ctype_digit_all_digits() { compile_ok(r#"<?php
echo ctype_digit('12345') ? 'yes' : 'no';
echo ctype_digit('123a5') ? 'yes' : 'no';
"#); }

#[test]
fn ctype_alnum_alphanumeric() { compile_ok(r#"<?php
echo ctype_alnum('abc123') ? 'yes' : 'no';
echo ctype_alnum('abc!23') ? 'yes' : 'no';
"#); }

#[test]
fn ctype_space_whitespace() { compile_ok(r#"<?php
echo ctype_space("  \t\n") ? 'yes' : 'no';
echo ctype_space('  x  ') ? 'yes' : 'no';
"#); }

#[test]
fn ctype_upper_uppercase() { compile_ok(r#"<?php
echo ctype_upper('HELLO') ? 'yes' : 'no';
echo ctype_upper('Hello') ? 'yes' : 'no';
"#); }

#[test]
fn ctype_lower_lowercase() { compile_ok(r#"<?php
echo ctype_lower('hello') ? 'yes' : 'no';
echo ctype_lower('Hello') ? 'yes' : 'no';
"#); }

#[test]
fn ctype_punct_punctuation() { compile_ok(r#"<?php
...
#[test]
fn is_object_check() { compile_ok(r#"<?php
...
#[test]
fn is_callable_check() { compile_ok(r#"<?php
...
#[test]
fn is_float_alias() { compile_ok(r#"<?php
...
#[test]
fn php_int_max_arithmetic() { compile_ok(r#"<?php
$big = PHP_INT_MAX;
$overflow = $big + 1;
...
#[test]
fn php_float_epsilon_comparison() { compile_ok(r#"<?php
$a = 0.1 + 0.2;
$b = 0.3;
