...
#[test] fn exception_previous_basic() {
...
#[test] fn exception_previous_is_null_when_not_chained() {
    assert_eq!(run_prints(r#"<?php
try { throw new Exception('plain'); }
catch (Exception $e) { echo $e->getPrevious() === null ? 'null' : 'has prev'; }
"#), vec!["null"]);
}
#[test] fn exception_chain_three_levels() {
    assert_eq!(run_prints(r#"<?php
try {
    try {
...
#[test] fn exception_message_after_rethrow() {
    assert_eq!(run_prints(r#"<?php
...
#[test] fn type_error_caught_by_error() {
    assert_eq!(run_prints(r#"<?php
...
#[test] fn value_error_from_array_function() {
    assert_eq!(run_prints(r#"<?php
try { array_chunk([], 0); }
catch (ValueError $e) { echo 'ValueError'; }
"#), vec!["ValueError"]);
}
#[test] fn division_by_zero_error() {
    assert_eq!(run_prints(r#"<?php
try { $r = intdiv(5, 0); }
catch (\DivisionByZeroError $e) { echo 'DivByZero'; }
"#), vec!["DivByZero"]);
}
#[test] fn error_is_not_exception() {
    assert_eq!(run_prints(r#"<?php
...
#[test] fn throwable_catches_both() {
    assert_eq!(run_prints(r#"<?php
...
#[test] fn finally_runs_even_when_rethrown() {
    assert_eq!(run_prints(r#"<?php
$log = [];
try {
    try {
...
#[test] fn finally_runs_on_return() {
    assert_eq!(run_prints(r#"<?php
