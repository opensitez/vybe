...
fn cobol_numeric_arithmetic_matrix() {
    let pics = [
        "9(1)", "9(2)", "9(3)", "9(4)", "9(5)", "9(2)V9(2)", "S9(2)", "S9(3)",
        "9(3)V9(1)", "9(4)V9(2)",
    ];
    let values = [
...
fn cobol_alphanumeric_move_matrix() {
    ...
    let values = [
        "A", "B", "C", "AB", "CD", "HELLO", "WORLD", "COBOL", "123", "ABC123", "X1Y2",
        "MIXED", "VALUE", "LEFT", "RIGHT", "ALPHA", "BETA", "GAMMA", "DELTA", "EPSILON",
    ];
    let mut index = 0;
    for size in sizes {
        for value in values {
    ...
    for left in lefts {
        for right in rights {
            for op in ops {
                ...
                let body = format!("    IF A {} B MOVE \"Y\" TO FLAG END-IF.\n    DISPLAY FLAG.", op);
                compile_case(index, &data, &body);
                index += 1;
            }
        }
...
fn cobol_evaluate_matrix() {
    ...
    let labels = ["ZERO", "ONE", "TWO", "THREE", "FOUR", "FIVE", "SIX", "SEVEN", "EIGHT", "NINE"];
    let mut index = 0;
    for value in values {
        for label in labels {
            let data = format!("01 N PIC 9(2) VALUE {}.\n01 OUT PIC X(10) VALUE \"NONE\".", value);
            let body = format!(
