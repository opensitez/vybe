...
fn handle_condition() {
    compile_ok(&p(
        d(),
        "    EXEC CICS HANDLE CONDITION ERROR(ERROR-PARA) NOTFND(NOTFOUND-PARA) END-EXEC.",
    ));
}
#[test]
fn handle_condition_multi() {
    compile_ok(&p(
        d(),
...
fn handle_aid() {
    compile_ok(&p(
        d(),
        "    EXEC CICS HANDLE AID PF1(HELP-PARA) PF3(EXIT-PARA) ENTER(PROCESS-PARA) CLEAR(CLEAR-PARA) END-EXEC.",
    ));
}
...
fn readq_ts_item() {
    compile_ok(&p(
        d(),
        "    EXEC CICS READQ TS QUEUE(MYQUEUE) INTO(WS-DATA) ITEM(3) END-EXEC.",
    ));
}
#[test]
fn deleteq_ts() {
    compile_ok(&p(d(), "    EXEC CICS DELETEQ TS QUEUE(MYQUEUE) END-EXEC."));
}
#[test]
fn writeq_td() {
    compile_ok(&p(
        d(),
        "    EXEC CICS WRITEQ TD QUEUE(ERRLOG) FROM(WS-MSG) END-EXEC.",
    ));
}
#[test]
fn readq_td() {
    compile_ok(&p(
        d(),
        "    EXEC CICS READQ TD QUEUE(ERRLOG) INTO(WS-MSG) END-EXEC.",
    ));
}
...
fn enq_resource() {
    compile_ok(&p(
        d(),
        "    EXEC CICS ENQ RESOURCE(CUSTLOCK) LENGTH(8) END-EXEC.",
    ));
}
#[test]
fn deq_resource() {
    compile_ok(&p(
        d(),
        "    EXEC CICS DEQ RESOURCE(CUSTLOCK) LENGTH(8) END-EXEC.",
    ));
}
#[test]
fn enq_deq_pair() {
...
#[test]
fn assign_userid() {
    compile_ok(&p(d(), "    EXEC CICS ASSIGN USERID(WS-USER) END-EXEC."));
}
#[test]
fn assign_sysid() {
    compile_ok(&p(d(), "    EXEC CICS ASSIGN SYSID(WS-SYSID) END-EXEC."));
}
#[test]
fn assign_multi() {
    compile_ok(&p(
        d(),
        "    EXEC CICS ASSIGN USERID(WS-USER) SYSID(WS-SYSID) END-EXEC.",
    ));
}
...
fn put_container() {
    compile_ok(&p(
        d(),
        "    EXEC CICS PUT CONTAINER(CUSTDATA) CHANNEL(MYCHANNEL) FROM(WS-DATA) END-EXEC.",
    ));
}
#[test]
fn get_container() {
    compile_ok(&p(
        d(),
        "    EXEC CICS GET CONTAINER(CUSTDATA) CHANNEL(MYCHANNEL) INTO(WS-DATA) END-EXEC.",
    ));
}
#[test]
...
#[test]
fn delay_seconds() {
    compile_ok(&p(d(), "    EXEC CICS DELAY SECONDS(5) END-EXEC."));
}
#[test]
fn delay_interval() {
    compile_ok(&p(d(), "    EXEC CICS DELAY INTERVAL(001000) END-EXEC."));
}
#[test]
fn start_trans() {
    compile_ok(&p(d(), "    EXEC CICS START TRANSID(NXTTX) END-EXEC."));
}
#[test]
fn suspend_task() {
...
fn web_receive() {
    compile_ok(&p(
        d(),
        "    EXEC CICS WEB RECEIVE INTO(WS-DATA) LENGTH(100) END-EXEC.",
    ));
}
...
fn document_create() {
    compile_ok(&p(
        d(),
        "    EXEC CICS DOCUMENT CREATE DOCTOKEN(WS-PTR) END-EXEC.",
    ));
}
...
#[test]
fn abend_code() {
    compile_ok(&p(d(), "    EXEC CICS ABEND ABCODE(ASRA) END-EXEC."));
}
#[test]
fn abend_custom() {
    compile_ok(&p(d(), "    EXEC CICS ABEND ABCODE(USR1) END-EXEC."));
}
...
#[test]
fn inquire_program() {
    compile_ok(&p(d(), "    EXEC CICS INQUIRE PROGRAM(MYPROG) END-EXEC."));
}
