...
#[test]
fn add_literal_to() {
    compile_ok(&p("    ADD 10 TO WS-A."));
}
#[test]
fn add_multiple() {
    compile_ok(&p("    ADD 1 2 3 TO WS-A."));
}

// ── SUBTRACT ───────────────────────────────────────────────
#[test]
fn subtract_from() {
    compile_ok(&p("    SUBTRACT 5 FROM WS-A."));
}
#[test]
fn subtract_giving() {
...
#[test]
fn multiply_by() {
    compile_ok(&p("    MULTIPLY 3 BY WS-A."));
}
#[test]
fn multiply_giving() {
...
#[test]
fn divide_giving() {
    compile_ok(&p("    DIVIDE WS-A BY 3 GIVING WS-C."));
}
#[test]
fn divide_remainder() {
...
#[test]
fn compute_pow() {
    compile_ok(&p("    COMPUTE WS-C = WS-A ** 2."));
}
#[test]
fn compute_complex() {
    compile_ok(&p("    COMPUTE WS-C = (WS-A + WS-B) * 2 - 1."));
}
#[test]
fn compute_nested_parens() {
    compile_ok(&p("    COMPUTE WS-C = ((WS-A + 1) * (WS-B + 2))."));
}
#[test]
fn compute_with_function() {
    compile_ok(&p("    COMPUTE WS-C = FUNCTION SQRT(WS-A)."));
}
#[test]
fn compute_mod() {
    compile_ok(&p("    COMPUTE WS-C = FUNCTION MOD(WS-A 3)."));
}
