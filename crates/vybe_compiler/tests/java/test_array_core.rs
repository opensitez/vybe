...
fn new_int_array_zero_fills_all_slots() {
    let out = run_main(
        "int[] nums = new int[4]; System.out.println(nums[0]); System.out.println(nums[3]);",
    );
...
fn arrays_copy_of_preserves_existing_elements() {
    let out = run_main(
        "int[] src = {5, 6, 7}; int[] copy = java.util.Arrays.copyOf(src, 3); System.out.println(copy[0]); System.out.println(copy[2]);",
    );
...
fn arrays_sort_orders_integers_ascending() {
    let out = run_main(
        "int[] nums = {3, 1, 4, 1, 5}; java.util.Arrays.sort(nums); System.out.println(nums[0]); System.out.println(nums[4]);",
    );
...
fn arrays_sort_leaves_already_sorted_array_unchanged() {
    let out = run_main(
        "int[] nums = {2, 4, 6}; java.util.Arrays.sort(nums); System.out.println(nums[1]);",
    );
    assert_eq!(out, vec!["4"]);
}
...
fn arrays_sort_handles_single_element_array() {
    let out =
        run_main("int[] nums = {42}; java.util.Arrays.sort(nums); System.out.println(nums[0]);");
    assert_eq!(out, vec!["42"]);
}
...
fn arrays_binary_search_finds_existing_value_index() {
    let out = run_main(
        "int[] nums = {1, 3, 5, 7}; System.out.println(java.util.Arrays.binarySearch(nums, 5));",
    );
    assert_eq!(out, vec!["2"]);
}
...
fn two_dimensional_array_column_count_on_first_row() {
    let out =
        run_main("int[][] grid = {{1, 2, 3}, {4, 5, 6}}; System.out.println(grid[0].length);");
    assert_eq!(out, vec!["3"]);
}
...
fn two_dimensional_array_writes_inner_element() {
    let out = run_main(
        "int[][] grid = {{1, 2}, {3, 4}}; grid[0][1] = 99; System.out.println(grid[0][1]);",
    );
...
fn jagged_two_d_array_rows_can_have_different_lengths() {
    let out = run_main(
        "int[][] grid = {{1}, {2, 3, 4}}; System.out.println(grid[0].length); System.out.println(grid[1].length);",
    );
...
fn new_boolean_array_defaults_to_false() {
    let out = run_main(
        "boolean[] flags = new boolean[2]; System.out.println(flags[0]); System.out.println(flags[1]);",
    );
...
fn string_array_initializer_stores_literals() {
    let out = run_main(
        "String[] words = {\"a\", \"bb\"}; System.out.println(words[0]); System.out.println(words[1]);",
    );
...
fn fill_then_copy_of_preserves_filled_values() {
    let out = run_main(
        "int[] nums = new int[3]; java.util.Arrays.fill(nums, 4); int[] copy = java.util.Arrays.copyOf(nums, 3); System.out.println(copy[1]);",
    );
    assert_eq!(out, vec!["4"]);
}
...
fn two_d_literal_nested_loop_prints_all_cells() {
    let out = run_main(
        "int[][] grid = {{1, 2}, {3, 4}}; for (int r = 0; r < grid.length; r++) { for (int c = 0; c < grid[r].length; c++) { System.out.println(grid[r][c]); } }",
    );
...
fn int_array_sum_via_indexed_while_loop() {
    let out = run_main(
        "int[] nums = {1, 2, 3, 4}; int i = 0; int sum = 0; while (i < nums.length) { sum += nums[i]; i++; } System.out.println(sum);",
    );
...
fn new_two_d_int_array_allocates_rectangular_grid() {
    let out = run_main(
        "int[][] grid = new int[2][3]; System.out.println(grid.length); System.out.println(grid[1].length); System.out.println(grid[0][2]);",
    );
