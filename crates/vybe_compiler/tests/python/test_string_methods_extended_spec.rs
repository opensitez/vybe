...
compile_case!(
    str_partition_compile,
    "parts = 'key=value'.partition('=')\n"
);
compile_case!(
    str_rpartition_compile,
    "parts = 'path/to/file'.rpartition('/')\n"
);
...
compile_case!(
    str_translate_compile,
    "tbl = str.maketrans({'a': 'x'})\ns = 'aba'.translate(tbl)\n"
);
compile_case!(str_rfind_compile, "i = 'banana'.rfind('na')\n");
compile_case!(str_rindex_compile, "i = 'banana'.rindex('na')\n");
compile_case!(
    str_format_named_compile,
    "s = '{greet}, {name}!'.format(greet='Hello', name='Ada')\n"
);
compile_case!(
    str_format_positional_compile,
    "s = '{0} + {0} = {1}'.format(2, 4)\n"
);
compile_case!(
    str_percent_format_string_compile,
    "s = 'hello %s' % 'world'\n"
);
compile_case!(
    str_percent_format_tuple_compile,
    "s = '%s:%d' % ('port', 80)\n"
);
