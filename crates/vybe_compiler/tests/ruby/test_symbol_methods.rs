...
ruby_test!(test_symbol_creation_dynamic, "puts :\"hello #{1}\".class.name", "Symbol");
ruby_test!(test_symbol_to_s, "puts :hello.to_s", "hello");
ruby_test!(test_symbol_id2name, "puts :hello.id2name", "hello");
ruby_test!(test_symbol_to_sym, "puts :hello.to_sym.equal?(:hello)", "true"); // returns self
ruby_test!(test_symbol_intern, "puts :hello.intern.equal?(:hello)", "true");
...
ruby_test!(test_symbol_casecmp_question, "puts :hello.casecmp?(:HELLO)", "true");
...
ruby_test!(test_symbol_start_with, "puts :hello.start_with?('he')", "true");
ruby_test!(test_symbol_end_with, "puts :hello.end_with?('lo')", "true");
ruby_test!(test_symbol_encoding, "puts :hello.encoding.name", "US-ASCII"); // or UTF-8 depending on literal
ruby_test!(test_symbol_all_symbols, "puts Symbol.all_symbols.class.name", "Array");
