...
fn symbol_to_s_converts_to_string() {
    compile_ok("s = :hello.to_s
");
}

#[test]
fn symbol_to_s_runtime() {
    assert_eq!(run_ruby_one("puts :world.to_s
"), "world");
}
...
fn symbol_inspect_wraps_in_colon() {
    compile_ok("s = :hello.inspect
");
}

#[test]
fn symbol_inspect_runtime() {
    assert_eq!(run_ruby_one("puts :hello.inspect
"), ":hello");
}
...
fn symbol_id2name_alias() {
    compile_ok("s = :hello.id2name
");
}

#[test]
fn symbol_id2name_runtime() {
    assert_eq!(run_ruby_one("puts :world.id2name
"), "world");
}
...
fn symbol_length_character_count() {
    compile_ok("n = :hello.length
");
}

#[test]
fn symbol_size_alias() {
    compile_ok("n = :hello.size
");
}
...
fn symbol_equality_comparison() {
    compile_ok("result = :hello == :hello
");
}

#[test]
fn symbol_equality_runtime() {
    assert_eq!(run_ruby_one("puts :foo == :foo
"), "true");
}
...
fn symbol_spaceship_operator() {
    compile_ok("result = :apple <=> :banana
");
}
...
fn symbol_identity_same_object_id() {
    compile_ok("result = :hello.object_id == :hello.object_id
");
}
...
fn symbol_to_proc_in_select() {
    compile_ok("result = [1, 2, 3, nil, false].select(&:itself)
");
}
...
fn symbol_to_proc_in_reject() {
    compile_ok("result = ['a', '', 'b', ''].reject(&:empty?)
");
}
...
fn symbol_percent_i_literal() {
    compile_ok("syms = %i[foo bar baz]
");
}

#[test]
fn symbol_percent_i_runtime_length() {
    assert_eq!(run_ruby_one("puts %i[foo bar baz].length
"), "3");
}
...
fn symbol_send_runtime() {
    assert_eq!(run_ruby_one("puts 'hello'.send(:reverse)
"), "olleh");
}
...
fn symbol_match_regex() {
    compile_ok("result = :hello.match(/ell/)
");
}
...
fn symbol_all_symbols() {
    compile_ok("syms = Symbol.all_symbols
");
}
