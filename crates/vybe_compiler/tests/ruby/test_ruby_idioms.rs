...
fn tap_receiver_unchanged_runtime() {
    let out = run_ruby(
        r#"[1, 2, 3].tap { |a| a.push(4) }.each { |x| puts x }
"#,
    );
...
fn then_chains_transformations() {
    compile_ok(
        r#"result = 5.then { |x| x + 1 }.then { |x| x * 2 }
"#,
    );
}
...
fn dup_not_frozen_runtime() {
    assert_eq!(
        run_ruby_one(
            r#"puts 'hello'.freeze.dup.frozen?
"#
        ),
        "false"
    );
...
fn defined_undefined_returns_nil() {
    compile_ok(
        r#"result = defined?(totally_undefined_var_xyz)
"#,
    );
}
