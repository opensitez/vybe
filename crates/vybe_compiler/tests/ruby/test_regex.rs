...
fn regexp_new_constructor() {
    compile_ok(r#"r = Regexp.new('hello')
"#);
}
...
fn regexp_flag_extended() {
    compile_ok("r = /hello   # match hello/x
");
}
...
fn regexp_capture_group_dollar_vars() {
    compile_ok(r#"'2024-01-15' =~ /(\d{4})-(\d{2})/
y = $1
m = $2
"#);
}
...
fn regexp_named_capture_group() {
    compile_ok(r#"m = 'John 30'.match(/(?<name>\w+) (?<age>\d+)/)
"#);
}
...
fn regexp_match_returns_matchdata() {
    compile_ok(r#"md = 'hello'.match(/e(l+)/)
"#);
}
...
fn matchdata_named_access() {
    compile_ok(r#"md = 'John'.match(/(?<first>\w+)/)
n = md[:first]
"#);
}
...
fn matchdata_pre_and_post_match() {
    compile_ok(r#"md = 'hello world'.match(/wor/)
pre = md.pre_match
post = md.post_match
"#);
}
...
fn regexp_match_predicate_no_side_effects() {
    compile_ok(r#"result = 'hello'.match?(/ell/)
"#);
}
...
fn regexp_gsub_with_block() {
    compile_ok(r#"result = 'hello world'.gsub(/\w+/) { |w| w.upcase }
"#);
}
...
fn regexp_gsub_with_hash() {
    compile_ok(r#"result = 'aeiou'.gsub(/[aeiou]/, 'a' => '1', 'e' => '2', 'i' => '3')
"#);
}
...
fn regexp_scan_capture_groups_nested() {
    compile_ok(r#"result = 'one 1, two 2'.scan(/(\w+) (\d)/)
"#);
}
...
fn regexp_split_with_capturing_regex() {
    compile_ok(r#"result = 'a1b2c3'.split(/(\d)/)
"#);
}
...
fn regexp_sub_backreference_in_replacement() {
    compile_ok(r#"result = 'hello world'.sub(/(\w+)/, '[\1]')
"#);
}
...
fn regexp_match_op_nil_on_no_match() {
    compile_ok(r#"result = 'hello' =~ /xyz/
"#);
}

#[test]
fn regexp_match_op_nil_runtime() {
    assert_eq!(run_ruby_one(r#"puts ('hello' =~ /xyz/).nil?
"#), "true");
}
...
fn regexp_character_class() {
    compile_ok(r#"result = 'Hello World'.gsub(/[a-z]/, '*')
"#);
}
...
fn regexp_source_returns_pattern() {
    compile_ok(r#"src = /hello\d+/i.source
"#);
}

#[test]
fn regexp_source_runtime() {
    assert_eq!(run_ruby_one(r#"puts /hello/.source
"#), "hello");
}
