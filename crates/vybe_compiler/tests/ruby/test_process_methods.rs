...
ruby_test!(
    ...
    "puts Process.clock_gettime(Process::CLOCK_MONOTONIC).class.name",
    "Float"
);
ruby_test!(
    test_process_times,
    "puts Process.times.class.name",
    "Process::Tms"
);
ruby_test!(
    test_process_wait2,
...
ruby_test!(
    ...
    "pid"
); // wait returns pid, but exit status would be term signal. just asserting wait finishes.
ruby_test!(
    test_process_spawn,
    "pid = Process.spawn('echo hello > /dev/null'); puts Process.wait(pid)",
    "pid"
);
