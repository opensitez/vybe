...
ruby_test!(
    test_numeric_nonzero_question_false,
    "puts 0.nonzero?.nil?",
    "true"
);
ruby_test!(test_numeric_positive_question, "puts 1.positive?", "true");
ruby_test!(
    test_numeric_negative_question,
    "puts (-1).negative?",
    "true"
);
ruby_test!(test_numeric_real_question, "puts 1.real?", "true");
ruby_test!(
    test_numeric_real_question_complex,
    "puts Complex(1, 2).real?",
    "false"
);
ruby_test!(test_numeric_integer_question, "puts 1.integer?", "true");
ruby_test!(
    test_numeric_integer_question_float,
    "puts 1.0.integer?",
    "false"
);
ruby_test!(
    test_numeric_step,
    "acc = []; 1.step(5, 2) { |i| acc << i }; puts acc.join('-')",
    "1-3-5"
);
ruby_test!(
    test_numeric_step_enumerator,
    "puts 1.step(5).class.name",
    "Enumerator"
);
