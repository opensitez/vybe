...
#[test]
fn string_upper() {
    compile_ok("var x = 'hello'.toUpperCase();");
}
#[test]
fn string_lower() {
    compile_ok("var x = 'HELLO'.toLowerCase();");
}
#[test]
fn string_trim() {
    compile_ok("var x = '  hi  '.trim();");
}
#[test]
fn string_split() {
    compile_ok("var x = 'a,b,c'.split(',');");
}
#[test]
fn string_contains() {
    compile_ok("var x = 'hello'.contains('ell');");
}
#[test]
fn string_starts_with() {
    compile_ok("var x = 'hello'.startsWith('he');");
}
#[test]
fn string_replace() {
    compile_ok("var x = 'hello'.replaceAll('l', 'r');");
}
...
#[test]
fn array_add() {
    compile_ok("var x = [1, 2]; x.add(3);");
}
#[test]
fn array_join() {
    compile_ok("var x = [1, 2, 3].join(',');");
}
#[test]
fn array_reversed() {
    compile_ok("var x = [1, 2, 3].reversed;");
}
...
#[test]
fn if_else() {
    compile_ok("void main() { if (true) { print(1); } else { print(2); } }");
}
#[test]
fn for_loop() {
    compile_ok("void main() { for (var i = 0; i < 10; i++) { print(i); } }");
}
#[test]
fn for_in() {
    compile_ok("void main() { for (var x in [1, 2, 3]) { print(x); } }");
}
#[test]
fn while_loop() {
    compile_ok("void main() { var i = 0; while (i < 5) { i = i + 1; } }");
}
#[test]
fn switch_stmt() {
    compile_ok(
        "void main() { var x = 1; switch(x) { case 1: print('one'); break; default: print('other'); } }",
    );
}
#[test]
fn try_catch() {
    compile_ok("void main() { try { throw 'error'; } catch (e) { print(e); } }");
}
...
#[test]
fn lambda() {
    compile_ok("var f = (x) => x * 2;");
}
#[test]
fn named_params() {
    compile_ok("void greet({String name = 'World'}) { print(name); }");
}
#[test]
fn optional_params() {
    compile_ok("void greet([String name = 'World']) { print(name); }");
...
#[test]
fn null_coalesce() {
    compile_ok("var x = null ?? 42;");
}
#[test]
fn null_aware_access() {
    compile_ok("class A { int x = 1; } void main() { var a = null; var b = a?.x; }");
}
...
fn string_interpolation() {
    compile_ok("var name = 'World'; var s = 'Hello $name!';");
}
#[test]
fn string_interpolation_expr() {
    compile_ok("var x = 42; var s = 'Value is ${x + 1}';");
}
...
#[test]
fn operator_plus_no_return_type() {
    compile_ok(
...
fn operator_equals() {
    compile_ok(
        "class Point { int x; int y; Point(this.x, this.y); bool operator ==(Object other) { return true; } }",
    );
}
#[test]
fn operator_index() {
