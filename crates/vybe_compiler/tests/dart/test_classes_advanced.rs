...
#[test] fn implements_interface() {
    compile_ok("abstract class Printable { void print_(); } class Doc implements Printable { void print_() { print('doc'); } }");
}

#[test] fn implements_multiple() {
    compile_ok(r#"
...
#[test] fn implements_with_extends() {
    compile_ok(r#"
...
#[test] fn multiple_mixins_result() {
...
#[test] fn abstract_with_concrete() {
    compile_ok(r#"
...
#[test] fn redirecting_constructor() {
    compile_ok("class Point { int x; int y; Point(this.x, this.y); Point.origin() : this(0, 0); Point.unit() : this(1, 1); }");
}

#[test] fn redirecting_constructor_result() {
    let out = run_prints(r#"
...
#[test] fn const_constructor_used() {
    compile_ok("class Vec { final int x; final int y; const Vec(this.x, this.y); } const origin = Vec(0, 0);");
}

// ── Static members ────────────────────────────────────────────

#[test] fn static_counter() {
    compile_ok(r#"
class Counter {
  static int _count = 0;
...
#[test] fn static_factory_pattern() {
    compile_ok(r#"
class Singleton {
...
#[test] fn to_string_override() {
    compile_ok("class Point { int x; int y; Point(this.x, this.y); String toString() => 'Point($x, $y)'; }");
}
...
#[test] fn three_level_inheritance() {
    compile_ok(r#"
...
#[test] fn private_field() {
    compile_ok("class Account { double _balance = 0; void deposit(double amt) { _balance += amt; } double get balance => _balance; }");
}

#[test] fn private_method() {
    compile_ok("class Parser { String _clean(String s) => s.trim(); String parse(String s) => _clean(s); }");
}

#[test] fn private_field_result() {
    let out = run_prints(r#"
...
#[test] fn extension_on_int() {
    compile_ok("extension IntExt on int { bool get isEven => this % 2 == 0; }");
}

#[test] fn extension_on_list() {
    compile_ok("extension ListExt<T> on List<T> { T? get secondOrNull => length > 1 ? this[1] : null; }");
}

#[test] fn extension_chain() {
    compile_ok(r#"
