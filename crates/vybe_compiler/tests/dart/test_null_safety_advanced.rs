...
#[test] fn nullable_chain_two_levels() {
    compile_ok("class B { int x = 1; } class A { B? b; } void main() { var a = A(); var v = a.b?.x; }");
}

#[test] fn nullable_chain_method() {
    compile_ok("class A { String? name; } void main() { var a = A(); var v = a.name?.toUpperCase(); }");
}

#[test] fn nullable_chain_result() {
    let out = run_prints(r#"
...
#[test] fn nullable_null_chain_result() {
    let out = run_prints(r#"
class A { String? name; }
...
#[test] fn coalesce_null() {
    let out = run_prints("void main() { var x = null; print(x ?? 'fallback'); }");
    assert_eq!(out, ["fallback"]);
}

#[test] fn coalesce_not_null() {
    let out = run_prints("void main() { var x = 'real'; print(x ?? 'fallback'); }");
    assert_eq!(out, ["real"]);
}

#[test] fn coalesce_chained() {
    let out = run_prints("void main() { String? a; String? b; var c = a ?? b ?? 'default'; print(c); }");
    assert_eq!(out, ["default"]);
}

#[test] fn coalesce_with_expr() {
...
#[test] fn nullable_param_provided() {
    let out = run_prints(r#"
void greet(String? name) { print('Hello ${name ?? 'stranger'}'); }
...
#[test] fn nullable_list_element() {
    compile_ok("List<String?> list = ['a', null, 'b'];");
}

#[test] fn nullable_map_value() {
    compile_ok("Map<String, int?> m = {'a': 1, 'b': null};");
}

#[test] fn nullable_list_filter() {
    compile_ok("List<String?> list = ['a', null, 'b']; var nonNull = list.where((e) => e != null).toList();");
}

// ── Late + nullable combos ───────────────────────────────────

#[test] fn late_nullable() {
    compile_ok("late String? name; void main() { name = null; print(name ?? 'empty'); }");
}

#[test] fn late_initialized_with_nullable() {
    compile_ok(r#"
class Loader {
  late String? data;
