...
/// The walker's declared scheduling contract must reach the script chunk —
/// otherwise the drain loop silently falls back to the ECMA default and the
/// declaration is decoration. Verified end to end because "the suites didn't
/// change" is equally consistent with the wiring being dead.
#[test]
fn declared_scheduling_policy_reaches_the_script_chunk() {
    use vybe_ast::{Lang, Module, QueueDiscipline, SchedulingPolicy};
    ...
    let module = Module {
        ...
        scheduling: SchedulingPolicy {
            queues: QueueDiscipline::SingleReadyQueue,
            ..Default::default()
        } };
    // A bare profile: the policy rides on the MODULE, so no language plugin
    // needs to be registered for this to be meaningful.
    let profile = vybe_compiler::profile::parse_profile("[compiler]\n").expect("profile");
    let compiled = vybe_compiler::primitives::Compiler::with_profile(profile)
        .compile_with_imports(&module)
        .expect("compile");
    assert_eq!(
        compiled.chunks[0].scheduling.queues,
        QueueDiscipline::SingleReadyQueue,
        "the declaration must survive compilation onto the script chunk"
    );
}

/// The `[async]` profile section is the CONTRACT's home: it must parse into
/// the policy, and an unknown spelling must be a HARD error — a typo silently
/// inheriting the ECMA default is the silent-inheritance failure the section
/// exists to end.
#[test]
fn profile_async_section_parses_and_rejects_typos() {
    let good = "[compiler]\n[async]\nqueues = \"single_ready_queue\"\ncontinuation = \"sync_if_settled\"\n";
    let profile = vybe_compiler::profile::parse_profile(good).expect("valid [async]");
    assert_eq!(
        profile.scheduling.queues,
        vybe_ast::QueueDiscipline::SingleReadyQueue
    );
    assert_eq!(
        profile.scheduling.continuation,
        vybe_ast::ContinuationTiming::SyncIfSettled
    );

    let typo = "[compiler]\n[async]\nqueues = \"singel_ready_queue\"\n";
    let err = vybe_compiler::profile::parse_profile(typo).expect_err("typo must be rejected");
    assert!(err.contains("unknown value"), "got: {err}");
}
