...
fn compile_dart(src: &str) -> Vec<Chunk> {
    ...
    let profile = vybe_compiler::profile::parse_profile(vybe_compiler::languages::dart::profile_source())
        .expect("dart profile");
    vybe_compiler::compiler::Compiler::with_profile(profile).compile(&module).expect("dart compile")
}

fn compile_ruby(src: &str) -> Vec<Chunk> {
    let module = vybe_compiler::languages::ruby::parse(src).expect("ruby parse");
    let profile = vybe_compiler::profile::parse_profile(vybe_compiler::languages::ruby::profile_source())
        .expect("ruby profile");
    vybe_compiler::compiler::Compiler::with_profile(profile).compile(&module).expect("ruby compile")
}
...
fn compile_cobol(src: &str) -> Vec<Chunk> {
    ...
    vybe_compiler::compiler::Compiler::with_profile(profile).compile(&module).expect("cobol compile")
}

fn compile_fortran(src: &str) -> Vec<Chunk> {
    let module = vybe_compiler::languages::fortran::parse(src).expect("fortran parse");
    let profile = vybe_compiler::profile::parse_profile(vybe_compiler::languages::fortran::profile_source())
        .expect("fortran profile");
    vybe_compiler::compiler::Compiler::with_profile(profile).compile(&module).expect("fortran compile")
}
...
fn compile_vb(src: &str) -> Vec<Chunk> {
    ...
    let profile = vybe_compiler::profile::parse_profile(vybe_compiler::languages::vb::profile_source())
        .expect("vb profile");
    vybe_compiler::compiler::Compiler::with_profile(profile).compile(&module).expect("vb compile")
}

fn compile_pascal(src: &str) -> Vec<Chunk> {
    let module = vybe_compiler::languages::pascal::parse(src).expect("pascal parse");
    let profile = vybe_compiler::profile::parse_profile(vybe_compiler::languages::pascal::profile_source())
        .expect("pascal profile");
    vybe_compiler::compiler::Compiler::with_profile(profile).compile(&module).expect("pascal compile")
}
