...
fn compile_chunks(src: &str) -> Result<Vec<vybe_bytecode::Chunk>, String> {
    let module = vybe_compiler::languages::lua::parse(src)?;
    let profile =
        vybe_compiler::profile::parse_profile(vybe_compiler::languages::lua::profile_source())
...
pub fn parse_ok(src: &str) {
    vybe_compiler::languages::lua::parse(src).expect("Lua parse failed");
}
