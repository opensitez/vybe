...
pub fn emit_throw_const(chunks: &mut [Chunk], current: usize, exc_name: &str, msg: &str, line: u32) {
    ...
    crate::emitter::errors::emit_exception_new_finalize(chunk, exc_name, line);
    ...
    for name in php_exception_chain(exc_name) {
        crate::emitter::classes::emit_instanceof_chain(chunks, current, this_slot, name, line);
    }
    let chunk = &mut chunks[current];
...
fn push_is_collection(chunk: &mut Chunk, slot: u16, line: u32) {
    ...
    crate::emitter::instructions::recipes::is_object(chunk, line);
