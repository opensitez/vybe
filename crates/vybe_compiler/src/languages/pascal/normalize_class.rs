    ...
    match &mut stmt.kind {
        ...
        StmtKind::VarDecl { declarations, .. } => {
            ...
            for decl in declarations {
                if let crate::ast::BindingPattern::Ident(name) = &decl.pattern {
        ...
        StmtKind::Switch {
            ...
            for case in cases {
                for cond in &mut case.conditions {
                    match cond {
                        crate::ast::CaseCondition::Value(expr)
                        | crate::ast::CaseCondition::Comparison { expr, .. } => {
                            rewrite_implicit_self_members_expr(expr, member_names, shadowed, false);
                        }
                        crate::ast::CaseCondition::Range { from, to } => {
...
fn call_self_form_create_body() -> Vec<Statement> {
    vec![Statement::new(StmtKind::Expr(Expression::new(
        ExprKind::Call {
            ...
            args: vec![crate::ast::Argument::positional(Expression::new(
                ExprKind::This,
            ))],
            optional: false,
        },
