...
enum LabelKind { Block, Loop }

struct LabelStack(Vec<(Option<String>, LabelKind)>);

impl LabelStack {
    fn new() -> Self { LabelStack(Vec::new()) }
    fn push(&mut self, name: Option<String>, kind: LabelKind) { self.0.push((name, kind)); }
    fn pop(&mut self) { self.0.pop(); }

    fn kind_of(&self, label: &str) -> Option<LabelKind> {
        for (name, kind) in self.0.iter().rev() {
            if name.as_deref() == Some(label) { return Some(kind.clone()); }
        }
        None
    }
}
...
pub fn parse(source: &str) -> Result<Module, String> {
        ...
        match top.as_rule() {
            Rule::program => {
                for cmd in top.into_inner() {
                    if cmd.as_rule() != Rule::EOI { walk_script_cmd(cmd, &mut body)?; }
                }
            }
            Rule::EOI => {}
    ...
    Ok(Module { name: "main".into(), language: Lang::Unknown, body, imports: Vec::new() })
}
...
fn walk_module(pair: Pair<Rule>) -> Result<Vec<Statement>, String> {
    ...
    for child in pair.into_inner() {
        match child.as_rule() {
            Rule::id => { module_name = Some(child.as_str()[1..].to_string()); }
            Rule::module_field => {
                let inner = child.into_inner().next().ok_or("Empty module_field")?;
                match inner.as_rule() {
                    Rule::func_field   => members.push(ClassMember::Method(Box::new(walk_func_field(inner)?))),
    ...
    let class = Statement::with_span(StmtKind::ClassDecl {
        name, parents: Vec::new(), interfaces: Vec::new(),
        members, modifiers: ClassModifiers::default(), decorators: Vec::new(),
    }, span);
...
fn walk_func_field(pair: Pair<Rule>) -> Result<Statement, String> {
    ...
    for child in pair.into_inner() {
        match child.as_rule() {
            Rule::id => { func_name = child.as_str()[1..].to_string(); }
            Rule::export_inline => {
            ...
            Rule::import_inline => {}
            Rule::typeuse => { params = walk_typeuse_params(child)?; }
            Rule::local   => { body.extend(walk_local(child)?); }
            Rule::instr   => { body.extend(walk_instr_as_stmts(child, &mut labels)?); }
            _ => {}
        }
    }

    if func_name.is_empty() {
        func_name = export_names.first().cloned().unwrap_or_else(|| "__wasm_func".to_string());
    }
    ...
    Ok(Statement::with_span(StmtKind::FunctionDecl {
        name: func_name, params, return_type: None, body, modifiers,
        handles: Vec::new(), is_async: false, is_generator: false, is_sub: false,
    }, span))
}
...
fn walk_typeuse_params(pair: Pair<Rule>) -> Result<Vec<Param>, String> {
    ...
    for child in pair.into_inner() {
        if child.as_rule() == Rule::param { params.extend(walk_param(child)?); }
    }
    Ok(params)
}
...
fn walk_param(pair: Pair<Rule>) -> Result<Vec<Param>, String> {
    ...
    if let Some(n) = name {
        return Ok(vec![Param { name: n, type_hint: types.into_iter().next(), default: None,
            pass_by: PassBy::Value, is_rest: false, is_kwargs: false, is_optional: false, is_nullable: false }]);
    }
    Ok(types.into_iter().enumerate().map(|(i, t)| Param {
        name: format!("p{}", i), type_hint: Some(t), default: None,
        pass_by: PassBy::Value, is_rest: false, is_kwargs: false, is_optional: false, is_nullable: false,
    }).collect())
}
...
fn walk_local(pair: Pair<Rule>) -> Result<Vec<Statement>, String> {
    ...
    Ok(types.iter().enumerate().map(|(i, t)| {
        ...
        Statement::new(StmtKind::VarDecl {
            declarations: vec![VarDeclarator {
                pattern: BindingPattern::Ident(var_name), type_hint: Some(t.clone()),
                init: Some(init), array_bounds: None, with_events: false,
            }],
            kind: VarDeclKind::Let,
        })
    }).collect())
...
fn walk_instr_as_stmts(pair: Pair<Rule>, labels: &mut LabelStack) -> Result<Vec<Statement>, String> {
    let span = to_span(&pair);
...
fn walk_plain_instr_as_stmts(pair: Pair<Rule>, span: Span, labels: &mut LabelStack) -> Result<Vec<Statement>, String> {
    ...
    match name.as_str() {
        ...
        "unreachable" => Ok(vec![Statement::with_span(StmtKind::Throw { expr: None, cause: None }, span)]),

        // ── br $label ─────────────────────────────────────────────────────
        "br" => {
            let lbl = raw_args.first()
        ...
        "br_if" => {
            ...
            Ok(vec![Statement::with_span(StmtKind::If {
                cond: cond_expr, then_body: vec![branch], else_body: None, elifs: Vec::new(),
            }, span)])
        }
...
fn walk_plain_instr_as_expr(pair: Pair<Rule>, span: Span, labels: &mut LabelStack) -> Result<Expression, String> {
    let mut name = String::new();
...
fn walk_folded_instr_as_stmts(pair: Pair<Rule>, span: Span, labels: &mut LabelStack) -> Result<Vec<Statement>, String> {
    ...
    if all_children.is_empty() { return Ok(Vec::new()); }
    ...
    match name.as_str() {
        ...
        "block" => {
            ...
            if let Some(lbl) = label {
                Ok(vec![Statement::with_span(StmtKind::Labeled { label: lbl, body: Box::new(block_stmt) }, span)])
            } else {
                Ok(vec![block_stmt])
            }
        }
        ...
        "loop" => {
            ...
            labels.pop();
            let while_stmt = Statement::with_span(StmtKind::While {
                cond: Expression::bool(true), body, else_body: None,
            }, span);
            if let Some(lbl) = label {
                Ok(vec![Statement::with_span(StmtKind::Labeled { label: lbl, body: Box::new(while_stmt) }, span)])
            } else {
                Ok(vec![while_stmt])
            }
        }
        ...
        // ── (return instr?) ───────────────────────────────────────────────
        "return" => {
            let val = all_children.into_iter()
        ...
        // ── (unreachable) = WASM trap ─────────────────────────────────────
        "unreachable" => {
            Ok(vec![Statement::with_span(StmtKind::Throw { expr: None, cause: None }, span)])
        }

        // ── (br $label) ───────────────────────────────────────────────────
        "br" => {
            let lbl = all_children.iter()
...
fn walk_folded_instr_as_expr(pair: Pair<Rule>, span: Span, labels: &mut LabelStack) -> Result<Expression, String> {
    let mut all_children: Vec<Pair<Rule>> = pair.into_inner().collect();
    if all_children.is_empty() { return Ok(Expression::null()); }
    let name = if all_children[0].as_rule() == Rule::instr_name {
        all_children.remove(0).as_str().to_string()
    } else {
        String::new()
...
fn walk_folded_core(name: String, children: Vec<Pair<Rule>>, span: Span, labels: &mut LabelStack) -> Result<Expression, String> {
    // ── (block $label instr*) used as expression ──────────────────────────
    if name == "block" || name == "loop" {
        let kind = if name == "block" { LabelKind::Block } else { LabelKind::Loop };
        let mut label: Option<String> = None;
        ...
        for _ in 0..exprs.len().saturating_sub(0) { labels.pop(); }
    ...
    if name == "try" {
        let mut args: Vec<Expression> = Vec::new();
        for child in children {
            if child.as_rule() == Rule::instr || child.as_rule() == Rule::catch_block || child.as_rule() == Rule::catch_all_block {
    ...
    if name == "if" || has_then {
        ...
        return Ok(Expression::with_span(ExprKind::Ternary {
            cond: Box::new(cond), then: Box::new(then_val), else_: Box::new(else_val),
        }, span));
    }
...
fn map_instr_to_ast(name: String, mut args: Vec<Expression>, span: Span) -> Result<Expression, String> {
    match name.as_str() {
        ...
        "local.set" | "global.set" => {
            ...
            Ok(Expression::with_span(ExprKind::Assign { target: Box::new(target), value: Box::new(value) }, span))
        }
        ...
        "local.tee" => {
            ...
            Ok(Expression::with_span(ExprKind::Sequence(vec![
                Expression::with_span(ExprKind::Assign {
        ...
        "i32.div_s" | "i32.div_u" | "i64.div_s" | "i64.div_u" | "f32.div" | "f64.div" => bin_op(args, BinOp::Div, span),
        ...
        "f32.neg" | "f64.neg" => {
            ...
            Ok(Expression::with_span(ExprKind::Unary { op: UnaryOp::Neg, expr: Box::new(operand) }, span))
        }
        ...
        "select" => {
            ...
            Ok(Expression::with_span(ExprKind::Ternary {
                cond: Box::new(cond), then: Box::new(val1), else_: Box::new(val2),
            }, span))
        }
...
fn make_br_stmt_opt(label: Option<&str>, labels: &LabelStack, span: Span) -> Statement {
    match label {
        Some(lbl) => match labels.kind_of(lbl) {
            Some(LabelKind::Loop) =>
                Statement::with_span(StmtKind::Continue(ContinueTarget::Label(lbl.to_string())), span),
            _ =>
                Statement::with_span(StmtKind::Break(BreakTarget::Label(lbl.to_string())), span),
        },
...
fn walk_export_field(pair: Pair<Rule>) -> Result<Expression, String> {
    ...
    for child in pair.into_inner() {
        if child.as_rule() == Rule::string { export_name = unquote(child.as_str()); break; }
    }
...
fn walk_start_field(pair: Pair<Rule>) -> Result<Expression, String> {
    ...
    Ok(Expression::new(ExprKind::Call { callee: Box::new(callee), args: Vec::new(), optional: false }))
}
...
fn walk_invoke_cmd(pair: Pair<Rule>) -> Result<Statement, String> {
    ...
    for child in pair.into_inner() {
        match child.as_rule() {
            Rule::id     => {}
            Rule::string => { if func_name.is_empty() { func_name = unquote(child.as_str()); } }
...
fn walk_assert_return(pair: Pair<Rule>) -> Result<Statement, String> {
    ...
    Ok(Statement::with_span(StmtKind::Expr(make_call("__wast_assert_return", args, span)), span))
}
...
fn walk_assert_trap(pair: Pair<Rule>) -> Result<Statement, String> {
    ...
    Ok(Statement::with_span(StmtKind::Expr(make_call("__wast_assert_trap", args, span)), span))
}
...
fn walk_assert_generic(pair: Pair<Rule>, fn_name: &str) -> Result<Statement, String> {
    ...
    let mut message = String::new();
    for child in pair.into_inner() {
        if child.as_rule() == Rule::string { message = unquote(child.as_str()); }
    }
    Ok(Statement::with_span(StmtKind::Expr(make_call(fn_name, vec![Expression::string(&message)], span)), span))
}
...
fn walk_register_cmd(pair: Pair<Rule>) -> Result<Statement, String> {
    ...
    Ok(Statement::with_span(StmtKind::Expr(make_call("__wasm_register", vec![Expression::string(&name)], span)), span))
}
...
fn walk_get_cmd(pair: Pair<Rule>) -> Result<Statement, String> {
    ...
    Ok(Statement::with_span(StmtKind::Expr(make_call("__wasm_get", vec![Expression::string(&name)], span)), span))
}
...
fn walk_action(pair: Pair<Rule>) -> Result<Expression, String> {
    ...
    match inner.as_rule() {
        Rule::invoke_cmd => {
            ...
            match stmt.kind { StmtKind::Expr(e) => Ok(e), _ => Ok(Expression::null()) }
        }
        Rule::get_cmd => {
...
fn bin_op(mut args: Vec<Expression>, op: BinOp, span: Span) -> Result<Expression, String> {
    ...
    Ok(Expression::with_span(ExprKind::Binary { op, left: Box::new(left), right: Box::new(right) }, span))
}
...
fn parse_integer(s: &str) -> Expression {
    ...
    let (neg, digits) = if s.starts_with("-0x") || s.starts_with("-0X") {
        (true, &s[3..])
    } else if s.starts_with("0x") || s.starts_with("0X") || s.starts_with("+0x") || s.starts_with("+0X") {
...
fn to_span(pair: &Pair<Rule>) -> Span {
    ...
    Span { start_line: start.0 as u32, start_col: start.1 as u32,
           end_line: end.0 as u32, end_col: end.1 as u32 }
}
