...
fn emit_str_concat(imports: &mut Chunk, chunk: &mut Chunk, line: u32) {
    crate::compiler::ops::emit_dyn_add_into(imports, chunk, line);
}
...
fn emit_const_index(chunk: &mut Chunk, idx: u16, line: u32) {
    match chunk.constants[idx as usize].clone() {
        ...
        Value::Undefined => crate::compiler::expressions::emit_undefined(chunk, line),
...
pub fn build_runtime_helpers(imports: &mut Chunk) -> RuntimeHelpers {
    ...
    // in `crate::compiler::collections::emit_reversed`.
    ...
    chunks.push(
        vybe_bytecode::registry::platform_numeric_format_helper().expect(
            "no platform registered a numeric-format helper",
        )(
            imports,
        ),
    );
    ...
    chunks.push(crate::compiler::sprintf::build_sprintf(imports));
...
fn build_runtime_helper_export(imports: &mut Chunk, name: &str) -> Option<Chunk> {
    let chunk = match name {
        ...
        "__stdlib_sprintf" => crate::compiler::sprintf::build_sprintf(imports),
...
fn build_sorted(imports: &mut Chunk) -> Chunk {
    ...
    crate::compiler::collections::emit_slice_into(imports, &mut c, 0);
    c.emit_op_u16(Op::LOCAL_SET, result, 0);

    // len = result.length
    c.emit_op_u16(Op::LOCAL_GET, result, 0);
    crate::compiler::collections::emit_len_into(imports, &mut c, 0);
    ...
    crate::compiler::ops::emit_dyn_lt_into(imports, &mut c, 0);
    ...
    c.emit_op_u16(Op::LOCAL_GET, result, 0);
    c.emit_op_u16(Op::LOCAL_GET, i, 0);
    crate::compiler::collections::emit_get_into(imports, &mut c, 0);
    ...
    core_wasm::i32_const(&mut c, 0, 0);
    crate::compiler::ops::emit_dyn_ge_into(imports, &mut c, 0);
    crate::compiler::ops::emit_dyn_not_into(imports, &mut c, 0);
    ...
    crate::compiler::ops::emit_dyn_gt_into(imports, &mut c, 0);
    crate::compiler::ops::emit_dyn_not_into(imports, &mut c, 0);
    ...
    c.emit_op_u16(Op::LOCAL_GET, j, 0);
    crate::compiler::collections::emit_get_into(imports, &mut c, 0);
    crate::compiler::collections::emit_set_into(imports, &mut c, 0);
    c.emit_op(Op::DROP, 0);
...
fn build_reversed(imports: &mut Chunk) -> Chunk {
    ...
    // result = []
    crate::compiler::collections::emit_array_new_into(imports, &mut c, 0, 0);
    c.emit_op_u16(Op::LOCAL_SET, result, 0);
    ...
    c.emit_op_u16(Op::LOCAL_GET, i, 0);
    crate::compiler::collections::emit_get_into(imports, &mut c, 0);
    crate::compiler::collections::emit_push_into(imports, &mut c, 0);
    c.emit_op(Op::DROP, 0);
...
fn build_iter_drain(imports: &mut Chunk) -> Chunk {
    ...
    c.emit_op_u16(Op::LOCAL_GET, v, 0);
    crate::compiler::collections::emit_import_call_into(
        imports,
        &mut c,
        "ecma:object",
        "iterForOf",
    ...
    crate::compiler::ops::emit_dyn_eq_into(imports, &mut c, 0);
    ...
    crate::compiler::functions::emit_await_into(imports, &mut c, 0);
    ...
    crate::compiler::ops::emit_dyn_to_bool_into(imports, &mut c, 0);
    c.emit_if(0);
    c.emit_op_u16(Op::LOCAL_GET, it, 0);
    c.emit_op_u16(Op::LOCAL_SET, v, 0);
    crate::compiler::generators::emit_drain_into_array_into(imports, &mut c, 0);
    c.emit_op_u16(Op::LOCAL_SET, result, 0);
    c.emit_br(1, 0); // exit
    c.emit_end(0);
    ...
    c.emit_op_u16(Op::LOCAL_GET, counter, 0);
    core_wasm::i32_const(&mut c, 0, 1);
    crate::compiler::ops::emit_dyn_add_into(imports, &mut c, 0);
    c.emit_op_u16(Op::LOCAL_SET, counter, 0);
...
// in `crate::compiler::generators` — this module only registers them.
use crate::compiler::generators::{build_async_generator_next, build_generator_next};
...
fn build_enumerate(imports: &mut Chunk) -> Chunk {
    ...
    c.emit_op_u16(Op::LOCAL_GET, arr, 0);
    c.emit_op_u16(Op::LOCAL_GET, i, 0);
    crate::compiler::collections::emit_get_into(imports, &mut c, 0); // arr[i]
    crate::compiler::collections::emit_array_pair_into(imports, &mut c, 0); // pair = [i, arr[i]]
    crate::compiler::collections::emit_push_into(imports, &mut c, 0); // result.push(pair)
    c.emit_op(Op::DROP, 0);
...
fn build_rotate(imports: &mut Chunk) -> Chunk {
    ...
    c.emit_op_u16(Op::LOCAL_GET, len, 0);
    crate::compiler::ops::emit_dyn_add_into(imports, &mut c, 0); // + len
    c.emit_op_u16(Op::LOCAL_GET, len, 0);
...
fn emit_pascal_write_buffer(c: &mut Chunk, buffer_key: u16, line: u32) {
    ...
    crate::compiler::ops::emit_dyn_eq(c, line);
    crate::compiler::ops::emit_dyn_to_bool(c, line);
    c.emit_if_value(line);
    c.emit_op(Op::DROP, line);
    emit_const_index(c, empty_key, line);
    c.emit_else(line);
...
fn build_pascal_write(imports: &mut Chunk) -> Chunk {
    ...
    crate::compiler::ops::emit_dyn_add_into(imports, &mut c, line);
...
fn build_qbcolor(_imports: &mut Chunk) -> Chunk {
    ...
    c.local_count = 2;
    crate::compiler::collections::emit_array_new_into(_imports, &mut c, 0, 0);
    ...
    for &val in palette.iter() {
        ...
        crate::compiler::collections::emit_push_into(_imports, &mut c, 0);
        c.emit_op(Op::DROP, 0);
    }
    ...
    emit_const_index(&mut c, mask, 0);
    c.emit_op(Op::I32_AND, 0);
    crate::compiler::collections::emit_get_into(_imports, &mut c, 0);
    c.emit_op(Op::RETURN, 0);
    c
}
...
fn build_js_get_method(imports: &mut Chunk) -> Chunk {
    ...
    crate::compiler::expressions::emit_undefined(&mut c, 0);
    c.emit_op(Op::RETURN, 0);
    c
}
...
fn build_slice_step(imports: &mut Chunk) -> Chunk {
    ...
    let empty = c.add_constant(Value::String(std::sync::Arc::from("")));
    emit_const_index(&mut c, empty, 0);
    crate::compiler::collections::emit_join_into(imports, &mut c, 0);
    c.emit_op(Op::RETURN, 0);
    c.emit_end(0);
...
fn build_concat(imports: &mut Chunk) -> Chunk {
    ...
    c.emit_op_u16(Op::LOCAL_GET, a, 0);
    c.emit_op_u16(Op::LOCAL_GET, b, 0);
    crate::compiler::collections::emit_concat_into(imports, &mut c, 0);
    c.emit_op(Op::RETURN, 0);
...
fn build_array_reverse_range(imports: &mut Chunk) -> Chunk {
    ...
    crate::compiler::ops::emit_dyn_add_into(imports, &mut c, 0);
    core_wasm::i32_const(&mut c, 0, 1);
    crate::compiler::ops::emit_dyn_neg_into(imports, &mut c, 0);
    crate::compiler::ops::emit_dyn_add_into(imports, &mut c, 0);
