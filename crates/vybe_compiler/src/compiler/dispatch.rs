...
use crate::compiler::threading as thread_adapter;
...
pub fn emit_common(
    ...
    match name {
        ...
        // ── Channel ops ──
        "channels.send" => crate::compiler::channels::emit_send(chunks, current, line),
        "channels.receive" => crate::compiler::channels::emit_receive(chunks, current, line),
        "channels.len" => crate::compiler::channels::emit_len(chunks, current, line),
        "channels.cap" => crate::compiler::channels::emit_cap(chunks, current, line),
        "channels.close" => crate::compiler::channels::emit_close(chunks, current, line),
        ...
        "sprintf.format" => crate::compiler::sprintf::emit_sprintf(chunks, current, argc, line),
        "sprintf.format_array" => {
            crate::compiler::sprintf::emit_sprintf_from_array(chunks, current, line)
        }

        // ── Expression ops ──
        "expressions.undefined" => {
            crate::compiler::expressions::emit_undefined(&mut chunks[current], line)
        }
        "expressions.i32_not" => {
            crate::compiler::expressions::emit_i32_not(&mut chunks[current], line)
        }
        "expressions.f64_mod" => {
            crate::compiler::expressions::emit_f64_mod(&mut chunks[current], line)
        }
        "math.clamp" => crate::compiler::math::emit_clamp(&mut chunks[current], line),
        "math.copysign" => crate::compiler::math::emit_copysign(&mut chunks[current], line),
        "math.signbit" => crate::compiler::math::emit_signbit(&mut chunks[current], line),
        "math.dim" => crate::compiler::math::emit_dim(&mut chunks[current], line),
        "math.nan" => crate::compiler::math::emit_nan(&mut chunks[current], line),
        "math.inf" => crate::compiler::math::emit_inf(&mut chunks[current], line),
        "math.is_inf" => crate::compiler::math::emit_is_inf(&mut chunks[current], line),
        "math.f64_bits" => crate::compiler::math::emit_f64_bits(&mut chunks[current], line),
        "math.f64_from_bits" => {
            crate::compiler::math::emit_f64_from_bits(&mut chunks[current], line)
        }
        "math.f32_bits" => crate::compiler::math::emit_f32_bits(&mut chunks[current], line),
        "math.f32_from_bits" => {
            crate::compiler::math::emit_f32_from_bits(&mut chunks[current], line)
        }
        "expressions.bool_not" => {
            crate::compiler::expressions::emit_bool_not(&mut chunks[current], line)
        }

        // ── Delegate ops ──
        "delegates.combine" => crate::compiler::delegates::emit_combine(chunks, current, line),
        "delegates.remove" => crate::compiler::delegates::emit_remove(chunks, current, line),
        "delegates.invocation_list" => {
            crate::compiler::delegates::emit_get_invocation_list(chunks, current, line)
        }
        ...
        "str_includes" => {
            ...
            chunks[current].emit_call(idx, 2, line);
            crate::compiler::instructions::core_wasm::i32_const(&mut chunks[current], line, 0);
