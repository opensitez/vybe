...
pub fn emit_spread_iterable(chunks: &mut [Chunk], current: usize, line: u32) {
    ...
    crate::compiler::ops::emit_dyn_to_bool(&mut chunks[current], line);
    chunks[current].emit_if_value(line);

    // Value is already a generator — drain via stack-switching
    chunks[current].emit_op_u16(Op::LOCAL_GET, slot, line);
    crate::compiler::generators::emit_drain_into_array(chunks, current, line);
    ...
    chunks[current].emit_op_u16(Op::LOCAL_GET, slot, line);
    crate::compiler::generators::emit_drain_custom_iterable(chunks, current, line);
...
pub fn emit_sort_by_key_in_place(chunks: &mut [Chunk], current: usize, line: u32) {
    ...
    let _ = chunk;
    let outer = crate::compiler::loops::emit_loop_start(chunks, current, line);
    let chunk = &mut chunks[current];
    lget(chunk, i, line);
    lget(chunk, len, line);
    chunk.emit_op(Op::I32_LT_S, line);
    let _ = chunk;
    crate::compiler::loops::emit_loop_cond(chunks, current, line);
    let chunk = &mut chunks[current];
    ...
    let _ = chunk;
    let inner = crate::compiler::loops::emit_loop_start(chunks, current, line);
    let chunk = &mut chunks[current];
    lget(chunk, j, line);
    lget(chunk, len, line);
    chunk.emit_op(Op::I32_LT_S, line);
    ...
    chunk.emit_op(Op::ARRAY_GET, line);
    chunk.emit_op_u8(Op::CALL_REF, 1, line);
    crate::compiler::ops::emit_dyn_lt(chunk, line);
    chunk.emit_if(line);
    lget(chunk, j, line);
    lset(chunk, best, line);
    chunk.emit_end(line);
    ...
    lset(chunk, j, line);
    let _ = chunk;
    crate::compiler::loops::emit_loop_end(chunks, current, inner, line);
    let chunk = &mut chunks[current];
    ...
    lset(chunk, i, line);
    let _ = chunk;
    crate::compiler::loops::emit_loop_end(chunks, current, outer, line);
...
pub fn emit_get_range(chunks: &mut [Chunk], current: usize, line: u32) {
    ...
    crate::compiler::ops::emit_dyn_add(&mut chunks[current], line); // [arr, index, end]
    emit_import_call(chunks, current, "ecma:array", "slice", 3, line);
}
...
pub fn emit_sequence_equal(chunks: &mut [Chunk], current: usize, line: u32) {
    ...
    lget(&mut chunks[current], len_slot, line);
    crate::compiler::ops::emit_dyn_lt(&mut chunks[current], line);
    crate::compiler::ops::emit_dyn_not(&mut chunks[current], line);
    crate::compiler::ops::emit_dyn_to_bool(&mut chunks[current], line);
    ...
    crate::compiler::ops::emit_dyn_add(&mut chunks[current], line);
    lset(&mut chunks[current], idx_slot, line);
...
pub fn emit_range(chunks: &mut [Chunk], current: usize, arg_count: u8, inclusive: bool, line: u32) {
    ...
    let state = crate::compiler::loops::emit_loop_start(chunks, current, line);
    // cond — continue while:
    ...
    if arg_count == 3 {
        ...
        core_wasm::i32_const(&mut chunks[current], line, 0);
        crate::compiler::ops::emit_dyn_gt(&mut chunks[current], line); // step > 0
        chunks[current].emit_if_value(line);
        ...
        chunks[current].emit_op_u16(Op::LOCAL_GET, i_s, line);
        chunks[current].emit_op_u16(Op::LOCAL_GET, stop_s, line);
        crate::compiler::ops::emit_dyn_gt(&mut chunks[current], line);
        chunks[current].emit_end(line);
    } else {
        chunks[current].emit_op_u16(Op::LOCAL_GET, i_s, line);
        chunks[current].emit_op_u16(Op::LOCAL_GET, stop_s, line);
        if inclusive {
            crate::compiler::ops::emit_dyn_le(&mut chunks[current], line);
        } else {
            crate::compiler::ops::emit_dyn_lt(&mut chunks[current], line);
        }
    }
    ...
    crate::compiler::ops::emit_dyn_add(&mut chunks[current], line);
    chunks[current].emit_op_u16(Op::LOCAL_SET, i_s, line);
    crate::compiler::loops::emit_loop_end(chunks, current, state, line);
...
pub fn emit_zip(chunks: &mut [Chunk], current: usize, n: u8, mode: ZipLen, line: u32) {
    ...
    crate::compiler::tuples::emit_tuple(chunks, current, n as u16, line);
    // result.push(tuple)
...
pub fn emit_reversed(chunks: &mut [Chunk], current: usize, line: u32) {
    ...
    core_wasm::i32_const(&mut chunks[current], line, 0);
    crate::compiler::ops::emit_dyn_ge(&mut chunks[current], line);
    crate::compiler::ops::emit_dyn_not(&mut chunks[current], line);
    chunks[current].emit_br_if(1, line);
    // result.push(seq[i])
