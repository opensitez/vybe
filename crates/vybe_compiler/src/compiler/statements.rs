...
//!
//! Extracted from `compiler/mod.rs` (`impl Compiler`) — same pattern as
            ...
            StmtKind::Try {
                ...
                if let Some((completion_slot, ret_slot)) = completion {
                    let emit_eq_branch = |c: &mut Self, code: f64| {
                        ...
                        let ln = c.line;
                        crate::compiler::ops::emit_dyn_eq(c.chunk(), ln);
                        crate::compiler::ops::emit_dyn_to_bool(c.chunk(), ln);
                        c.chunk().emit_if(ln);
                        c.label_depth += 1;
                    };
                ...
                if let Some(v) = val {
                    ...
                    if let Some((ctx_chunk, this_slot)) = self.js_derived_ctor_ctx {
                        if ctx_chunk == self.current && self.profile.name == "js" {
                            ...
                            self.emit_u16(Op::LOCAL_GET, return_slot);
                            self.chunk().emit_else(line);
                            crate::compiler::classes::emit_this_initialized_guard(
                                self.chunk(),
                                this_slot,
                                line,
                            );
                            ...
                            if self.is_python_profile() {
                                if let ExprKind::Slice { lower, upper, step } = &index.kind {
                                    if step.is_none() {
                                        ...
                                        crate::compiler::slices::emit_strided_del(
                                            &mut self.chunks,
                                            self.current,
                                            line,
                                            opts,
            ...
            ExprKind::Index { object, index, .. } => {
                ...
                if self.is_python_profile() {
                    if let ExprKind::Slice { lower, upper, step } = &index.kind {
                        ...
                        if step.is_none() {
                            self.emit_u16(Op::LOCAL_GET, value_tmp);
                            crate::compiler::slices::emit_splice_assign(
                                &mut self.chunks,
                                self.current,
                                line,
                            );
                            ...
                            crate::compiler::slices::emit_strided_assign(
                                &mut self.chunks,
                                self.current,
                                line,
                                opts,
