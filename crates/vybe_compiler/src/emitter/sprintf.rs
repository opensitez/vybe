...
fn lg(c: &mut Chunk, s: u16)  { c.emit_op_u16(Op::LOCAL_GET, s, 0); }
fn ls(c: &mut Chunk, s: u16)  { c.emit_op_u16(Op::LOCAL_SET, s, 0); c.emit_op(Op::DROP, 0); }
fn ci(c: &mut Chunk, v: i32)  { let k = c.add_constant(Value::I32(v));  c.emit_op_u16(Op::CONST, k, 0); }
fn cf(c: &mut Chunk, v: f64)  { let k = c.add_constant(Value::F64(v));  c.emit_op_u16(Op::CONST, k, 0); }
fn cs(c: &mut Chunk, v: &str) { let k = c.add_constant(Value::String(Arc::from(v))); c.emit_op_u16(Op::CONST, k, 0); }
fn hc(c: &mut Chunk, i: u16, a: u8) { c.emit_op_u16(Op::CALL_IMPORT, i, 0); c.emit(a, 0); }
fn inc(c: &mut Chunk, s: u16) { lg(c,s); c.emit_op(Op::I32_CONST_1,0); c.emit_op(Op::I32_ADD,0); ls(c,s); }
...
pub fn build_sprintf(imports: &mut Chunk) -> Chunk {
    ...
    // init
    lg(&mut c, FMT); hc(&mut c, str_len, 1); ls(&mut c, FLEN);
    cs(&mut c, ""); ls(&mut c, OUT);
    c.emit_op(Op::I32_CONST_0, 0); ls(&mut c, I);
    c.emit_op(Op::I32_CONST_0, 0); ls(&mut c, AIDX);

    // outer: block $ob, loop $ol
    let ob = c.emit_block(0);
    let (ol, _) = c.emit_loop_s(0);
    lg(&mut c, I); lg(&mut c, FLEN); c.emit_op(Op::I32_GE_S, 0); c.emit_br_if(1, 0);

    // read ch
    lg(&mut c, FMT); lg(&mut c, I); hc(&mut c, str_ccat, 2);
    c.emit_op(Op::I32_FROM_F64, 0); ls(&mut c, CH);

    // non-% path (depth: plain_blk=0, ol=1, ob=2)
    {
        let pb = c.emit_block(0);
        lg(&mut c, CH); ci(&mut c, 37); c.emit_op(Op::I32_EQ, 0); c.emit_br_if(0, 0);
        lg(&mut c, OUT); lg(&mut c, FMT); lg(&mut c, I); hc(&mut c, str_chat, 2);
        hc(&mut c, str_cat, 2); ls(&mut c, OUT);
        inc(&mut c, I);
        c.emit_br(1, 0); // continue ol
        c.emit_end(0); c.patch_block(pb);
    }
    inc(&mut c, I); // skip '%'
    ...
    // "%c" specifiers.
    {
        ...
        lg(&mut c, I); lg(&mut c, FLEN); c.emit_op(Op::I32_GE_S, 0); c.emit_br_if(0, 0);
        ...
        // %% -> literal percent
        {
            ...
            lg(&mut c, CH); ci(&mut c, 37); c.emit_op(Op::I32_NE, 0); c.emit_br_if(0, 0);
            lg(&mut c, OUT); cs(&mut c, "%"); hc(&mut c, str_cat, 2); ls(&mut c, OUT);
            inc(&mut c, I);
            c.emit_br(2, 0); // continue outer loop
            c.emit_end(0); c.patch_block(pct);
        }

        // %c -> next arg as character
        {
            let chr = c.emit_block(0);
            lg(&mut c, CH); ci(&mut c, 99); c.emit_op(Op::I32_NE, 0); c.emit_br_if(0, 0);
            lg(&mut c, ARGS); lg(&mut c, AIDX); hc(&mut c, arr_at, 2); ls(&mut c, ARG);
            inc(&mut c, AIDX);
            ...
            let line = 0;
            c.emit_if(line);
            lg(&mut c, ARG); ci(&mut c, 0); hc(&mut c, str_chat, 2); ls(&mut c, RAW);
            c.emit_else(line);
            lg(&mut c, ARG); hc(&mut c, num_num, 1); hc(&mut c, str_fcc, 1); ls(&mut c, RAW);
            c.emit_end(line);

            lg(&mut c, OUT); lg(&mut c, RAW); hc(&mut c, str_cat, 2); ls(&mut c, OUT);
            inc(&mut c, I);
            c.emit_br(2, 0); // continue outer loop
            c.emit_end(0); c.patch_block(chr);
        }

        c.emit_end(0); c.patch_block(fp);
    }
    ...
    // index (set AIDX = N-1), otherwise rewind so the digits parse as width.
    {
        ...
        lg(&mut c, I); ls(&mut c, SAVEI);
        c.emit_op(Op::I32_CONST_0, 0); ls(&mut c, POS);
        // scan digits into POS
        {
            ...
            lg(&mut c, POS); ci(&mut c, 10); c.emit_op(Op::I32_MUL, 0);
            lg(&mut c, CH); ci(&mut c, 48); c.emit_op(Op::I32_SUB, 0);
            c.emit_op(Op::I32_ADD, 0); ls(&mut c, POS);
            inc(&mut c, I);
            c.emit_br(0, 0);
            c.emit_end(0); c.patch_loop(plp);
            c.emit_end(0); c.patch_block(pl);
        }
        // if next char is '$' → use POS as arg index; else rewind I
        {
            ...
            lg(&mut c, FMT); lg(&mut c, I); hc(&mut c, str_ccat, 2);
            c.emit_op(Op::I32_FROM_F64, 0); ls(&mut c, CH);
            lg(&mut c, CH); ci(&mut c, 36); c.emit_op(Op::I32_NE, 0); c.emit_br_if(0, 0);
            // dollar present: AIDX = POS - 1 ; consume '$'
            lg(&mut c, POS); ci(&mut c, 1); c.emit_op(Op::I32_SUB, 0); ls(&mut c, AIDX);
            inc(&mut c, I);
            c.emit_br(1, 0); // exit pos_block, skipping the rewind
            c.emit_end(0); c.patch_block(nd);
        }
        lg(&mut c, SAVEI); ls(&mut c, I); // rewind: digits were width, not arg index
        c.emit_end(0); c.patch_block(pos_block);
    }

    // reset flags
    c.emit_op(Op::I32_CONST_0, 0); ls(&mut c, FLEFT);
    c.emit_op(Op::I32_CONST_0, 0); ls(&mut c, FSIGN);
    c.emit_op(Op::I32_CONST_0, 0); ls(&mut c, FZERO);
    c.emit_op(Op::I32_CONST_0, 0); ls(&mut c, FSPACE);
    c.emit_op(Op::I32_CONST_0, 0); ls(&mut c, FALT);
    c.emit_op(Op::I32_CONST_0, 0); ls(&mut c, WIDTH);
    c.emit_op(Op::I32_CONST_0, 0); ls(&mut c, FPAD);
    cs(&mut c, ""); ls(&mut c, CUSTPAD);
    ci(&mut c, -1); ls(&mut c, PREC);
    ...
    // flag loop
    {
        ...
        // custom pad char: `'X` takes the next char as the pad character.
        {
            ...
            lg(&mut c, CH); ci(&mut c, 39); c.emit_op(Op::I32_NE, 0); c.emit_br_if(0, 0);
            inc(&mut c, I); // skip the quote
            lg(&mut c, FMT); lg(&mut c, I); hc(&mut c, str_chat, 2); ls(&mut c, CUSTPAD);
            ci(&mut c, 1); ls(&mut c, FPAD);
            inc(&mut c, I); // skip the pad char
            c.emit_br(1, 0); // continue flag loop
            c.emit_end(0); c.patch_block(b);
        }
        c.emit_br(1, 0); // exit fl
        c.emit_end(0); c.patch_loop(flp);
        c.emit_end(0); c.patch_block(fl);
    }
    ...
    // width loop
    {
        ...
        lg(&mut c, WIDTH); ci(&mut c, 10); c.emit_op(Op::I32_MUL, 0);
        lg(&mut c, CH); ci(&mut c, 48); c.emit_op(Op::I32_SUB, 0);
        c.emit_op(Op::I32_ADD, 0); ls(&mut c, WIDTH);
        inc(&mut c, I);
        c.emit_br(0, 0);
        c.emit_end(0); c.patch_loop(wlp);
        c.emit_end(0); c.patch_block(wl);
    }
    read_ch(&mut c, FMT, I, FLEN, CH, str_ccat);
    ...
    // precision
    {
        ...
        lg(&mut c, I); lg(&mut c, FLEN); c.emit_op(Op::I32_GE_S, 0); c.emit_br_if(0, 0);
        lg(&mut c, CH); ci(&mut c, 46); c.emit_op(Op::I32_NE, 0); c.emit_br_if(0, 0);
        inc(&mut c, I);
        c.emit_op(Op::I32_CONST_0, 0); ls(&mut c, PREC);
        let pl = c.emit_block(0);
        let (plp, _) = c.emit_loop_s(0);
        lg(&mut c, I); lg(&mut c, FLEN); c.emit_op(Op::I32_GE_S, 0); c.emit_br_if(1, 0);
        lg(&mut c, FMT); lg(&mut c, I); hc(&mut c, str_ccat, 2);
        ...
        lg(&mut c, PREC); ci(&mut c, 10); c.emit_op(Op::I32_MUL, 0);
        lg(&mut c, CH); ci(&mut c, 48); c.emit_op(Op::I32_SUB, 0);
        c.emit_op(Op::I32_ADD, 0); ls(&mut c, PREC);
        inc(&mut c, I);
        c.emit_br(0, 0);
        c.emit_end(0); c.patch_loop(plp);
        c.emit_end(0); c.patch_block(pl);
        read_ch(&mut c, FMT, I, FLEN, CH, str_ccat);
        c.emit_end(0); c.patch_block(dot);
    }

    // read conversion char; if past end emit "%" and break loop
    {
        let eof = c.emit_block(0);
        lg(&mut c, I); lg(&mut c, FLEN); c.emit_op(Op::I32_LT_S, 0); c.emit_br_if(0, 0);
        lg(&mut c, OUT); cs(&mut c, "%"); hc(&mut c, str_cat, 2); ls(&mut c, OUT);
        c.emit_br(3, 0); // break ob
        c.emit_end(0); c.patch_block(eof);
    }
    lg(&mut c, FMT); lg(&mut c, I); hc(&mut c, str_ccat, 2);
    c.emit_op(Op::I32_FROM_F64, 0); ls(&mut c, CONV);
    inc(&mut c, I);

    // %% — depths: pct=0, spec=1, ol=2  → continue ol = br 2
    {
        let pct = c.emit_block(0);
        lg(&mut c, CONV); ci(&mut c, 37); c.emit_op(Op::I32_NE, 0); c.emit_br_if(0, 0);
        lg(&mut c, OUT); cs(&mut c, "%"); hc(&mut c, str_cat, 2); ls(&mut c, OUT);
        c.emit_br(2, 0);
        c.emit_end(0); c.patch_block(pct);
    }
    ...
    lg(&mut c, ARGS); lg(&mut c, AIDX); hc(&mut c, arr_at, 2); ls(&mut c, ARG);
    inc(&mut c, AIDX);
    lg(&mut c, ARG); hc(&mut c, num_num, 1); ls(&mut c, N);
    cs(&mut c, ""); ls(&mut c, RAW);

    // conversions
    conv_s(&mut c, str_tostr, str_slice);
    conv_d(&mut c, math_trunc, math_abs, num_radix, str_cat);
    conv_u(&mut c, math_abs, num_radix);
    conv_f(&mut c, num_fixed, str_cat);
    conv_e(&mut c, num_exp, str_upper, str_cat);
    conv_radix(&mut c, math_trunc, num_radix, str_upper, str_cat, 120, 16, false, "0x");
    conv_radix(&mut c, math_trunc, num_radix, str_upper, str_cat, 88,  16, true,  "0X");
    conv_radix(&mut c, math_trunc, num_radix, str_upper, str_cat, 111,  8, false, "0");
    conv_radix(&mut c, math_trunc, num_radix, str_upper, str_cat, 98,   2, false, "");
    ...
    // pad char: custom `'X` wins; else if fzero && !fleft → "0" else " "
    {
        ...
        lg(&mut c, CUSTPAD);
        c.emit_else(0);
        lg(&mut c, FZERO); lg(&mut c, FLEFT); c.emit_op(Op::I32_EQZ, 0); c.emit_op(Op::I32_AND, 0);
        c.emit_if_value(0);
        cs(&mut c, "0");
        c.emit_else(0);
        cs(&mut c, " ");
    ...
    // apply width
    {
        ...
        lg(&mut c, WIDTH); c.emit_op(Op::I32_CONST_0, 0); c.emit_op(Op::I32_LE_S, 0);
        c.emit_br_if(0, 0);
        {
            let la = c.emit_block(0);
            lg(&mut c, FLEFT); c.emit_op(Op::I32_EQZ, 0); c.emit_br_if(0, 0);
            lg(&mut c, RAW); lg(&mut c, WIDTH); c.emit_op(Op::F64_FROM_I32, 0);
            lg(&mut c, PADCH); hc(&mut c, str_pend, 3); ls(&mut c, RAW);
            c.emit_br(1, 0);
            c.emit_end(0); c.patch_block(la);
        }
        lg(&mut c, RAW); lg(&mut c, WIDTH); c.emit_op(Op::F64_FROM_I32, 0);
        lg(&mut c, PADCH); hc(&mut c, str_pstart, 3); ls(&mut c, RAW);
        c.emit_end(0); c.patch_block(pad);
    }

    lg(&mut c, OUT); lg(&mut c, RAW); hc(&mut c, str_cat, 2); ls(&mut c, OUT);

    c.emit_end(0); c.patch_block(spec);
    c.emit_br(0, 0); // continue ol
    c.emit_end(0); c.patch_loop(ol);
    c.emit_end(0); c.patch_block(ob);
...
fn read_ch(c: &mut Chunk, fmt: u16, i: u16, flen: u16, ch: u16, ccat: u16) {
    let b = c.emit_block(0);
    lg(c, i); lg(c, flen); c.emit_op(Op::I32_GE_S, 0); c.emit_br_if(0, 0);
    lg(c, fmt); lg(c, i); hc(c, ccat, 2); c.emit_op(Op::I32_FROM_F64, 0); ls(c, ch);
    c.emit_end(0); c.patch_block(b);
}
...
fn digit_or_break(c: &mut Chunk, ch: u16, depth: u32) {
    let b = c.emit_block(0);
    lg(c, ch); ci(c, 48); c.emit_op(Op::I32_GE_S, 0);
    lg(c, ch); ci(c, 57); c.emit_op(Op::I32_LE_S, 0);
    c.emit_op(Op::I32_AND, 0); c.emit_br_if(0, 0);
    c.emit_br(depth, 0);
    c.emit_end(0); c.patch_block(b);
}
...
fn flag_arm(c: &mut Chunk, ch: u16, code: i32, flag: u16) {
    let b = c.emit_block(0);
    lg(c, ch); ci(c, code); c.emit_op(Op::I32_NE, 0); c.emit_br_if(0, 0);
    ci(c, 1); ls(c, flag);
    inc(c, I);
...
fn conv_s(c: &mut Chunk, str_tostr: u16, str_slice: u16) {
    let b = c.emit_block(0);
    lg(c, CONV); ci(c, 115); c.emit_op(Op::I32_NE, 0); c.emit_br_if(0, 0);
    lg(c, ARG); hc(c, str_tostr, 1); ls(c, RAW);
    {
        let pb = c.emit_block(0);
        lg(c, PREC); ci(c, 0); c.emit_op(Op::I32_LT_S, 0); c.emit_br_if(0, 0);
        lg(c, RAW); ci(c, 0); c.emit_op(Op::F64_FROM_I32, 0);
        lg(c, PREC); c.emit_op(Op::F64_FROM_I32, 0);
        hc(c, str_slice, 3); ls(c, RAW);
        c.emit_end(0); c.patch_block(pb);
    }
    c.emit_end(0); c.patch_block(b);
}
...
fn conv_d(c: &mut Chunk, math_trunc: u16, math_abs: u16, num_radix: u16, str_cat: u16) {
    let b = c.emit_block(0);
    lg(c, CONV); ci(c, 100); c.emit_op(Op::I32_EQ, 0);
    lg(c, CONV); ci(c, 105); c.emit_op(Op::I32_EQ, 0);
    c.emit_op(Op::I32_OR, 0); c.emit_op(Op::I32_EQZ, 0); c.emit_br_if(0, 0);
    lg(c, N); hc(c, math_trunc, 1); ls(c, N);
    lg(c, N); hc(c, math_abs, 1); cf(c, 10.0); hc(c, num_radix, 2); ls(c, RAW);
    // sign
    {
        let nb = c.emit_block(0);
        lg(c, N); cf(c, 0.0); c.emit_op(Op::F64_LT, 0); c.emit_br_if(0, 0);
        // non-negative: fsign?
        {
            let sb = c.emit_block(0);
            lg(c, FSIGN); c.emit_op(Op::I32_EQZ, 0); c.emit_br_if(0, 0);
            cs(c, "+"); lg(c, RAW); hc(c, str_cat, 2); ls(c, RAW);
            ...
            c.emit_br(2, 0);
            c.emit_end(0); c.patch_block(sb);
        }
        // fspace?
        {
            let spb = c.emit_block(0);
            lg(c, FSPACE); c.emit_op(Op::I32_EQZ, 0); c.emit_br_if(0, 0);
            cs(c, " "); lg(c, RAW); hc(c, str_cat, 2); ls(c, RAW);
            c.emit_end(0); c.patch_block(spb);
        }
        c.emit_br(1, 0); // skip negative "-"
        c.emit_end(0); c.patch_block(nb);
    }
    cs(c, "-"); lg(c, RAW); hc(c, str_cat, 2); ls(c, RAW);
    c.emit_end(0); c.patch_block(b);
}
...
fn conv_u(c: &mut Chunk, math_abs: u16, num_radix: u16) {
    let b = c.emit_block(0);
    lg(c, CONV); ci(c, 117); c.emit_op(Op::I32_NE, 0); c.emit_br_if(0, 0);
    lg(c, N); hc(c, math_abs, 1); cf(c, 10.0); hc(c, num_radix, 2); ls(c, RAW);
    c.emit_end(0); c.patch_block(b);
}

fn conv_f(c: &mut Chunk, num_fixed: u16, str_cat: u16) {
    let b = c.emit_block(0);
    lg(c, CONV); ci(c, 102); c.emit_op(Op::I32_EQ, 0);
    lg(c, CONV); ci(c, 70);  c.emit_op(Op::I32_EQ, 0);
    c.emit_op(Op::I32_OR, 0); c.emit_op(Op::I32_EQZ, 0); c.emit_br_if(0, 0);
    lg(c, N);
    {
        lg(c, PREC); ci(c, 0); c.emit_op(Op::I32_GE_S, 0);
        c.emit_if_value(0);
        lg(c, PREC); c.emit_op(Op::F64_FROM_I32, 0);
        c.emit_else(0);
        cf(c, 6.0);
...
fn conv_e(c: &mut Chunk, num_exp: u16, str_upper: u16, str_cat: u16) {
    let b = c.emit_block(0);
    lg(c, CONV); ci(c, 101); c.emit_op(Op::I32_EQ, 0);
    lg(c, CONV); ci(c, 69);  c.emit_op(Op::I32_EQ, 0);
    c.emit_op(Op::I32_OR, 0); c.emit_op(Op::I32_EQZ, 0); c.emit_br_if(0, 0);
    lg(c, N);
    {
        lg(c, PREC); ci(c, 0); c.emit_op(Op::I32_GE_S, 0);
    ...
    hc(c, num_exp, 2); ls(c, RAW);
    {
        let ub = c.emit_block(0);
        lg(c, CONV); ci(c, 69); c.emit_op(Op::I32_NE, 0); c.emit_br_if(0, 0);
        lg(c, RAW); hc(c, str_upper, 1); ls(c, RAW);
        c.emit_end(0); c.patch_block(ub);
    }
    sign_pos(c, str_cat);
    c.emit_end(0); c.patch_block(b);
}
...
fn conv_g(c: &mut Chunk, num_prec: u16, str_upper: u16, str_cat: u16) {
    let b = c.emit_block(0);
    lg(c, CONV); ci(c, 103); c.emit_op(Op::I32_EQ, 0);
    lg(c, CONV); ci(c, 71);  c.emit_op(Op::I32_EQ, 0);
    c.emit_op(Op::I32_OR, 0); c.emit_op(Op::I32_EQZ, 0); c.emit_br_if(0, 0);

    // RAW = N.toPrecision(prec) ; prec = PREC>=0 ? max(PREC,1) : 6
    lg(c, N);
    {
        lg(c, PREC); ci(c, 0); c.emit_op(Op::I32_LT_S, 0);
        c.emit_if_value(0);
        cf(c, 6.0);
        c.emit_else(0);
        lg(c, PREC); ci(c, 1); c.emit_op(Op::I32_LT_S, 0);
        c.emit_if_value(0);
        cf(c, 1.0);
        c.emit_else(0);
        lg(c, PREC); c.emit_op(Op::F64_FROM_I32, 0);
    ...
    hc(c, num_prec, 2); ls(c, RAW);

    // Strip trailing zeros only for fixed notation (RAW has '.' and no 'e').
    {
        let sb = c.emit_block(0);
        lg(c, RAW); cs(c, "."); c.emit_op(Op::STR_INDEX_OF, 0);
        ci(c, 0); c.emit_op(Op::I32_LT_S, 0); c.emit_br_if(0, 0); // no '.' → skip
        lg(c, RAW); cs(c, "e"); c.emit_op(Op::STR_INDEX_OF, 0);
        ci(c, 0); c.emit_op(Op::I32_GE_S, 0); c.emit_br_if(0, 0); // has 'e' → skip

        // while last char == '0' { RAW = RAW.slice(0, len-1) }
        {
            let zb = c.emit_block(0);
            let (zlp, _) = c.emit_loop_s(0);
            lg(c, RAW); c.emit_op(Op::STR_LENGTH, 0); c.emit_op(Op::I32_EQZ, 0); c.emit_br_if(1, 0);
            lg(c, RAW); lg(c, RAW); c.emit_op(Op::STR_LENGTH, 0); ci(c, 1); c.emit_op(Op::I32_SUB, 0);
            c.emit_op(Op::STR_CHAR_CODE_AT, 0);
            ci(c, 48); c.emit_op(Op::I32_NE, 0); c.emit_br_if(1, 0); // last != '0' → break
            lg(c, RAW); ci(c, 0); lg(c, RAW); c.emit_op(Op::STR_LENGTH, 0); ci(c, 1); c.emit_op(Op::I32_SUB, 0);
            c.emit_op(Op::STR_SLICE, 0); ls(c, RAW);
            c.emit_br(0, 0);
            c.emit_end(0); c.patch_loop(zlp);
            c.emit_end(0); c.patch_block(zb);
        }
        // drop a bare trailing '.'
        {
            ...
            lg(c, RAW); lg(c, RAW); c.emit_op(Op::STR_LENGTH, 0); ci(c, 1); c.emit_op(Op::I32_SUB, 0);
            c.emit_op(Op::STR_CHAR_CODE_AT, 0);
            ci(c, 46); c.emit_op(Op::I32_NE, 0); c.emit_br_if(0, 0); // last != '.' → skip
            lg(c, RAW); ci(c, 0); lg(c, RAW); c.emit_op(Op::STR_LENGTH, 0); ci(c, 1); c.emit_op(Op::I32_SUB, 0);
            c.emit_op(Op::STR_SLICE, 0); ls(c, RAW);
            c.emit_end(0); c.patch_block(db);
        }
        c.emit_end(0); c.patch_block(sb);
    }
    ...
    {
        let ub = c.emit_block(0);
        lg(c, CONV); ci(c, 71); c.emit_op(Op::I32_NE, 0); c.emit_br_if(0, 0);
        lg(c, RAW); hc(c, str_upper, 1); ls(c, RAW);
        c.emit_end(0); c.patch_block(ub);
    }
...
fn conv_radix(c: &mut Chunk, math_trunc: u16, num_radix: u16, str_upper: u16, str_cat: u16,
              code: i32, radix: u16, uppercase: bool, prefix: &str) {
    let b = c.emit_block(0);
    lg(c, CONV); ci(c, code); c.emit_op(Op::I32_NE, 0); c.emit_br_if(0, 0);
    lg(c, N); hc(c, math_trunc, 1); cf(c, radix as f64); hc(c, num_radix, 2);
    if uppercase { hc(c, str_upper, 1); }
    ls(c, RAW);
    if !prefix.is_empty() {
        let ab = c.emit_block(0);
        lg(c, FALT); c.emit_op(Op::I32_EQZ, 0); c.emit_br_if(0, 0);
        cs(c, prefix); lg(c, RAW); hc(c, str_cat, 2); ls(c, RAW);
        c.emit_end(0); c.patch_block(ab);
    }
    c.emit_end(0); c.patch_block(b);
}
...
fn conv_c(c: &mut Chunk, num_num: u16, str_fcc: u16) {
    let b = c.emit_block(0);
    lg(c, CONV); ci(c, 99); c.emit_op(Op::I32_NE, 0); c.emit_br_if(0, 0);
    lg(c, ARG); hc(c, num_num, 1); hc(c, str_fcc, 1); ls(c, RAW);
    c.emit_end(0); c.patch_block(b);
}
...
fn ss_is_space(c: &mut Chunk) {
    lg(c, S_IC); ci(c, 32); c.emit_op(Op::I32_EQ, 0);
    lg(c, S_IC); ci(c, 9);  c.emit_op(Op::I32_EQ, 0); c.emit_op(Op::I32_OR, 0);
    lg(c, S_IC); ci(c, 10); c.emit_op(Op::I32_EQ, 0); c.emit_op(Op::I32_OR, 0);
    lg(c, S_IC); ci(c, 13); c.emit_op(Op::I32_EQ, 0); c.emit_op(Op::I32_OR, 0);
}
...
fn ss_skip_ws(c: &mut Chunk) {
    ...
    let (wlp, _) = c.emit_loop_s(0);
    lg(c, S_I); lg(c, S_ILEN); c.emit_op(Op::I32_GE_S, 0); c.emit_br_if(1, 0);
    lg(c, S_INP); lg(c, S_I); c.emit_op(Op::STR_CHAR_CODE_AT, 0); ls(c, S_IC);
    ss_is_space(c); c.emit_op(Op::I32_EQZ, 0); c.emit_br_if(1, 0);
    inc(c, S_I);
    c.emit_br(0, 0);
    c.emit_end(0); c.patch_loop(wlp);
    c.emit_end(0); c.patch_block(wb);
}
...
pub fn build_sscanf(imports: &mut Chunk) -> Chunk {
    ...
    // ilen / flen ; i = j = 0 ; out = []
    lg(&mut c, S_INP); c.emit_op(Op::STR_LENGTH, 0); ls(&mut c, S_ILEN);
    lg(&mut c, S_FMT); c.emit_op(Op::STR_LENGTH, 0); ls(&mut c, S_FLEN);
    c.emit_op(Op::I32_CONST_0, 0); ls(&mut c, S_I);
    c.emit_op(Op::I32_CONST_0, 0); ls(&mut c, S_J);
    c.emit_op(Op::I32_CONST_0, 0); hc(&mut c, arr_new, 1); ls(&mut c, S_OUT);

    let ob = c.emit_block(0);
    let (ol, _) = c.emit_loop_s(0);
    lg(&mut c, S_J); lg(&mut c, S_FLEN); c.emit_op(Op::I32_GE_S, 0); c.emit_br_if(1, 0);
    lg(&mut c, S_FMT); lg(&mut c, S_J); c.emit_op(Op::STR_CHAR_CODE_AT, 0); ls(&mut c, S_FC);

    // ── conversion specifier: '%' ─────────────────────────────────────────
    {
        let pct = c.emit_block(0);
        lg(&mut c, S_FC); ci(&mut c, 37); c.emit_op(Op::I32_NE, 0); c.emit_br_if(0, 0);
        inc(&mut c, S_J);
        lg(&mut c, S_FMT); lg(&mut c, S_J); c.emit_op(Op::STR_CHAR_CODE_AT, 0); ls(&mut c, S_CONV);
        inc(&mut c, S_J);
        ss_skip_ws(&mut c);
        ...
        {
            let nb = c.emit_block(0);
            lg(&mut c, S_CONV); ci(&mut c, 100); c.emit_op(Op::I32_EQ, 0);
            lg(&mut c, S_CONV); ci(&mut c, 105); c.emit_op(Op::I32_EQ, 0); c.emit_op(Op::I32_OR, 0);
            lg(&mut c, S_CONV); ci(&mut c, 102); c.emit_op(Op::I32_EQ, 0); c.emit_op(Op::I32_OR, 0);
            c.emit_op(Op::I32_EQZ, 0); c.emit_br_if(0, 0);
            lg(&mut c, S_I); ls(&mut c, S_START);
            // optional leading sign
            {
                let sgn = c.emit_block(0);
                lg(&mut c, S_I); lg(&mut c, S_ILEN); c.emit_op(Op::I32_GE_S, 0); c.emit_br_if(0, 0);
                lg(&mut c, S_INP); lg(&mut c, S_I); c.emit_op(Op::STR_CHAR_CODE_AT, 0); ls(&mut c, S_IC);
                lg(&mut c, S_IC); ci(&mut c, 45); c.emit_op(Op::I32_EQ, 0);
                lg(&mut c, S_IC); ci(&mut c, 43); c.emit_op(Op::I32_EQ, 0); c.emit_op(Op::I32_OR, 0);
                c.emit_op(Op::I32_EQZ, 0); c.emit_br_if(0, 0);
                inc(&mut c, S_I);
                c.emit_end(0); c.patch_block(sgn);
            }
            // digit run (and '.' when parsing %f)
            {
                let dl = c.emit_block(0);
                let (dlp, _) = c.emit_loop_s(0);
                lg(&mut c, S_I); lg(&mut c, S_ILEN); c.emit_op(Op::I32_GE_S, 0); c.emit_br_if(1, 0);
                lg(&mut c, S_INP); lg(&mut c, S_I); c.emit_op(Op::STR_CHAR_CODE_AT, 0); ls(&mut c, S_IC);
                lg(&mut c, S_IC); ci(&mut c, 48); c.emit_op(Op::I32_GE_S, 0);
                lg(&mut c, S_IC); ci(&mut c, 57); c.emit_op(Op::I32_LE_S, 0); c.emit_op(Op::I32_AND, 0);
                lg(&mut c, S_IC); ci(&mut c, 46); c.emit_op(Op::I32_EQ, 0);
                lg(&mut c, S_CONV); ci(&mut c, 102); c.emit_op(Op::I32_EQ, 0); c.emit_op(Op::I32_AND, 0);
                c.emit_op(Op::I32_OR, 0);
                c.emit_op(Op::I32_EQZ, 0); c.emit_br_if(1, 0);
                inc(&mut c, S_I);
                c.emit_br(0, 0);
                c.emit_end(0); c.patch_loop(dlp);
                c.emit_end(0); c.patch_block(dl);
            }
            // out.push(Number(input.slice(start, i)))
            lg(&mut c, S_OUT);
            lg(&mut c, S_INP); lg(&mut c, S_START); lg(&mut c, S_I); c.emit_op(Op::STR_SLICE, 0);
            hc(&mut c, num_num, 1);
            hc(&mut c, arr_push, 2); c.emit_op(Op::DROP, 0);
        ...
        {
            let sb = c.emit_block(0);
            lg(&mut c, S_CONV); ci(&mut c, 115); c.emit_op(Op::I32_NE, 0); c.emit_br_if(0, 0);
            lg(&mut c, S_I); ls(&mut c, S_START);
            {
                ...
                lg(&mut c, S_I); lg(&mut c, S_ILEN); c.emit_op(Op::I32_GE_S, 0); c.emit_br_if(1, 0);
                lg(&mut c, S_INP); lg(&mut c, S_I); c.emit_op(Op::STR_CHAR_CODE_AT, 0); ls(&mut c, S_IC);
                ss_is_space(&mut c); c.emit_br_if(1, 0);
                inc(&mut c, S_I);
                c.emit_br(0, 0);
                c.emit_end(0); c.patch_loop(slp);
                c.emit_end(0); c.patch_block(sl);
            }
            lg(&mut c, S_OUT);
            lg(&mut c, S_INP); lg(&mut c, S_START); lg(&mut c, S_I); c.emit_op(Op::STR_SLICE, 0);
            hc(&mut c, arr_push, 2); c.emit_op(Op::DROP, 0);
        ...
        // %c → single character
        {
            ...
            lg(&mut c, S_CONV); ci(&mut c, 99); c.emit_op(Op::I32_NE, 0); c.emit_br_if(0, 0);
            lg(&mut c, S_OUT);
            lg(&mut c, S_INP); lg(&mut c, S_I); c.emit_op(Op::STR_CHAR_AT, 0);
            hc(&mut c, arr_push, 2); c.emit_op(Op::DROP, 0);
            inc(&mut c, S_I);
            c.emit_br(2, 0); // continue ol
            c.emit_end(0); c.patch_block(cb);
        }
    ...
    // ── whitespace in format: match a run of whitespace in input ──────────
    {
        ...
        lg(&mut c, S_FC); ci(&mut c, 32); c.emit_op(Op::I32_EQ, 0);
        lg(&mut c, S_FC); ci(&mut c, 9);  c.emit_op(Op::I32_EQ, 0); c.emit_op(Op::I32_OR, 0);
        lg(&mut c, S_FC); ci(&mut c, 10); c.emit_op(Op::I32_EQ, 0); c.emit_op(Op::I32_OR, 0);
        lg(&mut c, S_FC); ci(&mut c, 13); c.emit_op(Op::I32_EQ, 0); c.emit_op(Op::I32_OR, 0);
        c.emit_op(Op::I32_EQZ, 0); c.emit_br_if(0, 0);
        inc(&mut c, S_J);
        ss_skip_ws(&mut c);
        c.emit_br(1, 0); // continue ol
        c.emit_end(0); c.patch_block(wsb);
    }

    // ── literal char: must match the input, else stop scanning ────────────
    {
        let lit = c.emit_block(0);
        lg(&mut c, S_I); lg(&mut c, S_ILEN); c.emit_op(Op::I32_GE_S, 0); c.emit_br_if(2, 0); // input exhausted → break
        lg(&mut c, S_INP); lg(&mut c, S_I); c.emit_op(Op::STR_CHAR_CODE_AT, 0);
        lg(&mut c, S_FC); c.emit_op(Op::I32_NE, 0); c.emit_br_if(2, 0); // mismatch → break
        inc(&mut c, S_I);
        inc(&mut c, S_J);
        c.emit_end(0); c.patch_block(lit);
    }
