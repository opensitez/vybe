...
impl Compiler {
    ...
    fn compile_assign_stmt(
        ...
        if let Some((_arity, idents)) = self.detect_multi_value_receive(targets, value) {
            ...
            let prefer_numeric_add =
                matches!(targets.as_slice(), [target] if self.expr_prefers_numeric_add(target));
            ...
            // measured across 900 php files, `is_ref_assign` was never
            ...
            match m {
                ClassMember::Method(stmt) => {
                    ...
                    } = &stmt.kind
                    {
                        ...
                        let global_name = if module_name.contains('.') || !contributes_unqualified {
    ...
    #[allow(clippy::ptr_arg)]
    #[inline(never)]
    fn compile_namespace_decl_stmt(
        &mut self,
        name: &String,
        body: &Vec<Statement>,
    ) -> Result<(), String> {
    ...
    #[allow(clippy::ptr_arg)]
    #[inline(never)]
    pub(super) fn compile_expr_stmt(&mut self, expr: &Expression) -> Result<(), String> {
        match &expr.kind {
            ...
            ExprKind::Ternary { cond, then, else_ } => {
                ...
                let line = self.line;
                self.chunk().emit_if(line);
                self.compile_expr_stmt(then)?;
                self.chunk().emit_else(line);
                self.compile_expr_stmt(else_)?;
                self.chunk().emit_end(line);
            }
    ...
    fn compile_forin_stmt(
        ...
        if self.is_direct_generator_call(iter) {
            ...
            if natural_object_iter {
                self.emit_u16(Op::LOCAL_GET, iter_slot);
                common::collections::emit_iter_natural(&mut self.chunks, self.current, self.line);
                self.emit_u16(Op::LOCAL_SET, iter_slot);
            }
    ...
    #[allow(clippy::ptr_arg, clippy::too_many_arguments)]
    #[inline(never)]
    fn compile_input_record_file_stmt(
        &mut self,
    ...
    #[allow(clippy::ptr_arg, clippy::too_many_arguments)]
    #[inline(never)]
    fn compile_wasm_try_table_stmt(
        &mut self,
        body: &Vec<Statement>,
    ...
    fn compile_stmt_inner(&mut self, stmt: &Statement) -> Result<(), String> {
        ...
        match &stmt.kind {
            ...
            StmtKind::Exit { status } => {
                return self.compile_exit_stmt(status.as_ref());
            }
            StmtKind::Return(val) => {
                ...
                if let Some(v) = val {
                    ...
                    if let Some((ctx_chunk, this_slot)) = self.js_derived_ctor_ctx {
                        // Redundant name check removed: `js_derived_ctor_ctx`
                        // is only ever set under `ecma_new_dispatch`.
            ...
            StmtKind::With { items, body, .. } => {
                ...
                if let Some(first) = items.first() {
                    ...
                    let slot =
                        self.define_source_local_typed(&binding, target_type.map(Into::into));
                    self.emit_u16(Op::LOCAL_SET, slot);
            ...
            StmtKind::Echo(exprs) => {
                ...
                if self.profile.echo_concatenates_operands {
                    ...
                    // PHP does NOT reach here: its walker rewrites every `echo`
                    // into a `__php_echo` call bound to `common:php.echo`, so
                    // the raw-stdout write, the null-skip and the ToString-slot
                    // probe that used to sit behind a `profile.name == "php"`
                    // check here were unreachable. (`merge_consecutive_echos`,
                    // the only php site building this node, is dead code.)
                    for expr in exprs {
                        self.compile_expr(expr)?;
                        common::io::emit_print_with_import(self.chunk(), log_idx, 1, line);
                    }
            ...
            StmtKind::Assert { test, msg } => {
                self.compile_condition_to_i32(test)?;
                self.emit(Op::I32_EQZ);
                let line = self.line;
                self.chunk().emit_if(line);
            ...
            StmtKind::ScopeDecl { kind, names } => {
                // All three kinds say how THIS scope resolves names, so they
                // all write to the scope and none needs to know the language.
                let canon: Vec<String> = names.iter().map(|name| self.canon(name)).collect();
            ...
            StmtKind::FileDecl {
                ...
                self.compile_file_decl(name, path, record, *organization, *access, *mode, keys)?;
            }
    ...
    pub(super) fn compile_var_declarator(
        ...
        match &decl.pattern {
            BindingPattern::Ident(name) => {
                ...
                let is_hoisted = *kind == VarDeclKind::FunctionScoped && self.scopes.len() == 1;
                if is_toplevel
                ...
                if is_toplevel || is_hoisted {
                    ...
                    {
                        let field_key =
                            self.resolve_slot_interned(&class_slots::ClassSlot::internal(&cn));
    ...
    pub(super) fn compile_assign_target_valued(
        ...
        match &target.kind {
            ExprKind::Ternary { cond, then, else_ } => {
                let value_tmp = self.define_local("__conditional_assign_value");
                ...
                self.compile_assign_target_valued(then, source)?;
                self.chunk().emit_else(line);
                self.emit_u16(Op::LOCAL_GET, value_tmp);
                self.compile_assign_target_valued(else_, source)?;
                self.chunk().emit_end(line);
            }
            ExprKind::Sequence(parts) => {
                let value_tmp = self.define_local("__sequence_assign_value");
                self.emit_u16(Op::LOCAL_SET, value_tmp);
                if let Some((last, prefix)) = parts.split_last() {
                    ...
                    self.emit_u16(Op::LOCAL_GET, value_tmp);
                    self.compile_assign_target_valued(last, source)?;
                }
            }
            ExprKind::Ident(name) => {
            ...
            ExprKind::Member { object, field, .. } => {
                    ...
                    && matches!(&object.kind, ExprKind::Super)
                {
                    ...
                    self.class_get(
                        class_slots::ObjSource::Stack,
                        &class_slots::ClassSlot::internal(&format!("__set_{}", field)),
                    );
                    ...
                    && !self.expr_user_value_type_name(object).is_some()
                {
                    if let Some(type_hint) = self.infer_expr_type_hint(object) {
                        ...
                        if let Some(target) = (!self.user_owns_type_spelling(&class_name))
                            .then(|| self.tree_property_setter_target(&class_name, field))
                            .flatten()
                        {
                            match target {
                ...
                let field_name = self
                    .field_storage_name_for_receiver(object, field)
                    .unwrap_or_else(|| self.js_member_storage_name(field));
                if self.profile.user_types_are_value_types {
                ...
                if matches!(op, UnaryOp::Deref | UnaryOp::AddrOf) =>
            {
                ...
                if matches!(op, UnaryOp::AddrOf) {
                    self.compile_address_of_expr(expr)?;
                } else {
                    self.compile_expr(expr)?;
                }
            ...
            ExprKind::Index { object, index, .. } => {
                ...
                // namespace to the real global. Stack on entry: `[value]`.
                if self.expr_is_global_namespace(object) {
                    return self.emit_global_namespace_index_set(index);
                }
                ...
                if self.profile.slice_bounds_inclusive {
                    if let ExprKind::Slice { lower, upper, step } = &index.kind {
                        if step.is_none() {
                            let line = self.line;
                ...
                if is_append {
                    ...
                    && self
                        .builtin_slot_target(object, vybe_ast::ProtocolSlot::SetItem)
                        .is_some()
                {
                    let target = self
                        .builtin_slot_target(object, vybe_ast::ProtocolSlot::SetItem)
                    ...
                    self.compile_array_index_operand_for_owner(object, index)?;
                    self.emit_u16(Op::LOCAL_GET, tmp);
                    self.emit_slot_target(&target, 3, line, "bytes set_item");
                    self.emit(Op::DROP);
                ...
                // gated on the same fact and probing the matching accessor.
                    ...
                    self.class_get(
                        class_slots::ObjSource::Stack,
                        &class_slots::ClassSlot::internal("__set___index__"),
                    );
                    ...
                    if std::env::var_os("VYBE_KEEP_PY_IDX").is_some() {
                        ...
                        // Redundant name check removed: the branch requires a
                        // `map[...]` type hint, a form only Go produces.
                        {
                ...
                for prop in props {
                    if let crate::ast::ObjectProperty::Shorthand(name) = prop {
                        ...
                        if let ExprKind::Lit(crate::ast::Literal::Str(ref s)) = key.kind {
                            self.class_get(
                                class_slots::ObjSource::Stack,
                                &class_slots::ClassSlot::internal(s),
                            );
                        } else {
                            self.compile_expr(key)?;
                            let l = self.line;
