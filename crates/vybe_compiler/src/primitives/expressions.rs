...
impl Compiler {
    fn emit_len_value_method(&mut self, def: &crate::profile::BuiltinDef) -> Result<(), String> {
        match &def.emit {
            ...
            vybe_runtime::profile::BuiltinEmit::Invoke(method_name) => {
                ...
                common::invoke::emit_invoke_method(&mut self.chunks, self.current, &name, 0, line);
            }
    ...
    pub(crate) fn emit_tree_ctor_construction(
        ...
        if is_control {
            ...
            if carries_dom_rtt {
                ...
                for key in [
                    "__node",
                    "__document",
                    crate::primitives::gui::CONTROL_TYPE_FIELD,
                ] {
        ...
        self.class_set(
            class_slots::ObjSource::Stack,
            &class_slots::ClassSlot::internal("__types"),
            class_slots::ValueSource::Stack,
        );
        ...
        if is_control && !control_type.is_empty() {
            ...
            self.class_set(
                class_slots::ObjSource::Stack,
                &class_slots::ClassSlot::internal(crate::primitives::gui::CONTROL_TYPE_FIELD),
                class_slots::ValueSource::Stack,
            );
        }
        ...
        self.class_set(
            class_slots::ObjSource::Stack,
            &class_slots::ClassSlot::internal("__controlfn"),
            class_slots::ValueSource::Stack,
        );
    ...
    fn emit_js_import_meta_object(&mut self) {
        ...
        let url_key = self.resolve_slot_interned(&class_slots::ClassSlot::internal("url"));
        ...
        self.class_set(
            class_slots::ObjSource::Stack,
            &class_slots::ClassSlot::internal("url"),
            class_slots::ValueSource::Stack,
        );
    ...
    pub(super) fn emit_generator_entry_control(&mut self, control_slot: u16) -> Result<(), String> {
        ...
        self.emit_u16(Op::LOCAL_GET, control_slot);
        let op_key = self.resolve_slot_interned(&class_slots::ClassSlot::internal("op"));
        self.class_get_resolved(class_slots::ObjSource::Stack, &op_key);
        ...
        self.emit_u16(Op::LOCAL_GET, control_slot);
        let value_key = self.resolve_slot_interned(&class_slots::ClassSlot::internal("value"));
        self.class_get_resolved(class_slots::ObjSource::Stack, &value_key);
        {
            let line = self.line;
    ...
    pub(crate) fn emit_generator_yield_value(&mut self, yielded_slot: u16) {
        ...
        self.class_get(
            class_slots::ObjSource::Stack,
            &class_slots::ClassSlot::internal("__vybe_generator_yield"),
        );
        crate::primitives::ops::emit_dyn_to_bool(self.chunk(), line);
        self.chunk().emit_if_value(line);
        ...
        self.class_get(
            class_slots::ObjSource::Stack,
            &class_slots::ClassSlot::internal("payload_id"),
        );
        ...
        match &expr.kind {
            ...
            ExprKind::Binary { op, left, right } => {
                ...
                if *op == BinOp::InstanceOf {
                    // ⛔⛔ A DECLARED CLASS IS ANSWERED BY THE rtt, WHATEVER THE
                    // DISPATCH MODEL.
                    //
                    // Prototype dispatch decides how a METHOD is found; it says
                    // nothing about type IDENTITY. Sending `instanceof` down the
                    // prototype path for a user class answered from
                    // `__type`/`__types` — ordinary writable properties — so
                    // identity was FORGEABLE:
                    //
                    //     const u = new User();
                    //     u.__type = "Admin";
                    //     u instanceof Admin   →  true      ⛔
                    //
                    // The rtt is stamped at allocation and no instruction can
                    // change it, so for a class this compiler declared,
                    // `ref.test` is both the correct answer and the unforgeable
                    // one. The prototype/string path stays for BUILTIN
                    // constructors and for a dynamic right-hand side, where
                    // there is no declared type index to test against.
                    // Prototype dispatch decides METHOD lookup; it says nothing
                    // about type IDENTITY. So a class this compiler declared is
                    // answered by the rtt in EVERY model — which is also the
                    // unforgeable answer, since the rtt is stamped at allocation
                ...
                if self.profile.ecma_in_operator && *op == BinOp::In {
                    if let ExprKind::Ident(field) = &left.kind {
                        ...
                        if self.member_access_is_private(field) {
                            ...
                            // ⛔ SECOND SITE OF THE SAME DEFECT. `in` here is a
                            // KEY test against the property map, which an
                            // indexed struct slot never populates — so the
                            // brand check answered `false` for an object that
                            // demonstrably carries the field. Identical cause to
                            // `emit_js_private_brand_check`, but it does not go
                            // through that helper, so fixing the helper alone
                            // left `#x in obj` wrong (`want [true false] got
                            // [false …]`).
                            //
                            // `ref.test` is what the question actually means —
                            ...
                            if let Some(class) = self.declaring_owner_of_storage(&storage_name) {
                                ...
                                // `in` is a VALUE position: `REF_TEST` leaves an
                                // `i32`, and ECMA-262 requires a `boolean` here.
                                // Without this the answers were already right
                                // and printed `1` / `0` instead of `true` /
                                // `false`. Branch conditions accept both, which
                                // is why the brand check above needs no wrapper.
                                crate::primitives::ops::emit_i32_to_bool(self.chunk(), line);
                                return Ok(());
                            }
                    ...
                    _ => {
                        ...
                        match op {
                            ...
                            UnaryOp::TrailingZeros(lane) => {
                                let l = self.line;
                                crate::primitives::bits::emit_trailing_zeros(
                                    self.chunk(),
                                    *lane,
                                    l,
                                );
            ...
            ExprKind::Member {
                ...
                let receiver_declares_result = self
                    ...
                    .is_some_and(|hint| self.tree_instance_member(&hint, "Result").is_some());
                if receiver_declares_result && self.canon(field) == self.canon("Result") {
                    ...
                    self.class_get(
                        class_slots::ObjSource::Stack,
                        &class_slots::ClassSlot::internal("Result"),
                    );
                    self.emit_u16(Op::LOCAL_SET, value_slot);
                    self.emit_u16(Op::LOCAL_GET, value_slot);
                ...
                if false
                    ...
                    && !*null_safe
                    && matches!(&object.kind, ExprKind::GlobalNamespace)
                {
                    ...
                    && !*null_safe
                {
                    if self.current_class.is_some() {
                        ...
                        let getter_key = self.resolve_slot_interned(
                            &class_slots::ClassSlot::internal(&format!("__get_{}", field)),
                        );
                        self.emit_js_super_home_base();
                        self.class_get_resolved(class_slots::ObjSource::Stack, &getter_key);
                ...
                if self.member_access_is_private(field) && !*null_safe {
                    ...
                    self.class_get(
                        class_slots::ObjSource::Stack,
                        &class_slots::ClassSlot::internal(&getter_name),
                    );
                ...
                if let Some(target) = namespace_tree_zero_arg_method {
                    if let crate::component_classes::InstanceMethodTarget::Common { emit, .. } =
                        &target
                    {
                        let line = self.line;
                        self.compile_expr(object)?;
                    ...
                    match target {
                        ...
                        crate::component_classes::InstanceMethodTarget::Common { emit, .. } => {
                            let line = self.line;
                            self.emit_common(&emit, 1, line);
                        }
                    }
            ...
            ExprKind::Index {
                ...
                } else if !matches!(index.kind, ExprKind::Range { .. } | ExprKind::Slice { .. })
                    && self.builtin_type_of(object)
                        == Some(vybe_ast::builtin_slots::BuiltinType::Bytes)
                    && self
                        .builtin_slot_target(object, vybe_ast::ProtocolSlot::GetItem)
                        .is_some()
                {
                    let target = self
                        .builtin_slot_target(object, vybe_ast::ProtocolSlot::GetItem)
                    ...
                    let line = self.line;
                    self.emit_slot_target(&target, 2, line, "bytes get_item");
                    return Ok(());
                ...
                // this path instead of the one that maintains its storage.
                    ...
                    self.class_get(
                        class_slots::ObjSource::Stack,
                        &class_slots::ClassSlot::internal("__get___index__"),
                    );
            ...
            ExprKind::New { class, args } => {
                ...
                if let Some(type_name) = dotted_type_name.as_ref() {
                    ...
                    if self.defined_classes.contains(&canon_type) {
                        ...
                        let js_ctor_rest_fixed: Option<usize> = if self.ecma_new_dispatch()
                            && !args
                                .iter()
                    ...
                    if self.ecma_new_dispatch() && self.defined_functions.contains(&canon_type) {
                        self.emit_global_read(&canon_type);
                    ...
                    let dotnet_constructor = if type_name.contains('.') {
                        self.tree_ctor_target(type_name)
                            .or_else(|| self.tree_ctor_target(bare_src_str))
                    } else {
                        self.tree_ctor_target(bare_src_str)
                    };
                    if let Some(target) = dotnet_constructor.clone() {
                        for a in args {
                            self.compile_expr(&a.value)?;
                        }
                        let tree_identity_name = if type_name.contains('.')
                            && self.tree_is_registered_type(type_name)
                        {
                            bare_src
                        } else {
                            bare_src_str
                        ...
                        let proper_name: String = tree_identity_name.to_string();
                        match target {
                        ...
                        self.emit_tree_ancestry_stamp(tree_identity_name);
                        ...
                        if matches!(bare_proper_name, "List" | "ArrayList") {
                            let sort_key = self
                                .resolve_slot_interned(&class_slots::ClassSlot::internal("sort"));
                            inst!(self, core_wasm::dup);
                    ...
                    if dotnet_constructor.is_none() {
                            ...
                            .map(|(m, f)| (m.to_string(), f.to_string()))
                        {
                            ...
                            if module == "common" {
                                ...
                                if self.ecma_new_dispatch() && module == "ecma:error" {
                                    let line = self.line;
            ...
            ExprKind::Assign { target, value } => {
                ...
                self.compile_expr(value)?;
                let assigned_value = self.define_local("__assign_expr_value");
                self.emit_u16(Op::LOCAL_TEE, assigned_value);
                self.compile_assign_target_valued(target, Some(value))?;
                self.emit_u16(Op::LOCAL_GET, assigned_value);
                        ...
                        .all(|p| matches!(p, ObjectProperty::KeyValue { .. }))
                {
                    ...
                    for prop in &props[1..] {
                        ...
                        let ExprKind::Lit(Literal::Str(field)) = &key.kind else {
                            return Err(
                                "copy-with-overrides: member key must be a name".to_string()
                            );
                        };
                        let field_name = self
                    ...
                    match prop {
                        ObjectProperty::KeyValue { key, value } => {
                            if let ExprKind::Lit(Literal::Str(k)) = &key.kind {
                                ...
                                if !self.profile.ecma_object_literals {
                                    inst!(self, core_wasm::dup);
                                    let keys_key = self.resolve_slot_interned(
                        ...
                        ObjectProperty::Shorthand(name) => {
                            ...
                            self.emit_var_get(name);
                            let idx =
                                self.resolve_slot_interned(&class_slots::ClassSlot::internal(name));
                            self.class_set_resolved(
                                class_slots::ObjSource::Stack,
                                &idx,
                                class_slots::ValueSource::Stack,
                        ...
                        ObjectProperty::Accessor { kind, key, value } => {
                            ...
                            self.class_set(
                                class_slots::ObjSource::Stack,
                                &class_slots::ClassSlot::internal(&accessor_slot),
                                class_slots::ValueSource::Stack,
                            );
                        }
            ...
            ExprKind::IsType {
                ...
                // ECMA-compatible Bool output.
                {
                    ...
                    match canon_type.as_str() {
                        ...
                        | "uint64" | "byte" | "sbyte" | "short" | "ushort" | "uint" | "ulong" => {
                            self.emit_u16(Op::LOCAL_GET, obj_slot);
                ...
                for candidate in type_name_candidates {
                    self.emit_u16(Op::LOCAL_GET, obj_slot);
                    let type_key =
                        self.resolve_slot_interned(&class_slots::ClassSlot::TypeIdentity);
                    self.class_get_resolved(class_slots::ObjSource::Stack, &type_key);
                        ...
                        if matches!(&inner.kind, ExprKind::Object(_)) {
                                ...
                                .map(|pending| pending.fields.clone())
                            {
                                for field_name in fields {
                                    ...
                                    let field_idx = self.resolve_slot_interned(
                                        &class_slots::ClassSlot::internal(&field_name),
                                    );
                ...
                if let Some(method_parts) = bound_parts {
                    if self.emit_self_ref() {
                        for part in method_parts {
                            let idx = self.resolve_slot_interned(
                                &class_slots::ClassSlot::internal(&self.canon(part)),
                            );
                            self.class_get_resolved(class_slots::ObjSource::Stack, &idx);
                        }
                        return Ok(());
            ...
            ExprKind::SuperCall { method, args } => {
                ...
                if is_ctor_call {
                    ...
                    if let Some(parent_name) = parent_name {
                        ...
                        // ⛔ THE NEAREST ANCESTOR THAT DECLARES IT, not the
                        // immediate parent. This read is an own-slot get on the
                        // parent's CLASS OBJECT, so when the parent declares
                        // nothing it yields undefined and the call throws
                        // `undefined is not callable`:
                        //
                        //     open class A5 { open fun v() = 1 }
                        //     open class B5 : A5() { }        // declares nothing
                        //     class C5 : B5() { override fun v() = super.v() + 2 }
                        //
                        // Measured in kotlin and java. It is the THIRD
                        // mechanism behind one symptom — the prototype path did
                        // not walk, python's `__mro__` was truncated, and this
                        // reads a single level — and, like the others, the
                        // trigger is "the immediate parent does not declare the
                        // method", NOT an empty intermediate class: a `B5`
                        // declaring an unrelated member fails identically.
                        //
                        // Answered from the declared class graph, so it is a
                        // fact about the program and not about any language.
                        let target = self
                            .current_class
                            .clone()
                            .and_then(|cn| self.nearest_declaring_ancestor(&cn, mname))
                        ...
                        self.emit_direct_callable_invoke((args.len() as u8).saturating_add(1));
                    } else {
                        self.emit_null();
                    }
                } else {
    ...
    fn try_compile_python_set_binary_operator(
        ...
        let size_key = self.resolve_slot_interned(&class_slots::ClassSlot::internal("size"));
        self.emit_u16(Op::LOCAL_GET, lhs_slot);
        self.class_get_resolved(class_slots::ObjSource::Stack, &size_key);
        self.emit(Op::REF_IS_NULL);
    ...
    fn try_compile_dotnet_datetime_timespan_binary_operator(
        ...
        if !self.tree_is_registered_type(&left_type) || !self.tree_is_registered_type(&right_type) {
            return Ok(false);
        }
    ...
    fn emit_c_unsigned_wrap(&mut self, modulus: f64) {
        ...
        common::math::emit_c_fmod(self.chunk(), line);
        self.emit_const(Value::F64(modulus));
        self.emit(Op::F64_ADD);
        self.emit_const(Value::F64(modulus));
        common::math::emit_c_fmod(self.chunk(), line);
...
pub fn emit_rich_arithmetic(
    ...
    right_slot: u16,
    dunder: &str,
    // `impl Fn` rather than `fn`: the fallback for `+` now carries the
    // language's `Directives::bool_text`, which a bare function pointer cannot.
    fallback_fn: impl Fn(&mut Chunk, u32),
    line: u32,
) {
    ...
    let key = class_slots::resolve_interned(
        chunk,
        &class_slots::ClassSlot::internal(dunder),
        &class_slots::PlainNames,
    );
    chunk.emit_op_u16(Op::LOCAL_GET, left_slot, line);
    class_slots::emit_class_get(
...
pub fn emit_rich_to_string(chunk: &mut Chunk, obj_slot: u16, line: u32) {
    ...
    class_slots::emit_class_get(
        ...
        class_slots::ObjSource::Stack,
        &key,
        class_slots::Dest::Local(method_slot),
        line,
    );
...
pub fn emit_rich_bool(chunk: &mut Chunk, obj_slot: u16, line: u32) {
    ...
    let slot = class_slots::resolve(
        &class_slots::ClassSlot::internal("__bool__"),
        &class_slots::PlainNames,
    );
    class_slots::emit_class_get(
        chunk,
...
pub fn emit_rich_compare_locals(
    ...
    for method_name in ["compare", "CompareTo", "compareTo", "__cmp__", "<=>"] {
        let method_key = class_slots::resolve_interned(
            chunk,
            &class_slots::ClassSlot::internal(method_name),
            &class_slots::PlainNames,
        );
...
pub fn emit_smart_length(chunk: &mut Chunk, obj_slot: u16, line: u32) {
    ...
    // declared the class.
    chunk.emit_op_u16(Op::LOCAL_GET, obj_slot, line);
    let slot_key = class_slots::resolve(
        &class_slots::ClassSlot::internal(
    ...
    chunk.emit_op_u16(Op::LOCAL_GET, obj_slot, line);
    let slot = class_slots::resolve(
        &class_slots::ClassSlot::internal("__get_length"),
        &class_slots::PlainNames,
    );
    class_slots::emit_class_get(
        chunk,
