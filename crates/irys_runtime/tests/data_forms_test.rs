...
#[test]
fn test_data_forms_functionality() {
    let source = std::fs::read_to_string("../../tests/test_data_forms.vb")
    ...
    assert!(has_success, "Data forms test failed - did not see SUCCESS");
    assert!(!has_failure, "Data forms test failed - saw FAILURE");
}
