...
fn parse(src: &str) -> vybe_parser_php::Program {
...
echo greet('World');
"#);
    let component = vybe_compiler_php::compile_component(&program, "greeting").unwrap();
...
"#);
    let component = vybe_compiler_php::compile_component(&program, "math").unwrap();
...
"#);
    let component = vybe_compiler_php::compile_component(&program, "math_utils").unwrap();
...
"#);
    let component = vybe_compiler_php::compile_component(&program, "items").unwrap();
...
"#);
    let chunks = vybe_compiler_php::Compiler::new().compile(&program).unwrap();
