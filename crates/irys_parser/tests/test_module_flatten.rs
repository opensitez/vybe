...
End Module
"#;
    let prog = irys_parser::parse_program(code).expect("parse with imports failed");
    println!("[with imports] Declarations: {}", prog.declarations.len());
    for d in &prog.declarations {
        match d {
            irys_parser::ast::Declaration::Sub(s) => println!("  Sub: {}", s.name.as_str()),
            irys_parser::ast::Declaration::Function(f) => println!("  Function: {}", f.name.as_str()),
            _ => println!("  (other kind)"),
        }
    }
    println!("[with imports] Statements: {}", prog.statements.len());
    assert!(prog.declarations.iter().any(|d| matches!(d, irys_parser::ast::Declaration::Sub(s) if s.name.as_str().eq_ignore_ascii_case("Main"))),
...
End Module
"#;
    let prog = irys_parser::parse_program(code).expect("parse without imports failed");
