...
fn standard_wasm_function_with_type(type_entries: &[u8], func_type_idx: u32, instructions: &[u8]) -> Vec<u8> {
    let mut wasm = Vec::new();
    wasm.extend_from_slice(&[0x00, 0x61, 0x73, 0x6d]);
    wasm.extend_from_slice(&[0x01, 0x00, 0x00, 0x00]);
...
fn reader_preserves_if_else_and_i32_conditions() {
    ...
    assert!(chunk.code.windows(2).any(|w| w == [Op::IF.prefix(), Op::IF.sub()]));
    assert!(chunk.code.windows(2).any(|w| w == [Op::ELSE.prefix(), Op::ELSE.sub()]));
    assert!(chunk.code.windows(2).any(|w| w == [Op::END.prefix(), Op::END.sub()]));
...
fn reader_preserves_br_if_depth() {
    ...
    assert!(chunk.code.windows(2).any(|w| w == [Op::BR_IF.prefix(), Op::BR_IF.sub()]));
...
fn reader_preserves_br_table_vector_and_default_depths() {
    ...
    let br_table = chunk.code.windows(2)
        .position(|w| w == [Op::BR_TABLE.prefix(), Op::BR_TABLE.sub()])
        .expect("br_table should be decoded");
    assert_eq!(&chunk.code[br_table + 2..br_table + 6], &[0x02, 0x00, 0x01, 0x01]);
    assert_eq!(run_chunk(chunk).as_i32(), 9);
}
...
fn vm_rejects_non_i32_if_condition() {
    ...
    let err = VM::new().run(vec![chunk]).expect_err("if must require an i32 condition");
...
fn reader_decodes_prefixed_proposal_opcodes() {
    ...
    assert!(chunk.code.windows(2).any(|w| w == [Op::I31_NEW.prefix(), Op::I31_NEW.sub()]));
    assert!(chunk.code.windows(2).any(|w| w == [Op::V128_CONST.prefix(), Op::V128_CONST.sub()]));
    assert!(chunk.code.windows(2).any(|w| w == [Op::ATOMIC_FENCE.prefix(), Op::ATOMIC_FENCE.sub()]));
}
