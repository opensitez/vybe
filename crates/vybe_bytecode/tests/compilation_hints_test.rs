...
use vybe_bytecode::wasm::compilation_hints::{
...
fn compilation_order_absent_for_non_entry_only_module() {
    ...
    let (count, _) = leb128(payload);
    assert_eq!(count, 1, "only <script> should be marked hot, not the helper");
}
...
fn inlining_section_present_for_leaf_function() {
    ...
    assert!(find_custom_section(&wasm, INLINING_SECTION_NAME).is_some(),
...
fn inlining_section_absent_for_non_leaf() {
    ...
    if let Some(p) = find_custom_section(&wasm, INLINING_SECTION_NAME) {
        ...
        for _ in 0..count {
            let (fn_idx, n) = leb128(&p[pos..]); pos += n;
            let (_off, n)   = leb128(&p[pos..]); pos += n;
            let (_len, n)   = leb128(&p[pos..]); pos += n;
            pos += 1; // priority byte
...
fn inlining_section_lists_multiple_leaf_functions() {
    ...
    let (count, _) = leb128(payload);
    assert_eq!(count, 3, "all three leaf functions must appear in inlining section");
}
...
fn inlining_section_does_not_include_script_entry() {
    ...
    assert!(find_custom_section(&wasm, INLINING_SECTION_NAME).is_none(),
