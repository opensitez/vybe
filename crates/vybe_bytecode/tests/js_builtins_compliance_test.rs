...
use vybe_bytecode::wasm::{
    ...
    js_set_builtins, js_string_builtins, js_typedarray_builtins, js_weakmap_builtins, write_wasm,
};
...
fn every_array_import_has_signature() {
    for name in js_array_builtins::IMPORTS {
        ...
        assert!(!buf.is_empty(), "vybe:js-array.{} signature is empty", name);
    }
}
...
fn every_arraybuffer_import_has_signature() {
    ...
    for name in js_arraybuffer_builtins::SHAREDARRAYBUFFER_IMPORTS {
        ...
        assert!(
            ok,
            "vybe:js-sharedarraybuffer.{} declared but no signature",
            name
        );
    }
...
fn rejects_unknown_import_names() {
    ...
    let mut buf = Vec::new();
    assert!(!js_array_builtins::write_signature(
        &mut buf,
        "nonsense_undeclared"
    ));
    assert!(!js_object_builtins::write_signature(
        &mut buf,
        "nonsense_undeclared"
    ));
    assert!(!js_map_builtins::write_signature(
        &mut buf,
        "nonsense_undeclared"
    ));
    assert!(!js_set_builtins::write_signature(
        &mut buf,
        "nonsense_undeclared"
    ));
    assert!(!js_arraybuffer_builtins::write_arraybuffer_signature(
        &mut buf, "xxx"
    ));
    assert!(!js_arraybuffer_builtins::write_dataview_signature(
        &mut buf, "xxx"
    ));
    assert!(!js_weakmap_builtins::write_weakmap_signature(
        &mut buf, "xxx"
    ));
    assert!(!js_weakmap_builtins::write_weakset_signature(
        &mut buf, "xxx"
    ));
...
fn module_names_follow_ecma_convention() {
    ...
    assert_eq!(
        js_arraybuffer_builtins::ARRAYBUFFER_MODULE,
        "ecma:arraybuffer"
    );
    assert_eq!(
        js_arraybuffer_builtins::SHAREDARRAYBUFFER_MODULE,
    ...
    for variant in js_typedarray_builtins::VARIANTS {
        ...
        assert!(
            ...
            "typed array module must start with ecma: or wasm:js- prefix, got: {}",
            m
        );
    }
}
...
fn no_duplicate_imports_within_any_module() {
    ...
    assert_unique(
        js_weakmap_builtins::WEAKMAP_MODULE,
        js_weakmap_builtins::WEAKMAP_IMPORTS,
    );
    assert_unique(
        js_weakmap_builtins::WEAKSET_MODULE,
    ...
    assert_unique(
        js_arraybuffer_builtins::DATAVIEW_MODULE,
...
fn array_push_signature_is_externref_externref_to_i32() {
    ...
    assert_eq!(
        params,
        vec![0x6F, 0x6F],
        "params should be (externref, externref)"
    );
...
fn map_has_returns_i32_boolean() {
    ...
    assert_eq!(
        results,
        vec![0x7F],
        "has must return i32 0/1 per convention"
    );
}
...
fn dataview_get_biguint64_returns_i64() {
    ...
    assert_eq!(
        results,
        vec![0x7E],
        "getBigUint64 result must be i64 (0x7E)"
    );
}
...
fn float32array_get_returns_f32() {
    ...
    assert!(js_typedarray_builtins::write_signature(
        &mut buf,
        js_typedarray_builtins::TypedElem::F32,
        "get"
    ));
    let (params, results) = decode_signature(&buf).unwrap();
...
fn float64array_set_takes_f64_value() {
    ...
    assert!(js_typedarray_builtins::write_signature(
        &mut buf,
        js_typedarray_builtins::TypedElem::F64,
        "set"
    ));
    let (params, results) = decode_signature(&buf).unwrap();
    assert_eq!(
...
fn bigint64array_get_returns_i64() {
    ...
    assert!(js_typedarray_builtins::write_signature(
        &mut buf,
        js_typedarray_builtins::TypedElem::BigI64,
        "get"
    ));
    let (_, results) = decode_signature(&buf).unwrap();
    assert_eq!(
...
fn typed_array_variants_have_expected_bytes_per_element() {
    ...
    for (variant, bpe) in expected {
        assert_eq!(
            variant.bytes_per_element(),
            *bpe,
            "{} must have BYTES_PER_ELEMENT = {}",
            variant.module(),
            bpe
        );
    }
...
fn typed_array_variants_list_covers_all_11() {
    assert_eq!(
        js_typedarray_builtins::VARIANTS.len(),
        11,
...
fn array_surface_covers_mdn_prototype_methods() {
    ...
    for name in required {
        assert!(
            js_array_builtins::IMPORTS.contains(name),
            "vybe:js-array missing required `{}` per ECMA-262 §23.1.3",
            name
        );
    }
}
...
fn map_surface_covers_mdn_prototype_methods() {
    let required: &[&str] = &[
        "get", "set", "has", "delete", "clear", "size", "keys", "values", "entries", "forEach",
    ];
    for name in required {
        assert!(
            js_map_builtins::IMPORTS.contains(name),
            "vybe:js-map missing required `{}` per ECMA-262 §24.1.3",
            name
        );
    }
}
...
fn set_surface_covers_mdn_prototype_methods_including_es2025() {
    ...
    for name in required {
        assert!(
            js_set_builtins::IMPORTS.contains(name),
            "vybe:js-set missing required `{}` per ECMA-262 §24.2 + ES2025 set algebra",
            name
        );
    }
}
...
fn dataview_has_all_get_and_set_variants() {
    ...
    for ty in element_types {
        ...
        assert!(
            js_arraybuffer_builtins::DATAVIEW_IMPORTS
                .iter()
                .any(|s| *s == getter.as_str()),
            "vybe:js-dataview missing {}",
            getter
        );
        assert!(
            js_arraybuffer_builtins::DATAVIEW_IMPORTS
                .iter()
                .any(|s| *s == setter.as_str()),
            "vybe:js-dataview missing {}",
            setter
        );
    }
...
fn object_surface_covers_mdn_static_and_prototype_methods() {
    ...
    for name in required {
        assert!(
            js_object_builtins::IMPORTS.contains(name),
            "vybe:js-object missing required `{}` per ECMA-262 §20.1",
            name
        );
    }
}
...
fn total_imported_surface_is_reasonable() {
    ...
    assert!(
        array_n >= 40,
        "Array surface should be ~40+ methods, got {}",
        array_n
    );
    assert!(
        object_n >= 25,
        "Object surface should be 25+ methods, got {}",
        object_n
    );
    assert!(
        map_n >= 10,
        "Map surface should be 10+ methods, got {}",
        map_n
    );
    assert!(
        set_n >= 15,
        "Set surface (with ES2025) should be 15+, got {}",
        set_n
    );
    assert!(
        weakmap_n >= 5,
        "WeakMap surface should be 5+, got {}",
        weakmap_n
    );
    assert!(
        weakset_n >= 4,
        "WeakSet surface should be 4+, got {}",
        weakset_n
    );
    assert!(
        ab_n >= 10,
        "ArrayBuffer surface should be 10+, got {}",
        ab_n
    );
    assert!(
        sab_n >= 6,
        "SharedArrayBuffer surface should be 6+, got {}",
        sab_n
    );
    assert!(
        dv_n >= 22,
        "DataView surface should be 22+ (11 get + 11 set + helpers), got {}",
        dv_n
    );
    assert!(
        ta_methods >= 30,
        "TypedArray method set should be 30+, got {}",
        ta_methods
    );
