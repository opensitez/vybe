...
fn i32_atomic_store_overwrites() {
    let r = run_with_memory(|c| {
        push_i32(c, 0); push_i32(c, 1); c.emit_op(Op::I32_ATOMIC_STORE, 0);
        push_i32(c, 0); push_i32(c, 2); c.emit_op(Op::I32_ATOMIC_STORE, 0);
        push_i32(c, 0); c.emit_op(Op::I32_ATOMIC_LOAD, 0);
    });
...
fn i32_atomic_rmw_add_returns_old() {
    let r = run_with_memory(|c| {
        push_i32(c, 0); push_i32(c, 10); c.emit_op(Op::I32_ATOMIC_STORE, 0);
        push_i32(c, 0); push_i32(c, 5);  c.emit_op(Op::I32_ATOMIC_RMW_ADD, 0);
...
fn i32_atomic_rmw_add_updates_memory() {
    let r = run_with_memory(|c| {
        ...
        push_i32(c, 0); push_i32(c, 5);  c.emit_op(Op::I32_ATOMIC_RMW_ADD, 0);
        c.emit_op(Op::DROP, 0);
        push_i32(c, 0); c.emit_op(Op::I32_ATOMIC_LOAD, 0); // should be 15
    });
...
fn i32_atomic_rmw_sub() {
    let r = run_with_memory(|c| {
        push_i32(c, 0); push_i32(c, 20); c.emit_op(Op::I32_ATOMIC_STORE, 0);
        push_i32(c, 0); push_i32(c, 8);  c.emit_op(Op::I32_ATOMIC_RMW_SUB, 0);
        c.emit_op(Op::DROP, 0);
        push_i32(c, 0); c.emit_op(Op::I32_ATOMIC_LOAD, 0);
    });
...
fn i32_atomic_rmw_and() {
    let r = run_with_memory(|c| {
        push_i32(c, 0); push_i32(c, 0b1100); c.emit_op(Op::I32_ATOMIC_STORE, 0);
        push_i32(c, 0); push_i32(c, 0b1010); c.emit_op(Op::I32_ATOMIC_RMW_AND, 0);
        c.emit_op(Op::DROP, 0);
        push_i32(c, 0); c.emit_op(Op::I32_ATOMIC_LOAD, 0);
    });
...
fn i32_atomic_rmw_or() {
    let r = run_with_memory(|c| {
        push_i32(c, 0); push_i32(c, 0b1100); c.emit_op(Op::I32_ATOMIC_STORE, 0);
        push_i32(c, 0); push_i32(c, 0b0011); c.emit_op(Op::I32_ATOMIC_RMW_OR, 0);
        c.emit_op(Op::DROP, 0);
        push_i32(c, 0); c.emit_op(Op::I32_ATOMIC_LOAD, 0);
    });
...
fn i32_atomic_rmw_xor() {
    let r = run_with_memory(|c| {
        push_i32(c, 0); push_i32(c, 0b1111); c.emit_op(Op::I32_ATOMIC_STORE, 0);
        push_i32(c, 0); push_i32(c, 0b1010); c.emit_op(Op::I32_ATOMIC_RMW_XOR, 0);
        c.emit_op(Op::DROP, 0);
        push_i32(c, 0); c.emit_op(Op::I32_ATOMIC_LOAD, 0);
    });
...
fn i32_atomic_rmw_xchg_returns_old() {
    let r = run_with_memory(|c| {
        push_i32(c, 0); push_i32(c, 7);  c.emit_op(Op::I32_ATOMIC_STORE, 0);
        push_i32(c, 0); push_i32(c, 99); c.emit_op(Op::I32_ATOMIC_RMW_XCHG, 0);
    });
...
fn i32_atomic_cmpxchg_succeeds_when_expected_matches() {
    let r = run_with_memory(|c| {
        push_i32(c, 0); push_i32(c, 42);  c.emit_op(Op::I32_ATOMIC_STORE, 0);
        push_i32(c, 0); push_i32(c, 42); push_i32(c, 100); c.emit_op(Op::I32_ATOMIC_RMW_CMPXCHG, 0);
        c.emit_op(Op::DROP, 0);
        push_i32(c, 0); c.emit_op(Op::I32_ATOMIC_LOAD, 0);
    });
...
fn i32_atomic_cmpxchg_fails_when_expected_mismatches() {
    let r = run_with_memory(|c| {
        push_i32(c, 0); push_i32(c, 42);  c.emit_op(Op::I32_ATOMIC_STORE, 0);
        push_i32(c, 0); push_i32(c, 99); push_i32(c, 100); c.emit_op(Op::I32_ATOMIC_RMW_CMPXCHG, 0);
        c.emit_op(Op::DROP, 0);
        push_i32(c, 0); c.emit_op(Op::I32_ATOMIC_LOAD, 0);
    });
...
fn i64_atomic_rmw_add() {
    let r = run_with_memory(|c| {
        push_i32(c, 0); push_i64(c, 100); c.emit_op(Op::I64_ATOMIC_STORE, 0);
        push_i32(c, 0); push_i64(c, 42);  c.emit_op(Op::I64_ATOMIC_RMW_ADD, 0);
        c.emit_op(Op::DROP, 0);
        push_i32(c, 0); c.emit_op(Op::I64_ATOMIC_LOAD, 0);
    });
...
fn i64_atomic_rmw_sub() {
    let r = run_with_memory(|c| {
        push_i32(c, 0); push_i64(c, 200); c.emit_op(Op::I64_ATOMIC_STORE, 0);
        push_i32(c, 0); push_i64(c, 58);  c.emit_op(Op::I64_ATOMIC_RMW_SUB, 0);
        c.emit_op(Op::DROP, 0);
        push_i32(c, 0); c.emit_op(Op::I64_ATOMIC_LOAD, 0);
    });
    assert_eq!(r.as_i64(), 142);
...
fn i64_atomic_cmpxchg_succeeds() {
    let r = run_with_memory(|c| {
        push_i32(c, 0); push_i64(c, 7);  c.emit_op(Op::I64_ATOMIC_STORE, 0);
        push_i32(c, 0); push_i64(c, 7); push_i64(c, 99); c.emit_op(Op::I64_ATOMIC_RMW_CMPXCHG, 0);
        c.emit_op(Op::DROP, 0);
        push_i32(c, 0); c.emit_op(Op::I64_ATOMIC_LOAD, 0);
    });
...
fn memory_atomic_wait32_returns_not_equal() {
    ...
    let r = run_with_memory(|c| {
        push_i32(c, 0); push_i32(c, 0); c.emit_op(Op::I32_ATOMIC_STORE, 0);
        push_i32(c, 0);   // address
