...
fn i32x4_relaxed_trunc_f32x4_s() {
    ...
    let r = i32_lanes(&as_v128(run(|c| {
        emit_v128(c, input);
        c.emit_op(Op::I32X4_RELAXED_TRUNC_F32X4_S, 0);
    })));
...
fn i32x4_relaxed_trunc_f32x4_u() {
    ...
    let r = i32_lanes(&as_v128(run(|c| {
        emit_v128(c, input);
        c.emit_op(Op::I32X4_RELAXED_TRUNC_F32X4_U, 0);
    })));
    assert_eq!(r[0] as u32, 3);
    assert_eq!(r[1] as u32, 0);
    assert_eq!(r[2] as u32, 255);
    assert_eq!(r[3] as u32, 1000);
}
...
fn i32x4_relaxed_trunc_f64x2_s_zero() {
    ...
    let r = i32_lanes(&as_v128(run(|c| {
        emit_v128(c, input);
        c.emit_op(Op::I32X4_RELAXED_TRUNC_F64X2_S_ZERO, 0);
    })));
    assert_eq!(r[0], 3);
    assert_eq!(r[1], -2);
    assert_eq!(r[2], 0);
...
fn i32x4_relaxed_trunc_f64x2_u_zero() {
    ...
    let r = i32_lanes(&as_v128(run(|c| {
        emit_v128(c, input);
        c.emit_op(Op::I32X4_RELAXED_TRUNC_F64X2_U_ZERO, 0);
    })));
    assert_eq!(r[0] as u32, 5);
    assert_eq!(r[1] as u32, 100);
    assert_eq!(r[2], 0);
    assert_eq!(r[3], 0);
}
...
fn i16x8_relaxed_q15mulr_s_basic() {
    ...
    let mut b = [0u8; 16];
    for i in 0..8 {
        a[i * 2..i * 2 + 2].copy_from_slice(&v.to_le_bytes());
        b[i * 2..i * 2 + 2].copy_from_slice(&v.to_le_bytes());
    }
    let r = as_v128(run(|c| {
        emit_v128(c, a);
        emit_v128(c, b);
        c.emit_op(Op::I16X8_RELAXED_Q15MULR_S, 0);
    }));
