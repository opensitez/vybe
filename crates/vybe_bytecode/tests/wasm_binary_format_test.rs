...
fn memory64_memarg_encoder_accepts_u64_offsets() {
    let mut bytes = Vec::new();
    wasm::encoding::encode_memarg_with_memidx(&mut bytes, 3, 0x1_0000_0000, 0);
    assert_eq!(
        bytes,
...
fn table64_section_uses_i64_limits_flag() {
    let section = wasm::sections::encode_table64_section_with(1, None, 0x70);
...
fn reader_preserves_multi_memory_load_memarg() {
    ...
    let pattern = [Op::I32_LOAD.prefix(), Op::I32_LOAD.sub(), 0x42, 0x05, 0x01];
    assert!(
