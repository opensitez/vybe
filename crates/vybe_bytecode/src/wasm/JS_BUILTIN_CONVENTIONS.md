# JS Builtin Conventions (Vybe)

This file pins the contract every JS-shape host import in Vybe must
follow. It covers:
...
5. **Naming: which namespace a primitive belongs under**

## Namespace policy

- **`wasm:js-string`** — real. Merged `js-string-builtins` proposal. V8
  implements natively. Use this for string built-ins defined in that
  proposal.
- **`wasm:js-{number,boolean,undefined,symbol,bigint}`** — stage-1
...
- **`ecma:*`** — every other JS-shape primitive Vybe provides:
  ...
  providing these — they do NOT link automatically.
...
glue layer is literally a one-to-one method wrapping in an
`importObject`:
