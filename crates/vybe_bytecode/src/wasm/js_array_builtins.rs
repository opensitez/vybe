...
pub const MODULE: &str = "wasm:js-array";

/// Every `wasm:js-array` import. Matches ECMA-262 §23.1 surface.
...
pub fn write_signature(out: &mut Vec<u8>, name: &str) -> bool {
    match name {
        ...
        "of" | "from" | "fromAsync" => {
            ...
            out.push(TYPE_I32); out.push(TYPE_EXTERNREF);
            write_leb128_u32(out, 1); out.push(TYPE_EXTERNREF);
        }
        "isArray" => {
            write_leb128_u32(out, 1); out.push(TYPE_EXTERNREF);
        ...
        "set" => {
            ...
            out.push(TYPE_EXTERNREF); out.push(TYPE_I32); out.push(TYPE_EXTERNREF);
            write_leb128_u32(out, 0);
        }
        "length" => {
            write_leb128_u32(out, 1); out.push(TYPE_EXTERNREF);
