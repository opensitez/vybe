...
impl Debugger {
    ...
    fn decide_stop(&mut self, vm: &mut VM, chunk_index: usize, ip: usize, op: Op) -> Option<PauseReason> {
        ...
        for id in matched {
            let (ignore, hit, condition, log, one_shot) = {
                ...
                b.hit_count += 1;
                (b.ignore_count, b.hit_count, b.condition.clone(), b.log_message.clone(), b.one_shot)
            };
            if hit <= ignore {
        ...
        for wid in wp_ids {
            let target = self.watchpoints.iter().find(|w| w.id == wid).unwrap().target.clone();
    ...
    fn handle_command(
        ...
        match command {
            ...
            BreakOffset { chunk, offset, condition } => {
            ...
            SetIgnoreCount { id, count } => {
                if let Some(bp) = self.breakpoints.iter_mut().find(|b| b.id == id) {
                    ...
                    Control::stay(DebugResponse::Value(format!("breakpoint #{id} ignores next {count}")))
                } else {
                    Control::stay(DebugResponse::Error(format!("no breakpoint #{id}")))
                }
            }
            ExceptionBreak { on_throw, on_uncaught } => {
            ...
            AddWatchpoint { target } => {
                ...
                self.next_wp_id += 1;
                self.watchpoints.push(Watchpoint { id, target: target.clone(), last: last.clone() });
                Control::stay(DebugResponse::Value(format!("watchpoint #{id} on {target} (= {last})")))
            }
            ListWatchpoints => {
            ...
            AddWatch { expr } => {
                ...
                Control::stay(DebugResponse::Value(format!("watching `{expr}` (#{})", self.watches.len())))
            }
            ListWatches => {
...
fn resolve_source_line(vm: &VM, target: u32) -> Option<(u32, Vec<(usize, usize)>)> {
    ...
    let nearest_in = |lines: &BTreeMap<u32, usize>| -> Option<(u32, usize, u32)> {
        ...
        let pick = match (fwd, bwd) {
            (Some((&f, &fo)), Some((&b, &bo))) => {
                if f - target <= target - b { (f, fo) } else { (b, bo) }
            }
            (Some((&f, &fo)), None) => (f, fo),
            (None, Some((&b, &bo))) => (b, bo),
...
fn fiber_list(vm: &VM) -> Vec<String> {
    let name_of = |ci: usize| vm.chunks.get(ci).map(|c| c.name.as_str()).unwrap_or("?").to_string();
    let mut out = Vec::new();
    let cur_top = vm
        .frames
        .last()
    ...
    for (n, ac) in vm.debug_suspended_fibers().into_iter().enumerate() {
        let top = ac.0.last().map(|&ci| name_of(ci)).unwrap_or_else(|| "?".into());
        out.push(format!(
    ...
    while i < bytes.len() {
        match bytes[i] {
            ...
            b'[' => {
                ...
                let idx: usize = path[s..i].trim().parse().map_err(|_| "index must be an integer")?;
...
fn navigate(val: &crate::Value, acc: &Access) -> Result<crate::Value, String> {
    ...
    match acc {
        ...
        Access::Index(i) => match &obj.kind {
            crate::value::ObjectKind::Array(elems) => {
                elems.get(*i).cloned().ok_or_else(|| format!("index {i} out of range"))
            }
            _ => obj
                .fields
                .get(*i)
...
fn eval_path(vm: &VM, path: &str) -> Result<String, String> {
    ...
    let mut val = read_named_value(vm, &base).ok_or_else(|| format!("no variable `{base}` in scope"))?;
...
fn set_var(vm: &mut VM, name: &str, literal: &str) -> Result<String, String> {
    ...
    if let Some(frame) = vm.frames.last() {
        let slot = vm
            .chunks
            .get(frame.chunk_index)
            .and_then(|c| c.local_names.iter().rev().find(|(_, n)| n == name).map(|(s, _)| *s));
