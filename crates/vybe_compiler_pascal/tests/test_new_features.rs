...
#[test] fn forin_array() {
    assert_eq!(run(r#"program T; var item: Integer; begin
      for item in [10, 20, 30] do WriteLn(item); end."#), &["10", "20", "30"]);
}

#[test] fn forin_string() {
    assert_eq!(run(r#"program T; var ch: String; begin
...
#[test] fn compound_plus() {
    assert_eq!(run("program T; var x: Integer; begin x := 10; x += 5; WriteLn(x); end."), &["15"]);
}
#[test] fn compound_minus() {
    assert_eq!(run("program T; var x: Integer; begin x := 10; x -= 3; WriteLn(x); end."), &["7"]);
}
#[test] fn compound_mul() {
    assert_eq!(run("program T; var x: Integer; begin x := 5; x *= 3; WriteLn(x); end."), &["15"]);
}
#[test] fn compound_div() {
    assert_eq!(run("program T; var x: Real; begin x := 10.0; x /= 4.0; WriteLn(x); end."), &["2.5"]);
}
#[test] fn compound_string_concat() {
    assert_eq!(run("program T; var s: String; begin s := 'hello'; s += ' world'; WriteLn(s); end."), &["hello world"]);
}
...
#[test] fn str_stringreplace() {
    assert_eq!(run("program T; begin WriteLn(StringReplace('hello world', 'world', 'pascal')); end."), &["hello pascal"]);
}

#[test] fn str_stringofchar() {
    assert_eq!(run("program T; begin WriteLn(StringOfChar('*', 5)); end."), &["*****"]);
}

#[test] fn str_leftstr() {
    assert_eq!(run("program T; begin WriteLn(LeftStr('hello', 3)); end."), &["hel"]);
}
...
#[test] fn typed_const() {
    assert_eq!(run("program T; const MaxSize: Integer = 100; begin WriteLn(MaxSize); end."), &["100"]);
}

#[test] fn typed_const_string() {
    assert_eq!(run("program T; const Greeting: String = 'Hello'; begin WriteLn(Greeting); end."), &["Hello"]);
}
