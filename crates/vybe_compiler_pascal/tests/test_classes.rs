...
#[test] fn class_create_and_field() {
...
var f: TFoo;
begin f := TFoo.Create(42); WriteLn(f.FVal); end."#), &["42"]);
}

#[test] fn class_method_returns_value() {
    assert_eq!(run(r#"program T;
type TAnimal = class
...
var a: TAnimal;
begin a := TAnimal.Create('Rex'); WriteLn(a.Speak()); end."#), &["Rex speaks"]);
}

#[test] fn class_zero_param_constructor() {
    assert_eq!(run(r#"program T;
type TFoo = class public FX: Integer; constructor Create; end;
constructor TFoo.Create; begin FX := 99; end;
var f: TFoo; begin f := TFoo.Create; WriteLn(f.FX); end."#), &["99"]);
}

#[test] fn class_multiple_fields() {
    assert_eq!(run(r#"program T;
...
var p: TPoint;
begin p := TPoint.Create(10, 20); WriteLn(p.FX + p.FY); end."#), &["30"]);
}

#[test] fn class_method_modifies_state() {
    assert_eq!(run(r#"program T;
...
var c: TCounter;
begin c := TCounter.Create; c.Increment(); c.Increment(); c.Increment(); WriteLn(c.FCount); end."#), &["3"]);
}

#[test] fn class_multiple_methods() {
    assert_eq!(run(r#"program T;
...
var c: TCalc;
begin c := TCalc.Create(10); c.Add(5); c.Add(3); WriteLn(c.GetVal()); end."#), &["18"]);
}

#[test] fn class_method_with_params() {
    assert_eq!(run(r#"program T;
...
var m: TMath;
begin m := TMath.Create; WriteLn(m.Add(3, 4)); end."#), &["7"]);
}

#[test] fn class_string_field() {
    assert_eq!(run(r#"program T;
...
var p: TPerson;
begin p := TPerson.Create('Alice', 30); WriteLn(p.Desc()); end."#), &["Alice is 30"]);
}

// ── Multiple instances ───────────────────────────────────────────────────

#[test] fn class_two_instances_independent() {
    assert_eq!(run(r#"program T;
...
var a, b: TBox;
begin a := TBox.Create(10); b := TBox.Create(20); WriteLn(a.GetVal()); WriteLn(b.GetVal()); end."#), &["10", "20"]);
}

#[test] fn class_instances_mutate_independently() {
    assert_eq!(run(r#"program T;
type TCounter = class public FCount: Integer;
...
begin a := TCounter.Create(0); b := TCounter.Create(0);
  ...
  WriteLn(a.GetCount()); WriteLn(b.GetCount()); end."#), &["3", "1"]);
}

#[test] fn class_three_instances() {
    assert_eq!(run(r#"program T;
...
begin a := TVal.Create(1); b := TVal.Create(2); c := TVal.Create(3);
  WriteLn(a.FV + b.FV + c.FV); end."#), &["6"]);
}

// ── Inheritance ──────────────────────────────────────────────────────────

#[test] fn class_child_inherits_method() {
    assert_eq!(run(r#"program T;
...
var d: TDog;
begin d := TDog.Create('Rex'); WriteLn(d.Speak()); end."#), &["Rex speaks"]);
}

#[test] fn class_child_inherits_field() {
    assert_eq!(run(r#"program T;
...
var c: TChild;
begin c := TChild.Create(42); WriteLn(c.FVal); end."#), &["42"]);
}

#[test] fn class_child_overrides_method() {
    assert_eq!(run(r#"program T;
...
var c: TChild;
begin c := TChild.Create; WriteLn(c.Greet()); end."#), &["child"]);
}

#[test] fn class_child_adds_own_field() {
    assert_eq!(run(r#"program T;
...
var c: TChild;
begin c := TChild.Create(10, 20); WriteLn(c.Sum()); end."#), &["30"]);
}

#[test] fn class_child_adds_own_method() {
    assert_eq!(run(r#"program T;
...
var c: TChild;
begin c := TChild.Create('hello'); WriteLn(c.GetName()); WriteLn(c.Upper()); end."#), &["hello", "HELLO"]);
}
