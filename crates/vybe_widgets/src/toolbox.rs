...
impl Toolbox {
    ...
    pub fn paint(&self, pixmap: &mut Pixmap, x: f32, y: f32, scale: f32) {
        ...
        p.set_color_rgba8(40, 40, 40, 220);
        let w = 160.0 * scale;
        let h = (self.items.len() as f32 * 28.0 + 16.0) * scale;
        ...
        for _item in &self.items {
            ...
            b.set_color_rgba8(220, 220, 220, 255);
            // Draw label (simple rectangle placeholder for now)
            let mut pb2 = PathBuilder::new();
            pb2.push_rect(
                tiny_skia::Rect::from_xywh(x + 8.0 * scale, iy, w - 16.0 * scale, 22.0 * scale)
                    .unwrap(),
            );
            if let Some(pth) = pb2.finish() {
                pixmap.stroke_path(&pth, &b, &Stroke::default(), Transform::identity(), None);
            }
            iy += 28.0 * scale;
        }
    }
    ...
    fn handle_mouse(&mut self, event: &MouseEvent) -> bool {
        ...
        if let MouseEventKind::Press(LayoutMouseButton::Left) = event.kind {
            if let Some(idx) = self.hit_test(event.x, event.y, r.x, r.y, 1.0) {
                self.pending_events.push(WidgetEvent::Action(format!(
                    "toolbox:{}:{}",
                    self.name, idx
                )));
                return true;
