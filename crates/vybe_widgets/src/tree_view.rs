...
fn scan_dir(path: &str) -> Vec<FileEntry> {
    ...
    if let Ok(rd) = fs::read_dir(path) {
        for e in rd.flatten() {
            ...
            entries.push(FileEntry { name, path: p, is_dir, is_expanded: false, children: Vec::new(), buffer: None });
        }
    }
...
impl TreeView {
    ...
    pub fn render(&mut self, pix: &mut Pixmap, fs: &mut FontSystem, sc: &mut SwashCache, x: f32, y: f32, width: f32, text_color: CosmicColor, selection_color: (u8, u8, u8, u8)) {
        ...
        for entry in &mut self.entries {
            Self::render_entry(pix, fs, sc, entry, x, &mut current_y, 0, scale, item_height, indent, &self.selected_path, width, text_color, selection_color);
        }
    }

    fn render_entry(pix: &mut Pixmap, fs: &mut FontSystem, sc: &mut SwashCache, entry: &mut FileEntry, x: f32, y: &mut f32, depth: u32, scale: f32, item_height: f32, indent: f32, selected_path: &Option<String>, width: f32, text_color: CosmicColor, selection_color: (u8, u8, u8, u8)) {
        ...
        if let Some(sel) = selected_path {
            if sel == &entry.path {
                let mut p = Paint::default(); p.set_color_rgba8(selection_color.0, selection_color.1, selection_color.2, selection_color.3);
                pix.fill_rect(tiny_skia::Rect::from_xywh(x, active_y, width, item_height).unwrap(), &p, Transform::identity(), None);
        ...
        if entry.is_dir {
            ...
            if let Some(path) = pb.finish() {
                let mut p = Paint::default(); p.set_color_rgba8(text_color.r(), text_color.g(), text_color.b(), text_color.a());
                pix.fill_path(&path, &p, tiny_skia::FillRule::Winding, Transform::identity(), None);
            }
        }
        ...
        let mut ic_p = Paint::default(); ic_p.set_color_rgba8(r, g, b, 255);
        let ic_sz = 6.0 * scale;
        pix.fill_rect(tiny_skia::Rect::from_xywh(x_off + 18.0 * scale, active_y + (item_height - ic_sz) / 2.0, ic_sz, ic_sz).unwrap(), &ic_p, Transform::identity(), None);
        ...
        *y += item_height;
        ...
        if entry.is_expanded && entry.is_dir {
            if entry.children.is_empty() { entry.children = scan_dir(&entry.path); }
            for child in &mut entry.children {
                Self::render_entry(pix, fs, sc, child, x, y, depth + 1, scale, item_height, indent, selected_path, width, text_color, selection_color);
            }
        }
    }
    ...
    pub fn handle_mouse(&mut self, mx: f32, my: f32, x: f32, y: f32) -> TreeEvent {
        ...
        for entry in &mut self.entries {
            if let Some(ev) = Self::check_mouse_entry(entry, mx, my, x, &mut current_y, 0, item_height, indent) {
                result = ev;
                break;
            }
        }
    ...
    fn check_mouse_entry(entry: &mut FileEntry, mx: f32, my: f32, x: f32, y: &mut f32, depth: u32, item_height: f32, indent: f32) -> Option<TreeEvent> {
        ...
        *y += item_height;
        if entry.is_expanded && entry.is_dir {
            for child in &mut entry.children {
                if let Some(ev) = Self::check_mouse_entry(child, mx, my, x, y, depth + 1, item_height, indent) {
                    return Some(ev);
                }
            }
        }
