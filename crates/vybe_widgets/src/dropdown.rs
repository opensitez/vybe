...
impl Dropdown {
    pub fn new(
        ...
        let actual_cols = num_cols
            .unwrap_or_else(|| {
                if items.len() <= 15 {
                    1
                } else {
                    (items.len() as f32 / 15.0).ceil() as usize
                }
            })
            .max(1);
    ...
    pub fn render_list(
        ...
        hover: (u8, u8, u8, u8),
        active_text: CosmicColor,
        inactive_text: CosmicColor,
    ) {
        ...
        mp.set_color_rgba8(bg.0, bg.1, bg.2, bg.3);
        pix.fill_rect(
            Rect::from_xywh(rx, ry, rw, rh).unwrap(),
            &mp,
            Transform::identity(),
            None,
        );
        ...
        bp.set_color_rgba8(border.0, border.1, border.2, border.3);
        let mut pb = PathBuilder::new();
        pb.push_rect(Rect::from_xywh(rx, ry, rw, rh).unwrap());
        ...
        for (i, item) in self.items.iter().enumerate() {
            ...
            if Some(i) == self.hover_idx {
                ...
                hp.set_color_rgba8(hover.0, hover.1, hover.2, hover.3);
                pix.fill_rect(
                    Rect::from_xywh(ix, iy, iw, ih).unwrap(),
                    &hp,
                    Transform::identity(),
                    None,
                );
    ...
    fn handle_mouse(&mut self, event: &MouseEvent) -> bool {
        ...
        match self.handle_mouse_at(event.x, event.y, r.x, r.y, is_click) {
            DropdownEvent::Selected(idx) => {
                self.pending_events
                    .push(WidgetEvent::DropdownSelected(self.name.clone(), idx));
                true
            }
        ...
        match cmd {
            WidgetCommand::SetSelectedIndex(i) => {
                if *i < self.items.len() && self.selected_idx != *i {
                    self.selected_idx = *i;
                    self.pending_events
                        .push(WidgetEvent::DropdownSelected(self.name.clone(), *i));
                }
                CommandValue::None
            }
