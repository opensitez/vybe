use cosmic_text::{Color as CColor, FontSystem, SwashCache};
use tiny_skia::{Paint, Pixmap, Rect};

use vybe_widgets::{Button, Checkbox, Dropdown, Radio, Slider, TextInput, TreeView};
...
fn main() {
    ...
    TreeView::draw_text_static_internal(
        &mut pixmap,
        &mut fs,
        &mut sc,
        "Accept terms",
    ...
    let col = if ti.is_placeholder() {
        CColor::rgb(150, 150, 150)
    } else {
        CColor::rgb(0, 0, 0)
    };
    TreeView::draw_text_static_internal(
    ...
    TreeView::draw_text_static_internal(
        ...
        &mut fs,
        &mut sc,
        &format!("Value: {:.0}", slider.actual_value()),
        240.0,
        200.0,
        CColor::rgb(51, 51, 51),
        1.0,
