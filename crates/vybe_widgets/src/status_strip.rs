...
impl PanelWidget for StatusStrip {
    ...
    fn render(&mut self, ctx: &mut RenderContext) {
        ...
        if !self.text.is_empty() {
            super::ide_text::draw_text(ctx.pixmap, ctx.font_system, ctx.swash_cache, &self.text, r.x + 6.0, r.y + 4.0, 12.0, col, ctx.scale);
    ...
    fn handle_mouse(&mut self, event: &MouseEvent) -> bool {
        ...
        if let MouseEventKind::Press(LayoutMouseButton::Left) = event.kind {
            self.pending_events.push(WidgetEvent::StatusBarClick(self.name.clone()));
            return true;
        }
        false
    }
