...
pub enum DrawCmd {
    ...
    Rotate(f32),
    Scale(f32, f32),
    Transform { m11: f32, m12: f32, m21: f32, m22: f32, dx: f32, dy: f32 },
    ResetTransform,
}
    ...
    pub fn replay(&self, target: &mut dyn Canvas) {
        for cmd in &self.commands {
            match cmd {
                ...
                DrawCmd::BezierCurveTo { cx1, cy1, cx2, cy2, x, y } =>
                    target.bezier_curve_to(*cx1, *cy1, *cx2, *cy2, *x, *y),
                DrawCmd::Arc { x, y, r, start, end, ccw } =>
                ...
                DrawCmd::Transform { m11, m12, m21, m22, dx, dy } =>
...
impl Canvas for RecordingCanvas {
    // Paint state
    fn set_fill_color(&mut self, color: Color)      { self.commands.push(DrawCmd::SetFillColor(color)); }
    fn set_stroke_color(&mut self, color: Color)    { self.commands.push(DrawCmd::SetStrokeColor(color)); }
    fn set_line_width(&mut self, width: f32)        { self.commands.push(DrawCmd::SetLineWidth(width)); }
    fn set_line_cap(&mut self, cap: LineCap)        { self.commands.push(DrawCmd::SetLineCap(cap)); }
    fn set_line_join(&mut self, join: LineJoin)     { self.commands.push(DrawCmd::SetLineJoin(join)); }
    ...
    fn close_path(&mut self)                        { self.commands.push(DrawCmd::ClosePath); }
    fn move_to(&mut self, x: f32, y: f32)           { self.commands.push(DrawCmd::MoveTo(x, y)); }
    fn line_to(&mut self, x: f32, y: f32)           { self.commands.push(DrawCmd::LineTo(x, y)); }
    fn quadratic_curve_to(&mut self, cx: f32, cy: f32, x: f32, y: f32) {
        self.commands.push(DrawCmd::QuadraticCurveTo { cx, cy, x, y });
    }
    fn bezier_curve_to(&mut self, cx1: f32, cy1: f32, cx2: f32, cy2: f32, x: f32, y: f32) {
        self.commands.push(DrawCmd::BezierCurveTo { cx1, cy1, cx2, cy2, x, y });
    }
    fn arc(&mut self, x: f32, y: f32, r: f32, start: f32, end: f32, ccw: bool) {
        self.commands.push(DrawCmd::Arc { x, y, r, start, end, ccw });
    }
    ...
    // Drawing
    fn fill(&mut self)                              { self.commands.push(DrawCmd::Fill); }
    fn stroke(&mut self)                            { self.commands.push(DrawCmd::Stroke); }
    ...
    fn fill_text(&mut self, text: &str, x: f32, y: f32) {
        self.commands.push(DrawCmd::FillText { text: text.to_string(), x, y });
    }
    fn stroke_text(&mut self, text: &str, x: f32, y: f32) {
        self.commands.push(DrawCmd::StrokeText { text: text.to_string(), x, y });
    }
    ...
    fn restore(&mut self)                           { self.commands.push(DrawCmd::Restore); }

    // Transforms
    fn translate(&mut self, x: f32, y: f32)         { self.commands.push(DrawCmd::Translate(x, y)); }
    fn rotate(&mut self, rad: f32)                  { self.commands.push(DrawCmd::Rotate(rad)); }
    fn scale(&mut self, sx: f32, sy: f32)           { self.commands.push(DrawCmd::Scale(sx, sy)); }
    fn transform(&mut self, m11: f32, m12: f32, m21: f32, m22: f32, dx: f32, dy: f32) {
        self.commands.push(DrawCmd::Transform { m11, m12, m21, m22, dx, dy });
    }
    fn reset_transform(&mut self)                   { self.commands.push(DrawCmd::ResetTransform); }
}
...
mod tests {
    ...
    fn captures_calls_in_order() {
        ...
        assert_eq!(c.commands[1], DrawCmd::FillRect { x: 0.0, y: 0.0, w: 10.0, h: 10.0 });
