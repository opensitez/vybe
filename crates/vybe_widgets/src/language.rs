            ...
            if let Ok(json) = serde_json::from_str::<LanguageJson>(&content) {
                ...
                for (k, v) in &json.language {
                    if let Some(arr) = v.as_array() {
                        let target_set = if k.to_lowercase().contains("keyword") {
                            Some(&mut lang.keywords)
                        } else if k.to_lowercase().contains("function") {
                            Some(&mut lang.keywords)
                        } else if k.to_lowercase().contains("constant") {
                            Some(&mut lang.constants)
                        } else if k.to_lowercase().contains("operator") {
                            Some(&mut lang.operators)
                        } else if k.to_lowercase().contains("type") {
                            Some(&mut lang.type_keywords)
                        } else {
                            None
                        };
