...
impl ProjectPropsDialog {
    ...
    pub fn open(&mut self, project: &vybe_compiler::projects::project::Project) {
        ...
        self.selected_startup = match &project.startup_object {
            ...
            vybe_compiler::projects::project::StartupObject::Form(name) => {
    ...
    pub fn apply(&self, project: &mut vybe_compiler::projects::project::Project) {
        match self.selected_startup {
            0 => { project.startup_object = vybe_compiler::projects::project::StartupObject::SubMain; project.startup_form = None; }
            1 => { project.startup_object = vybe_compiler::projects::project::StartupObject::None; project.startup_form = None; }
            n => {
                let idx = n - 2;
                if let Some(fm) = project.forms.get(idx) {
                    let name = fm.form.name.clone();
                    project.startup_object = vybe_compiler::projects::project::StartupObject::Form(name.clone());
    ...
    pub fn startup_options(&self, project: &vybe_compiler::projects::project::Project) -> Vec<String> {
        let mut opts = vec!["Sub Main".into(), "(None)".into()];
        for fm in &project.forms { opts.push(fm.form.name.clone()); }
        opts
    }

    pub fn render(&self, pix: &mut Pixmap, fs: &mut FontSystem, sc: &mut SwashCache, win_w: f32, win_h: f32, scale: f32, project: &vybe_compiler::projects::project::Project) {
    ...
    pub fn handle_click(&mut self, mx: f32, my: f32, win_w: f32, win_h: f32, project: &vybe_compiler::projects::project::Project) -> bool {
