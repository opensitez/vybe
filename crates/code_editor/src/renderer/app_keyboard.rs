    ...
    pub(super) fn handle_key_press(&mut self, event: vybe_widgets::KeyEvent) {
                    ...
                    match ch.as_str() {
                        ...
                        "c" => {
                            ...
                            self.control_clipboard = f
                                ...
                                .iter()
                                .filter_map(|id| {
                                    f.form.controls.iter().find(|c| c.id == *id).cloned()
                                })
                                .collect();
                        }
                        "x" => {
        ...
        if w.autocomplete_visible {
            match event.logical_key {
                ...
                Key::Named(NamedKey::ArrowDown) => {
                    if w.autocomplete_selected < w.autocomplete_items.len().saturating_sub(1) {
                        w.autocomplete_selected += 1;
                    }
        ...
        for kb in &self.keybindings {
            if kb.key == key_str && kb.cmd == cmd && kb.shift == shift && kb.alt == alt {
                match kb.action.as_str() {
                    ...
                    "MoveBufferStart" => {
                        w.action_motion(&mut self.font_system, CursorMotion::BufferStart)
                    }
                    "MoveBufferEnd" => {
                        w.action_motion(&mut self.font_system, CursorMotion::BufferEnd)
                    }
                    ...
                    "MoveWordLeft" => {
                        w.action_motion(&mut self.font_system, CursorMotion::LeftWord)
                    }
                    "MoveWordRight" => {
                        w.action_motion(&mut self.font_system, CursorMotion::RightWord)
                    }
                    "Save" => {
                    ...
                    "Find" => {
                        ...
                        if let Some(t) = w.copy_selection_text() {
                            if !t.is_empty() {
                                w.search_query = t;
                            }
                        } else {
                            w.search_query.clear();
                        }
                    }
                    "Replace" => {
                        w.is_search_open = true;
                        w.is_replace_open = !w.is_replace_open;
                    }
                    "ToggleOutput" => {
                    ...
                    "ToggleComment" => {
                        ...
                        if let Some(prefix) = prefix_opt {
                            ...
                            w.my_editor
                                .save_snapshot(w.cursor_pos().0, w.cursor_pos().1);
                            ...
                            for (i, li) in (start_line..=end_line).rev().enumerate() {
                                ...
                                if all_commented {
                                    ...
                                    w.my_editor.delete_range(
                                        del_start,
                                        del_start + remove_len,
                                        &w.lang_def,
                                    );
                                } else {
                                    let ins_pos = w.compute_byte_offset(li, *leading);
                                    w.my_editor.insert_str(
                                        ins_pos,
                                        &format!("{} ", prefix),
                                        &w.lang_def,
                                    );
                    ...
                    "Unindent" => {
                        ...
                        for (i, li) in (start_line..=end_line).rev().enumerate() {
                            ...
                            if n > 0 {
                                ...
                                w.my_editor
                                    .delete_range(del_start, del_start + n, &w.lang_def);
                            }
                        }
                        let new_text = w.my_editor.rope().to_string();
                        w.set_buffer_text(&mut self.font_system, &new_text);
        ...
        match event.key_without_modifiers.clone() {
            Key::Character(c) if cmd && (c == "=" || c == "+") => {
                w.set_zoom(&mut self.font_system, 1.0);
            }
            ...
            Key::Character(c) if cmd && c == "0" => {
                w.font_size = 14.0;
                w.set_zoom(&mut self.font_system, 0.0);
            }
            Key::Character(c) if cmd && (c == "w" || c == "W") => {
                w.show_whitespace = !w.show_whitespace;
            }
            Key::Character(c) if alt && (c == "z" || c == "Z") => {
                w.wrap_lines = !w.wrap_lines;
                w.needs_reshape = true;
            }
            Key::Character(c) if cmd && (c == "m" || c == "M") => {
                let (cl, ci) = w.cursor_pos();
                if let Some(p) = w.my_editor.find_matching_bracket(cl, ci, &w.lang_def) {
                    w.set_cursor_pos(p.0, p.1);
                }
            }
            ...
            Key::Character(c) if cmd && (c == "g" || c == "G") => {
            ...
            Key::Character(c) if cmd && shift && c == "[" => {
                ...
                if w.my_editor.folds.iter().any(|(s, _)| *s == line)
                    && !w.my_editor.collapsed_starts.contains(&line)
                {
            ...
            Key::Character(c) if cmd && shift && c == "0" => {
                ...
                for s in fold_starts {
                    w.my_editor.collapsed_starts.insert(s);
                }
            }
            ...
            Key::Named(NamedKey::Home) => {
                ...
                let first_byte_idx = line_text
                    .char_indices()
                    .find(|&(_, c)| !c.is_whitespace())
                    .map(|(i, _)| i)
                    .unwrap_or(line_text.len());
                if cur == first_byte_idx {
                    w.action_motion(&mut self.font_system, CursorMotion::Home);
                } else {
                    w.set_cursor_pos(cli, first_byte_idx);
                }
            }
            Key::Named(NamedKey::End) => w.action_motion(&mut self.font_system, CursorMotion::End),
            Key::Character(c) if cmd && shift && (c == "k" || c == "K") => {
                w.action_motion(&mut self.font_system, CursorMotion::End);
                w.action_backspace(&mut self.font_system);
                w.action_motion(&mut self.font_system, CursorMotion::Home);
                let len = w.line_text(w.cursor_pos().0).len();
                for _ in 0..len {
            ...
            Key::Named(NamedKey::Backspace) => {
                ...
                } else if w.is_search_open {
                    if w.is_replace_open && alt {
                        w.replace_query.pop();
                    } else {
                        w.search_query.pop();
                    }
            ...
            Key::Named(NamedKey::Enter) => {
                if self.goto_line_open {
                    if let Ok(line_num) = self.goto_line_query.trim().parse::<usize>() {
                        ...
                        w.scroll_y =
                            (safe_line as f32 * w.font_size * 1.4 - w.font_size * 10.0).max(0.0);
                        w.needs_reshape = true;
                    }
                ...
                if let Some(cb) = &mut self.clipboard {
                    if let Ok(t) = cb.get_text() {
                        ...
                        let (new_line, new_col) =
                            w.my_editor.insert_string(byte_off, &t, &w.lang_def);
            ...
            Key::Character(c) if cmd && c == "z" => {
                if shift {
                    if let Some((text, line, col)) =
                        w.my_editor.redo(w.cursor_pos().0, w.cursor_pos().1)
                    {
                        w.set_buffer_text(&mut self.font_system, &text);
                        let safe_line = line.min(w.line_count().saturating_sub(1));
                        let safe_col = if safe_line < w.line_count() {
                            col.min(w.line_text(safe_line).len())
                        } else {
                            0
                        };
                    ...
                    if let Some((text, line, col)) =
                        w.my_editor.undo(w.cursor_pos().0, w.cursor_pos().1)
                    {
                        w.set_buffer_text(&mut self.font_system, &text);
                        let safe_line = line.min(w.line_count().saturating_sub(1));
                        let safe_col = if safe_line < w.line_count() {
            ...
            Key::Named(NamedKey::F12) => {
                ...
                if let Some(path) = &tab.path {
                    ...
                    let _ = self.lsp.send(crate::lsp_client::LspRequest::Definition(
                        uri,
                        line as u32,
                        col as u32,
                    ));
                    ...
                    if !cmd {
                        ...
                        for ch in t.chars() {
                            if !ch.is_control() || ch == '\t' || ch == '\n' {
                                if w.is_search_open {
                                    if w.is_replace_open && alt {
                                        w.replace_query.push(ch);
                                    } else {
                                        w.search_query.pop();
                                        w.search_query.push(ch);
                                    }
                                } else {
                                    let mut skip = false;
                                    if let Some(cl) = match ch {
                                        ')' => Some(')'),
                                        '}' => Some('}'),
                                        ']' => Some(']'),
                                        '"' => Some('"'),
                                    ...
                                    if !skip {
                                        w.action_insert(&mut self.font_system, ch);
                                        tab.is_modified = true;
                                        if let Some(cl) = match ch {
                                            '(' => Some(')'),
                                            '{' => Some('}'),
            ...
            Key::Named(NamedKey::ArrowDown) => {
                ...
                if len > 0 {
                    self.command_palette_selected =
                        (self.command_palette_selected + 1).min(len - 1);
                }
            }
            Key::Named(NamedKey::ArrowUp) => {
        ...
        match &event.logical_key {
            ...
            _ => {
                if let Some(t) = &event.text {
                    for ch in t.chars() {
                        if !ch.is_control() {
                            self.project_search_query.push(ch);
                        }
                    }
                    self.run_project_search();
                }
        ...
        let mut hits: Vec<(i64, usize)> = super::palette_commands()
            ...
            .filter_map(|(i, c)| {
                if self.command_palette_query.is_empty() {
                    ...
                    matcher
                        .fuzzy_match(c.label, &self.command_palette_query)
                        .map(|s| (s, i))
                }
            })
            .collect();
